# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/quadtree-0.1.0/src/node.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/quadtree-0.1.0/src/node.c"
...
# 2 "/home/enumclass/Documents/C2RustTranslation/bkp/quadtree-0.1.0/src/node.c" 2
...
quadtree_node_with_bounds(double minx, double miny, double maxx, double maxy){
  quadtree_node_t* node;
  if(!(node = quadtree_node_new())) return 0;
  if(!(node->bounds = quadtree_bounds_new())) return 0;
...
quadtree_node_free(quadtree_node_t* node, void (*key_free)(void*)) {
  if(node->nw != 0) quadtree_node_free(node->nw, key_free);
  if(node->ne != 0) quadtree_node_free(node->ne, key_free);
  if(node->sw != 0) quadtree_node_free(node->sw, key_free);
  if(node->se != 0) quadtree_node_free(node->se, key_free);
