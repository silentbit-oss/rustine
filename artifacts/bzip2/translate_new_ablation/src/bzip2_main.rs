...
pub fn main() {
    ...
    if NUM_FILE_NAMES.load(Ordering::SeqCst) == 0 {
        ...
        OPMODE.store(3, Ordering::SeqCst);
    }
            ...
            if OPMODE.load(Ordering::SeqCst) == 1 {
                ...
                while let Some(cell) = current {
                    if let Some(name) = &cell.name {
                        ...
                        // compress expects `Option<&[u8]>`
                        compress(Some(name_str.as_bytes()));
                    }
                    current = &cell.link;
                }
            }
            ...
            if OPMODE.load(Ordering::SeqCst) == 1 {
                ...
                while let Some(cell) = current {
                    if let Some(name) = &cell.name {
                        ...
                        NUM_FILES_PROCESSED.fetch_add(1, Ordering::SeqCst);
                        // uncompress expects `Option<&[u8]>`
                        uncompress(Some(name_str.as_bytes()));
                    }
                    current = &cell.link;
                }
            }
            ...
            if OPMODE.load(Ordering::SeqCst) == 1 {
                ...
                while let Some(cell) = current {
                    if let Some(name) = &cell.name {
                        ...
                        NUM_FILES_PROCESSED.fetch_add(1, Ordering::SeqCst);
                        // FIX: `testf` expects `Option<&[char]>`. Provide a `Vec<Char>` and pass a slice.
