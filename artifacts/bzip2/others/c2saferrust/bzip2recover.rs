...
/*---------------------------------------------*/
fn readError() {
    let prog_name = unsafe { std::ffi::CStr::from_ptr(progName.as_ptr()) };
    let in_file_name = unsafe { std::ffi::CStr::from_ptr(inFileName.as_ptr()) };

    eprintln!("{}: I/O error reading `{}`, possible reason follows.", 
              prog_name.to_string_lossy(), 
              in_file_name.to_string_lossy());
    eprintln!("{}", std::io::Error::last_os_error());
    eprintln!("{}: warning: output file(s) may be incomplete.", 
              prog_name.to_string_lossy());
    std::process::exit(1);
}
...
/*---------------------------------------------*/
fn writeError() {
    let in_file_name = unsafe { std::ffi::CStr::from_ptr(inFileName.as_mut_ptr()) };
    let prog_name = unsafe { std::ffi::CStr::from_ptr(progName.as_mut_ptr()) };
...
/*---------------------------------------------*/
fn mallocFail(n: usize) {
    let prog_name_str = unsafe { std::ffi::CStr::from_ptr(progName.as_ptr()) }.to_string_lossy();
...
/*---------------------------------------------*/
fn tooManyBlocks(max_handled_blocks: i32) {
    eprintln!("{}: `{}` appears to contain more than {} blocks", 
              ...
              max_handled_blocks);
    eprintln!("{}: and cannot be handled.  To fix, increase", 
              unsafe { std::ffi::CStr::from_ptr(progName.as_ptr()).to_string_lossy() });
    eprintln!("{}: BZ_MAX_HANDLED_BLOCKS in bzip2recover.c, and recompile.", 
              unsafe { std::ffi::CStr::from_ptr(progName.as_ptr()).to_string_lossy() });
    std::process::exit(1);
}

/*---------------------------------------------*/
fn bsOpenReadStream(stream: *mut FILE) -> *mut BitStream {
    let bs = Box::new(BitStream {
        handle: stream,
        buffer: 0,
        buffLive: 0,
        mode: 'r' as i32 as Char,
    });
    Box::into_raw(bs)
}

/*---------------------------------------------*/
fn bsOpenWriteStream(stream: *mut FILE) -> *mut BitStream {
    let bs = Box::new(BitStream {
        handle: stream,
        buffer: 0,
        buffLive: 0,
        mode: 'w' as i32 as Char,
    });
...
/*---------------------------------------------*/
fn bsPutUInt32(bs: *mut BitStream, c: u32) {
    unsafe {
        let bs_ref = &mut *bs;
        for i in (0..32).rev() {
...
pub fn main() {
    ...
    let arg_count = args.len() - 1; // Exclude the program name
