...
pub fn cleanUpAndFail(ec: Int32) -> ! {
    ...
    if (srcMode.load(Ordering::Relaxed) == 3 && OPMODE.load(Ordering::Relaxed) != 3) && DELETE_OUTPUT_ON_INTERRUPT.load(Ordering::Relaxed) {
        let in_name = IN_NAME.lock().unwrap();
...
pub fn notAStandardFile(name: &str) -> Bool {
...
pub fn redundant(flag: Option<&String>) {
    let flag = match flag {
        Some(f) => f,
        None => return,
    };
...
pub fn snocString(root: Option<Box<Cell>>, name: &String) -> Option<Box<Cell>> {
    if root.is_none() {
        let mut tmp = mkCell()?;
        let name_len = name.to_string().len();
        tmp.name = Some(Box::new(name.clone()));
        Some(tmp)
    } else {
...
pub fn testf(name: Option<&[Char]>) {
    ...
    if name.is_none() && srcMode.load(Ordering::Relaxed)  != 1 {
    }
...

    {
        ...
        match srcMode.load(Ordering::Relaxed) {
    ...
    if srcMode.load(Ordering::Relaxed) != 1 {
        let in_name = IN_NAME.lock().unwrap();
    ...
    all_ok = match srcMode.load(Ordering::Relaxed) {
        1 | 2 | 3 => {
...
pub fn compress(name: Option<&[Char]>) {
    ...
    if name.is_none() && srcMode.load(Ordering::Relaxed) != 1 {
       println!("compress here 1");
        //{};
    }
    
    println!("==================>>>>> {}", srcMode.load(Ordering::Relaxed));
    match srcMode.load(Ordering::Relaxed) {
        1 => {
    ...
    if srcMode.load(Ordering::Relaxed) != 1 && containsDubiousChars(Some(&IN_NAME.lock().unwrap()[..])) {
        if noisy.load(Ordering::SeqCst) {
    ...
    if srcMode.load(Ordering::Relaxed) != 1 && fileExists(&IN_NAME.lock().unwrap().iter().collect::<String>()) == 0 {
        let prog_name_bytes: Vec<u8> = PROG_NAME.lock().unwrap().iter().map(|&c| c as u8).collect();
        let prog_name_str = CStr::from_bytes_until_nul(&prog_name_bytes).unwrap().to_str().unwrap();
        let in_name_bytes: Vec<u8> = IN_NAME.lock().unwrap().iter().map(|&c| c as u8).collect();
        let in_name_str = CStr::from_bytes_until_nul(&in_name_bytes).unwrap().to_str().unwrap();
    ...
    let in_name_str = in_name_str.trim_end_matches('\0');

    if srcMode.load(Ordering::Relaxed) == 3 || srcMode.load(Ordering::Relaxed) == 2 {
        //let in_name_str = IN_NAME.lock().unwrap().iter().collect::<String>();
        //let in_name_str = in_name_str.trim_end_matches('\0');
        println!("################ {}", in_name_str);
        let path = Path::new(in_name_str.trim());
    ...
    if srcMode.load(Ordering::Relaxed) == 3 && !forceOverwrite.load(Ordering::SeqCst) && notAStandardFile(in_name_str) != 0 {
        if noisy.load(Ordering::SeqCst) {
            let prog_name_bytes: Vec<u8> = PROG_NAME.lock().unwrap().iter().map(|&c| c as u8).collect();
            let prog_name_str = CStr::from_bytes_with_nul(&prog_name_bytes).unwrap().to_str().unwrap();
            let in_name_bytes: Vec<u8> = IN_NAME.lock().unwrap().iter().map(|&c| c as u8).collect();
    ...
    if srcMode.load(Ordering::Relaxed) == 3 && fileExists(&outName.lock().unwrap().iter().collect::<String>()) != 0 {
        if forceOverwrite.load(Ordering::SeqCst) {
    ...
    if srcMode.load(Ordering::Relaxed) == 3 && !forceOverwrite.load(Ordering::SeqCst) {
    ...
    if srcMode.load(Ordering::Relaxed) == 3 {
        saveInputFileMetaInfo(Some(&IN_NAME.lock().unwrap()[0]));
    }

    let mut in_file: Option<InFile> = match srcMode.load(Ordering::Relaxed) {
        1 => {
    ...
    let mut out_file: Option<File> = match srcMode.load(Ordering::Relaxed) {
        1 | 2 => {
        ...
        3 => {
            ...
            let out_name_str = outName.lock().unwrap().iter().collect::<String>();
            let out_name_str = out_name_str.trim_end_matches('\0');
    ...
    *OUTPUT_HANDLE_JUST_IN_CASE.lock().unwrap() = match srcMode.load(Ordering::Relaxed) {
        3 => {
            let out_name_str = outName.lock().unwrap().iter().collect::<String>();
            let out_name_str = out_name_str.trim_end_matches('\0');
...
pub fn uncompress(name: Option<&[Char]>) {
    ...
    if srcMode.load(Ordering::Relaxed) != 1 && containsDubiousChars(Some(&IN_NAME.lock().unwrap()[..])) {
        if noisy.load(Ordering::SeqCst) {
            eprintln!(
                "{}: There are no files matching `{}'.",
                unsafe { CStr::from_ptr(PROG_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap(),
                unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()
            );
        }
        setExit(1);
        return;
    ...
    if srcMode.load(Ordering::Relaxed) != 1 && fileExists(unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()) == 0 {
        eprintln!(
            "{}: Can't open input file {}: {}.",
            unsafe { CStr::from_ptr(PROG_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap(),
            unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap(),
    ...
    if cant_guess != 0 && noisy.load(Ordering::SeqCst) {
        eprintln!(
            ...
            unsafe { CStr::from_ptr(PROG_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap(),
            unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap(),
            unsafe { CStr::from_ptr(outName.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()
        );
    }

    if srcMode.load(Ordering::Relaxed) == 3 && fileExists(unsafe { CStr::from_ptr(outName.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()) != 0 {
        if forceOverwrite.load(Ordering::SeqCst) {
            std::fs::remove_file(Path::new(unsafe { CStr::from_ptr(outName.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap())).unwrap_or_else(|_| ioError());
        } else {
            eprintln!(
    ...
    if srcMode.load(Ordering::Relaxed) == 3 && !forceOverwrite.load(Ordering::SeqCst) {
        n = countHardLinks(Some(unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()));
        if n > 0 {
            eprintln!(
    ...
    match srcMode.load(Ordering::Relaxed) {
        ...
        2 => {
            in_str = File::open(Path::new(unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap())).ok();
            out_str = Some(unsafe { File::from_raw_fd(libc::STDOUT_FILENO) });
            if in_str.is_none() {
                eprintln!(
                    "{}: Can't open input file {}:{}.",
        ...
        3 => {
            in_str = File::open(Path::new(unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap())).ok();
            out_str = fopen_output_safely(Some(Path::new(unsafe { CStr::from_ptr(outName.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap())), "wb");
            if out_str.is_none() {
                eprintln!(
    ...
    if VERBOSITY.load(Ordering::SeqCst) >= 1 {
        eprint!("  {}: ", unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap());
        pad(&IN_NAME.lock().unwrap()[..]);
        io::stderr().flush().unwrap();
    }
    ...
    *OUTPUT_HANDLE_JUST_IN_CASE.lock().unwrap() = None;
    ...
    if magic_number_ok != 0 {
        if srcMode.load(Ordering::Relaxed) == 3 {
            ...
            DELETE_OUTPUT_ON_INTERRUPT.store(false, Ordering::SeqCst);
            if !KEEP_INPUT_FILES.load(Ordering::SeqCst) {
                let ret_val = std::fs::remove_file(Path::new(unsafe { CStr::from_ptr(IN_NAME.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()));
                if ret_val.is_err() {
                    ioError();
                }
            }
        ...
        DELETE_OUTPUT_ON_INTERRUPT.store(false, Ordering::SeqCst);
        if srcMode.load(Ordering::Relaxed) == 3 {
            let ret_val = std::fs::remove_file(Path::new(unsafe { CStr::from_ptr(outName.lock().unwrap().as_ptr() as *const u8) }.to_str().unwrap()));
            if ret_val.is_err() {
                ioError();
            }
        }
