...
unsafe fn main_0() -> libc::c_int {
    ...
    let mut jso: *mut crate::src::json_object::json_object = crate::src::json_tokener::json_tokener_parse(input);
    printf(
        b"jso.to_string()=%s\n\0" as *const u8 as *const libc::c_char,
        crate::src::json_object::json_object_to_json_string(jso),
    );
    let mut rv: libc::c_int = 0;
    rv = crate::src::json_visit::json_c_visit(
        jso,
        0 as libc::c_int,
...
unsafe extern "C" fn emit_object(
    ...
    printf(
        ...
        if !jso_key.is_null() {
            jso_key
        } else {std::intrinsics::assume((jso_key).addr() == 0);
            b"(null)\0" as *const u8 as *const libc::c_char
        },
        if !jso_index.is_null() {
            *jso_index as libc::c_long
        } else {std::intrinsics::assume((jso_index).addr() == 0);
            -(1 as libc::c_long)
        },
        crate::src::json_object::json_object_to_json_string(jso),
    );
...
unsafe extern "C" fn skip_arrays(
    ...
    if crate::src::json_object::json_object_get_type(jso) as libc::c_uint
...
unsafe extern "C" fn pop_array(
    ...
    emit_object(jso, flags, parent_jso, jso_key, jso_index, userarg);
    if !jso_index.is_null() && *jso_index == 0 as libc::c_int as size_t {
