...
pub unsafe extern "C" fn json_util_get_last_err() -> *const libc::c_char {
    if _last_err[0 as libc::c_int as usize] as libc::c_int == '\0' as i32 {
...
#[no_mangle]
pub unsafe extern "C" fn json_object_from_fd(mut fd: libc::c_int) -> *mut crate::src::json_object::json_object {
...
pub unsafe extern "C" fn json_object_from_fd_ex(
    ...
    if pb.is_null() {std::intrinsics::assume((pb).addr() == 0);
    ...
    loop {
        ...
        if crate::src::printbuf::printbuf_memappend(pb, buf.as_mut_ptr(), ret as libc::c_int)
            < 0 as libc::c_int
        {
            _json_c_set_last_err(
    ...
    obj = crate::src::json_tokener::json_tokener_parse_ex(tok, (*pb).buf, (*pb).bpos);
    if obj.is_null() {std::intrinsics::assume((obj).addr() == 0);
        _json_c_set_last_err(
            b"json_tokener_parse_ex failed: %s\n\0" as *const u8 as *const libc::c_char,
            crate::src::json_tokener::json_tokener_error_desc(crate::src::json_tokener::json_tokener_get_error(tok)),
        );
    }
    crate::src::json_tokener::json_tokener_free(tok);
    crate::src::printbuf::printbuf_free(pb);
...
unsafe extern "C" fn _json_object_to_fd(
    ...
    filename = if !filename.is_null() {
        filename
    } else {std::intrinsics::assume((filename).addr() == 0);
        b"(fd)\0" as *const u8 as *const libc::c_char
    };
    json_str = crate::src::json_object::json_object_to_json_string_ext(obj, flags);
    if json_str.is_null() {std::intrinsics::assume((json_str).addr() == 0);
        return -(1 as libc::c_int);
    }
    ...
    while wpos < wsize {
        ...
        wpos = wpos.wrapping_add(ret as size_t);
    }
...
pub unsafe extern "C" fn json_parse_int64(
    ...
    *__errno_location() = 0 as libc::c_int;
    val = strtoll(buf, &raw mut end, 10 as libc::c_int) as int64_t;
    if end != buf as *mut libc::c_char {
        *retval = val;
    }
    if val == 0 as libc::c_int as int64_t && *__errno_location() != 0 as libc::c_int
...
pub unsafe extern "C" fn json_parse_uint64(
    ...
    *__errno_location() = 0 as libc::c_int;
    ...
    val = strtoull(buf, &raw mut end, 10 as libc::c_int) as uint64_t;
    if end != buf as *mut libc::c_char {
        *retval = val;
    }
    if val == 0 as libc::c_int as uint64_t && *__errno_location() != 0 as libc::c_int
        || end == buf as *mut libc::c_char
    {
        *__errno_location() = 22 as libc::c_int;
        return 1 as libc::c_int;
