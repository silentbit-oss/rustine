...
pub unsafe extern "C" fn array_list_new2(
    ...
    arr = malloc(::core::mem::size_of::<array_list>() as libc::c_ulong)
        as *mut array_list;
    if arr.is_null() {std::intrinsics::assume((arr).addr() == 0);
        return 0 as *mut array_list;
    }
    ...
    if ((*arr).array).is_null() {std::intrinsics::assume(((*arr).array).addr() == 0);
...
pub unsafe extern "C" fn array_list_free(mut arr: *mut array_list) {
    ...
    while i < (*arr).length {
        ...
        }else { std::intrinsics::assume((*(*arr).array.offset(i as isize)).addr() == 0); }
        i = i.wrapping_add(1);
        i;
    }
...
pub unsafe extern "C" fn array_list_shrink(
    ...
    if new_size == 0 as libc::c_int as size_t {
...
pub unsafe extern "C" fn array_list_insert_idx(
    ...
    if {let crown_promoted_local_0 = ((*arr).length).wrapping_add(1 as libc::c_int as size_t);array_list_expand_internal(
        arr,
    ...
    move_amount = ((*arr).length)
        .wrapping_sub(idx)
        .wrapping_mul(::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong);
    memmove(
        ((*arr).array).offset(idx as isize).offset(1 as libc::c_int as isize)
            as *mut libc::c_void,
        ((*arr).array).offset(idx as isize) as *const libc::c_void,
        move_amount,
    );
...
pub unsafe extern "C" fn array_list_put_idx(
    ...
    if array_list_expand_internal(arr, idx.wrapping_add(1 as libc::c_int as size_t)) != 0
    {
        return -(1 as libc::c_int);
    }
    ...
    *((*arr).array).offset(idx as isize) = data;
    ...
    if (*arr).length <= idx {
        (*arr).length = idx.wrapping_add(1 as libc::c_int as size_t);
    }
    return 0 as libc::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn array_list_del_idx(
    ...
    (*arr).length = ((*arr).length).wrapping_sub(count);
    return 0 as libc::c_int;
}
