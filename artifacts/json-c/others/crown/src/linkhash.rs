...
unsafe extern "C" fn lh_char_hash(mut k: *const libc::c_void) -> libc::c_ulong {
    ...
    if random_seed == -(1 as libc::c_int) {
        ...
        loop {
            seed = crate::src::random_seed::json_c_get_random_seed();
...
pub unsafe extern "C" fn lh_table_new(
    ...
    let mut t: *mut crate::src::json_object::lh_table = 0 as *mut crate::src::json_object::lh_table;
    ...
    t = calloc(
        ...
        ::core::mem::size_of::<crate::src::json_object::lh_table>() as libc::c_ulong,
    ) as *mut crate::src::json_object::lh_table;
    if t.is_null() {std::intrinsics::assume((t).addr() == 0);
        return 0 as *mut crate::src::json_object::lh_table;
    }
    ...
    (*t)
        ...
        ::core::mem::size_of::<crate::src::json_object::lh_entry>() as libc::c_ulong,
    ) as *mut crate::src::json_object::lh_entry;
    if ((*t).table).is_null() {std::intrinsics::assume(((*t).table).addr() == 0);
    ...
    i = 0 as libc::c_int;
    while i < size {
        (*((*t).table).offset(i as isize)).k = -(1 as libc::c_int) as *mut libc::c_void;
        i += 1;
        i;
    }
    return t;
...
pub unsafe extern "C" fn lh_table_resize(
    ...
    let mut new_t: *mut crate::src::json_object::lh_table = 0 as *mut crate::src::json_object::lh_table;
    let mut ent: *mut crate::src::json_object::lh_entry = 0 as *mut crate::src::json_object::lh_entry;
    new_t = lh_table_new(new_size, None, (*t).hash_fn, (*t).equal_fn);
    if new_t.is_null() {std::intrinsics::assume((new_t).addr() == 0);
    ...
    while !ent.is_null() {
        ...
        ent = (*ent).next;
    }std::intrinsics::assume((ent).addr() == 0);
...
pub unsafe extern "C" fn lh_table_free(mut t: *mut crate::src::json_object::lh_table) {
    let mut c: *mut crate::src::json_object::lh_entry = 0 as *mut crate::src::json_object::lh_entry;
    if ((*t).free_fn).is_some() {
        c = (*t).head;
        while !c.is_null() {
            ((*t).free_fn).expect("non-null function pointer")(c);
            c = (*c).next;
        }std::intrinsics::assume((c).addr() == 0);
    }
    free((*t).table as *mut libc::c_void);
    free(t as *mut libc::c_void);
}
...
pub unsafe extern "C" fn lh_table_insert_w_hash(
    ...
    (*t).count += 1;
    (*t).count;
    if ((*t).head).is_null() {std::intrinsics::assume(((*t).head).addr() == 0);
        (*t).tail = &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry;
        (*t).head = (*t).tail;
        (*((*t).table).offset(n as isize)).prev = 0 as *mut crate::src::json_object::lh_entry; (*((*t).table).offset(n as isize)).next = (*((*t).table).offset(n as isize)).prev;
    } else {
        (*(*t).tail).next = &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry;
        (*((*t).table).offset(n as isize)).prev = (*t).tail;
        (*((*t).table).offset(n as isize)).next = 0 as *mut crate::src::json_object::lh_entry;
        (*t).tail = &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry;
    }
    return 0 as libc::c_int;
}
...
pub unsafe extern "C" fn lh_table_lookup_entry_w_hash(
    ...
    while count < (*t).size {
        if (*((*t).table).offset(n as isize)).k
            == -(1 as libc::c_int) as *mut libc::c_void as *const libc::c_void
        {
            return 0 as *mut crate::src::json_object::lh_entry;
        }
        if (*((*t).table).offset(n as isize)).k
            != -(2 as libc::c_int) as *mut libc::c_void as *const libc::c_void
            && ((*t).equal_fn)
        ...
        {
            return &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry;
        }
        n = n.wrapping_add(1);
        if n as libc::c_int == (*t).size {
            n = 0 as libc::c_int as libc::c_ulong;
...
pub unsafe extern "C" fn lh_table_lookup_entry(
    ...
    return {let crown_promoted_local_0 = lh_get_hash(t, k);lh_table_lookup_entry_w_hash(t, k, crown_promoted_local_0)};
}
#[no_mangle]
pub unsafe extern "C" fn lh_table_lookup_ex(
    ...
    if !e.is_null() {
        if !v.is_null() {
            *v = lh_entry_v(e);
        }else { std::intrinsics::assume((v).addr() == 0); }
        return 1 as libc::c_int;
    }else { std::intrinsics::assume((e).addr() == 0); }
    if !v.is_null() {
...
pub unsafe extern "C" fn lh_table_delete_entry(
    ...
    (*((*t).table).offset(n as isize)).v = 0 as *const libc::c_void;
    (*((*t).table).offset(n as isize)).k = -(2 as libc::c_int) as *mut libc::c_void;
    if (*t).tail == &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry
        && (*t).head == &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry
    {
        (*t).tail = 0 as *mut crate::src::json_object::lh_entry;
        (*t).head = (*t).tail;
    } else if (*t).head == &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry {
        (*(*(*t).head).next).prev = 0 as *mut crate::src::json_object::lh_entry;
        (*t).head = (*(*t).head).next;
    } else if (*t).tail == &raw mut *((*t).table).offset(n as isize) as *mut crate::src::json_object::lh_entry {
        (*(*(*t).tail).prev).next = 0 as *mut crate::src::json_object::lh_entry;
        (*t).tail = (*(*t).tail).prev;
    } else {
        (*(*((*t).table).offset(n as isize)).prev).next = (*((*t).table).offset(n as isize)).next;
        (*(*((*t).table).offset(n as isize)).next).prev = (*((*t).table).offset(n as isize)).prev;
    }
    (*((*t).table).offset(n as isize)).prev = 0 as *mut crate::src::json_object::lh_entry; (*((*t).table).offset(n as isize)).next = (*((*t).table).offset(n as isize)).prev;
    return 0 as libc::c_int;
}
...
pub unsafe extern "C" fn lh_table_delete(
    ...
    let mut e: *mut crate::src::json_object::lh_entry = lh_table_lookup_entry(t, k);
    if e.is_null() {std::intrinsics::assume((e).addr() == 0);
        return -(1 as libc::c_int);
    }
    return lh_table_delete_entry(t, e);
}
#[no_mangle]
pub unsafe extern "C" fn lh_table_length(mut t: *mut crate::src::json_object::lh_table) -> libc::c_int {
    return (*t).count;
}
