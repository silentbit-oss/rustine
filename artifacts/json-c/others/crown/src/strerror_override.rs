...
pub unsafe extern "C" fn _json_c_strerror(
    ...
    if _json_c_strerror_enable == 0 {
        _json_c_strerror_enable = if (getenv(
            ...
            .is_null()
        {std::intrinsics::assume((getenv(b"_JSON_C_STRERROR_ENABLE\x00" as *const u8 as *const libc::c_char)).addr() == 0);
            -(1 as libc::c_int)
        } else {
            1 as libc::c_int
        };
    ...
    while !(errno_list[ii as usize].errno_str).is_null() {
        ...
        if errno_list[ii as usize].errno_value != errno_in {
            ...
            start_idx = (::core::mem::size_of::<[libc::c_char; 7]>() as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong) as libc::c_int;
            jj = 0 as libc::c_int;
            while *errno_str.offset(jj as isize) as libc::c_int != '\0' as i32 {
            ...
            errno_buf[start_idx as usize] = '\0' as i32 as libc::c_char;
            return errno_buf.as_mut_ptr();
        }
    }std::intrinsics::assume((errno_list[ii as usize].errno_str).addr() == 0);
    ii = 0 as libc::c_int;
