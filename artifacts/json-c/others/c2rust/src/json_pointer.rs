...
unsafe extern "C" fn json_pointer_set_single_path(
    ...
    if json_object_is_type(parent, json_type_array) != 0 {
        ...
        if is_valid_index(path, &mut idx) == 0 {
            return -(1 as libc::c_int);
        }
        return array_set_cb
...
unsafe extern "C" fn json_pointer_result_get_recursive(
    ...
    rc = json_pointer_get_single_path(obj, path, &mut obj, &mut idx);
    if rc != 0 {
        return rc;
    }
    if !endp.is_null() {
...
unsafe extern "C" fn json_pointer_object_get_recursive(
    ...
    let mut rc: libc::c_int = 0;
    rc = json_pointer_result_get_recursive(obj, path, &mut res);
    if rc != 0 {
        return rc;
    }
    if !value.is_null() {
...
pub unsafe extern "C" fn json_pointer_get_internal(
    ...
    if *path.offset(0 as libc::c_int as isize) as libc::c_int == '\0' as i32 {
        (*res).parent = 0 as *mut json_object;
        (*res).obj = obj;
...
pub unsafe extern "C" fn json_pointer_get(
    ...
    let mut rc: libc::c_int = 0;
    rc = json_pointer_get_internal(obj, path, &mut jpres);
    if rc != 0 {
        return rc;
    }
    if !res.is_null() {
...
pub unsafe extern "C" fn json_pointer_set_with_array_cb(
    ...
    *path_copy
    ...
    rc = json_pointer_object_get_recursive(*obj, path_copy, &mut set);
    free(path_copy as *mut libc::c_void);
    if rc != 0 {
        return rc;
    }
