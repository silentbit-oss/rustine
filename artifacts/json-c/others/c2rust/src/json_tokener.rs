...
pub unsafe extern "C" fn json_tokener_new_ex(
    ...
    tok = calloc(
        ...
        ::core::mem::size_of::<json_tokener>() as libc::c_ulong,
    ) as *mut json_tokener;
    if tok.is_null() {
        return 0 as *mut json_tokener;
    }
    (*tok)
        ...
        ::core::mem::size_of::<json_tokener_srec>() as libc::c_ulong,
            ...
            match (*((*tok).stack).offset((*tok).depth as isize)).state as libc::c_uint {
                0 => {
                        ...
                        && (*tok).flags & 0x1 as libc::c_int == 0
                    {
                        ...
                        if printbuf_memappend((*tok).pb, &mut c, 1 as libc::c_int)
                            < 0 as libc::c_int
                        {
                ...
                26 => {
                    ...
                    let ref mut fresh4 = (*((*tok).stack).offset((*tok).depth as isize))
                        .current;
                ...
                13 => {
                        ...
                        < 0 as libc::c_int
                    {
                            ...
                            ) == 0 as libc::c_int
                        {
                            ...
                            let ref mut fresh8 = (*((*tok).stack)
                                .offset((*tok).depth as isize))
                                .current;
                            *fresh8 = json_object_new_boolean(1 as libc::c_int);
                            if ((*((*tok).stack).offset((*tok).depth as isize)).current)
                                .is_null()
                            {
                                (*tok).err = json_tokener_error_memory;
                            ...
                            ) == 0 as libc::c_int
                        {
                            ...
                            let ref mut fresh9 = (*((*tok).stack)
                                .offset((*tok).depth as isize))
                                .current;
                            *fresh9 = json_object_new_boolean(0 as libc::c_int);
                            if ((*((*tok).stack).offset((*tok).depth as isize)).current)
                                .is_null()
                            {
                                (*tok).err = json_tokener_error_memory;
                ...
                loop {
                    if c as libc::c_int == (*tok).quote_char as libc::c_int {
                        ...
                        ) < 0 as libc::c_int
                        {
                            ...
                            let ref mut fresh7 = (*((*tok).stack)
                                .offset((*tok).depth as isize))
                                .current;
                            *fresh7 = json_object_new_string_len(
                                (*(*tok).pb).buf,
                                (*(*tok).pb).bpos,
                            );
                            if ((*((*tok).stack).offset((*tok).depth as isize)).current)
