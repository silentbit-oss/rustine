            ...
            match current_block {
                9270490131672724345 => {
                    ...
                    if (*s).distance_code > (*s).max_distance {
                        ...
                        let crown_promoted_local_0 = ((*s).dist_rb_idx & 3 as libc::c_int)
                            as usize;(*s).dist_rb[crown_promoted_local_0]= (*s).distance_code;
                        (*s).dist_rb_idx+= 1;
            ...
            if result as libc::c_int == BROTLI_DECODER_NEEDS_MORE_INPUT as libc::c_int {
                ...
                if (*s).buffer_length != 0 as libc::c_int as libc::c_uint {
                    if (*br).avail_in == 0 as libc::c_int as libc::c_ulong {
                        ...
                        result= BROTLI_DECODER_SUCCESS;
                        let crown_promoted_local_0 = (*s).buffer_length as usize;(*s).buffer.u8_0[crown_promoted_local_0]= (*(*next_in));
                    ...
                    *available_in= (*br).avail_in;
                    while (*available_in) != 0 {
                        let crown_promoted_local_1 = (*s).buffer_length as usize;(*s).buffer.u8_0[crown_promoted_local_1]= (*(*next_in));
                        (*s).buffer_length= (*s).buffer_length.wrapping_add(1);
                        *next_in= (*next_in).offset(1);
                        *available_in= (*available_in).wrapping_sub(1);
                    }
                ...
                6909244940670113449 => {
                    if (*s).loop_counter >= 3 as libc::c_int {
                        ...
                        if result as libc::c_int != BROTLI_DECODER_SUCCESS as libc::c_int
                        {
                            ...
                            let crown_promoted_local_2 = (*s).loop_counter as usize;(*s).num_block_types[crown_promoted_local_2]= (*s).num_block_types[(*s).loop_counter as usize].wrapping_add(1);
