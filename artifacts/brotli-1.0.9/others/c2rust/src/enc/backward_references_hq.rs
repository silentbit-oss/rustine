...
unsafe extern "C" fn InitCommand(
    ...
    (*self_0).copy_len_ = (copylen | (delta << 25 as libc::c_int) as size_t) as uint32_t;
...
unsafe extern "C" fn InitDictionaryBackwardMatch(
    ...
    (*self_0)
        ...
        | (if len == len_code { 0 as libc::c_int as size_t } else { len_code }))
        as uint32_t;
}
#[inline(always)]
...
unsafe extern "C" fn BackwardMatchLengthCode(
    ...
    let mut code: size_t = ((*self_0).length_and_code & 31 as libc::c_int as uint32_t)
        as size_t;
...
unsafe extern "C" fn FindAllMatchesH10(
    ...
    let short_match_max_backward: size_t = (if (*params).quality != 11 as libc::c_int {
        16 as libc::c_int
    } else {
        64 as libc::c_int
    }) as size_t;
    ...
    i = cur_ix.wrapping_sub(1 as libc::c_int as size_t);
    while i > stop && best_len <= 2 as libc::c_int as size_t {
        ...
        if !(*data.offset(cur_ix_masked as isize) as libc::c_int
            ...
            || *data
                .offset(cur_ix_masked.wrapping_add(1 as libc::c_int as size_t) as isize)
                as libc::c_int
                != *data
                    .offset(prev_ix.wrapping_add(1 as libc::c_int as size_t) as isize)
                    as libc::c_int)
        {
    ...
    while i <= 37 as libc::c_int as size_t {
    ...
    let mut minlen: size_t = brotli_max_size_t(
        ...
        best_len.wrapping_add(1 as libc::c_int as size_t),
    );
    ...
    {
        let mut maxlen: size_t = brotli_min_size_t(
        ...
        while l <= maxlen {
            ...
            if dict_id < kInvalidMatch {
                let mut distance: size_t = dictionary_distance
                    .wrapping_add((dict_id >> 5 as libc::c_int) as size_t)
                    .wrapping_add(1 as libc::c_int as size_t);
                if distance <= (*params).dist.max_distance {
                    ...
                    InitDictionaryBackwardMatch(
                        ...
                        distance,
                        l,
                        (dict_id & 31 as libc::c_int as uint32_t) as size_t,
                    );
                }
            }
            l = l.wrapping_add(1);
...
unsafe extern "C" fn StoreRangeH10(
    ...
    if ix_start.wrapping_add(63 as libc::c_int as size_t) <= ix_end {
        i = ix_end.wrapping_sub(63 as libc::c_int as size_t);
    }
    if ix_start.wrapping_add(512 as libc::c_int as size_t) <= i {
        while j < i {
            StoreH10(self_0, data, mask, j);
            j = j.wrapping_add(8 as libc::c_int as size_t);
        }
    }
    while i < ix_end {
...
unsafe extern "C" fn ZopfliNodeCopyLength(mut self_0: *const ZopfliNode) -> uint32_t {
    return (*self_0).length & 0x1ffffff as libc::c_int as uint32_t;
}
#[inline(always)]
...
unsafe extern "C" fn ZopfliNodeDistanceCode(mut self_0: *const ZopfliNode) -> uint32_t {
    let short_code: uint32_t = (*self_0).dcode_insert_length >> 27 as libc::c_int;
    return if short_code == 0 as libc::c_int as uint32_t {
        (ZopfliNodeCopyDistance(self_0))
            .wrapping_add(16 as libc::c_int as uint32_t)
            .wrapping_sub(1 as libc::c_int as uint32_t)
    } else {
        short_code.wrapping_sub(1 as libc::c_int as uint32_t)
    };
}
#[inline(always)]
unsafe extern "C" fn ZopfliNodeCommandLength(mut self_0: *const ZopfliNode) -> uint32_t {
    return (ZopfliNodeCopyLength(self_0))
        .wrapping_add(
            (*self_0).dcode_insert_length & 0x7ffffff as libc::c_int as uint32_t,
        );
}
unsafe extern "C" fn InitZopfliCostModel(
    ...
    (*self_0)
        .literal_costs_ = if num_bytes.wrapping_add(2 as libc::c_int as size_t)
    ...
    (*self_0)
        .cost_dist_ = if (*dist).alphabet_size_limit > 0 as libc::c_int as uint32_t {
        BrotliAllocate(
            m,
...
unsafe extern "C" fn SetCost(
    ...
    while i < histogram_size {
        sum = sum.wrapping_add(*histogram.offset(i as isize) as size_t);
        i = i.wrapping_add(1);
        i;
    }
    ...
    if literal_histogram == 0 {
        i = 0 as libc::c_int as size_t;
        while i < histogram_size {
            if *histogram.offset(i as isize) == 0 as libc::c_int as uint32_t {
...
unsafe extern "C" fn ZopfliCostModelSetFromCommands(
    ...
    while i < num_commands {
        ...
        let mut copylength: size_t = CommandCopyLen(&*commands.offset(i as isize))
            as size_t;
        let mut distcode: size_t = ((*commands.offset(i as isize)).dist_prefix_
            as libc::c_int & 0x3ff as libc::c_int) as size_t;
        ...
        if cmdcode >= 128 as libc::c_int as size_t {
    ...
    *literal_costs.offset(0 as libc::c_int as isize) = 0.0f64 as libc::c_float;
    ...
    while i < num_bytes {
        ...
        *literal_costs
        ...
        literal_carry
            -= *literal_costs.offset(i.wrapping_add(1 as libc::c_int as size_t) as isize)
    ...
    *literal_costs.offset(0 as libc::c_int as isize) = 0.0f64 as libc::c_float;
    ...
    while i < 704 as libc::c_int as size_t {
        *cost_cmd
            ...
            (11 as libc::c_int as uint32_t).wrapping_add(i as uint32_t) as size_t,
        ) as libc::c_float;
        i = i.wrapping_add(1);
        i;
    }
    ...
    while i < (*self_0).distance_histogram_size as size_t {
        *cost_dist
            ...
            (20 as libc::c_int as uint32_t).wrapping_add(i as uint32_t) as size_t,
        ) as libc::c_float;
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn UpdateZopfliNode(
    ...
    (*next)
        .length = (len
        | len.wrapping_add(9 as libc::c_uint as size_t).wrapping_sub(len_code)
...
unsafe extern "C" fn StartPosQueuePush(
    ...
    let mut offset: size_t = !fresh3 & 7 as libc::c_int as size_t;
    ...
    *q.offset(offset as isize) = *posdata;
    ...
    while i < len {
        if (*q.offset((offset & 7 as libc::c_int as size_t) as isize)).costdiff
            > (*q
                .offset(
                    (offset.wrapping_add(1 as libc::c_int as size_t)
                        & 7 as libc::c_int as size_t) as isize,
                ))
                .costdiff
        {
            let mut __brotli_swap_tmp: PosData = *q
                .offset((offset & 7 as libc::c_int as size_t) as isize);
            *q
                .offset(
                    (offset & 7 as libc::c_int as size_t) as isize,
                ) = *q
                .offset(
                    (offset.wrapping_add(1 as libc::c_int as size_t)
                        & 7 as libc::c_int as size_t) as isize,
...
unsafe extern "C" fn StartPosQueueAt(
    ...
    return &*((*self_0).q_)
        .as_ptr()
        .offset((k.wrapping_sub((*self_0).idx_) & 7 as libc::c_int as size_t) as isize)
        as *const PosData;
}
unsafe extern "C" fn ComputeMinimumCopyLength(
        ...
        && (*nodes.offset(pos.wrapping_add(len) as isize)).u.cost <= min_cost
    {
        ...
        if len == next_len_offset {
            ...
            next_len_bucket = next_len_bucket * 2 as libc::c_int as size_t;
        }
    }
    return len;
}
unsafe extern "C" fn ComputeDistanceShortcut(
    ...
    let clen: size_t = ZopfliNodeCopyLength(&*nodes.offset(pos as isize)) as size_t;
    let ilen: size_t = ((*nodes.offset(pos as isize)).dcode_insert_length
        & 0x7ffffff as libc::c_int as uint32_t) as size_t;
    let dist: size_t = ZopfliNodeCopyDistance(&*nodes.offset(pos as isize)) as size_t;
...
unsafe extern "C" fn ComputeDistanceCache(
    ...
    while idx < 4 as libc::c_int && p > 0 as libc::c_int as size_t {
        let ilen: size_t = ((*nodes.offset(p as isize)).dcode_insert_length
            & 0x7ffffff as libc::c_int as uint32_t) as size_t;
        let clen: size_t = ZopfliNodeCopyLength(&*nodes.offset(p as isize)) as size_t;
        let dist: size_t = ZopfliNodeCopyDistance(&*nodes.offset(p as isize)) as size_t;
        let fresh4 = idx;
        idx = idx + 1;
...
unsafe extern "C" fn UpdateNodes(
    ...
    let mut posdata: *const PosData = StartPosQueueAt(queue, 0 as libc::c_int as size_t);
    ...
    while k < max_iters && k < StartPosQueueSize(queue) {
        ...
        let base_cost: libc::c_float = start_costdiff
            ...
            + ZopfliCostModelGetLiteralCosts(model, 0 as libc::c_int as size_t, pos);
        let mut best_len: size_t = min_len.wrapping_sub(1 as libc::c_int as size_t);
        let mut j: size_t = 0 as libc::c_int as size_t;
        while j < 16 as libc::c_int as size_t && best_len < max_len {
                ...
                if backward <= max_distance {
                    if !(prev_ix >= cur_ix) {
                                    ...
                                    as libc::c_int)
                        {
                            ...
                            let mut l: size_t = 0;
                            l = best_len.wrapping_add(1 as libc::c_int as size_t);
                            while l <= len {
                                let copycode: uint16_t = GetCopyLengthCode(l);
                                let cmdcode: uint16_t = CombineLengthCodes(
                                    inscode,
                                    copycode,
                                    (j == 0 as libc::c_int as size_t) as libc::c_int,
                                );
                                let cost: libc::c_float = (if (cmdcode as libc::c_int)
                                    < 128 as libc::c_int
                                {
                                    base_cost
                                } else {
                                    ...
                                    < (*nodes.offset(pos.wrapping_add(l) as isize)).u.cost
                                {
                                    UpdateZopfliNode(
                                        ...
                                        l,
                                        backward,
                                        j.wrapping_add(1 as libc::c_int as size_t),
                                        cost,
                                    );
        ...
        if !(k >= 2 as libc::c_int as size_t) {
            ...
            while j < num_matches {
                ...
                let mut is_dictionary_match: libc::c_int = if dist
...
unsafe extern "C" fn ComputeShortestPathFromNodes(
    ...
    while (*nodes.offset(index as isize)).dcode_insert_length
        & 0x7ffffff as libc::c_int as uint32_t == 0 as libc::c_int as uint32_t
        && (*nodes.offset(index as isize)).length == 1 as libc::c_int as uint32_t
    {
    ...
    while index != 0 as libc::c_int as size_t {
        let mut len: size_t = ZopfliNodeCommandLength(&mut *nodes.offset(index as isize))
            as size_t;
...
pub unsafe extern "C" fn BrotliZopfliCreateCommands(
    ...
    let stream_offset: size_t = (*params).stream_offset;
    let max_backward_limit: size_t = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as size_t);
    let mut pos: size_t = 0 as libc::c_int as size_t;
    let mut offset: uint32_t = (*nodes.offset(0 as libc::c_int as isize)).u.next;
    let mut i: size_t = 0;
    let mut gap: size_t = 0 as libc::c_int as size_t;
    i = 0 as libc::c_int as size_t;
    while offset != !(0 as libc::c_int as uint32_t) {
        let mut next: *const ZopfliNode = &*nodes
            .offset(pos.wrapping_add(offset as size_t) as isize) as *const ZopfliNode;
        ...
        let mut is_dictionary: libc::c_int = if distance
            > dictionary_start.wrapping_add(gap)
        {
            1 as libc::c_int
        } else {
        ...
        if is_dictionary == 0 && dist_code > 0 as libc::c_int as size_t {
            *dist_cache
                .offset(
...
unsafe extern "C" fn ZopfliIterate(
    ...
    i = 0 as libc::c_int as size_t;
    while i.wrapping_add(3 as libc::c_int as size_t) < num_bytes {
        ...
        if skip < 16384 as libc::c_int as size_t {
            skip = 0 as libc::c_int as size_t;
        }
        cur_match_pos = cur_match_pos
            .wrapping_add(*num_matches.offset(i as isize) as size_t);
        if *num_matches.offset(i as isize) == 1 as libc::c_int as uint32_t
            && BackwardMatchLength(
                &*matches
                    .offset(
                        cur_match_pos.wrapping_sub(1 as libc::c_int as size_t) as isize,
                    ),
            ) > max_zopfli_len
        {
            skip = brotli_max_size_t(
                BackwardMatchLength(
                    &*matches
                        .offset(
                            cur_match_pos.wrapping_sub(1 as libc::c_int as size_t)
                                as isize,
                        ),
                ),
                skip,
        ...
        if skip > 1 as libc::c_int as size_t {
            skip = skip.wrapping_sub(1);
            skip;
            while skip != 0 {
                i = i.wrapping_add(1);
                i;
                if i.wrapping_add(3 as libc::c_int as size_t) >= num_bytes {
                    break;
                }
                EvaluateNode(
...
pub unsafe extern "C" fn BrotliZopfliComputeShortestPath(
    ...
    InitStartPosQueue(&mut queue);
    i = 0 as libc::c_int as size_t;
    while i.wrapping_add(HashTypeLengthH10()).wrapping_sub(1 as libc::c_int as size_t)
        < num_bytes
    {
        ...
        if num_matches > 0 as libc::c_int as size_t
            && BackwardMatchLength(
                &mut *matches
                    .as_mut_ptr()
                    .offset(
                        num_matches.wrapping_sub(1 as libc::c_int as size_t) as isize,
                    ),
            ) > max_zopfli_len
        {
            matches[0 as libc::c_int
                as usize] = matches[num_matches.wrapping_sub(1 as libc::c_int as size_t)
                as usize];
        ...
        if skip > 1 as libc::c_int as size_t {
            ...
            while skip != 0 {
                ...
                if i
                    ...
                    .wrapping_sub(1 as libc::c_int as size_t) >= num_bytes
                {
                    break;
                }
                EvaluateNode(
...
pub unsafe extern "C" fn BrotliCreateZopfliBackwardReferences(
    ...
    let mut nodes: *mut ZopfliNode = if num_bytes
        .wrapping_add(1 as libc::c_int as size_t) > 0 as libc::c_int as size_t
    {
        BrotliAllocate(
            m,
            num_bytes
                .wrapping_add(1 as libc::c_int as size_t)
                .wrapping_mul(::core::mem::size_of::<ZopfliNode>() as libc::c_ulong),
        ) as *mut ZopfliNode
    } else {
        0 as *mut ZopfliNode
    };
    ...
    BrotliInitZopfliNodes(nodes, num_bytes.wrapping_add(1 as libc::c_int as size_t));
...
pub unsafe extern "C" fn BrotliCreateHqZopfliBackwardReferences(
    ...
    let max_backward_limit: size_t = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as size_t);
    let mut num_matches: *mut uint32_t = if num_bytes > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            num_bytes.wrapping_mul(::core::mem::size_of::<uint32_t>() as libc::c_ulong),
        ) as *mut uint32_t
    } else {
        0 as *mut uint32_t
    };
    let mut matches_size: size_t = 4 as libc::c_int as size_t * num_bytes;
    let store_end: size_t = if num_bytes >= StoreLookaheadH10() {
        position
            .wrapping_add(num_bytes)
            .wrapping_sub(StoreLookaheadH10())
    ...
    let mut matches: *mut BackwardMatch = if matches_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            matches_size
                .wrapping_mul(::core::mem::size_of::<BackwardMatch>() as libc::c_ulong),
        ...
        < num_bytes
    {
        ...
        {
            let mut _new_size: size_t = if matches_size == 0 as libc::c_int as size_t {
                cur_match_pos
                    .wrapping_add(128 as libc::c_int as size_t)
                    .wrapping_add(shadow_matches)
            } else {
            ...
            new_array = if _new_size > 0 as libc::c_int as size_t {
                BrotliAllocate(
                    ...
                    _new_size
                        .wrapping_mul(
                            ::core::mem::size_of::<BackwardMatch>() as libc::c_ulong,
                        ),
                ) as *mut BackwardMatch
            } else {
                0 as *mut BackwardMatch
        ...
        j = cur_match_pos;
        while j.wrapping_add(1 as libc::c_int as size_t) < cur_match_end {
            j = j.wrapping_add(1);
            j;
        }
        *num_matches.offset(i as isize) = num_found_matches as uint32_t;
        if num_found_matches > 0 as libc::c_int as size_t {
            let match_len: size_t = BackwardMatchLength(
                &mut *matches
                    .offset(
                        cur_match_end.wrapping_sub(1 as libc::c_int as size_t) as isize,
                    ),
            );
            if match_len > 325 as libc::c_int as size_t {
                let skip: size_t = match_len.wrapping_sub(1 as libc::c_int as size_t);
    ...
    nodes = if num_bytes.wrapping_add(1 as libc::c_int as size_t)
        > 0 as libc::c_int as size_t
    {
        BrotliAllocate(
            m,
