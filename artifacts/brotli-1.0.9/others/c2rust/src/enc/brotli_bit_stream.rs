...
unsafe extern "C" fn CommandDistanceContext(mut self_0: *const Command) -> uint32_t {
    let mut r: uint32_t = ((*self_0).cmd_prefix_ as libc::c_int >> 6 as libc::c_int)
        as uint32_t;
    let mut c: uint32_t = ((*self_0).cmd_prefix_ as libc::c_int & 7 as libc::c_int)
        as uint32_t;
    if (r == 0 as libc::c_int as uint32_t || r == 2 as libc::c_int as uint32_t
        || r == 4 as libc::c_int as uint32_t || r == 7 as libc::c_int as uint32_t)
        && c <= 2 as libc::c_int as uint32_t
    {
        return c;
    }
...
unsafe extern "C" fn CommandCopyLenCode(mut self_0: *const Command) -> uint32_t {
    ...
    let mut delta: int32_t = (modifier
        | (modifier & 0x40 as libc::c_int as uint32_t) << 1 as libc::c_int) as uint8_t
        as int8_t as int32_t;
    return (((*self_0).copy_len_ & 0x1ffffff as libc::c_int as uint32_t) as int32_t
        + delta) as uint32_t;
}
#[inline(always)]
...
unsafe extern "C" fn StoreStaticCodeLengthCode(
    ...
    BrotliWriteBits(
        ...
        (0xff as libc::c_uint as uint64_t) << 32 as libc::c_int
            | 0x55555554 as libc::c_uint as uint64_t,
        storage_ix,
        storage,
    );
}
...
unsafe extern "C" fn StoreStaticCommandHuffmanTree(
    ...
    BrotliWriteBits(
        ...
        (0x926244 as libc::c_uint as uint64_t) << 32 as libc::c_int
            | 0x16307003 as libc::c_uint as uint64_t,
        storage_ix,
        storage,
    );
    BrotliWriteBits(
...
unsafe extern "C" fn BlockLengthPrefixCode(mut len: uint32_t) -> uint32_t {
    let mut code: uint32_t = (if len >= 177 as libc::c_int as uint32_t {
        if len >= 753 as libc::c_int as uint32_t {
            20 as libc::c_int
        } else {
            14 as libc::c_int
        }
    } else if len >= 41 as libc::c_int as uint32_t {
        7 as libc::c_int
    } else {
        0 as libc::c_int
    }) as uint32_t;
    while code < (26 as libc::c_int - 1 as libc::c_int) as uint32_t
        && len
            >= _kBrotliPrefixCodeRanges[code.wrapping_add(1 as libc::c_int as uint32_t)
                    as usize]
                .offset as uint32_t
    {
...
unsafe extern "C" fn GetBlockLengthPrefixCode(
    ...
    *extra = len
        .wrapping_sub(_kBrotliPrefixCodeRanges[*code as usize].offset as uint32_t);
}
...
unsafe extern "C" fn NextBlockTypeCode(
    ...
    let mut type_code: size_t = (if type_0 as size_t
        == ((*calculator).last_type).wrapping_add(1 as libc::c_int as size_t)
    {
        1 as libc::c_uint
...
unsafe extern "C" fn BrotliEncodeMlen(
    ...
    let mut lg: size_t = (if length == 1 as libc::c_int as size_t {
        1 as libc::c_int as uint32_t
    } else {
        (Log2FloorNonZero(
            length.wrapping_sub(1 as libc::c_int as size_t) as uint32_t as size_t,
        ))
            .wrapping_add(1 as libc::c_int as uint32_t)
    }) as size_t;
    let mut mnibbles: size_t = (if lg < 16 as libc::c_int as size_t {
        16 as libc::c_int as size_t
    } else {
        lg.wrapping_add(3 as libc::c_int as size_t)
    }) / 4 as libc::c_int as size_t;
    *nibblesbits = mnibbles.wrapping_sub(4 as libc::c_int as size_t);
    *numbits = mnibbles * 4 as libc::c_int as size_t;
    *bits = length.wrapping_sub(1 as libc::c_int as size_t);
}
#[inline(always)]
unsafe extern "C" fn StoreCommandExtra(
    ...
    let mut insextraval: uint64_t = ((*cmd).insert_len_)
        .wrapping_sub(GetInsertBase(inscode)) as uint64_t;
    let mut copyextraval: uint64_t = copylen_code.wrapping_sub(GetCopyBase(copycode))
        as uint64_t;
...
unsafe extern "C" fn BrotliStoreHuffmanTreeOfHuffmanTreeToBitMask(
    ...
    let mut codes_to_store: size_t = (17 as libc::c_int + 1 as libc::c_int) as size_t;
    if num_codes > 1 as libc::c_int {
        while codes_to_store > 0 as libc::c_int as size_t {
            if *code_length_bitdepth
                .offset(
                    kStorageOrder[codes_to_store.wrapping_sub(1 as libc::c_int as size_t)
                        as usize] as isize,
    ...
    while i < codes_to_store {
        let mut l: size_t = *code_length_bitdepth
            .offset(kStorageOrder[i as usize] as isize) as size_t;
        BrotliWriteBits(
...
unsafe extern "C" fn StoreSimpleHuffmanTree(
    ...
    BrotliWriteBits(
        2 as libc::c_int as size_t,
        num_symbols.wrapping_sub(1 as libc::c_int as size_t),
        storage_ix,
        storage,
    );
    let mut i: size_t = 0;
    ...
    if num_symbols == 2 as libc::c_int as size_t {
        BrotliWriteBits(
            max_bits,
    ...
    } else if num_symbols == 3 as libc::c_int as size_t {
        BrotliWriteBits(
            max_bits,
            *symbols.offset(0 as libc::c_int as isize),
            storage_ix,
...
pub unsafe extern "C" fn BrotliStoreHuffmanTree(
    ...
    i = 0 as libc::c_int as size_t;
    while i < (17 as libc::c_int + 1 as libc::c_int) as size_t {
...
unsafe extern "C" fn BuildAndStoreHuffmanTree(
    ...
    let mut max_bits_counter: size_t = alphabet_size
    ...
    memset(
        ...
        0 as libc::c_int,
        histogram_length.wrapping_mul(::core::mem::size_of::<uint8_t>() as libc::c_ulong),
    );
...
pub unsafe extern "C" fn BrotliBuildAndStoreHuffmanTreeFast(
    ...
    while total != 0 as libc::c_int as size_t {
        if *histogram.offset(length as isize) != 0 {
            ...
            count = count.wrapping_add(1);
            count;
            total = total.wrapping_sub(*histogram.offset(length as isize) as size_t);
        }
    ...
    let max_tree_size: size_t = (2 as libc::c_int as size_t * length)
        .wrapping_add(1 as libc::c_int as size_t);
    let mut tree: *mut HuffmanTree = if max_tree_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            max_tree_size
    ...
    loop {
        ...
        let n: libc::c_int = node.offset_from(tree) as libc::c_long as libc::c_int;
    ...
    if count <= 4 as libc::c_int as size_t {
        ...
        BrotliWriteBits(
            2 as libc::c_int as size_t,
            count.wrapping_sub(1 as libc::c_int as size_t),
            storage_ix,
            storage,
        );
        ...
        while i_0 < count {
            ...
            j_0 = i_0.wrapping_add(1 as libc::c_int as size_t);
            while j_0 < count {
        ...
        } else if count == 3 as libc::c_int as size_t {
            BrotliWriteBits(
                max_bits,
                symbols[0 as libc::c_int as usize],
                storage_ix,
        ...
        while i_1 < length {
            ...
            k_0 = i_1.wrapping_add(1 as libc::c_int as size_t);
            while k_0 < length
            ...
            if value as libc::c_int == 0 as libc::c_int {
                ...
                if reps < 3 as libc::c_int as size_t {
                    while reps != 0 as libc::c_int as size_t {
                        reps = reps.wrapping_sub(1);
                        reps;
                        BrotliWriteBits(
                            kCodeLengthDepth[value as usize] as size_t,
                    ...
                    reps = reps.wrapping_sub(3 as libc::c_int as size_t);
                    BrotliWriteBits(
...
unsafe extern "C" fn MoveToFront(mut v: *mut uint8_t, mut index: size_t) {
    ...
    while i != 0 as libc::c_int as size_t {
        *v
            .offset(
                i as isize,
            ) = *v.offset(i.wrapping_sub(1 as libc::c_int as size_t) as isize);
        i = i.wrapping_sub(1);
        i;
    }
...
unsafe extern "C" fn MoveToFrontTransform(
    ...
    if v_size == 0 as libc::c_int as size_t {
        return;
    }
    ...
    let mut mtf_size: size_t = max_value.wrapping_add(1 as libc::c_int as uint32_t)
        as size_t;
    i = 0 as libc::c_int as size_t;
    while i < v_size {
...
unsafe extern "C" fn RunLengthCodeZeros(
    ...
    while i < in_size {
        ...
        while i < in_size && *v.offset(i as isize) != 0 as libc::c_int as uint32_t {
            i = i.wrapping_add(1);
            i;
        }
        while i < in_size && *v.offset(i as isize) == 0 as libc::c_int as uint32_t {
    ...
    max_prefix = if max_reps > 0 as libc::c_int as uint32_t {
    ...
    *out_size = 0 as libc::c_int as size_t;
    i = 0 as libc::c_int as size_t;
    while i < in_size {
        if *v.offset(i as isize) != 0 as libc::c_int as uint32_t {
            *v
                .offset(
                    *out_size as isize,
            ...
            *out_size;
            ...
            while k < in_size && *v.offset(k as isize) == 0 as libc::c_int as uint32_t {
                reps_0 = reps_0.wrapping_add(1);
                reps_0;
                k = k.wrapping_add(1);
                k;
            }
            i = i.wrapping_add(reps_0 as size_t);
            while reps_0 != 0 as libc::c_int as uint32_t {
                if reps_0 < (2 as libc::c_uint) << max_prefix {
                    let mut run_length_prefix: uint32_t = Log2FloorNonZero(
                        reps_0 as size_t,
                    );
                    ...
                    *out_size;
                    ...
                    let extra_bits_0: uint32_t = ((1 as libc::c_uint) << max_prefix)
                        .wrapping_sub(1 as libc::c_uint);
                    *v
                        .offset(
                            *out_size as isize,
...
unsafe extern "C" fn EncodeContextMap(
    ...
    StoreVarLenUint8(
        num_clusters.wrapping_sub(1 as libc::c_int as size_t),
        storage_ix,
        storage,
    );
    if num_clusters == 1 as libc::c_int as size_t {
        return;
    }
    rle_symbols = if context_map_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            context_map_size
    ...
    let mut use_rle: libc::c_int = if max_run_length_prefix
        > 0 as libc::c_int as uint32_t
    {
        1 as libc::c_int
    } else {
    ...
    if use_rle != 0 {
        BrotliWriteBits(
            4 as libc::c_int as size_t,
            max_run_length_prefix.wrapping_sub(1 as libc::c_int as uint32_t) as uint64_t,
            storage_ix,
            storage,
        );
    }
    ...
    while i < num_rle_symbols {
        ...
        let extra_bits_val: uint32_t = *rle_symbols.offset(i as isize)
            >> 9 as libc::c_int;
        BrotliWriteBits(
        ...
        if rle_symbol > 0 as libc::c_int as uint32_t
...
unsafe extern "C" fn StoreBlockSwitch(
    ...
    let mut typecode: size_t = NextBlockTypeCode(
        &mut (*code).type_code_calculator,
        block_type,
    );
...
unsafe extern "C" fn BuildAndStoreBlockSplitCode(
    ...
    StoreVarLenUint8(
        num_types.wrapping_sub(1 as libc::c_int as size_t),
        storage_ix,
        storage,
    );
    if num_types > 1 as libc::c_int as size_t {
        BuildAndStoreHuffmanTree(
            &mut *type_histo.as_mut_ptr().offset(0 as libc::c_int as isize),
            num_types.wrapping_add(2 as libc::c_int as size_t),
            num_types.wrapping_add(2 as libc::c_int as size_t),
            tree,
...
unsafe extern "C" fn StoreTrivialContextMap(
    ...
    if num_types > 1 as libc::c_int as size_t {
        let mut repeat_code: size_t = context_bits
        ...
        BrotliWriteBits(
            4 as libc::c_int as size_t,
            repeat_code.wrapping_sub(1 as libc::c_int as size_t),
            storage_ix,
            storage,
        );
        ...
        i = 0 as libc::c_int as size_t;
        while i < num_types {
            let mut code: size_t = if i == 0 as libc::c_int as size_t {
                0 as libc::c_int as size_t
            } else {
                i.wrapping_add(context_bits).wrapping_sub(1 as libc::c_int as size_t)
            };
            BrotliWriteBits(
...
unsafe extern "C" fn InitBlockEncoder(
    ...
    (*self_0)
        .block_len_ = (if num_blocks == 0 as libc::c_int as size_t {
        0 as libc::c_int as uint32_t
    } else {
...
unsafe extern "C" fn StoreSymbol(
    ...
    if (*self_0).block_len_ == 0 as libc::c_int as size_t {
        (*self_0).block_ix_ = ((*self_0).block_ix_).wrapping_add(1);
        let mut block_ix: size_t = (*self_0).block_ix_;
        let mut block_len: uint32_t = *((*self_0).block_lengths_)
            .offset(block_ix as isize);
        let mut block_type: uint8_t = *((*self_0).block_types_)
            .offset(block_ix as isize);
        (*self_0).block_len_ = block_len as size_t;
        (*self_0).entropy_ix_ = block_type as size_t * (*self_0).histogram_length_;
        StoreBlockSwitch(
            &mut (*self_0).block_split_code_,
            block_len,
            block_type,
...
unsafe extern "C" fn StoreSymbolWithContext(
    ...
    let mut ix: size_t = (histo_ix * (*self_0).histogram_length_).wrapping_add(symbol);
    BrotliWriteBits(
        *((*self_0).depths_).offset(ix as isize) as size_t,
        *((*self_0).bits_).offset(ix as isize) as uint64_t,
        storage_ix,
...
unsafe extern "C" fn BuildAndStoreEntropyCodesLiteral(
    ...
    let table_size: size_t = histograms_size * (*self_0).histogram_length_;
    (*self_0)
        .depths_ = if table_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            table_size.wrapping_mul(::core::mem::size_of::<uint8_t>() as libc::c_ulong),
        ) as *mut uint8_t
    } else {
        0 as *mut uint8_t
    };
    (*self_0)
        .bits_ = if table_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            table_size.wrapping_mul(::core::mem::size_of::<uint16_t>() as libc::c_ulong),
        ) as *mut uint16_t
    } else {
        0 as *mut uint16_t
    };
    ...
    while i < histograms_size {
        let mut ix: size_t = i * (*self_0).histogram_length_;
        BuildAndStoreHuffmanTree(
...
pub unsafe extern "C" fn BrotliStoreMetaBlock(
    ...
    let mut num_effective_distance_symbols: uint32_t = (*params)
        .dist
        .alphabet_size_limit;
    ...
    if (*mb).literal_context_map_size == 0 as libc::c_int as size_t {
    ...
    if (*mb).distance_context_map_size == 0 as libc::c_int as size_t {
        StoreTrivialContextMap(
    ...
    while i < n_commands {
        ...
        if (*mb).literal_context_map_size == 0 as libc::c_int as size_t {
            ...
            while j_0 != 0 as libc::c_int as size_t {
        ...
        if CommandCopyLen(&cmd) != 0 {
            ...
            if cmd.cmd_prefix_ as libc::c_int >= 128 as libc::c_int {
                ...
                let mut distnumextra: uint32_t = (cmd.dist_prefix_ as libc::c_int
                    >> 10 as libc::c_int) as uint32_t;
                let mut distextra: uint64_t = cmd.dist_extra_ as uint64_t;
                if (*mb).distance_context_map_size == 0 as libc::c_int as size_t {
                    StoreSymbol(&mut distance_enc, dist_code, storage_ix, storage);
                } else {
                    let mut context_0: size_t = CommandDistanceContext(&cmd) as size_t;
                    StoreSymbolWithContext(
                        &mut distance_enc,
                        dist_code,
                        context_0,
...
pub unsafe extern "C" fn BrotliStoreMetaBlockFast(
    ...
    let mut distance_alphabet_bits: uint32_t = (Log2FloorNonZero(
        num_distance_symbols.wrapping_sub(1 as libc::c_int as uint32_t) as size_t,
    ))
        .wrapping_add(1 as libc::c_int as uint32_t);
    StoreCompressedMetaBlockHeader(is_last, length, storage_ix, storage);
    BrotliWriteBits(
    ...
    if n_commands <= 128 as libc::c_int as size_t {
        ...
        while i < n_commands {
            ...
            num_literals = num_literals.wrapping_add(cmd.insert_len_ as size_t);
            pos = pos.wrapping_add(CommandCopyLen(&cmd) as size_t);
            i = i.wrapping_add(1);
            i;
        }
...
pub unsafe extern "C" fn BrotliStoreUncompressedMetaBlock(
    ...
    JumpToByteBoundary(storage_ix, storage);
    if masked_pos.wrapping_add(len) > mask.wrapping_add(1 as libc::c_int as size_t) {
        ...
        *storage_ix = (*storage_ix).wrapping_add(len1 << 3 as libc::c_int);
