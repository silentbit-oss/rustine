...
unsafe extern "C" fn UTF8Position(
    ...
    if c < 128 as libc::c_int as size_t {
        return 0 as libc::c_int as size_t
    } else if c >= 192 as libc::c_int as size_t {
        return brotli_min_size_t(1 as libc::c_int as size_t, clamp)
    } else if last < 0xe0 as libc::c_int as size_t {
        return 0 as libc::c_int as size_t
    } else {
...
unsafe extern "C" fn DecideMultiByteStatsLevel(
    ...
    while i < len {
        let mut c: size_t = *data.offset((pos.wrapping_add(i) & mask) as isize)
            as size_t;
    ...
    if counts[2 as libc::c_int as usize] < 500 as libc::c_int as size_t {
        max_utf8 = 1 as libc::c_int as size_t;
    }
    if (counts[1 as libc::c_int as usize])
        .wrapping_add(counts[2 as libc::c_int as usize]) < 25 as libc::c_int as size_t
    {
...
unsafe extern "C" fn EstimateBitCostsForLiteralsUTF8(
    ...
    while i < len {
        if i >= window_half {
            let mut c_0: size_t = (if i
                < window_half.wrapping_add(1 as libc::c_int as size_t)
            {
                0 as libc::c_int
            } else {
                *data
                    .offset(
                        (pos
                            ...
                            .wrapping_sub(1 as libc::c_int as size_t) & mask) as isize,
                    ) as libc::c_int
            }) as size_t;
            let mut last_c_0: size_t = (if i
                < window_half.wrapping_add(2 as libc::c_int as size_t)
            {
                0 as libc::c_int
            } else {
                *data
                    .offset(
                        (pos
                            .wrapping_add(i)
                            .wrapping_sub(window_half)
                            .wrapping_sub(2 as libc::c_int as size_t) & mask) as isize,
                    ) as libc::c_int
            }) as size_t;
        ...
        let mut c_2: size_t = (if i < 1 as libc::c_int as size_t {
            0 as libc::c_int
        } else {
            *data
                .offset(
                    (pos.wrapping_add(i).wrapping_sub(1 as libc::c_int as size_t) & mask)
                        as isize,
                ) as libc::c_int
        }) as size_t;
        let mut last_c_2: size_t = (if i < 2 as libc::c_int as size_t {
            0 as libc::c_int
        } else {
            *data
                .offset(
                    (pos.wrapping_add(i).wrapping_sub(2 as libc::c_int as size_t) & mask)
                        as isize,
                ) as libc::c_int
        }) as size_t;
        let mut utf8_pos_0: size_t = UTF8Position(last_c_2, c_2, max_utf8);
        let mut masked_pos: size_t = pos.wrapping_add(i) & mask;
        let mut histo: size_t = histogram[utf8_pos_0
            as usize][*data.offset(masked_pos as isize) as usize];
        let mut lit_cost: libc::c_double = 0.;
        if histo == 0 as libc::c_int as size_t {
        ...
        if i < 2000 as libc::c_int as size_t {
            lit_cost
                += 0.7f64
                    - (2000 as libc::c_int as size_t).wrapping_sub(i) as libc::c_double
                        / 2000.0f64 * 0.35f64;
        }
...
pub unsafe extern "C" fn BrotliEstimateBitCostsForLiterals(
    ...
    if BrotliIsMostlyUTF8(data, pos, mask, len, kMinUTF8Ratio) != 0 {
        ...
        while i < len {
            ...
            let mut lit_cost: libc::c_double = FastLog2(in_window) - FastLog2(histo);
            lit_cost += 0.029f64;
            if lit_cost < 1.0f64 {
                lit_cost *= 0.5f64;
                lit_cost += 0.5f64;
