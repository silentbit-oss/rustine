...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueLiteral(
    ...
    if (*out.offset(idx1 as isize)).total_count_ == 0 as libc::c_int as size_t {
        p.cost_combo = (*out.offset(idx2 as isize)).bit_cost_;
        is_good_pair = 1 as libc::c_int;
    } else if (*out.offset(idx2 as isize)).total_count_ == 0 as libc::c_int as size_t {
        p.cost_combo = (*out.offset(idx1 as isize)).bit_cost_;
        is_good_pair = 1 as libc::c_int;
    } else {
        let mut threshold: libc::c_double = if *num_pairs == 0 as libc::c_int as size_t {
            1e99f64
        } else {
            brotli_max_double(
                0.0f64,
    ...
    if is_good_pair != 0 {
        p.cost_diff += p.cost_combo;
        if *num_pairs > 0 as libc::c_int as size_t
...
pub unsafe extern "C" fn BrotliHistogramCombineLiteral(
    ...
    while idx1 < num_clusters {
        ...
        idx2 = idx1.wrapping_add(1 as libc::c_int as size_t);
    ...
    while num_clusters > min_cluster_size {
        ...
        if (*pairs.offset(0 as libc::c_int as isize)).cost_diff >= cost_diff_threshold {
            ...
            *fresh0 = (*fresh0).wrapping_add(*cluster_size.offset(best_idx2 as isize));
            i = 0 as libc::c_int as size_t;
            ...
            while i < num_pairs {
                    ...
                    || (*p).idx1 == best_idx2 || (*p).idx2 == best_idx2)
                {
                    ...
                    {
                        let mut front: HistogramPair = *pairs
...
pub unsafe extern "C" fn BrotliHistogramBitCostDistanceLiteral(
    ...
    if (*histogram).total_count_ == 0 as libc::c_int as size_t {
        return 0.0f64
    } else {
...
pub unsafe extern "C" fn BrotliHistogramRemapLiteral(
    ...
    i = 0 as libc::c_int as size_t;
    while i < in_size {
        let mut best_out: uint32_t = if i == 0 as libc::c_int as size_t {
            *symbols.offset(0 as libc::c_int as isize)
        } else {
            *symbols.offset(i.wrapping_sub(1 as libc::c_int as size_t) as isize)
        };
        let mut best_bits: libc::c_double = BrotliHistogramBitCostDistanceLiteral(
...
pub unsafe extern "C" fn BrotliHistogramReindexLiteral(
    ...
    let mut new_index: *mut uint32_t = if length > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            length.wrapping_mul(::core::mem::size_of::<uint32_t>() as libc::c_ulong),
        ) as *mut uint32_t
    } else {
        0 as *mut uint32_t
    };
    ...
    tmp = if next_index > 0 as libc::c_int as uint32_t {
        BrotliAllocate(
            m,
...
pub unsafe extern "C" fn BrotliClusterHistogramsLiteral(
    ...
    let mut cluster_size: *mut uint32_t = if in_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            in_size.wrapping_mul(::core::mem::size_of::<uint32_t>() as libc::c_ulong),
        ) as *mut uint32_t
    } else {
        0 as *mut uint32_t
    };
    let mut clusters: *mut uint32_t = if in_size > 0 as libc::c_int as size_t {
        BrotliAllocate(
            m,
            in_size.wrapping_mul(::core::mem::size_of::<uint32_t>() as libc::c_ulong),
        ) as *mut uint32_t
    ...
    let mut pairs_capacity: size_t = max_input_histograms * max_input_histograms
        / 2 as libc::c_int as size_t;
    let mut pairs: *mut HistogramPair = if pairs_capacity
    ...
    i = 0 as libc::c_int as size_t;
    while i < in_size {
        let mut num_to_combine: size_t = brotli_min_size_t(
    ...
    if pairs_capacity < max_num_pairs.wrapping_add(1 as libc::c_int as size_t) {
        let mut _new_size: size_t = if pairs_capacity == 0 as libc::c_int as size_t {
            max_num_pairs.wrapping_add(1 as libc::c_int as size_t)
        } else {
            pairs_capacity
        };
        let mut new_array: *mut HistogramPair = 0 as *mut HistogramPair;
        while _new_size < max_num_pairs.wrapping_add(1 as libc::c_int as size_t) {
        ...
        if 0 as libc::c_int == 0 && 0 as libc::c_int == 0
            && pairs_capacity != 0 as libc::c_int as size_t
        {
            memcpy(
...
pub unsafe extern "C" fn BrotliHistogramCombineCommand(
    ...
    while num_clusters > min_cluster_size {
        ...
        if (*pairs.offset(0 as libc::c_int as isize)).cost_diff >= cost_diff_threshold {
            ...
            *fresh1 = (*fresh1).wrapping_add(*cluster_size.offset(best_idx2 as isize));
            i = 0 as libc::c_int as size_t;
            while i < symbols_size {
                if *symbols.offset(i as isize) == best_idx2 {
                    *symbols.offset(i as isize) = best_idx1;
...
pub unsafe extern "C" fn BrotliHistogramRemapCommand(
    ...
    while i < in_size {
        ...
        let mut best_bits: libc::c_double = BrotliHistogramBitCostDistanceCommand(
            &*in_0.offset(i as isize),
            &mut *out.offset(best_out as isize),
        );
        let mut j: size_t = 0;
...
pub unsafe extern "C" fn BrotliHistogramCombineDistance(
    ...
    while num_clusters > min_cluster_size {
        ...
        if (*pairs.offset(0 as libc::c_int as isize)).cost_diff >= cost_diff_threshold {
            ...
            *fresh2 = (*fresh2).wrapping_add(*cluster_size.offset(best_idx2 as isize));
            i = 0 as libc::c_int as size_t;
            while i < symbols_size {
                if *symbols.offset(i as isize) == best_idx2 {
                    *symbols.offset(i as isize) = best_idx1;
...
pub unsafe extern "C" fn BrotliHistogramRemapDistance(
    ...
    while i < in_size {
        ...
        let mut best_bits: libc::c_double = BrotliHistogramBitCostDistanceDistance(
            &*in_0.offset(i as isize),
            &mut *out.offset(best_out as isize),
        );
        let mut j: size_t = 0;
