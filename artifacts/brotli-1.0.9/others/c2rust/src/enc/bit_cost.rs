...
unsafe extern "C" fn ShannonEntropy(
    ...
    if size & 1 as libc::c_int as size_t != 0 {
        current_block = 2779694069880731857;
    } else {
...
pub unsafe extern "C" fn BrotliPopulationCostLiteral(
    ...
    while i < data_size {
        if (*histogram).data_[i as usize] > 0 as libc::c_int as uint32_t {
            s[count as usize] = i;
            count += 1;
            count;
    ...
    if count == 3 as libc::c_int {
        let histo0: uint32_t = (*histogram).data_[s[0 as libc::c_int as usize] as usize];
        let histo1: uint32_t = (*histogram).data_[s[1 as libc::c_int as usize] as usize];
        let histo2: uint32_t = (*histogram).data_[s[2 as libc::c_int as usize] as usize];
        let histomax: uint32_t = brotli_max_uint32_t(
            histo0,
            brotli_max_uint32_t(histo1, histo2),
        );
        return kThreeSymbolHistogramCost
            + (2 as libc::c_int as uint32_t
                * histo0.wrapping_add(histo1).wrapping_add(histo2)) as libc::c_double
    ...
    if count == 4 as libc::c_int {
        ...
        return kFourSymbolHistogramCost
            + (3 as libc::c_int as uint32_t * h23) as libc::c_double
            + (2 as libc::c_int as uint32_t
                * (histo[0 as libc::c_int as usize])
                    .wrapping_add(histo[1 as libc::c_int as usize])) as libc::c_double
    ...
    while i < data_size {
        if (*histogram).data_[i as usize] > 0 as libc::c_int as uint32_t {
            let mut log2p: libc::c_double = log2total
            ...
            if depth > 15 as libc::c_int as size_t {
            ...
            while k < data_size
                && (*histogram).data_[k as usize] == 0 as libc::c_int as uint32_t
            {
            ...
            i = i.wrapping_add(reps as size_t);
            if i == data_size {
                break;
            }
            if reps < 3 as libc::c_int as uint32_t {
                depth_histo[0 as libc::c_int
                    as usize] = (depth_histo[0 as libc::c_int as usize])
                    .wrapping_add(reps);
            } else {
                reps = reps.wrapping_sub(2 as libc::c_int as uint32_t);
                while reps > 0 as libc::c_int as uint32_t {
    ...
    bits
        += (18 as libc::c_int as size_t)
            .wrapping_add(2 as libc::c_int as size_t * max_depth) as libc::c_double;
    bits
        += BitsEntropy(
