...
pub unsafe extern "C" fn BrotliInitBitReader(br: *mut BrotliBitReader) {
    ...
    (*br)
        .bit_pos_ = ((::core::mem::size_of::<uint64_t>() as libc::c_ulong)
...
pub unsafe extern "C" fn BrotliWarmupBitReader(br: *mut BrotliBitReader) -> libc::c_int {
    let mut aligned_read_mask: size_t = (::core::mem::size_of::<uint64_t>()
        as libc::c_ulong >> 1 as libc::c_int)
        .wrapping_sub(1 as libc::c_int as libc::c_ulong);
    if 0 as libc::c_int == 0 {
        aligned_read_mask = 0 as libc::c_int as size_t;
    }
    if BrotliGetAvailableBits(br) == 0 as libc::c_int as uint32_t {
        if BrotliPullByte(br) == 0 {
            return 0 as libc::c_int;
        }
    }
    while (*br).next_in as size_t & aligned_read_mask != 0 as libc::c_int as size_t {
        if BrotliPullByte(br) == 0 {
            return 1 as libc::c_int;
        }
    }
