...
unsafe extern "C" fn ParseAlias(mut name: *const libc::c_char) -> Command {
    let mut unbrotli: *const libc::c_char = b"unbrotli\0" as *const u8
...
unsafe extern "C" fn ParseParams(mut params: *mut Context) -> Command {
    ...
    while i < argc {
        ...
        if arg_len == 0 as libc::c_int as size_t {
            ...
            if after_dash_dash != 0
                ...
                || arg_len == 1 as libc::c_int as size_t
            {
                ...
                if arg_len == 2 as libc::c_int as size_t
                    ...
                    != '-' as i32
                {
                    ...
                    while j < arg_len {
                            ...
                            || c as libc::c_int == 'k' as i32
                        {
                            ...
                            if j.wrapping_add(1 as libc::c_int as size_t) != arg_len {
                                fprintf(
                                    stderr,
    ...
    if input_count > 1 as libc::c_int as size_t && output_set != 0 {
        return COMMAND_INVALID;
    }
...
unsafe extern "C" fn PrintVersion() {
    ...
    let mut minor: libc::c_int = 0x1000009 as libc::c_int >> 12 as libc::c_int
        & 0xfff as libc::c_int;
...
unsafe extern "C" fn NextFile(mut context: *mut Context) -> libc::c_int {
    ...
    if (*context).input_count == 0 as libc::c_int as size_t {
    ...
    arg_len = strlen(arg);
    if arg_len == 1 as libc::c_int as size_t
        && *arg.offset(0 as libc::c_int as isize) as libc::c_int == '-' as i32
    {
        (*context).current_input_path = 0 as *const libc::c_char;
        (*context).current_output_path = (*context).output_path;
    ...
    if (*context).decompress != 0 {
        ...
        let mut name: *mut libc::c_char = FileName((*context).modified_path)
            as *mut libc::c_char;
        let mut name_suffix: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut name_len: size_t = strlen(name);
        if name_len < suffix_len.wrapping_add(1 as libc::c_int as size_t) {
            fprintf(
                stderr,
...
unsafe extern "C" fn OpenFiles(mut context: *mut Context) -> libc::c_int {
    let mut is_ok: libc::c_int = OpenInputFile(
...
unsafe extern "C" fn ProvideInput(mut context: *mut Context) -> libc::c_int {
    ...
    (*context).total_in = ((*context).total_in).wrapping_add((*context).available_in);
...
unsafe extern "C" fn WriteOutput(mut context: *mut Context) -> libc::c_int {
    let mut out_size: size_t = ((*context).next_out).offset_from((*context).output)
        as libc::c_long as size_t;
    (*context).total_out = ((*context).total_out).wrapping_add(out_size);
...
unsafe extern "C" fn PrintBytes(mut value: size_t) {
    if value < 1024 as libc::c_int as size_t {
        fprintf(
            stderr,
            b"%d B\0" as *const u8 as *const libc::c_char,
            value as libc::c_int,
        );
    } else if value < 1048576 as libc::c_int as size_t {
        fprintf(
            stderr,
            b"%0.3f KiB\0" as *const u8 as *const libc::c_char,
            value as libc::c_double / 1024.0f64,
        );
    } else if value < 1073741824 as libc::c_int as size_t {
        fprintf(
            stderr,
...
unsafe extern "C" fn DecompressFile(
    ...
    loop {
            ...
            == BROTLI_DECODER_RESULT_SUCCESS as libc::c_int as libc::c_uint
        {
            ...
            let mut has_more_input: libc::c_int = ((*context).available_in
...
unsafe extern "C" fn DecompressFiles(mut context: *mut Context) -> libc::c_int {
    while NextFile(context) != 0 {
        let mut is_ok: libc::c_int = 1 as libc::c_int;
        let mut s: *mut BrotliDecoderState = BrotliDecoderCreateInstance(
            None,
            None,
...
unsafe extern "C" fn CompressFile(
    ...
    InitializeBuffers(context);
    loop {
        if (*context).available_in == 0 as libc::c_int as size_t && is_eof == 0 {
            if ProvideInput(context) == 0 {
                return 0 as libc::c_int;
            }
        ...
        if (*context).available_out == 0 as libc::c_int as size_t {
            if ProvideOutput(context) == 0 {
                return 0 as libc::c_int;
            }
        }
...
unsafe extern "C" fn CompressFiles(mut context: *mut Context) -> libc::c_int {
    while NextFile(context) != 0 {
        let mut is_ok: libc::c_int = 1 as libc::c_int;
        let mut s: *mut BrotliEncoderState = BrotliEncoderCreateInstance(
            None,
            None,
            0 as *mut libc::c_void,
        );
        ...
        if (*context).lgwin > 0 as libc::c_int {
            ...
            if (*context).input_file_length >= 0 as libc::c_int as int64_t {
                    ...
                    < (*context).input_file_length as uint64_t
                {
                    ...
                    lgwin;
                    if lgwin == 24 as libc::c_int as uint32_t {
                        break;
                    }
                }
            }
...
unsafe fn main_0(
            ...
            == COMMAND_TEST_INTEGRITY as libc::c_int as libc::c_uint
    {
        if is_ok != 0 {
            ...
            context
                .buffer = malloc(kFileBufferSize * 2 as libc::c_int as size_t)
                as *mut uint8_t;
