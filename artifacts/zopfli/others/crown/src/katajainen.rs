...
unsafe extern "C" fn BoundaryPM(
    ...
    (*lists.offset(index as isize))[0 as libc::c_int as usize] = oldchain;
    (*lists.offset(index as isize))[1 as libc::c_int as usize] = newchain;
...
unsafe extern "C" fn InitLists(
    ...
    i = 0 as libc::c_int;
    while i < maxbits {
        (*lists.offset(i as isize))[0 as libc::c_int as usize] = node0;
        (*lists.offset(i as isize))[1 as libc::c_int as usize] = node1;
        i += 1;
        i;
    }
}
unsafe extern "C" fn ExtractBitLengths(
    ...
    while !node.is_null() {
        ...
        node = (*node).tail;
    }std::intrinsics::assume((node).addr() == 0);
...
pub unsafe extern "C" fn ZopfliLengthLimitedCodeLengths(
    ...
    if numsymbols == 2 as libc::c_int {
        *bitlengths
            .offset((*leaves.offset(0 as libc::c_int as isize)).count as isize) = (*fresh9).wrapping_add(1); *bitlengths
            .offset((*leaves.offset(0 as libc::c_int as isize)).count as isize);
        *bitlengths
            .offset((*leaves.offset(1 as libc::c_int as isize)).count as isize) = (*fresh10).wrapping_add(1); *bitlengths
    ...
    while i < numBoundaryPMRuns - 1 as libc::c_int {
        BoundaryPM(lists, leaves, numsymbols, &raw mut pool, maxbits - 1 as libc::c_int);
        i += 1;
        i;
    }
    BoundaryPMFinal(lists, leaves, numsymbols, &raw mut pool, maxbits - 1 as libc::c_int);
    ExtractBitLengths(
        (*lists
