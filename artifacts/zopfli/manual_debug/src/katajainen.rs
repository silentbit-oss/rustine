...
pub fn InitLists(pool: &mut NodePool, leaves: &[Node], maxbits: usize, lists: &mut Vec<[Option<Box<Node>>; 2]>) {
    // Take ownership of the next two nodes from the pool
    let mut node0 = pool.next[pool.index].clone();
    pool.index += 1;
    let mut node1 = pool.next[pool.index].clone();
    pool.index += 1;
    ...
    // Populate the lists
    for i in 0..maxbits {
        lists.push([Some(node0.clone()), Some(node1.clone())]);
    }
}
...
pub fn BoundaryPM(
    lists: &mut Vec<[Option<Box<Node>>; 2]>,
    leaves: &[Node],
    numsymbols: i32,
    pool: &mut NodePool,
    index: usize,
    ...
    let mut newchain_idx = pool.next[pool.index].clone();
    pool.index += 1;
    let oldchain_idx = lists[index][1].clone();
    
    lists[index][0] = oldchain_idx.clone();
    lists[index][1] = Some(newchain_idx.clone());
    ...
    if index == 0 {
        ...
        if (lastcount < numsymbols) && (sum > leaves[lastcount as usize].weight) {
            ...
            //let newchain = right[0].as_mut();
            let oldchain_tail = oldchain_idx.unwrap().tail.clone();
            InitNode(
                leaves[lastcount as usize].weight,
                lastcount + 1,
                oldchain_tail,
            ...
            let (left, right) = lists[index - 1].split_at_mut(1);
            let newchain = right[0].as_mut().unwrap();
            let lists_index_1 = left[0].clone();
...
pub fn ZopfliLengthLimitedCodeLengths(
    ...
    let mut pool = NodePool { next: vec![Box::new(Node::default()); (maxbits * 2 * numsymbols * 10) as usize], index: 0 };
