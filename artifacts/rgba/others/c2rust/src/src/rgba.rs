...
pub unsafe extern "C" fn rgba_new(mut rgba: uint32_t) -> rgba_t {
    ...
    color
        .g = (rgba >> 16 as libc::c_int & 0xff as libc::c_int as uint32_t)
        as libc::c_double / 255 as libc::c_int as libc::c_double;
    color
        .b = (rgba >> 8 as libc::c_int & 0xff as libc::c_int as uint32_t)
        as libc::c_double / 255 as libc::c_int as libc::c_double;
    color
        .a = (rgba & 0xff as libc::c_int as uint32_t) as libc::c_double
        / 255 as libc::c_int as libc::c_double;
    return color;
}
#[no_mangle]
...
unsafe extern "C" fn rgba_from_hex_string(
    ...
    *ok = 1 as libc::c_int as libc::c_short;
    if 6 as libc::c_int as size_t == len {
        return rgba_from_hex6_string(str) as int32_t;
    }
    if 3 as libc::c_int as size_t == len {
...
pub unsafe extern "C" fn rgba_inspect(mut rgba: uint32_t) {
    printf(
        ...
        rgba >> 24 as libc::c_int & 0xff as libc::c_int as uint32_t,
        rgba >> 16 as libc::c_int & 0xff as libc::c_int as uint32_t,
        rgba >> 8 as libc::c_int & 0xff as libc::c_int as uint32_t,
        rgba & 0xff as libc::c_int as uint32_t,
    );
}
