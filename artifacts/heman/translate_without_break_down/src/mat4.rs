...
pub fn kmMat4Fill<'a>(pOut: &'a mut KmMat4, pMat: Option<&[f32]>) -> Option<&'a mut KmMat4> {
    ...
    // Ensure the slice has exactly 16 elements (size of kmMat4.mat)
    if pMat.len() != 16 {
        return None;
    }
...
pub fn kmMat4Identity(pOut: &mut KmMat4) -> &mut KmMat4 {
    ...
    pOut.mat.iter_mut().for_each(|x| *x = 0.0f32);
...
pub fn kmMat4Assign<'a>(pOut: &'a mut KmMat4, pIn: &KmMat4) -> &'a mut KmMat4 {
    // Use Rust's assert macro to check for self-assignment
    assert!(pOut as *const _ != pIn as *const _, "You have tried to self-assign!!");
    
    // Use Rust's array copy instead of memcpy
    pOut.mat.copy_from_slice(&pIn.mat);
    
    pOut
}

pub fn kmMat4AreEqual(pMat1: &KmMat4, pMat2: &KmMat4) -> i32 {
    // Assert that the matrices are not the same object
    assert!(pMat1 as *const _ != pMat2 as *const _, "You are comparing the same thing!");

    for i in 0..16 {
        // Check if the values are approximately equal within a tolerance of 0.0001
...
pub fn kmMat4ExtractPlane<'a>(pOut: &'a mut KmPlane, pIn: &KmMat4, plane: u32) -> &'a mut KmPlane {
    ...
    let t = ((pOut.a * pOut.a + pOut.b * pOut.b) + pOut.c * pOut.c).sqrt();
    pOut.a /= t;
    pOut.b /= t;
    pOut.c /= t;
    pOut.d /= t;
...
pub fn kmMat4AssignMat3<'a>(pOut: &'a mut KmMat4, pIn: &KmMat3) -> &'a mut KmMat4 {
    kmMat4Identity(pOut);
...
pub fn kmMat4RotationTranslation<'a>(
    pOut: &'a mut KmMat4,
...
pub fn kmMat4GetUpVec3<'a>(pOut: &'a mut KmVec3, pIn: &KmMat4) -> &'a mut KmVec3 {
    // First operation: multiply
    kmVec3MultiplyMat4(pOut, &KM_VEC3_POS_Y, pIn);
    
    // Second operation: normalize
    kmVec3Normalize(Some(pOut), None).unwrap()
}
...
pub fn kmMat4GetForwardVec3RH<'a>(
    ...
    // Normalize the resulting vector - we only need mutable access
    kmVec3Normalize(Some(pOut), None)
}
pub fn kmMat4GetForwardVec3LH<'a>(pOut: &'a mut KmVec3, pIn: &KmMat4) -> &'a mut KmVec3 {
    // First operation: multiply
    kmVec3MultiplyMat4(pOut, &KM_VEC3_POS_Z, pIn);
    
    // Inlined normalization: replaces kmVec3Normalize call
...
pub fn km_mat4_rotation_axis_angle<'a>(
    ...
    let axis = axis?; // Return None if axis is None
    ...
    kmQuaternionRotationAxisAngle(Some(&mut quat), Some(axis), radians)?;
    km_mat4_rotation_quaternion(p_out, &quat);
...
pub fn kmMat4LookAt<'a>(
    ...
    // Recalculate up vector (u) as cross product of s and f
    ...
    // Create translation matrix and multiply with rotation matrix
    kmMat4Translation(&mut translate, -pEye.x, -pEye.y, -pEye.z);
    let out_copy = pOut.clone();
    kmMat4Multiply(pOut, &out_copy, &translate);
