...
pub fn kmLine2WithLineIntersection(
    ...
    if let Some(out_inter) = outIntersection {
        out_inter.x = x;
        out_inter.y = y;
    }
...
pub fn kmSegment2WithSegmentIntersection(
    ...
    if intersects && (0.0 <= ua) && (ua <= 1.0) && (0.0 <= ub) && (ub <= 1.0) {
...
pub fn kmRay2IntersectLineSegment(
    ...
    let mut ub = 0.0;
    let mut pt = KmVec2 { x: 0.0, y: 0.0 };
    let mut other_segment = KmRay2 {
        start: KmVec2 { x: 0.0, y: 0.0 },
        dir: KmVec2 { x: 0.0, y: 0.0 },
    };

    kmRay2FillWithEndpoints(&mut other_segment, p1, p2);
...
pub fn calculate_line_normal(
    ...
    // Check if normal_out is None (equivalent to NULL check in C)
    ...
    // Normalize edges - first create copies for immutable references
    let edge_copy = edge.clone();
    kmVec2Normalize(Some(&mut edge), Some(&edge_copy));
    let other_edge_copy = other_edge.clone();
    kmVec2Normalize(Some(&mut other_edge), Some(&other_edge_copy));
    ...
    let d = kmVec2Dot(Some(&n), Some(&other_edge)).unwrap_or(0.0);
    if d > 0.0 {
        n.x = -n.x;
        n.y = -n.y;
    }
    ...
    // Normalize output - create copy for immutable reference
    let normal_out_copy = normal_out.clone();
    kmVec2Normalize(Some(normal_out), Some(&normal_out_copy));
}
pub fn kmRay2IntersectTriangle(
    ...
    if kmRay2IntersectLineSegment(ray, p1, p2, Some(&mut intersect)) != 0 {
        ...
        if this_distance < distance && kmVec2Dot(Some(&this_normal), Some(&ray.dir)).unwrap_or(1.0) < 0.0 {
...
pub fn kmRay2IntersectBox(
    ...
    for i in 0..4 {
        ...
        let mut tmp_intersect = KmVec2 { x: 0.0, y: 0.0 };
        if kmRay2IntersectLineSegment(ray, this_point, next_point, Some(&mut tmp_intersect)) != 0 {
            ...
            let dot_product = kmVec2Dot(Some(&this_normal), Some(&ray.dir)).unwrap_or(0.0);
            if this_distance < distance && dot_product < 0.0 {
                kmVec2Assign(Some(&mut final_intersect), Some(&tmp_intersect));
                distance = this_distance;
                intersected = 1;
                kmVec2Assign(Some(&mut normal), Some(&this_normal));
            }
