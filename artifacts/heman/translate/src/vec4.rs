...
pub fn kmVec4Dot(pV1: Option<&KmVec4>, pV2: Option<&KmVec4>) -> Option<f32> {
    ...
    // Calculate dot product using safe references
    Some(
...
pub fn kmVec4Div<'a>(pOut: &'a mut KmVec4, pV1: &KmVec4, pV2: &KmVec4) -> Option<&'a mut KmVec4> {
    // Check if none of the components of pV2 are zero to avoid division by zero
...
pub fn kmVec4Assign<'a>(pOut: &'a mut KmVec4, pIn: &KmVec4) -> &'a mut KmVec4 {
    // Assert that pOut and pIn are not the same reference
    assert!(!std::ptr::eq(pOut, pIn), "pOut and pIn must not be the same");

    // Directly copy the fields since Rust allows this safely
    pOut.x = pIn.x;
    pOut.y = pIn.y;
    pOut.z = pIn.z;
    pOut.w = pIn.w;
...
pub fn kmVec4Length(pIn: Option<&KmVec4>) -> f32 {
    ...
    let pIn = match pIn {
        Some(vec) => vec,
        None => return f32::NAN, // Return NaN for NULL input (similar to domain error in C)
    };

    // Calculate the squared components and sum them
    let sum = kmSQR(pIn.x) + kmSQR(pIn.y) + kmSQR(pIn.z) + kmSQR(pIn.w);
    
    // Compute the square root using Rust's built-in sqrt method
    sum.sqrt()
}
...
pub fn kmVec4Normalize<'a>(pOut: &'a mut KmVec4, pIn: Option<&KmVec4>) -> Option<&'a mut KmVec4> {
    // Check if pIn is None (NULL in C)
    let pIn = match pIn {
        Some(p) => p,
        None => return None,
    };

    // Check if all components are zero (using approx_eq for floating point comparison)
