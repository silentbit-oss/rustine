...
pub fn kmMat3Fill<'a>(pOut: &'a mut KmMat3, pMat: Option<&[f32]>) -> Option<&'a mut KmMat3> {
    ...
    // Ensure the slice has exactly 9 elements (for 3x3 matrix)
    if pMat.len() != 9 {
        return None;
    }
...
pub fn kmMat3Determinant(pIn: &KmMat3) -> f32 {
    let output = (((pIn.mat[0] * pIn.mat[4]) * pIn.mat[8]) + ((pIn.mat[1] * pIn.mat[5]) * pIn.mat[6])) + ((pIn.mat[2] * pIn.mat[3]) * pIn.mat[7]);
    let output = output - (((pIn.mat[2] * pIn.mat[4]) * pIn.mat[6]) + ((pIn.mat[0] * pIn.mat[5]) * pIn.mat[7])) + ((pIn.mat[1] * pIn.mat[3]) * pIn.mat[8]);
    output
}
...
pub fn kmMat3ScalarMultiply<'a>(pOut: &'a mut KmMat3, pM: &KmMat3, pFactor: f32) -> &'a mut KmMat3 {
    ...
    // Safe because:
    // 1. Both pOut.mat and mat are valid arrays of the same size (9 f32 elements)
    // 2. The memory regions don't overlap (mat is local and pOut is a reference)
    unsafe {
        ptr::copy_nonoverlapping(mat.as_ptr(), pOut.mat.as_mut_ptr(), 9);
    }
    
    pOut
}
pub fn kmMat3Assign<'a>(pOut: &'a mut KmMat3, pIn: &KmMat3) -> &'a mut KmMat3 {
    // In Rust, we don't need to explicitly check for pointer equality since references can't be null
    // and the borrow checker prevents aliasing of mutable references
    assert!(!std::ptr::eq(pOut, pIn), "pOut and pIn must not be the same object");

    // Direct array assignment in Rust is safe and performs a copy
    pOut.mat = pIn.mat;
...
pub fn km_mat3_rotation_axis_angle<'a>(
    p_out: &'a mut KmMat3,
    axis: &KmVec3,
    radians: f32,
) -> &'a mut KmMat3 {
...
pub fn kmMat3GetRightVec3<'a>(pOut: Option<&'a mut KmVec3>, pIn: Option<&'a KmMat3>) -> Option<&'a mut KmVec3> {
    ...
    // Assign values from pIn.mat to pOut
    pOut.x = pIn.mat[0];
    pOut.y = pIn.mat[1];
    pOut.z = pIn.mat[2];
...
pub fn kmMat3GetForwardVec3<'a>(pOut: Option<&'a mut KmVec3>, pIn: Option<&'a KmMat3>) -> Option<&'a mut KmVec3> {
    ...
    // Assign values directly to pOut
    pOut.x = pIn.mat[6];
    pOut.y = pIn.mat[7];
    pOut.z = pIn.mat[8];

    // Normalize pOut in place
    let length_squared = pOut.x * pOut.x + pOut.y * pOut.y + pOut.z * pOut.z;
    if length_squared < f32::EPSILON * f32::EPSILON {
        // Avoid division by zero; set to zero vector
        pOut.x = 0.0;
        pOut.y = 0.0;
        pOut.z = 0.0;
    } else {
...
pub fn kmMat3Inverse<'a>(pOut: &'a mut KmMat3, pM: &KmMat3) -> Option<&'a mut KmMat3> {
    let determinate = kmMat3Determinant(pM);
    if determinate == 0.0 {
        return None;
    }
...
pub fn kmMat3RotationToAxisAngle<'a>(
    ...
    // We can safely unwrap here because we checked for None above
    kmQuaternionToAxisAngle(
        Some(&temp),
        pAxis.as_deref_mut(),
        radians.as_deref_mut(),
    );

    // Return pAxis - since we've already checked it's Some, we can unwrap safely
    pAxis
}
pub fn kmMat3LookAt<'a>(
    pOut: Option<&'a mut KmMat3>,
    pEye: Option<&'a KmVec3>,
    pCenter: Option<&'a KmVec3>,
    pUp: Option<&'a KmVec3>,
) -> Option<&'a mut KmMat3> {
    // Early return if any input is None
    if pOut.is_none() || pEye.is_none() || pCenter.is_none() || pUp.is_none() {
        return Option::None;
    }

    // Unwrap all options safely since we've checked they're Some
    let pOut = pOut.unwrap();
    let pEye = pEye.unwrap();
    let pCenter = pCenter.unwrap();
    let pUp = pUp.unwrap();

    // Create local vectors (no need for Option here since they're stack-allocated)
    let mut f = KmVec3 {
        x: 0.0,
        y: 0.0,
        z: 0.0,
    ...
    let f_normalized = {
        ...
        kmVec3Normalize(Some(&mut temp), Some(&f));
        temp
    };
    f = f_normalized;
    ...
    let up_normalized = {
        let mut temp = up.clone();
        kmVec3Normalize(Some(&mut temp), Some(&up));
        temp
    };
    up = up_normalized;
    ...
    let s_normalized = {
        ...
        kmVec3Normalize(Some(&mut temp), Some(&s));
        temp
    };
    s = s_normalized;
