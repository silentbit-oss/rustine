...
unsafe extern "C" fn get_window_color(
    ...
    let mut status: libc::c_int = 0;
    root_window = XRootWindow(display, XDefaultScreen(display));
    target_window = select_window(display, &mut x, &mut y);
    log_debug(
...
unsafe fn main_0(
    ...
    while i < argc {
        ...
        match *option.offset(1 as libc::c_int as isize) as libc::c_int {
            ...
            87 => {
                ...
                let mut window: Window = select_window(display, &mut x, &mut y);
                if window != 0 as *mut libc::c_void as Window {
                    log_debug(
    ...
    target_window = get_window_color(display, &mut color);
    if target_window != 0 as *mut libc::c_void as Window {
        status = XGetWindowAttributes(display, target_window, &mut window_attributes);
        ...
        XQueryColor(display, window_attributes.colormap, &mut color);
