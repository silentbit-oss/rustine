...
pub fn zdivmod(a: &mut z_t, b: &mut z_t, c: &mut z_t, d: &mut z_t) {
...

    {
        ...
        let mut tmp_d_copy = [Z { sign: tmp_d[0].sign, used: tmp_d[0].used, alloced: tmp_d[0].alloced, chars: tmp_d[0].chars.clone() }];
        if zcmpmag(&mut tmp_d_copy, c) > 0 {
    ...
    if bit < 32 {
        loop {
            let (should_break, new_bit) = {
                ...
                let mut tmp_b = libzahl_tmp_divmod_b.lock().unwrap();
                let mut tmp_b_copy = [Z { sign: tmp_b[0].sign, used: tmp_b[0].used, alloced: tmp_b[0].alloced, chars: tmp_b[0].chars.clone() }];
                
                let mut tmp_a = libzahl_tmp_divmod_a.lock().unwrap();
                let mut tmp_a_copy = [Z { sign: tmp_a[0].sign, used: tmp_a[0].used, alloced: tmp_a[0].alloced, chars: tmp_a[0].chars.clone() }];

                if zcmpmag(&mut tmp_d_copy, &mut tmp_b_copy) <= 0 {
        ...
        loop {
            let (should_break, new_bit) = {
                let mut tmp_ds = LIBZAHL_TMP_DIVMOD_DS.lock().unwrap();
                let mut tmp_ds_copies: Vec<z_t> = tmp_ds.iter().map(|z| [Z { sign: z[0].sign, used: z[0].used, alloced: z[0].alloced, chars: z[0].chars.clone() }]).collect();
                ...
                for i in 0..32 {
                    if zcmpmag(&mut tmp_ds_copies[i], &mut tmp_b_copy) <= 0 {
