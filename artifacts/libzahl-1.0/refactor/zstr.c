...
char *zstr(z_t a, char *b)
{
  ...
  for (;;)
  {
    helper_zstr_1(&b_idx, &n, &len, &overridden, b, buf);
  }
...
void helper_zstr_1(unsigned int * const b_idx_ref, size_t * const n_ref, size_t * const len_ref, char * const overridden_ref, char * const b, char buf[9 + 1])
{
