...
pub unsafe extern "C" fn zsub_unsigned(
    ...
    while i < n {
        carry[(!i & 1 as libc::c_int as size_t)
            as usize] = (if carry[(i & 1 as libc::c_int as size_t) as usize] != 0 {
            (*((*a).chars).offset(i as isize) <= *s.offset(i as isize)) as libc::c_int
        } else {
            (*((*a).chars).offset(i as isize) < *s.offset(i as isize)) as libc::c_int
        }) as zahl_char_t;
        let ref mut fresh0 = *((*a).chars).offset(i as isize);
        *fresh0 = (*fresh0).wrapping_sub(*s.offset(i as isize));
        let ref mut fresh1 = *((*a).chars).offset(i as isize);
        *fresh1 = (*fresh1)
            .wrapping_sub(carry[(i & 1 as libc::c_int as size_t) as usize]);
        i = i.wrapping_add(1);
        i;
    }
