...
pub unsafe extern "C" fn libzahl_realloc(mut a: *mut C2RustUnnamed, mut need: size_t) {
    ...
    if need & (!need).wrapping_add(1 as libc::c_int as size_t) != need {
        ...
        need = need.wrapping_add(1 as libc::c_int as size_t);
    }
    ...
    if libzahl_pool_n[i as usize] != 0 {
        ...
        (*a)
            ...
            need.wrapping_mul(::core::mem::size_of::<zahl_char_t>() as libc::c_ulong),
        ) as *mut zahl_char_t;
        if ((*a).chars).is_null() {
