...
extern "C" {
    ...
    #[no_mangle]
    fn read(__fd: std::os::raw::c_int, __buf: * mut core::ffi::c_void, __nbytes: std::os::raw::c_ulong)
     -> std::os::raw::c_long;
    ...
    #[no_mangle]
    fn memchr(_: * const core::ffi::c_void, _: std::os::raw::c_int, _: std::os::raw::c_ulong)
     -> * mut core::ffi::c_void;
}
...
unsafe extern "C" fn gz_load<'a1>(mut state: * mut crate::src::zlib::gzlib::gz_state,
                             mut buf: * mut std::os::raw::c_uchar,
                             mut len: std::os::raw::c_uint,
                             mut have: Option<&'a1 mut std::os::raw::c_uint>) -> std::os::raw::c_int {
    let mut ret: i32 = 0;
    let mut get: u32 = 0;
    let mut max: u32 =
        (-(1 as std::os::raw::c_int) as std::os::raw::c_uint >>
             2 as std::os::raw::c_int).wrapping_add(1 as std::os::raw::c_int as std::os::raw::c_uint);
    *(borrow_mut(&mut have)).unwrap() = 0 as std::os::raw::c_int as std::os::raw::c_uint;
    loop  {
        get = len.wrapping_sub(*(borrow(& have)).unwrap());
        if get > max { get = max }
        ret =
            read((*state).fd, buf.offset(*(borrow(& have)).unwrap() as isize) as *mut std::os::raw::c_void,
                 get as size_t) as std::os::raw::c_int;
        if ret <= 0 as std::os::raw::c_int { break ; }
        *(borrow_mut(&mut have)).unwrap() = (*(borrow(& have)).unwrap()).wrapping_add(ret as std::os::raw::c_uint);
        if !(*(borrow(& have)).unwrap() < len) { break ; }
    }
...
unsafe extern "C" fn gz_avail(mut state: * mut crate::src::zlib::gzlib::gz_state) -> std::os::raw::c_int {
    let mut got: u32 = 0;
    ...
    if (*state).eof == 0 as std::os::raw::c_int {
        if (*(borrow(& strm)).unwrap()).avail_in != 0 {
            ...
            let mut p: * mut u8 = (*state).in_0;
            let mut q: * const u8 = (*(borrow(& strm)).unwrap()).next_in as *mut u8;
            let mut n: u32 = (*(borrow_mut(&mut strm)).unwrap()).avail_in;
            loop  {
                let mut fresh0 = q;
                q = q.offset(1);
                let mut fresh1 = p;
        ...
        if gz_load(state, (*state).in_0.offset((*(borrow(& strm)).unwrap()).avail_in as isize),
                   (*state).size.wrapping_sub((*(borrow(& strm)).unwrap()).avail_in), Some(&mut got)) ==
               -(1 as std::os::raw::c_int) {
            return -(1 as std::os::raw::c_int)
        }
        (*(borrow_mut(&mut strm)).unwrap()).avail_in =
            ((*(borrow(& strm)).unwrap()).avail_in as std::os::raw::c_uint).wrapping_add(got) as uInt as
                uInt;
...
unsafe extern "C" fn gz_look(mut state: * mut crate::src::zlib::gzlib::gz_state) -> std::os::raw::c_int {
...
unsafe extern "C" fn gz_decomp(mut state: * mut crate::src::zlib::gzlib::gz_state) -> std::os::raw::c_int {
...
unsafe extern "C" fn gz_fetch(mut state: * mut crate::src::zlib::gzlib::gz_state) -> std::os::raw::c_int {
    let mut strm: Option<&'_ mut crate::src::libpng::png::z_stream_s> = Some(&mut (*state).strm);
    loop  {
        match (*state).how {
            0 => {
            ...
            1 => {
                ...
                if gz_load(state, (*state).out,
                           ...
                           Some(&mut (*state).x.have)) == -(1 as std::os::raw::c_int) {
                    return -(1 as std::os::raw::c_int)
                }
                (*state).x.next = (*state).out;
                return 0 as std::os::raw::c_int
            ...
            2 => {
                ...
                (*(borrow_mut(&mut strm)).unwrap()).avail_out = (*state).size << 1 as std::os::raw::c_int;
                (*(borrow_mut(&mut strm)).unwrap()).next_out = (*state).out;
        ...
        if !((*state).x.have == 0 as std::os::raw::c_int as std::os::raw::c_uint &&
                 ((*state).eof == 0 || (*(borrow(& strm)).unwrap()).avail_in != 0)) {
            break ;
        }
    }
    return 0 as std::os::raw::c_int;
...
unsafe extern "C" fn gz_skip(mut state: * mut crate::src::zlib::gzlib::gz_state, mut len: std::os::raw::c_long)
 -> std::os::raw::c_int {
    let mut n: u32 = 0;
...
unsafe extern "C" fn gz_read(mut state: * mut crate::src::zlib::gzlib::gz_state, mut buf: * mut core::ffi::c_void,
        ...
        if (*state).x.have != 0 {
           ...
           buffer */
            ...
            if (*state).how == 1 as std::os::raw::c_int {
                /* read directly */
                if gz_load(state, buf as *mut std::os::raw::c_uchar, n, Some(&mut n)) ==
                       -(1 as std::os::raw::c_int) {
                    return 0 as std::os::raw::c_int as z_size_t
                }
            } else {
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzread(mut file: * mut crate::src::zlib::gzclose::gzFile_s, mut buf: * mut core::ffi::c_void,
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzfread(mut buf: * mut core::ffi::c_void, mut size: std::os::raw::c_ulong,
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzgetc(mut file: * mut crate::src::zlib::gzclose::gzFile_s) -> std::os::raw::c_int {
    let mut ret: i32 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn gzgetc_(mut file: * mut crate::src::zlib::gzclose::gzFile_s) -> std::os::raw::c_int {
    return gzgetc(file);
}
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzungetc(mut c: std::os::raw::c_int, mut file: * mut crate::src::zlib::gzclose::gzFile_s)
 -> std::os::raw::c_int {
    let mut state: * mut crate::src::zlib::gzlib::gz_state = 0 as *mut gz_state;
    /* get internal structure */
    if file.is_null() { return -(1 as std::os::raw::c_int) }
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzgets(mut file: * mut crate::src::zlib::gzclose::gzFile_s, mut buf: * mut std::os::raw::c_char,
                                mut len: std::os::raw::c_int) -> * mut std::os::raw::c_char {
    let mut left: u32 = 0;
    let mut n: u32 = 0;
    let mut str: * mut i8 = 0 as *mut std::os::raw::c_char;
    let mut eol: * mut u8 = 0 as *mut std::os::raw::c_uchar;
    let mut state: * mut crate::src::zlib::gzlib::gz_state = 0 as *mut gz_state;
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzdirect(mut file: * mut crate::src::zlib::gzclose::gzFile_s) -> std::os::raw::c_int {
    let mut state: * mut crate::src::zlib::gzlib::gz_state = 0 as *mut gz_state;
    /* get internal structure */
...
/* -- see zlib.h -- */
#[no_mangle]
pub unsafe extern "C" fn gzclose_r(mut file: * mut crate::src::zlib::gzclose::gzFile_s) -> std::os::raw::c_int {
    let mut ret: i32 = 0;
    let mut err: i32 = 0;
    let mut state: * mut crate::src::zlib::gzlib::gz_state = 0 as *mut gz_state;
    /* get internal structure */
