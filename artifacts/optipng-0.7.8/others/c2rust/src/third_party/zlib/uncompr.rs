...
pub unsafe extern "C" fn uncompress2(
    ...
    loop {
        ...
        if stream.avail_in == 0 as libc::c_int as uInt {
            ...
            len = len.wrapping_sub(stream.avail_in as uLong);
        }
    ...
    *sourceLen = (*sourceLen).wrapping_sub(len.wrapping_add(stream.avail_in as uLong));
    ...
    return if err == 1 as libc::c_int {
        ...
        && left.wrapping_add(stream.avail_out as uLong) != 0
    {
        -(3 as libc::c_int)
    } else {
        err
