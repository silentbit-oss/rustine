...
pub fn opl_emu_opl_clock_noise_and_lfo(
    ...
    let mut lfo_pm_counter_idx = 0;
    let mut lfo_am_counter_idx = 0;
    ...
    *noise_lfsr |= ((opl_emu_bitfield(*noise_lfsr, 23, 1) ^ opl_emu_bitfield(*noise_lfsr, 9, 1))
    ...
    let am_counter = lfo_am_counter[lfo_am_counter_idx] as u32;
    ...
    *lfo_am = ((if am_counter < (105 * 64) {
    ...
    let pm_counter = lfo_pm_counter[lfo_pm_counter_idx] as u32;
...
pub fn opl_loadbank_ibk(opl: &mut OplT, file: Option<&CStr>) -> i32 {
    ...
    // Convert CStr to &str (UTF-8) safely
    ...
    let mut parts = file_str.splitn(2, ',');
    let instruments_part = parts.next().unwrap_or("");
    let percussion_part = parts.next().unwrap_or("");

    // Convert to Path
    let instruments_path = std::path::Path::new(instruments_part);
    let percussion_path = std::path::Path::new(percussion_part);

    // First load the instruments part
    let res = match opl_loadbank_internal(opl, instruments_path, 0) {
        Ok(_) => 0,
        Err(e) => e,
    };

    // Then load percussion if available and first load succeeded
    let res = if res == 0 && !percussion_part.is_empty() {
        match opl_loadbank_internal(opl, percussion_path, 128) {
            Ok(_) => 0,
            Err(e) => e,
        }
    } else {
...
pub fn opl_emu_generate(emu: &mut OplEmuT, output: &mut [i16], numsamples: u32, volume: f32) {
    let volume = volume.clamp(0.0, 1.0); // Clamp volume between 0.0 and 1.0
    let mut output_idx = 0;

    for _ in 0..numsamples {
        opl_emu_clock(emu, (1 << 18) - 1);
        
        // We need to split the mutable borrow of output to avoid overlapping
        {
            let output_slice = &mut output[output_idx..output_idx + 2];
            opl_emu_out(emu, output_slice, 0, 32767, (1 << 18) - 1);
            
            // Apply volume to both channels
            output_slice[0] = (output_slice[0] as f32 * volume) as i16;
            output_slice[1] = (output_slice[1] as f32 * volume) as i16;
        }
...
pub fn opl_destroy(opl: Option<Box<OplT>>) {
    // Check if opl is None (equivalent to NULL check in C)
    if opl.is_none() {
        return;
    }
