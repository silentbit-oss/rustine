...
pub fn destroy(avlt: &mut AvlTree, n: Option<Box<AvlNode>>) {
    if let Some(mut node) = n {
        ...
        if let Some(ref destroy_fn) = avlt.destroy {
...
pub fn avl_create(
    compare_func: Option<Box<dyn Fn(&dyn std::any::Any, &dyn std::any::Any) -> i32>>,
    destroy_func: Option<Box<dyn Fn(&dyn std::any::Any)>>,
) -> Option<Box<AvlTree>> {
    // Create separate nil nodes for root's pointers
    let nil_node_left = Box::new(AvlNode {
        left: None,
        right: None,
        parent: None,
        bf: 0,
    ...
    let nil_node_right = Box::new(AvlNode {
        left: None,
        right: None,
        parent: None,
        bf: 0,
    ...
    let nil_node_parent = Box::new(AvlNode {
        left: None,
        right: None,
        parent: None,
        bf: 0,
    ...
    let root_node = Box::new(AvlNode {
        left: Some(nil_node_left),
        right: Some(nil_node_right),
        parent: Some(nil_node_parent),
        bf: 0,
        data: None,
    });
...
pub fn avl_find<'a>(avlt: &'a AvlTree, data: &dyn std::any::Any) -> Option<&'a AvlNode> {
    // Start from the root node
    let mut p = avlt.root.as_ref()?;
    
    // While we haven't reached the nil node
    loop {
        // Compare the data using the tree's comparison function
        let cmp = avlt.compare.as_ref()?.as_ref()(data, p.data.as_ref()?);
...
pub fn check_order(
    ...
    let n = n.unwrap();
    let compare = avlt.compare.as_ref().unwrap();

    // Get references to the data that live long enough for the comparison
    let n_data = n.data.as_ref().unwrap();
    let min_val = min.unwrap();
    let max_val = max.unwrap();

    // Check if data is out of bounds (equivalent to avlt->compare(n->data, min) < 0 || ... > 0)
    if compare(n_data, min_val) < 0 || compare(n_data, max_val) > 0 {
        return false;
    }

    // Recursively check left and right subtrees
    check_order(avlt, n.left.as_ref(), min, Some(n_data))
        && check_order(avlt, n.right.as_ref(), Some(n_data), max)
}
...
pub fn avl_insert(avlt: &mut AvlTree, data: Box<dyn Any>) -> Option<Box<AvlNode>> {
    ...
    let mut current = &avlt.root;
    let mut parent_ptr: Option<*mut AvlNode> = None;
    
    // Find insertion point
    while let Some(node) = current {
        let cmp = avlt.compare.as_ref().unwrap()(&data, node.data.as_ref().unwrap());
    ...
    if let Some(parent) = parent_ptr {
        unsafe {
            ...
            let cmp = avlt.compare.as_ref().unwrap()(
