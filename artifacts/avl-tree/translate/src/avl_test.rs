...
pub fn tree_create() -> Option<Box<AvlTree>> {
    avl_create(
        Some(Box::new(|a: &dyn std::any::Any, b: &dyn std::any::Any| -> i32 {
            let d1 = a.downcast_ref::<Mydata>();
            let d2 = b.downcast_ref::<Mydata>();
            compare_func(d1, d2)
        })),
        Some(Box::new(|a: &dyn std::any::Any| {
            let d = a.downcast_ref::<Mydata>();
            if let Some(d) = d {
                destroy_func(Some(Box::new(d.clone())));
            }
        })),
    )
}
...
pub fn unit_test_find() -> i32 {
    ...
    if avl_find(&avlt, &('R' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('R' as i32)) ||
       avl_find(&avlt, &('E' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('E' as i32)) ||
       avl_find(&avlt, &('D' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('D' as i32)) ||
       avl_find(&avlt, &('S' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('S' as i32)) ||
       avl_find(&avlt, &('O' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('O' as i32)) ||
       avl_find(&avlt, &('X' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('X' as i32)) ||
       avl_find(&avlt, &('C' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('C' as i32)) ||
       avl_find(&avlt, &('U' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('U' as i32)) ||
       avl_find(&avlt, &('B' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('B' as i32)) ||
       avl_find(&avlt, &('T' as i32)).and_then(|n| n.data.as_ref().and_then(|d| d.downcast_ref::<i32>())) != Some(&('T' as i32))
    {
        println!("find failed");
        avl_destroy(avlt);
        return 0;
...
pub fn unit_test_create() -> i32 {
    ...
    let conditions_met = {
        ...
        let compare_ok = avlt.compare.as_ref().map_or(false, |f| {
            let f_ptr: *const _ = &**f;
            let wrapper = |a: &dyn std::any::Any, b: &dyn std::any::Any| -> i32 {
                let d1 = a.downcast_ref::<Mydata>();
                let d2 = b.downcast_ref::<Mydata>();
                compare_func(d1, d2)
            };
            let cmp_ptr: *const _ = &wrapper as *const _;
            f_ptr == cmp_ptr
        });
        
        let destroy_ok = avlt.destroy.as_ref().map_or(false, |f| {
            let f_ptr: *const _ = &**f;
            let wrapper = |a: &dyn std::any::Any| {
                let d = a.downcast_ref::<Mydata>().map(|x| Box::new(x.clone()));
                destroy_func(d)
            };
            let dest_ptr: *const _ = &wrapper as *const _;
            f_ptr == dest_ptr
        });
...
pub fn all_tests() {
    ...
    {
        MU_TESTS.fetch_add(1, Ordering::SeqCst);
        println!("#{:03} {}", MU_TESTS.load(Ordering::SeqCst), "unit_test_permutation_insertion");
    ...
    {
        MU_TESTS.fetch_add(1, Ordering::SeqCst);
        println!("#{:03} {}", MU_TESTS.load(Ordering::SeqCst), "unit_test_permutation_deletion");
