...
pub type binn = Option<BinnStruct>;
...
pub fn binn_ptr(ptr: Option<*mut u8>) -> Option<*mut u8> {
    ...
    match binn_get_ptr_type(ptr) {
        1 => {
            if let Some(p) = ptr {
                let mut item_val = Some(unsafe { std::ptr::read(p as *const BinnStruct) });
...
pub fn binn_memdup(src: Option<&[u8]>, size: i32) -> Option<Box<[u8]>> {
    ...
    if let Ok(mut dest_box) = dest.downcast::<Box<[u8]>>() {
        ...
        let dest_slice_t = &mut dest_slice[..src.len()];
        dest_slice_t.copy_from_slice(src);
...
pub fn binn_alloc_item() -> Option<BinnStruct> {
	println!("\n* binn_alloc_item");
    ...
    match storage_type {
        ...
        0xA0 | 0xC0 | 0xE0 => {
            ...
            if storage_type == 0xA0 && size == 0 {
                let c_str = unsafe { CStr::from_ptr(pvalue.as_ptr() as *const u8) };
...
pub fn CheckAllocation(item: &mut Option<BinnStruct>, add_size: i32) -> BOOL {
	println!("\n* CheckAllocation");
...
pub fn binn_create(item: &mut Option<BinnStruct>, type_: i32, size: i32, pointer: Option<&mut [u8]>) -> BOOL {
	println!("\n* binn_create");
    ...
    *item = Some(BinnStruct {
    ...
    if let Some(_ptr) = pointer {
        item.as_mut().unwrap().pre_allocated = 1;
        item.as_mut().unwrap().pbuf = Some(Box::new(())); // Simplified representation of pointer assignment
        item.as_mut().unwrap().alloc_size = actual_size;
    } else {
        item.as_mut().unwrap().pre_allocated = 0;
        ...
        //}
        item.as_mut().unwrap().pbuf = Some(Box::new(())); // Simplified representation of pointer assignment
        item.as_mut().unwrap().alloc_size = alloc_size;
    }

    item.as_mut().unwrap().header = 0x1F22B11F;
    item.as_mut().unwrap().writable = 1;
    item.as_mut().unwrap().used_size = 9;
    item.as_mut().unwrap().type_ = type_;
    item.as_mut().unwrap().dirty = 1;
    
    1
}
pub fn binn_new(type_: i32, size: i32, pointer: Option<&mut [u8]>) -> Option<BinnStruct> {
	println!("\n* binn_new");
    ...
    let mut item: Option<BinnStruct> = None;
...
pub fn binn_list() -> Option<BinnStruct> {
	println!("\n* binn_list");
    binn_new(0xE0, 0, None)
}
pub fn binn_map() -> Option<BinnStruct> {
	println!("\n* binn_map");
    binn_new(0xE1, 0, None)
}
pub fn binn_object() -> Option<BinnStruct> {
	println!("\n* binn_object");
    binn_new(0xE2, 0, None)
}
pub fn binn_create_list(list: &mut Option<BinnStruct>) -> BOOL {
	println!("\n* binn_create_list");
    binn_create(list, 0xE0, 0, None)
}
pub fn binn_create_map(map: &mut Option<BinnStruct>) -> BOOL {
	println!("\n* binn_create_map");
    binn_create(map, 0xE1, 0, None)
}
pub fn binn_create_object(object: &mut Option<BinnStruct>) -> BOOL {
	println!("\n* binn_create_object");
    binn_create(object, 0xE2, 0, None)
}
pub fn binn_free(item: &mut Option<BinnStruct>) {
	println!("\n* binn_free");
    let _ = item.take();
    //if item.is_none() {
    //    return;
...
pub fn binn_size(ptr: Option<*mut u8>) -> i32 {
    ...
    match binn_get_ptr_type(ptr) {
        1 => {
            let mut item = unsafe { ptr.unwrap() as *mut BinnStruct };
            //let mut item = Some(Box::new(*item));
            
            let needs_save = {
                //let item_ref = item.as_ref().unwrap();
                unsafe {(*item).writable != 0 && (*item).dirty != 0 }
            };
            
            let size = unsafe {(*item).size};
            //if needs_save {
            //    binn_save_header(&mut Some(*item));
            //}
...
pub fn binn_type(ptr: Option<*mut u8>) -> i32 {
	println!("\n* binn_type");
    let item: Option<BinnStruct>;
    let mut item_idx: u32 = 0;
    
    match binn_get_ptr_type(ptr) {
        1 => {
            ////item = unsafe { ptr.map(|p| *(p as * BinnStruct)) };
            //item = unsafe { ptr.map(|p| *p) }; // If BinnStruct is Copy
            //item.as_ref().map(|i| i.type_).unwrap_or(-1)
            unsafe {ptr.map(|p| (*(p as *mut BinnStruct)).type_).unwrap() }
        }
...
pub fn binn_count(ptr: Option<*mut u8>) -> i32 {
    ...
    match binn_get_ptr_type(ptr) {
        ...
        2 => binn_buf_count( 
            Some(unsafe {std::slice::from_raw_parts(ptr.unwrap() as *const u8, std::mem::size_of::<BinnStruct>())})
        ), // Assuming ptr can be converted to Option<&[u8]>
        _ => -1,
    }
}
...
pub fn store_value(value: Option<BinnStruct>) -> Option<Box<()>> {
	println!("\n* store_value");
...
lazy_static! {
    pub static ref LOCAL_VALUE: Mutex<Option<BinnStruct>> = {
        let empty_binn: BinnStruct = unsafe { mem::zeroed() };
        Mutex::new(Some(empty_binn))
    };
}
...
pub fn binn_copy(mut old: Option<&mut [u8]>) -> Option<BinnStruct> {
	println!("\n* binn_copy");
    let mut type_ = 0;
    let mut count = 0;
    let mut size = 0;
...
pub fn binn_list_add_new(list: &mut Option<BinnStruct>, value: &mut Option<BinnStruct>) -> BOOL {
	println!("\n* binn_list_add_new");
...
pub fn binn_map_set_new(map: &mut binn, id: i32, mut value: Option<BinnStruct>) -> BOOL {
	println!("\n* binn_map_set_new");
    let retval;
    {
...
pub fn binn_open(data: Option<*mut u8>) -> Option<BinnStruct> {
	println!("\n* binn_open");
    // Allocate memory for the binn struct
...
pub fn binn_list_value(ptr: Option<&[u8]>, pos: i32) -> Option<BinnStruct> {
	println!("\n* binn_list_value");
...
pub fn binn_map_value(ptr: Option<&[u8]>, id: i32) -> Option<BinnStruct> {
	println!("\n* binn_map_value");
    // Allocate memory for a new BinnStruct
    let mut value: Option<BinnStruct> = match binn_malloc(std::mem::size_of::<BinnStruct>() as i32) {
        Some(alloc) => {
            // SAFETY: We know binn_malloc returns a Box<BinnStruct> when successful
            unsafe { Some(*(alloc.downcast::<BinnStruct>().ok().unwrap())) }
        }
        None => return None,
    };
...
pub fn binn_object_value(ptr: Option<&[u8]>, key: Option<&str>) -> Option<BinnStruct> {
	println!("\n* binn_object_value");
    // Allocate memory for the binn struct
    ...
    // Create an Option<BinnStruct> to pass to the function
...
pub fn binn_map_pair(map: Option<&[u8]>, pos: i32, pid: Option<&mut i32>) -> Option<BinnStruct> {
	println!("\n* binn_map_pair");
    // Allocate memory for the binn struct
    let mut value = match binn_malloc(std::mem::size_of::<BinnStruct>() as i32) {
        Some(alloc) => {
    ...
    let mut boxed_value = Some(*value.unwrap());//value.as_mut().map(|v| *v);

    // Call binn_read_pair with the allocated value
    if binn_read_pair(0xE1, map, pos, pid, Option::None, Some(&mut boxed_value)) == 0 {
    ...
    if let Some(v) = boxed_value.as_mut() {
        v.allocated = 1;
    }
    //Some(*(value.unwrap()))
    boxed_value
}
pub fn binn_object_pair(obj: Option<&[u8]>, pos: i32, pkey: Option<&mut [u8]>) -> Option<BinnStruct> {
	println!("\n* binn_object_pair");
    // Allocate memory for the binn struct
    let value = match binn_malloc(std::mem::size_of::<BinnStruct>() as i32) {
        Some(ptr) => ptr,
    ...
    let mut binn_value: Option<BinnStruct> = Some(value);
    let value_ref: Option<&mut Option<BinnStruct>> = Some(&mut binn_value);
    ...
    match value.type_ {
        ...
        0xA0 => {
            return value.ptr.as_ref().map(|p| {
                Box::new(unsafe { CString::from_raw(Box::into_raw(p.clone()) as *mut u8) })
            });
        }
        0x80061 => {
...
pub fn binn_list_next_value(iter: Option<&mut BinnIter>) -> Option<BinnStruct> {
    ...
    let mut value: Option<BinnStruct> = match binn_malloc(std::mem::size_of::<BinnStruct>() as i32) {
        Some(alloc) => {
            ...
            Some(unsafe {
                ...
                std::ptr::read(ptr.cast::<BinnStruct>())
            })
        }
        None => return None,
    };
    ...
    match value.type_ {
        ...
        0xA0 => {
            if let Some(ptr) = value.ptr.as_ref() {
                let c_str = unsafe { CStr::from_ptr(ptr.as_ref() as *const _ as *const u8) };
