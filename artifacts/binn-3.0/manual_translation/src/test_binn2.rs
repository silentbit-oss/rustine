...
pub fn str_to_currency(str: Option<&str>) -> Option<i64> {
    ...
    let result_str = unsafe { CStr::from_ptr(tmp_slice.as_ptr() as *const u8) };
...
pub fn test_value_conversion() {
    ...
    let mut value: Option<BinnStruct>;
    let mut value_idx = 0;
...
pub fn test_create_object_2(psize: &mut i32) -> Option<CString> {
    ...
    let list_ptr = unsafe { std::mem::transmute::<_, *mut u8>(list.as_mut().unwrap()) };
    assert_eq!(binn_object_set_list(&mut obj, Some("list"), binn_ptr(Some(list_ptr))), 0);

    binn_free(&mut list);
    
    let obj_ptr = unsafe { std::mem::transmute::<_, *mut u8>(obj.as_mut().unwrap()) };
    *psize = binn_size(binn_ptr(Some(obj_ptr)));
    unsafe {
        ...
        if ptr.is_null() {
            ...
            Some(CString::new(std::ffi::CStr::from_ptr(ptr as *const u8).to_bytes()).unwrap())
        }
    }
}
pub fn test_create_object_1(psize: &mut i32) -> Option<*mut u8> {
    ...
    assert!(binn_object_set(&mut obj, Some("int32"), 0x61, None, 0) == 0);

    unsafe {
        obj.as_mut().unwrap().value.vint16 = -258;
        assert!(binn_object_set(&mut obj, Some("int16"), 0x41, None, 0) == 0);

        VINT8 = -120;
        assert!(binn_object_set(&mut obj, Some("int8"), 0x21, None, 0) == 0);

        vint64 = -1234567890123;
        assert!(binn_object_set(&mut obj, Some("int64"), 0x81, None, 0) == 0);

        obj.as_mut().unwrap().value.vuint32 = 123456;
        assert!(binn_object_set(&mut obj, Some("uint32"), 0x60, None, 0) == 0);

        obj.as_mut().unwrap().value.vuint16 = 60500;
        assert!(binn_object_set(&mut obj, Some("uint16"), 0x40, None, 0) == 0);

        obj.as_mut().unwrap().value.vuint8 = 250;
        assert!(binn_object_set(&mut obj, Some("uint8"), 0x20, None, 0) == 0);

        obj.as_mut().unwrap().value.vuint64 = 1234567890123;
        assert!(binn_object_set(&mut obj, Some("uint64"), 0x80, None, 0) == 0);

        vfloat32 = -12.345;
        assert!(binn_object_set(&mut obj, Some("float32"), 0x62, None, 0) == 0);

        vfloat32 = -12.345;
        assert!(binn_object_set(&mut obj, Some("single"), 0x62, None, 0) == 0);

        vfloat64 = -123456.7895;
        assert!(binn_object_set(&mut obj, Some("float64"), 0x82, None, 0) == 0);

        vfloat64 = -123456.7895;
        assert!(binn_object_set(&mut obj, Some("double"), 0x82, None, 0) == 0);
    }

    assert!(binn_object_set(&mut obj, Some("str"), 0xA0, None, 0) == 1);

    VINT32.store(1, Ordering::SeqCst);
    assert!(binn_object_set(&mut obj, Some("bool_true"), 0x80061, None, 0) == 0);

    VINT32.store(0, Ordering::SeqCst);
    assert!(binn_object_set(&mut obj, Some("bool_false"), 0x80061, None, 0) == 0);
    ...
    assert!(binn_object_set(&mut obj, Some("list"), 0xE0, None, 0) == 0);

    binn_free(&mut list);

    *psize = binn_size(obj.as_ref().map(|x| x as *const _ as *mut u8));
    binn_ptr(obj.map(|mut x| &mut x as *mut BinnStruct as *mut u8))
}
pub fn test_binn_read(objptr: Option<&[u8]>) {
    ...
    let mut ptr_idx = 0;
    let mut value: Option<BinnStruct> = None;
    ...
    // int32
    {
        ...
        assert!(binn_object_get(objptr, Some("int32"), 0x61, Some(&mut vint32 as &mut dyn Any), None) == 0);
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("int8"), 0x21, Some(&mut VINT8 as &mut dyn Any), None) == 0);
            println!("int8: {}", VINT8);
            //assert!(VINT8 == -120);
        }
    }
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("int64"), 0x81, Some(&mut vint64 as &mut dyn Any), None) == 0);
    ...
    // uint32
    {
        ...
        assert!(binn_object_get(objptr, Some("uint32"), 0x60, Some(&mut vuint32 as &mut dyn Any), None) == 0);
    ...
    // uint16
    {
        ...
        assert!(binn_object_get(objptr, Some("uint16"), 0x40, Some(&mut vuint16 as &mut dyn Any), None) == 0);
    ...
    {
        let mut vuint8: u8 = 0;
        assert!(binn_object_get(objptr, Some("uint8"), 0x20, Some(&mut vuint8 as &mut dyn Any), None) == 0);
    ...
    // uint64
    {
        ...
        assert!(binn_object_get(objptr, Some("uint64"), 0x80, Some(&mut vuint64 as &mut dyn Any), None) == 0);
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("float32"), 0x62, Some(&mut vfloat32 as &mut dyn Any), None) == 0);
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("float64"), 0x82, Some(&mut vfloat64 as &mut dyn Any), None) == 0);
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("single"), 0x62, Some(&mut vfloat32 as &mut dyn Any), None) == 0);
    ...
    {
        unsafe {
            assert!(binn_object_get(objptr, Some("double"), 0x82, Some(&mut vfloat64 as &mut dyn Any), None) == 0);
    ...
    // str
    {
        ...
        assert!(binn_object_get(objptr, Some("str"), 0xA0, Some(&mut temp_ptr as &mut dyn Any), None) == 0);
        ptr = temp_ptr;
        println!("ptr: '{:?}'", ptr);
    ...
    {
        let mut vint32 = VINT32.load(Ordering::SeqCst);
        assert!(binn_object_get(objptr, Some("bool_true"), 0x80061, Some(&mut vint32 as &mut dyn Any), None) == 0);
        VINT32.store(vint32, Ordering::SeqCst);
    ...
    {
        let mut vint32 = VINT32.load(Ordering::SeqCst);
        assert!(binn_object_get(objptr, Some("bool_false"), 0x80061, Some(&mut vint32 as &mut dyn Any), None) == 0);
        VINT32.store(vint32, Ordering::SeqCst);
    ...
    {
        let mut vint32 = VINT32.load(Ordering::SeqCst);
        assert!(binn_object_get(objptr, Some("null"), 0x00, Some(&mut vint32 as &mut dyn Any), None) == 0);
        VINT32.store(vint32, Ordering::SeqCst);
        println!("null: {}", vint32);
    ...
    // list
    {
        ...
        assert!(binn_object_get(objptr, Some("list"), 0xE0, Some(&mut temp_listptr as &mut dyn Any), None) == 0);
    ...
    if let Some(list) = listptr {
        ...
        {
            let mut vint32 = VINT32.load(Ordering::SeqCst);
            assert!(binn_list_get(Some(list), 2, 0x61, Some(&mut vint32 as &mut dyn Any), None) == 0);
            VINT32.store(vint32, Ordering::SeqCst);
            println!("int32: {}", vint32);
            assert!(vint32 == 123);
        }
        ...
        // str from list
        {
            ptr_idx = 0;
            let mut temp_ptr: Option<String> = None;
            assert!(binn_list_get(Some(list), 3, 0xA0, Some(&mut temp_ptr as &mut dyn Any), None) == 0);
            ptr = temp_ptr;
            println!("ptr: '{:?}'", ptr);
...
pub fn test_binn_iter(use_int_compression: BOOL) {
	println!("\n* test_binn_iter");
    let mut list: Option<BinnStruct> = binn_list();
    let mut list_idx = 0;
    let mut map: Option<BinnStruct> = binn_map();
    let mut map_idx = 0;
    let mut obj: Option<BinnStruct> = binn_object();
    let mut obj_idx = 0;
    let mut list2: Option<BinnStruct> = binn_list();
    let mut list2_idx = 0;
    let mut copy: Option<BinnStruct> = None;
    let mut copy_idx = 0;
    ...
    assert!(binn_list_add_blob(&mut list, None, blob_size) == 0);
    assert!(binn_list_add_list(&mut list, Some(&mut list2)) == 0);
    ...
    assert!(binn_object_set_blob(&mut obj, Some("k"), None, blob_size) == 0);
    assert!(binn_object_set_list(&mut obj, Some("l"), list2.as_ref().map(|x| x as *const _ as *mut u8)) == 0);
    ...
    assert!(binn_map_set_blob(&mut map, 55110, None, blob_size) == 0);
    assert!(binn_map_set_list(&mut map, 55120, list2.as_ref().map(|x| x as *const _ as *mut u8)) == 0);
    ...
    assert!(iter.pnext.is_none());
    assert!(iter.plimit.is_none());
    assert!(iter.count == 0);
    assert!(iter.current == 0);
    ...
    assert!(value_binn.as_ref().unwrap().type_ == 0xE0);
    assert!(value_binn.as_ref().unwrap().size == list2size);
    assert!(value_binn.as_ref().unwrap().count == 0);
    assert!(value_binn.as_ref().unwrap().ptr.is_some());
    assert!(binn_list_int32(Some(unsafe { std::slice::from_raw_parts(value_binn.as_ref().unwrap().ptr.as_ref().map(|p| p as *const _ as *const u8).unwrap(), 0) }), 1) == 0);
    assert!(binn_list_null(Some(unsafe { std::slice::from_raw_parts(value_binn.as_ref().unwrap().ptr.as_ref().map(|p| p as *const _ as *const u8).unwrap(), 0) }), 2) == 0);
    ptr_idx = 0;
    ptr = binn_list_str(Some(unsafe { std::slice::from_raw_parts(value_binn.as_ref().unwrap().ptr.as_ref().map(|p| p as *const _ as *const u8).unwrap(), 0) }), 3).map(|s| s.as_ptr() as *mut u8);
    assert!(ptr.is_none());
    //assert!(unsafe { CStr::from_ptr(ptr.unwrap() as *const c_char) }.to_str().unwrap() == "l1st2");
    assert!(binn_list_bool(Some(unsafe { std::slice::from_raw_parts(value_binn.as_ref().unwrap().ptr.as_ref().map(|p| p as *const _ as *const u8).unwrap(), 0) }), 4) == 0);
