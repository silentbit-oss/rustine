...
pub fn allocate_images() {
    // Declare X11 functions we'll use
    extern "C" {
        fn XCreateImage(
            display: *mut ffi::_XDisplay,
            visual: *mut ffi::Visual,
            depth: i32,
            format: i32,
            offset: i32,
            data: *mut u8,
            ...
            bitmap_pad: i32,
            bytes_per_line: i32,
        ) -> *mut XImage;  // Changed to match the actual type
        fn XDefaultVisualOfScreen(screen: *mut ffi::Screen) -> *mut ffi::Visual;
        fn XDefaultDepthOfScreen(screen: *mut ffi::Screen) -> i32;
    }
    ...
    let scr = SCR.with(|s| {
        let borrow = s.borrow();
        borrow.as_ref().unwrap().as_ref() as *const _ as *mut _
    });

    for i in 0..2 {
        // Allocate memory for image data
        let data_size = (width[i] * height[i]) as usize;
        let mut data = vec![0u8; data_size]; // Changed to mut since we'll need a mutable pointer

        // Create XImage
        let visual = unsafe { XDefaultVisualOfScreen(scr) };
        let depth = unsafe { XDefaultDepthOfScreen(scr) };
        
        let display_ptr = display.as_ref().unwrap().as_ref() as *const _ as *mut _;
        let new_ximage = unsafe {
            XCreateImage(
                display_ptr,
                visual,
                depth,
        ...
        if new_ximage.is_null() {
            eprintln!("XCreateImage failed");
            process::exit(-1);
        }

        // Store the XImage value directly (not boxed)
        ximage[i] = Some(unsafe { *Box::from_raw(new_ximage) });
        
        // Prevent Rust from freeing the data vector since XImage now owns it
        std::mem::forget(data);
    }

    // Mark images as created
