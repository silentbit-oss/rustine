void lil_freemem(void *ptr)
{
  free(ptr);
}


/*** DEPENDENCIES:
***/


static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


/*** DEPENDENCIES:
***/


static unsigned long hm_hash(const char *key)
{
  unsigned long hash = 5381;
  int c;
  while (c = *(key++))
    hash = ((hash << 5) + hash) + c;

  return hash;
}


/*** DEPENDENCIES:
***/


static int needs_escape(const char *str)
{
  size_t i;
  if ((!str) || (!str[0]))
  {
    return 1;
  }
  for (i = 0; str[i]; i += 1)
    if (ispunct(str[i]) || isspace(str[i]))
  {
    return 1;
  }

  return 0;
}


/*** DEPENDENCIES:
***/


static int islilspecial(char ch)
{
  return (((((((ch == '$') || (ch == '{')) || (ch == '}')) || (ch == '[')) || (ch == ']')) || (ch == '"')) || (ch == '\'')) || (ch == ';');
}


/*** DEPENDENCIES:
***/


static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


/*** DEPENDENCIES:
***/


// hint:  ['cont_idx_ref is a mutable refrence to unsigned int', 'contlen_ref is a mutable refrence to size_t', 'head_ref is a mutable refrence to size_t']
void helper_helper_lil_embedded_1_1(unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const cont)
{
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  if ((code[head] == '{') || (code[head] == '}'))
  {
    cont_idx = realloc(cont_idx, contlen + 6);
    cont[contlen + cont_idx] = '}';
    contlen += 1;
    cont[contlen + cont_idx] = '"';
    contlen += 1;
    cont[contlen + cont_idx] = '\\';
    contlen += 1;
    if (code[head] == '{')
    {
      cont[contlen + cont_idx] = 'o';
      contlen += 1;
    }
    else
      cont[contlen + cont_idx] = 'c';
    contlen += 1;
    cont[(contlen++) + cont_idx] = '"';
    cont[(contlen++) + cont_idx] = '{';
    head++;
  }
  else
  {
    cont_idx = realloc(cont_idx, contlen + 1);
    cont[contlen + cont_idx] = code[head];
    head += 1;
    contlen += 1;
  }
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


/*** DEPENDENCIES:
***/


// hint:  ['lilcode_idx_ref is a mutable refrence to unsigned int', 'lilcodelen_ref is a mutable refrence to size_t', 'cont_idx_ref is a mutable refrence to unsigned int', 'contlen_ref is a mutable refrence to size_t', 'head_ref is a mutable refrence to size_t']
void helper_helper_lil_embedded_1_2(unsigned int * const lilcode_idx_ref, size_t * const lilcodelen_ref, unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const lilcode, char * const cont, size_t codelen)
{
  unsigned int lilcode_idx = *lilcode_idx_ref;
  size_t lilcodelen = *lilcodelen_ref;
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  head += 5;
  if (contlen)
  {
    lilcode_idx = realloc(lilcode_idx, (lilcodelen + contlen) + 10);
    memcpy((&lilcode[lilcode_idx]) + lilcodelen, "\nwrite {", 8);
    memcpy(((&lilcode[lilcode_idx]) + lilcodelen) + 8, cont, contlen);
    lilcode[((lilcodelen + contlen) + 8) + lilcode_idx] = '}';
    lilcode[((lilcodelen + contlen) + 9) + lilcode_idx] = '\n';
    lilcodelen += contlen + 10;
    free(cont);
    cont_idx = 0;
    contlen = 0;
  }
  while (head < codelen)
  {
    if (((head < (codelen - 1)) && (code[head] == '?')) && (code[head + 1] == '>'))
    {
      head += 2;
      break;
    }
    lilcode_idx = realloc(lilcode_idx, lilcodelen + 1);
    lilcode[lilcodelen + lilcode_idx] = code[head];
    head += 1;
    lilcodelen += 1;
  }

  lilcode_idx = realloc(lilcode_idx, lilcodelen + 1);
  lilcode[lilcodelen + lilcode_idx] = '\n';
  lilcodelen += 1;
  *lilcode_idx_ref = lilcode_idx;
  *lilcodelen_ref = lilcodelen;
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


/*** DEPENDENCIES:
***/


int lil_error(lil_t lil, const char **msg, size_t *pos)
{
  if (!lil->error)
  {
    return 0;
  }
  *msg = lil->err_msg;
  *pos = lil->err_head;
  lil->error = 0;
  return 1;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
***/


const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t alloc_value_len(const char *str, size_t len)
{
  lil_value_t val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  if (str)
  {
    val->l = len;
    val->d = malloc(len + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, str, len);
    val->d[len] = 0;
  }
  else
  {
    val->l = 0;
    val->d = 0;
  }
  return val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


/*** DEPENDENCIES:
static lil_value_t alloc_value_len(const char *str, size_t len)
{
  lil_value_t val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  if (str)
  {
    val->l = len;
    val->d = malloc(len + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, str, len);
    val->d[len] = 0;
  }
  else
  {
    val->l = 0;
    val->d = 0;
  }
  return val;
}


----------------------------
***/


lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


/*** DEPENDENCIES:
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
***/


lil_value_t lil_alloc_double(double num)
{
  char buff[128];
  sprintf(buff, "%f", num);
  return alloc_value(buff);
}


/*** DEPENDENCIES:
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
***/


void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


/*** DEPENDENCIES:
typedef struct _lil_list_t *lil_list_t
----------------------------
***/


size_t lil_list_size(lil_list_t list)
{
  return list->c;
}


/*** DEPENDENCIES:
typedef struct _lil_list_t *lil_list_t
----------------------------
***/


lil_value_t lil_list_get(lil_list_t list, size_t index)
{
  return (index >= list->c) ? (0) : (list->v[index]);
}


/*** DEPENDENCIES:
typedef struct _lil_list_t *lil_list_t
----------------------------
***/


lil_env_t lil_alloc_env(lil_env_t parent)
{
  lil_env_t env;
  env = calloc(1, sizeof(struct _lil_env_t));
  env->parent = parent;
  return env;
}


/*** DEPENDENCIES:
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


lil_value_t lil_arg(lil_value_t *argv, size_t index)
{
  return (argv) ? (argv[index]) : (0);
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


void lil_set_data(lil_t lil, void *data)
{
  lil->data = data;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
***/


void *lil_get_data(lil_t lil)
{
  return lil->data;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
***/


static void hm_init(hashmap_t *hm)
{
  memset(hm, 0, sizeof(hashmap_t));
}


/*** DEPENDENCIES:
typedef struct _hashmap_t
{
  struct hashcell_t cell[256];
} hashmap_t
----------------------------
***/


static void hm_destroy(hashmap_t *hm)
{
  size_t i;
  size_t j;
  for (i = 0; i < 256; i += 1)
  {
    for (j = 0; j < hm->cell[i].c; j += 1)
      free(hm->cell[i].e[j].k);

    free(hm->cell[i].e);
  }

}


/*** DEPENDENCIES:
typedef struct _hashmap_t
{
  struct hashcell_t cell[256];
} hashmap_t
----------------------------
***/


int lil_append_string_len(lil_value_t val, const char *s, size_t len)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!s) || (!s[0]))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + len) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, s, len + 1);
  val->d = &new[new_idx];
  val->l += len;
  return 1;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static int ateol(lil_t lil)
{
  return (!lil->ignoreeol) && (((lil->code[lil->head] == '\n') || (lil->code[lil->head] == '\r')) || (lil->code[lil->head] == ';'));
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
***/


static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
***/


lil_value_t lil_alloc_string_len(const char *str, size_t len)
{
  return alloc_value_len(str, len);
}


/*** DEPENDENCIES:
static lil_value_t alloc_value_len(const char *str, size_t len)
{
  lil_value_t val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  if (str)
  {
    val->l = len;
    val->d = malloc(len + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, str, len);
    val->d[len] = 0;
  }
  else
  {
    val->l = 0;
    val->d = 0;
  }
  return val;
}


----------------------------
***/


static void fnc_embed_write(lil_t lil, const char *msg)
{
  size_t len = strlen(msg) + 1;
  lil->embed = realloc(lil->embed, lil->embedlen + len);
  memcpy(lil->embed + lil->embedlen, msg, len);
  lil->embedlen += len - 1;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
***/


void lil_callback(lil_t lil, int cb, lil_callback_proc_t proc)
{
  if ((cb < 0) || (cb > 8))
  {
    return;
  }
  lil->callback[cb] = proc;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
typedef void (*lil_callback_proc_t)(void)
----------------------------
***/


void lil_set_error(lil_t lil, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 2;
  lil->err_head = 0;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


void lil_set_error_at(lil_t lil, size_t pos, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 1;
  lil->err_head = pos;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


double lil_to_double(lil_value_t val)
{
  return atof(lil_to_string(val));
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
***/


lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
***/


int lil_to_boolean(lil_value_t val)
{
  const char *s = lil_to_string(val);
  size_t i;
  size_t dots = 0;
  if (!s[0])
  {
    return 0;
  }
  for (i = 0; s[i]; i += 1)
  {
    if ((s[i] != '0') && (s[i] != '.'))
    {
      return 1;
    }
    if (s[i] == '.')
    {
      if (dots)
      {
        return 1;
      }
      dots = 1;
    }
  }

  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
***/


lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


/*** DEPENDENCIES:
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef int64_t lilint_t
----------------------------
***/


int lil_append_string(lil_value_t val, const char *s)
{
  return lil_append_string_len(val, s, strlen(s));
}


/*** DEPENDENCIES:
int lil_append_string_len(lil_value_t val, const char *s, size_t len)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!s) || (!s[0]))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + len) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, s, len + 1);
  val->d = &new[new_idx];
  val->l += len;
  return 1;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


/*** DEPENDENCIES:
typedef struct _lil_list_t *lil_list_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
***/


static void *hm_get(hashmap_t *hm, const char *key)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    return cell->e[i].v;
  }

  return 0;
}


/*** DEPENDENCIES:
struct hashcell_t
{
  struct hashentry_t *e;
  size_t c;
}
----------------------------
static unsigned long hm_hash(const char *key)
{
  unsigned long hash = 5381;
  int c;
  while (c = *(key++))
    hash = ((hash << 5) + hash) + c;

  return hash;
}


----------------------------
typedef struct _hashmap_t
{
  struct hashcell_t cell[256];
} hashmap_t
----------------------------
***/


static lil_var_t lil_find_local_var(lil_t lil, lil_env_t env, const char *name)
{
  return hm_get(&env->varmap, name);
}


/*** DEPENDENCIES:
static void *hm_get(hashmap_t *hm, const char *key)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    return cell->e[i].v;
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


static lil_var_t lil_find_var(lil_t lil, lil_env_t env, const char *name)
{
  lil_var_t r = lil_find_local_var(lil, env, name);
  return (r) ? (r) : ((env == lil->rootenv) ? (0) : (lil_find_var(lil, lil->rootenv, name)));
}


/*** DEPENDENCIES:
static lil_var_t lil_find_local_var(lil_t lil, lil_env_t env, const char *name)
{
  return hm_get(&env->varmap, name);
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
None
----------------------------
***/


lil_value_t lil_get_var_or(lil_t lil, const char *name, lil_value_t defvalue)
{
  lil_var_t var = lil_find_var(lil, lil->env, name);
  lil_value_t retval = (var) ? (var->v) : (defvalue);
  if (lil->callback[7] && ((!var) || (var->env == lil->rootenv)))
  {
    lil_getvar_callback_proc_t proc = (lil_getvar_callback_proc_t) lil->callback[7];
    lil_value_t newretval = retval;
    if (proc(lil, name, &newretval))
    {
      retval = newretval;
    }
  }
  return retval;
}


/*** DEPENDENCIES:
static lil_var_t lil_find_var(lil_t lil, lil_env_t env, const char *name)
{
  lil_var_t r = lil_find_local_var(lil, env, name);
  return (r) ? (r) : ((env == lil->rootenv) ? (0) : (lil_find_var(lil, lil->rootenv, name)));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef int (*lil_getvar_callback_proc_t)(lil_t lil, const char *name, lil_value_t *value)
----------------------------
typedef struct _lil_t *lil_t
----------------------------
None
----------------------------
***/


lil_value_t lil_get_var(lil_t lil, const char *name)
{
  return lil_get_var_or(lil, name, lil->empty);
}


/*** DEPENDENCIES:
lil_value_t lil_get_var_or(lil_t lil, const char *name, lil_value_t defvalue)
{
  lil_var_t var = lil_find_var(lil, lil->env, name);
  lil_value_t retval = (var) ? (var->v) : (defvalue);
  if (lil->callback[7] && ((!var) || (var->env == lil->rootenv)))
  {
    lil_getvar_callback_proc_t proc = (lil_getvar_callback_proc_t) lil->callback[7];
    lil_value_t newretval = retval;
    if (proc(lil, name, &newretval))
    {
      retval = newretval;
    }
  }
  return retval;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


void lil_write(lil_t lil, const char *msg)
{
  if (lil->callback[1])
  {
    lil_write_callback_proc_t proc = (lil_write_callback_proc_t) lil->callback[1];
    proc(lil, msg);
  }
  else
    printf("%s", msg);
}


/*** DEPENDENCIES:
typedef void (*lil_write_callback_proc_t)(lil_t lil, const char *msg)
----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


/*** DEPENDENCIES:
static void *hm_get(hashmap_t *hm, const char *key)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    return cell->e[i].v;
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static void skip_spaces(lil_t lil)
{
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '#')
    {
      if ((lil->code[lil->head + 1] == '#') && (lil->code[lil->head + 2] != '#'))
      {
        lil->head += 2;
        while (lil->head < lil->clen)
        {
          if (((lil->code[lil->head] == '#') && (lil->code[lil->head + 1] == '#')) && (lil->code[lil->head + 2] != '#'))
          {
            lil->head += 2;
            break;
          }
          lil->head += 1;
        }

      }
      else
      {
        while ((lil->head < lil->clen) && (!ateol(lil)))
          lil->head++;

      }
    }
    else
      if ((lil->code[lil->head] == '\\') && ((lil->code[lil->head + 1] == '\r') || (lil->code[lil->head + 1] == '\n')))
    {
      lil->head += 1;
      while ((lil->head < lil->clen) && ateol(lil))
        lil->head++;

    }
    else
      if ((lil->code[lil->head] == '\r') || (lil->code[lil->head] == '\n'))
    {
      if (lil->ignoreeol)
      {
        lil->head += 1;
      }
      else
        break;
    }
    else
      if (isspace(lil->code[lil->head]))
    {
      lil->head += 1;
    }
    else
      break;
  }

}


/*** DEPENDENCIES:
static int ateol(lil_t lil)
{
  return (!lil->ignoreeol) && (((lil->code[lil->head] == '\n') || (lil->code[lil->head] == '\r')) || (lil->code[lil->head] == ';'));
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static void ee_numeric_element(expreval_t *ee)
{
  lilint_t fpart = 0;
  lilint_t fpartlen = 1;
  ee->type = 0;
  ee_skip_spaces(ee);
  ee->ival = 0;
  ee->dval = 0;
  while (ee->head < ee->len)
  {
    if (ee->code[ee->head] == '.')
    {
      if (ee->type == 1)
      {
        break;
      }
      ee->type = 1;
      ee->head += 1;
    }
    else
      if (!isdigit(ee->code[ee->head]))
    {
      break;
    }
    if (ee->type == 0)
    {
      ee->ival = (ee->ival * 10) + (ee->code[ee->head] - '0');
    }
    else
    {
      fpart = (fpart * 10) + (ee->code[ee->head] - '0');
      fpartlen *= 10;
    }
    ee->head += 1;
  }

  if (ee->type == 1)
  {
    ee->dval = ee->ival + (((double) fpart) / ((double) fpartlen));
  }
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
***/


static void ee_element(expreval_t *ee)
{
  if (isdigit(ee->code[ee->head]))
  {
    ee_numeric_element(ee);
    return;
  }
  ee->type = 0;
  ee->ival = 1;
  ee->error = 4;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_numeric_element(expreval_t *ee)
{
  lilint_t fpart = 0;
  lilint_t fpartlen = 1;
  ee->type = 0;
  ee_skip_spaces(ee);
  ee->ival = 0;
  ee->dval = 0;
  while (ee->head < ee->len)
  {
    if (ee->code[ee->head] == '.')
    {
      if (ee->type == 1)
      {
        break;
      }
      ee->type = 1;
      ee->head += 1;
    }
    else
      if (!isdigit(ee->code[ee->head]))
    {
      break;
    }
    if (ee->type == 0)
    {
      ee->ival = (ee->ival * 10) + (ee->code[ee->head] - '0');
    }
    else
    {
      fpart = (fpart * 10) + (ee->code[ee->head] - '0');
      fpartlen *= 10;
    }
    ee->head += 1;
  }

  if (ee->type == 1)
  {
    ee->dval = ee->ival + (((double) fpart) / ((double) fpartlen));
  }
}


----------------------------
***/


// hint:  ['lilcode_idx_ref is a mutable refrence to unsigned int', 'lilcodelen_ref is a mutable refrence to size_t', 'cont_idx_ref is a mutable refrence to unsigned int', 'contlen_ref is a mutable refrence to size_t', 'head_ref is a mutable refrence to size_t']
void helper_lil_embedded_1(unsigned int * const lilcode_idx_ref, size_t * const lilcodelen_ref, unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const lilcode, char * const cont, size_t codelen)
{
  unsigned int lilcode_idx = *lilcode_idx_ref;
  size_t lilcodelen = *lilcodelen_ref;
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  if ((((((head < (codelen - 4)) && (code[head] == '<')) && (code[head + 1] == '?')) && (code[head + 2] == 'l')) && (code[head + 3] == 'i')) && (code[head + 4] == 'l'))
  {
    helper_helper_lil_embedded_1_2(&lilcode_idx, &lilcodelen, &cont_idx, &contlen, &head, code, lilcode, cont, codelen);
  }
  else
  {
    helper_helper_lil_embedded_1_1(&cont_idx, &contlen, &head, code, cont);
  }
  *lilcode_idx_ref = lilcode_idx;
  *lilcodelen_ref = lilcodelen;
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


/*** DEPENDENCIES:
void helper_helper_lil_embedded_1_1(unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const cont)
{
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  if ((code[head] == '{') || (code[head] == '}'))
  {
    cont_idx = realloc(cont_idx, contlen + 6);
    cont[contlen + cont_idx] = '}';
    contlen += 1;
    cont[contlen + cont_idx] = '"';
    contlen += 1;
    cont[contlen + cont_idx] = '\\';
    contlen += 1;
    if (code[head] == '{')
    {
      cont[contlen + cont_idx] = 'o';
      contlen += 1;
    }
    else
      cont[contlen + cont_idx] = 'c';
    contlen += 1;
    cont[(contlen++) + cont_idx] = '"';
    cont[(contlen++) + cont_idx] = '{';
    head++;
  }
  else
  {
    cont_idx = realloc(cont_idx, contlen + 1);
    cont[contlen + cont_idx] = code[head];
    head += 1;
    contlen += 1;
  }
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


----------------------------
void helper_helper_lil_embedded_1_2(unsigned int * const lilcode_idx_ref, size_t * const lilcodelen_ref, unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const lilcode, char * const cont, size_t codelen)
{
  unsigned int lilcode_idx = *lilcode_idx_ref;
  size_t lilcodelen = *lilcodelen_ref;
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  head += 5;
  if (contlen)
  {
    lilcode_idx = realloc(lilcode_idx, (lilcodelen + contlen) + 10);
    memcpy((&lilcode[lilcode_idx]) + lilcodelen, "\nwrite {", 8);
    memcpy(((&lilcode[lilcode_idx]) + lilcodelen) + 8, cont, contlen);
    lilcode[((lilcodelen + contlen) + 8) + lilcode_idx] = '}';
    lilcode[((lilcodelen + contlen) + 9) + lilcode_idx] = '\n';
    lilcodelen += contlen + 10;
    free(cont);
    cont_idx = 0;
    contlen = 0;
  }
  while (head < codelen)
  {
    if (((head < (codelen - 1)) && (code[head] == '?')) && (code[head + 1] == '>'))
    {
      head += 2;
      break;
    }
    lilcode_idx = realloc(lilcode_idx, lilcodelen + 1);
    lilcode[lilcodelen + lilcode_idx] = code[head];
    head += 1;
    lilcodelen += 1;
  }

  lilcode_idx = realloc(lilcode_idx, lilcodelen + 1);
  lilcode[lilcodelen + lilcode_idx] = '\n';
  lilcodelen += 1;
  *lilcode_idx_ref = lilcode_idx;
  *lilcodelen_ref = lilcodelen;
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


----------------------------
***/


void helper_helper_ee_equals_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval != ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval != ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival != ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival != ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_compare_1_4(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval >= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval >= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival >= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival >= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_compare_1_3(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval <= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval <= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival <= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival <= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_compare_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval < ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval < ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival < ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival < ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_compare_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval > ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval > ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival > ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival > ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_ee_compare_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  int op = 4;
  if ((ee->code[ee->head] == '<') && (!ee_invalidpunct(ee->code[ee->head + 1])))
  {
    op = 1;
  }
  else
    if ((ee->code[ee->head] == '>') && (!ee_invalidpunct(ee->code[ee->head + 1])))
  {
    op = 2;
  }
  else
    if ((ee->code[ee->head] == '<') && (ee->code[ee->head + 1] == '='))
  {
    op = 3;
  }
  ee->head += (op > 2) ? (2) : (1);
  switch (op)
  {
    case 1:
    {
      helper_helper_ee_compare_1_1(ee, odval, oival);
    }

    case 2:
    {
      helper_helper_ee_compare_1_2(ee, odval, oival);
    }

    case 3:
    {
      helper_helper_ee_compare_1_3(ee, odval, oival);
    }

    case 4:
    {
      helper_helper_ee_compare_1_4(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
void helper_helper_ee_compare_1_4(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval >= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval >= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival >= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival >= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
void helper_helper_ee_compare_1_3(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval <= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval <= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival <= ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival <= ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
void helper_helper_ee_compare_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval < ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval < ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival < ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival < ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
void helper_helper_ee_compare_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval > ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval > ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_shift(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival > ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival > ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
***/


void helper_helper_ee_addsub_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = odval - ee->dval;
          break;
        }

        case 0:
        {
          ee->dval = odval - ee->ival;
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ((double) oival) - ee->dval;
          ee->type = 1;
          break;
        }

        case 0:
        {
          ee->ival = oival - ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_helper_ee_muldiv_1_4_1(expreval_t * const ee, double odval)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (odval / ee->dval);
      }
      ee->type = 0;
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (odval / ((double) ee->ival));
      }
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
None
----------------------------
***/


void helper_helper_helper_ee_muldiv_1_4_2(expreval_t * const ee, lilint_t oival)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (((double) oival) / ee->dval);
      }
      ee->type = 0;
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = oival / ee->ival;
      }
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_muldiv_1_4(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      helper_helper_helper_ee_muldiv_1_4_1(ee, odval);
    }

    case 0:
    {
      helper_helper_helper_ee_muldiv_1_4_2(ee, oival);
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
void helper_helper_helper_ee_muldiv_1_4_1(expreval_t * const ee, double odval)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (odval / ee->dval);
      }
      ee->type = 0;
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (odval / ((double) ee->ival));
      }
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
void helper_helper_helper_ee_muldiv_1_4_2(expreval_t * const ee, lilint_t oival)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = (lilint_t) (((double) oival) / ee->dval);
      }
      ee->type = 0;
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->ival = oival / ee->ival;
      }
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
***/


void helper_helper_helper_ee_muldiv_1_3_2(expreval_t * const ee, lilint_t oival)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = ((double) oival) / ee->dval;
      }
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = ((double) oival) / ((double) ee->ival);
      }
      ee->type = 1;
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_muldiv_1_3(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      helper_helper_helper_ee_muldiv_1_3_1(ee, odval);
    }

    case 0:
    {
      helper_helper_helper_ee_muldiv_1_3_2(ee, oival);
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
void helper_helper_helper_ee_muldiv_1_3_2(expreval_t * const ee, lilint_t oival)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = ((double) oival) / ee->dval;
      }
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = ((double) oival) / ((double) ee->ival);
      }
      ee->type = 1;
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
typedef int64_t lilint_t
----------------------------
void helper_helper_helper_ee_muldiv_1_3_1(expreval_t * const ee, double odval)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = odval / ee->dval;
      }
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = odval / ((double) ee->ival);
      }
      ee->type = 1;
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
***/


void helper_helper_ee_muldiv_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          if (ee->dval == 0.0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(odval, ee->dval);
          }
          break;
        }

        case 0:
        {
          if (ee->ival == 0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(odval, ee->ival);
          }
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          if (ee->dval == 0.0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(oival, ee->dval);
          }
          break;
        }

        case 0:
        {
          if (ee->ival == 0)
          {
            ee->error = 3;
          }
          else
          {
            ee->ival = oival % ee->ival;
          }
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_helper_ee_muldiv_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval * odval;
          break;
        }

        case 0:
        {
          ee->dval = ee->ival * odval;
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval * oival;
          ee->type = 1;
          break;
        }

        case 0:
        {
          ee->ival = ee->ival * oival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


void helper_ee_muldiv_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  switch (ee->code[ee->head])
  {
    case '*':
    {
      helper_helper_ee_muldiv_1_1(ee, odval, oival);
    }

    case '%':
    {
      helper_helper_ee_muldiv_1_2(ee, odval, oival);
    }

    case '/':
    {
      helper_helper_ee_muldiv_1_3(ee, odval, oival);
    }

    case '\\':
    {
      helper_helper_ee_muldiv_1_4(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
void helper_helper_ee_muldiv_1_4(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      helper_helper_helper_ee_muldiv_1_4_1(ee, odval);
    }

    case 0:
    {
      helper_helper_helper_ee_muldiv_1_4_2(ee, oival);
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
void helper_helper_ee_muldiv_1_3(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      helper_helper_helper_ee_muldiv_1_3_1(ee, odval);
    }

    case 0:
    {
      helper_helper_helper_ee_muldiv_1_3_2(ee, oival);
    }

  }

  break;
}


----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
void helper_helper_ee_muldiv_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          if (ee->dval == 0.0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(odval, ee->dval);
          }
          break;
        }

        case 0:
        {
          if (ee->ival == 0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(odval, ee->ival);
          }
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          if (ee->dval == 0.0)
          {
            ee->error = 3;
          }
          else
          {
            ee->dval = fmod(oival, ee->dval);
          }
          break;
        }

        case 0:
        {
          if (ee->ival == 0)
          {
            ee->error = 3;
          }
          else
          {
            ee->ival = oival % ee->ival;
          }
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

  }

  break;
}


----------------------------
void helper_helper_ee_muldiv_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval * odval;
          break;
        }

        case 0:
        {
          ee->dval = ee->ival * odval;
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_unary(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval * oival;
          ee->type = 1;
          break;
        }

        case 0:
        {
          ee->ival = ee->ival * oival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
***/


void helper_helper_ee_shift_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = ((lilint_t) odval) >> ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = ((lilint_t) odval) >> ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = oival >> ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = oival >> ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_ee_equals_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  int op = (ee->code[ee->head] == '=') ? (1) : (2);
  ee->head += 2;
  switch (op)
  {
    case 1:
    {
      helper_helper_ee_equals_1_1(ee, odval, oival);
    }

    case 2:
    {
      helper_helper_ee_equals_1_2(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
void helper_helper_ee_equals_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval != ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval != ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival != ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival != ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_shift(expreval_t *ee)
{
  ee_addsub(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && (((ee->code[ee->head] == '<') && (ee->code[ee->head + 1] == '<')) || ((ee->code[ee->head] == '>') && (ee->code[ee->head + 1] == '>'))))
  {
    helper_ee_shift_1(ee);
  }

}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_equals(expreval_t *ee)
{
  ee_compare(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && (((ee->code[ee->head] == '=') && (ee->code[ee->head + 1] == '=')) || ((ee->code[ee->head] == '!') && (ee->code[ee->head + 1] == '='))))
  {
    helper_ee_equals_1(ee);
  }

}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_helper_ee_shift_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = ((lilint_t) odval) << ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = ((lilint_t) odval) << ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = oival << ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = oival << ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_compare(expreval_t *ee)
{
  ee_shift(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && (((((ee->code[ee->head] == '<') && (!ee_invalidpunct(ee->code[ee->head + 1]))) || ((ee->code[ee->head] == '>') && (!ee_invalidpunct(ee->code[ee->head + 1])))) || ((ee->code[ee->head] == '<') && (ee->code[ee->head + 1] == '='))) || ((ee->code[ee->head] == '>') && (ee->code[ee->head + 1] == '='))))
  {
    helper_ee_compare_1(ee);
  }

}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
void helper_ee_compare_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  int op = 4;
  if ((ee->code[ee->head] == '<') && (!ee_invalidpunct(ee->code[ee->head + 1])))
  {
    op = 1;
  }
  else
    if ((ee->code[ee->head] == '>') && (!ee_invalidpunct(ee->code[ee->head + 1])))
  {
    op = 2;
  }
  else
    if ((ee->code[ee->head] == '<') && (ee->code[ee->head + 1] == '='))
  {
    op = 3;
  }
  ee->head += (op > 2) ? (2) : (1);
  switch (op)
  {
    case 1:
    {
      helper_helper_ee_compare_1_1(ee, odval, oival);
    }

    case 2:
    {
      helper_helper_ee_compare_1_2(ee, odval, oival);
    }

    case 3:
    {
      helper_helper_ee_compare_1_3(ee, odval, oival);
    }

    case 4:
    {
      helper_helper_ee_compare_1_4(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_ee_addsub_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  switch (ee->code[ee->head])
  {
    case '+':
    {
      helper_helper_ee_addsub_1_1(ee, odval, oival);
    }

    case '-':
    {
      helper_helper_ee_addsub_1_2(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


/*** DEPENDENCIES:
void helper_helper_ee_addsub_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = odval - ee->dval;
          break;
        }

        case 0:
        {
          ee->dval = odval - ee->ival;
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ((double) oival) - ee->dval;
          ee->type = 1;
          break;
        }

        case 0:
        {
          ee->ival = oival - ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_muldiv(expreval_t *ee)
{
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  ee_skip_spaces(ee);
  while ((((ee->head < ee->len) && (!ee->error)) && (!ee_invalidpunct(ee->code[ee->head + 1]))) && ((((ee->code[ee->head] == '*') || (ee->code[ee->head] == '/')) || (ee->code[ee->head] == '\\')) || (ee->code[ee->head] == '%')))
  {
    helper_ee_muldiv_1(ee);
  }

}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
void helper_ee_muldiv_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  switch (ee->code[ee->head])
  {
    case '*':
    {
      helper_helper_ee_muldiv_1_1(ee, odval, oival);
    }

    case '%':
    {
      helper_helper_ee_muldiv_1_2(ee, odval, oival);
    }

    case '/':
    {
      helper_helper_ee_muldiv_1_3(ee, odval, oival);
    }

    case '\\':
    {
      helper_helper_ee_muldiv_1_4(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_bitand(expreval_t *ee)
{
  ee_equals(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && ((ee->code[ee->head] == '&') && (!ee_invalidpunct(ee->code[ee->head + 1]))))
  {
    double odval = ee->dval;
    lilint_t oival = ee->ival;
    ee->head += 1;
    switch (ee->type)
    {
      case 1:
      {
        ee_equals(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = ((lilint_t) odval) & ((lilint_t) ee->dval);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = ((lilint_t) odval) & ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case 0:
      {
        ee_equals(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = oival & ((lilint_t) ee->dval);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = oival & ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      default:
      {
        ee->error = 2;
        break;
      }

    }

    ee_skip_spaces(ee);
  }

}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_helper_ee_equals_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (odval == ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (odval == ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee_compare(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = (oival == ee->dval) ? (1) : (0);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = (oival == ee->ival) ? (1) : (0);
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_helper_ee_addsub_1_1(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval + odval;
          break;
        }

        case 0:
        {
          ee->dval = ee->ival + odval;
          ee->type = 1;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_muldiv(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->dval = ee->dval + oival;
          ee->type = 1;
          break;
        }

        case 0:
        {
          ee->ival = ee->ival + oival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_unary(expreval_t *ee)
{
  ee_skip_spaces(ee);
  if (((ee->head < ee->len) && (!ee->error)) && ((((ee->code[ee->head] == '-') || (ee->code[ee->head] == '+')) || (ee->code[ee->head] == '~')) || (ee->code[ee->head] == '!')))
  {
    char op = ee->code[ee->head++];
    ee_unary(ee);
    if (ee->error)
    {
      return;
    }
    switch (op)
    {
      case '-':
      {
        switch (ee->type)
        {
          case 1:
          {
            ee->dval = -ee->dval;
            break;
          }

          case 0:
          {
            ee->ival = -ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case '+':
      {
        break;
      }

      case '~':
      {
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = ~((lilint_t) ee->dval);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = ~ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case '!':
      {
        switch (ee->type)
        {
          case 1:
          {
            ee->dval = !ee->dval;
            break;
          }

          case 0:
          {
            ee->ival = !ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

    }

  }
  else
  {
    ee_paren(ee);
  }
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_bitor(expreval_t *ee)
{
  ee_bitand(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && ((ee->code[ee->head] == '|') && (!ee_invalidpunct(ee->code[ee->head + 1]))))
  {
    double odval = ee->dval;
    lilint_t oival = ee->ival;
    ee->head += 1;
    switch (ee->type)
    {
      case 1:
      {
        ee_bitand(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = ((lilint_t) odval) | ((lilint_t) ee->dval);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = ((lilint_t) odval) | ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case 0:
      {
        ee_bitand(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = oival | ((lilint_t) ee->dval);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = oival | ee->ival;
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      default:
      {
        ee->error = 2;
        break;
      }

    }

    ee_skip_spaces(ee);
  }

}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
void helper_ee_shift_1(expreval_t * const ee)
{
  double odval = ee->dval;
  lilint_t oival = ee->ival;
  ee->head += 1;
  switch (ee->code[ee->head])
  {
    case '<':
    {
      helper_helper_ee_shift_1_1(ee, odval, oival);
    }

    case '>':
    {
      helper_helper_ee_shift_1_2(ee, odval, oival);
    }

  }

  ee_skip_spaces(ee);
}


/*** DEPENDENCIES:
void helper_helper_ee_shift_1_2(expreval_t * const ee, double odval, lilint_t oival)
{
  switch (ee->type)
  {
    case 1:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = ((lilint_t) odval) >> ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = ((lilint_t) odval) >> ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    case 0:
    {
      ee->head += 1;
      ee_addsub(ee);
      if (ee->error)
      {
        return;
      }
      switch (ee->type)
      {
        case 1:
        {
          ee->ival = oival >> ((lilint_t) ee->dval);
          ee->type = 0;
          break;
        }

        case 0:
        {
          ee->ival = oival >> ee->ival;
          break;
        }

        default:
        {
          ee->error = 2;
          break;
        }

      }

      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_addsub(expreval_t *ee)
{
  ee_muldiv(ee);
  ee_skip_spaces(ee);
  while ((((ee->head < ee->len) && (!ee->error)) && (!ee_invalidpunct(ee->code[ee->head + 1]))) && ((ee->code[ee->head] == '+') || (ee->code[ee->head] == '-')))
  {
    helper_ee_addsub_1(ee);
  }

}


/*** DEPENDENCIES:
static int ee_invalidpunct(int ch)
{
  return (((((ispunct(ch) && (ch != '!')) && (ch != '~')) && (ch != '(')) && (ch != ')')) && (ch != '-')) && (ch != '+');
}


----------------------------
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_paren(expreval_t *ee)
{
  ee_skip_spaces(ee);
  if (ee->code[ee->head] == '(')
  {
    ee->head += 1;
    ee_expr(ee);
    ee_skip_spaces(ee);
    if (ee->code[ee->head] == ')')
    {
      ee->head += 1;
    }
    else
      ee->error = 1;
  }
  else
    ee_element(ee);
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
static void ee_element(expreval_t *ee)
{
  if (isdigit(ee->code[ee->head]))
  {
    ee_numeric_element(ee);
    return;
  }
  ee->type = 0;
  ee->ival = 1;
  ee->error = 4;
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_logor(expreval_t *ee)
{
  ee_logand(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && ((ee->code[ee->head] == '|') && (ee->code[ee->head + 1] == '|')))
  {
    double odval = ee->dval;
    lilint_t oival = ee->ival;
    ee->head += 2;
    switch (ee->type)
    {
      case 1:
      {
        ee_logand(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = (odval || ee->dval) ? (1) : (0);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = (odval || ee->ival) ? (1) : (0);
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case 0:
      {
        ee_logand(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = (oival || ee->dval) ? (1) : (0);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = (oival || ee->ival) ? (1) : (0);
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      default:
      {
        ee->error = 2;
        break;
      }

    }

    ee_skip_spaces(ee);
  }

}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_logand(expreval_t *ee)
{
  ee_bitor(ee);
  ee_skip_spaces(ee);
  while (((ee->head < ee->len) && (!ee->error)) && ((ee->code[ee->head] == '&') && (ee->code[ee->head + 1] == '&')))
  {
    double odval = ee->dval;
    lilint_t oival = ee->ival;
    ee->head += 2;
    switch (ee->type)
    {
      case 1:
      {
        ee_bitor(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = (odval && ee->dval) ? (1) : (0);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = (odval && ee->ival) ? (1) : (0);
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      case 0:
      {
        ee_bitor(ee);
        if (ee->error)
        {
          return;
        }
        switch (ee->type)
        {
          case 1:
          {
            ee->ival = (oival && ee->dval) ? (1) : (0);
            ee->type = 0;
            break;
          }

          case 0:
          {
            ee->ival = (oival && ee->ival) ? (1) : (0);
            break;
          }

          default:
          {
            ee->error = 2;
            break;
          }

        }

        break;
      }

      default:
      {
        ee->error = 2;
        break;
      }

    }

    ee_skip_spaces(ee);
  }

}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
typedef int64_t lilint_t
----------------------------
static void ee_skip_spaces(expreval_t *ee)
{
  while ((ee->head < ee->len) && isspace(ee->code[ee->head]))
    ee->head++;

}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_ee_equals_1,ee_shift,ee_equals,helper_helper_ee_shift_1_1,ee_compare,helper_ee_addsub_1,ee_muldiv,ee_bitand,helper_helper_ee_equals_1_1,helper_helper_ee_addsub_1_1,ee_unary,ee_bitor,helper_ee_shift_1,ee_addsub,ee_paren,ee_logor,ee_logand,ee_expr
static void ee_expr(expreval_t *ee)
{
  ee_logor(ee);
  if (ee->error == 4)
  {
    ee->error = 0;
    ee->ival = 1;
  }
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
None
----------------------------
***/


void helper_helper_helper_ee_muldiv_1_3_1(expreval_t * const ee, double odval)
{
  ee->head += 1;
  ee_unary(ee);
  if (ee->error)
  {
    return;
  }
  switch (ee->type)
  {
    case 1:
    {
      if (ee->dval == 0.0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = odval / ee->dval;
      }
      break;
    }

    case 0:
    {
      if (ee->ival == 0)
      {
        ee->error = 3;
      }
      else
      {
        ee->dval = odval / ((double) ee->ival);
      }
      ee->type = 1;
      break;
    }

    default:
    {
      ee->error = 2;
      break;
    }

  }

  break;
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
None
----------------------------
***/


void lil_free_env(lil_env_t env)
{
  size_t i;
  if (!env)
  {
    return;
  }
  lil_free_value(env->retval);
  hm_destroy(&env->varmap);
  for (i = 0; i < env->vars; i += 1)
  {
    free(env->var[i]->n);
    lil_free_value(env->var[i]->v);
    free(env->var[i]->w);
    free(env->var[i]);
  }

  free(env->var);
  free(env);
}


/*** DEPENDENCIES:
typedef struct _lil_env_t *lil_env_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
static void hm_destroy(hashmap_t *hm)
{
  size_t i;
  size_t j;
  for (i = 0; i < 256; i += 1)
  {
    for (j = 0; j < hm->cell[i].c; j += 1)
      free(hm->cell[i].e[j].k);

    free(hm->cell[i].e);
  }

}


----------------------------
***/


void lil_free(lil_t lil)
{
  size_t i;
  if (!lil)
  {
    return;
  }
  free(lil->err_msg);
  lil_free_value(lil->empty);
  while (lil->env)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }

  for (i = 0; i < lil->cmds; i += 1)
  {
    if (lil->cmd[i]->argnames)
    {
      lil_free_list(lil->cmd[i]->argnames);
    }
    lil_free_value(lil->cmd[i]->code);
    free(lil->cmd[i]->name);
    free(lil->cmd[i]);
  }

  hm_destroy(&lil->cmdmap);
  free(lil->cmd);
  free(lil->dollarprefix);
  free(lil->catcher);
  free(lil);
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
void lil_free_env(lil_env_t env)
{
  size_t i;
  if (!env)
  {
    return;
  }
  lil_free_value(env->retval);
  hm_destroy(&env->varmap);
  for (i = 0; i < env->vars; i += 1)
  {
    free(env->var[i]->n);
    lil_free_value(env->var[i]->v);
    free(env->var[i]->w);
    free(env->var[i]);
  }

  free(env->var);
  free(env);
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
static void hm_destroy(hashmap_t *hm)
{
  size_t i;
  size_t j;
  for (i = 0; i < 256; i += 1)
  {
    for (j = 0; j < hm->cell[i].c; j += 1)
      free(hm->cell[i].e[j].k);

    free(hm->cell[i].e);
  }

}


----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static void hm_put(hashmap_t *hm, const char *key, void *value)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    cell->e[i].v = value;
    return;
  }

  cell->e = realloc(cell->e, (sizeof(struct hashentry_t)) * (cell->c + 1));
  cell->e[cell->c].k = strclone(key);
  cell->e[cell->c].v = value;
  cell->c += 1;
}


/*** DEPENDENCIES:
struct hashcell_t
{
  struct hashentry_t *e;
  size_t c;
}
----------------------------
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
static unsigned long hm_hash(const char *key)
{
  unsigned long hash = 5381;
  int c;
  while (c = *(key++))
    hash = ((hash << 5) + hash) + c;

  return hash;
}


----------------------------
typedef struct _hashmap_t
{
  struct hashcell_t cell[256];
} hashmap_t
----------------------------
***/


static lil_func_t add_func(lil_t lil, const char *name)
{
  lil_func_t cmd;
  lil_func_t *ncmd;
  unsigned int ncmd_idx = 0;
  cmd = find_cmd(lil, name);
  if (cmd)
  {
    if (cmd->argnames)
    {
      lil_free_list(cmd->argnames);
    }
    lil_free_value(cmd->code);
    cmd->argnames = 0;
    cmd->code = 0;
    cmd->proc = 0;
    return cmd;
  }
  cmd = calloc(1, sizeof(struct _lil_func_t));
  cmd->name = strclone(name);
  ncmd_idx = realloc(lil->cmd, (sizeof(lil_func_t)) * (lil->cmds + 1));
  if (!(&ncmd[ncmd_idx]))
  {
    free(cmd);
    return 0;
  }
  lil->cmd = &ncmd[ncmd_idx];
  ncmd[(lil->cmds++) + ncmd_idx] = cmd;
  hm_put(&lil->cmdmap, name, cmd);
  return cmd;
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
static void hm_put(hashmap_t *hm, const char *key, void *value)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    cell->e[i].v = value;
    return;
  }

  cell->e = realloc(cell->e, (sizeof(struct hashentry_t)) * (cell->c + 1));
  cell->e[cell->c].k = strclone(key);
  cell->e[cell->c].v = value;
  cell->c += 1;
}


----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


int lil_register(lil_t lil, const char *name, lil_func_proc_t proc)
{
  lil_func_t cmd = add_func(lil, name);
  if (!cmd)
  {
    return 0;
  }
  cmd->proc = proc;
  return 1;
}


/*** DEPENDENCIES:
typedef struct _lil_func_t *lil_func_t
----------------------------
typedef lil_value_t (*lil_func_proc_t)(lil_t lil, size_t argc, lil_value_t *argv)
----------------------------
static lil_func_t add_func(lil_t lil, const char *name)
{
  lil_func_t cmd;
  lil_func_t *ncmd;
  unsigned int ncmd_idx = 0;
  cmd = find_cmd(lil, name);
  if (cmd)
  {
    if (cmd->argnames)
    {
      lil_free_list(cmd->argnames);
    }
    lil_free_value(cmd->code);
    cmd->argnames = 0;
    cmd->code = 0;
    cmd->proc = 0;
    return cmd;
  }
  cmd = calloc(1, sizeof(struct _lil_func_t));
  cmd->name = strclone(name);
  ncmd_idx = realloc(lil->cmd, (sizeof(lil_func_t)) * (lil->cmds + 1));
  if (!(&ncmd[ncmd_idx]))
  {
    free(cmd);
    return 0;
  }
  lil->cmd = &ncmd[ncmd_idx];
  ncmd[(lil->cmds++) + ncmd_idx] = cmd;
  hm_put(&lil->cmdmap, name, cmd);
  return cmd;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


lil_env_t lil_push_env(lil_t lil)
{
  lil_env_t env = lil_alloc_env(lil->env);
  lil->env = env;
  return env;
}


/*** DEPENDENCIES:
lil_env_t lil_alloc_env(lil_env_t parent)
{
  lil_env_t env;
  env = calloc(1, sizeof(struct _lil_env_t));
  env->parent = parent;
  return env;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


void lil_pop_env(lil_t lil)
{
  if (lil->env->parent)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
void lil_free_env(lil_env_t env)
{
  size_t i;
  if (!env)
  {
    return;
  }
  lil_free_value(env->retval);
  hm_destroy(&env->varmap);
  for (i = 0; i < env->vars; i += 1)
  {
    free(env->var[i]->n);
    lil_free_value(env->var[i]->v);
    free(env->var[i]->w);
    free(env->var[i]);
  }

  free(env->var);
  free(env);
}


----------------------------
***/


lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


/*** DEPENDENCIES:
int lil_append_string(lil_value_t val, const char *s)
{
  return lil_append_string_len(val, s, strlen(s));
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
static int needs_escape(const char *str)
{
  size_t i;
  if ((!str) || (!str[0]))
  {
    return 1;
  }
  for (i = 0; str[i]; i += 1)
    if (ispunct(str[i]) || isspace(str[i]))
  {
    return 1;
  }

  return 0;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t get_bracketpart(lil_t lil)
{
  size_t cnt = 1;
  lil_value_t val;
  lil_value_t cmd = alloc_value(0);
  lil->head += 1;
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '[')
    {
      lil->head += 1;
      cnt += 1;
      lil_append_char(cmd, '[');
    }
    else
      if (lil->code[lil->head] == ']')
    {
      lil->head += 1;
      if ((--cnt) == 0)
      {
        break;
      }
      else
        lil_append_char(cmd, ']');
    }
    else
    {
      lil_append_char(cmd, lil->code[lil->head++]);
    }
  }

  val = lil_parse_value(lil, cmd, 0);
  lil_free_value(cmd);
  return val;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_next_word_1,next_word,helper_helper_next_word_1_1,get_dollarpart
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t']
void helper_next_word_1(lil_t * const lil_ref, lil_value_t * const val_ref)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  char sc = lil->code[lil->head++];
  val = alloc_value(0);
  while (lil->head < lil->clen)
  {
    helper_helper_next_word_1_1(&lil, val, sc);
  }

  *lil_ref = lil;
  *val_ref = val;
}


/*** DEPENDENCIES:
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_next_word_1,next_word,helper_helper_next_word_1_1,get_dollarpart
static lil_value_t next_word(lil_t lil)
{
  lil_value_t val;
  size_t start;
  skip_spaces(lil);
  if (lil->code[lil->head] == '$')
  {
    val = get_dollarpart(lil);
  }
  else
    if (lil->code[lil->head] == '{')
  {
    size_t cnt = 1;
    lil->head += 1;
    val = alloc_value(0);
    while (lil->head < lil->clen)
    {
      if (lil->code[lil->head] == '{')
      {
        lil->head += 1;
        cnt += 1;
        lil_append_char(val, '{');
      }
      else
        if (lil->code[lil->head] == '}')
      {
        lil->head += 1;
        if ((--cnt) == 0)
        {
          break;
        }
        else
          lil_append_char(val, '}');
      }
      else
      {
        lil_append_char(val, lil->code[lil->head++]);
      }
    }

  }
  else
    if (lil->code[lil->head] == '[')
  {
    val = get_bracketpart(lil);
  }
  else
    if ((lil->code[lil->head] == '"') || (lil->code[lil->head] == '\''))
  {
    helper_next_word_1(&lil, &val);
  }
  else
  {
    start = lil->head;
    while (((lil->head < lil->clen) && (!isspace(lil->code[lil->head]))) && (!islilspecial(lil->code[lil->head])))
    {
      lil->head += 1;
    }

    val = alloc_value_len(lil->code + start, lil->head - start);
  }
  return (val) ? (val) : (alloc_value(0));
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
static lil_value_t get_bracketpart(lil_t lil)
{
  size_t cnt = 1;
  lil_value_t val;
  lil_value_t cmd = alloc_value(0);
  lil->head += 1;
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '[')
    {
      lil->head += 1;
      cnt += 1;
      lil_append_char(cmd, '[');
    }
    else
      if (lil->code[lil->head] == ']')
    {
      lil->head += 1;
      if ((--cnt) == 0)
      {
        break;
      }
      else
        lil_append_char(cmd, ']');
    }
    else
    {
      lil_append_char(cmd, lil->code[lil->head++]);
    }
  }

  val = lil_parse_value(lil, cmd, 0);
  lil_free_value(cmd);
  return val;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
static int islilspecial(char ch)
{
  return (((((((ch == '$') || (ch == '{')) || (ch == '}')) || (ch == '[')) || (ch == ']')) || (ch == '"')) || (ch == '\'')) || (ch == ';');
}


----------------------------
static lil_value_t alloc_value_len(const char *str, size_t len)
{
  lil_value_t val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  if (str)
  {
    val->l = len;
    val->d = malloc(len + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, str, len);
    val->d[len] = 0;
  }
  else
  {
    val->l = 0;
    val->d = 0;
  }
  return val;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
static void skip_spaces(lil_t lil)
{
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '#')
    {
      if ((lil->code[lil->head + 1] == '#') && (lil->code[lil->head + 2] != '#'))
      {
        lil->head += 2;
        while (lil->head < lil->clen)
        {
          if (((lil->code[lil->head] == '#') && (lil->code[lil->head + 1] == '#')) && (lil->code[lil->head + 2] != '#'))
          {
            lil->head += 2;
            break;
          }
          lil->head += 1;
        }

      }
      else
      {
        while ((lil->head < lil->clen) && (!ateol(lil)))
          lil->head++;

      }
    }
    else
      if ((lil->code[lil->head] == '\\') && ((lil->code[lil->head + 1] == '\r') || (lil->code[lil->head + 1] == '\n')))
    {
      lil->head += 1;
      while ((lil->head < lil->clen) && ateol(lil))
        lil->head++;

    }
    else
      if ((lil->code[lil->head] == '\r') || (lil->code[lil->head] == '\n'))
    {
      if (lil->ignoreeol)
      {
        lil->head += 1;
      }
      else
        break;
    }
    else
      if (isspace(lil->code[lil->head]))
    {
      lil->head += 1;
    }
    else
      break;
  }

}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


// element in merge of:  helper_next_word_1,next_word,helper_helper_next_word_1_1,get_dollarpart
// hint:  ['lil_ref is a mutable refrence to lil_t']
void helper_helper_next_word_1_1(lil_t * const lil_ref, lil_value_t val, char sc)
{
  lil_t lil = *lil_ref;
  if ((lil->code[lil->head] == '[') || (lil->code[lil->head] == '$'))
  {
    lil_value_t tmp = (lil->code[lil->head] == '$') ? (get_dollarpart(lil)) : (get_bracketpart(lil));
    lil_append_val(val, tmp);
    lil_free_value(tmp);
    lil->head -= 1;
  }
  else
    if (lil->code[lil->head] == '\\')
  {
    lil->head += 1;
    switch (lil->code[lil->head])
    {
      case 'b':
      {
        lil_append_char(val, '\b');
        break;
      }

      case 't':
      {
        lil_append_char(val, '\t');
        break;
      }

      case 'n':
      {
        lil_append_char(val, '\n');
        break;
      }

      case 'v':
      {
        lil_append_char(val, '\v');
        break;
      }

      case 'f':
      {
        lil_append_char(val, '\f');
        break;
      }

      case 'r':
      {
        lil_append_char(val, '\r');
        break;
      }

      case '0':
      {
        lil_append_char(val, 0);
        break;
      }

      case 'a':
      {
        lil_append_char(val, '\a');
        break;
      }

      case 'c':
      {
        lil_append_char(val, '}');
        break;
      }

      case 'o':
      {
        lil_append_char(val, '{');
        break;
      }

      default:
      {
        lil_append_char(val, lil->code[lil->head]);
        break;
      }

    }

  }
  else
    if (lil->code[lil->head] == sc)
  {
    lil->head += 1;
    break;
  }
  else
  {
    lil_append_char(val, lil->code[lil->head]);
  }
  lil->head += 1;
  *lil_ref = lil;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
static lil_value_t get_bracketpart(lil_t lil)
{
  size_t cnt = 1;
  lil_value_t val;
  lil_value_t cmd = alloc_value(0);
  lil->head += 1;
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '[')
    {
      lil->head += 1;
      cnt += 1;
      lil_append_char(cmd, '[');
    }
    else
      if (lil->code[lil->head] == ']')
    {
      lil->head += 1;
      if ((--cnt) == 0)
      {
        break;
      }
      else
        lil_append_char(cmd, ']');
    }
    else
    {
      lil_append_char(cmd, lil->code[lil->head++]);
    }
  }

  val = lil_parse_value(lil, cmd, 0);
  lil_free_value(cmd);
  return val;
}


----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  helper_next_word_1,next_word,helper_helper_next_word_1_1,get_dollarpart
static lil_value_t get_dollarpart(lil_t lil)
{
  lil_value_t val;
  lil_value_t name;
  lil_value_t tmp;
  lil->head += 1;
  name = next_word(lil);
  tmp = alloc_value(lil->dollarprefix);
  lil_append_val(tmp, name);
  lil_free_value(name);
  val = lil_parse_value(lil, tmp, 0);
  lil_free_value(tmp);
  return val;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_list_t substitute(lil_t lil)
{
  lil_list_t words = lil_alloc_list();
  skip_spaces(lil);
  while (((lil->head < lil->clen) && (!ateol(lil))) && (!lil->error))
  {
    lil_value_t w = alloc_value(0);
    do
    {
      size_t head = lil->head;
      lil_value_t wp = next_word(lil);
      if (head == lil->head)
      {
        lil_free_value(w);
        lil_free_value(wp);
        lil_free_list(words);
        return 0;
      }
      lil_append_val(w, wp);
      lil_free_value(wp);
    }
    while ((((lil->head < lil->clen) && (!ateol(lil))) && (!isspace(lil->code[lil->head]))) && (!lil->error));
    skip_spaces(lil);
    lil_list_append(words, w);
  }

  return words;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
static int ateol(lil_t lil)
{
  return (!lil->ignoreeol) && (((lil->code[lil->head] == '\n') || (lil->code[lil->head] == '\r')) || (lil->code[lil->head] == ';'));
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
static void skip_spaces(lil_t lil)
{
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '#')
    {
      if ((lil->code[lil->head + 1] == '#') && (lil->code[lil->head + 2] != '#'))
      {
        lil->head += 2;
        while (lil->head < lil->clen)
        {
          if (((lil->code[lil->head] == '#') && (lil->code[lil->head + 1] == '#')) && (lil->code[lil->head + 2] != '#'))
          {
            lil->head += 2;
            break;
          }
          lil->head += 1;
        }

      }
      else
      {
        while ((lil->head < lil->clen) && (!ateol(lil)))
          lil->head++;

      }
    }
    else
      if ((lil->code[lil->head] == '\\') && ((lil->code[lil->head + 1] == '\r') || (lil->code[lil->head + 1] == '\n')))
    {
      lil->head += 1;
      while ((lil->head < lil->clen) && ateol(lil))
        lil->head++;

    }
    else
      if ((lil->code[lil->head] == '\r') || (lil->code[lil->head] == '\n'))
    {
      if (lil->ignoreeol)
      {
        lil->head += 1;
      }
      else
        break;
    }
    else
      if (isspace(lil->code[lil->head]))
    {
      lil->head += 1;
    }
    else
      break;
  }

}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
lil_value_t lil_parse(lil_t lil, const char *code, size_t codelen, int funclevel)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  lil_value_t val = 0;
  lil_list_t words = 0;
  if (!save_code)
  {
    lil->rootcode = code;
  }
  lil->code = code;
  lil->clen = (codelen) ? (codelen) : (strlen(code));
  lil->head = 0;
  skip_spaces(lil);
  lil->parse_depth += 1;
  if (lil->parse_depth == 1)
  {
    lil->error = 0;
  }
  if (funclevel)
  {
    lil->env->breakrun = 0;
  }
  while ((lil->head < lil->clen) && (!lil->error))
  {
    helper_lil_parse_1(&lil, &val, &words);
  }

  cleanup:
  if ((lil->error && lil->callback[5]) && (lil->parse_depth == 1))
  {
    lil_error_callback_proc_t proc = (lil_error_callback_proc_t) lil->callback[5];
    proc(lil, lil->err_head, lil->err_msg);
  }

  if (words)
  {
    lil_free_list(words);
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  if (funclevel && lil->env->retval_set)
  {
    if (val)
    {
      lil_free_value(val);
    }
    val = lil->env->retval;
    lil->env->retval = 0;
    lil->env->retval_set = 0;
    lil->env->breakrun = 0;
  }
  lil->parse_depth -= 1;
  return (val) ? (val) : (alloc_value(0));
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef void (*lil_error_callback_proc_t)(lil_t lil, size_t pos, const char *msg)
----------------------------
static void skip_spaces(lil_t lil)
{
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '#')
    {
      if ((lil->code[lil->head + 1] == '#') && (lil->code[lil->head + 2] != '#'))
      {
        lil->head += 2;
        while (lil->head < lil->clen)
        {
          if (((lil->code[lil->head] == '#') && (lil->code[lil->head + 1] == '#')) && (lil->code[lil->head + 2] != '#'))
          {
            lil->head += 2;
            break;
          }
          lil->head += 1;
        }

      }
      else
      {
        while ((lil->head < lil->clen) && (!ateol(lil)))
          lil->head++;

      }
    }
    else
      if ((lil->code[lil->head] == '\\') && ((lil->code[lil->head + 1] == '\r') || (lil->code[lil->head + 1] == '\n')))
    {
      lil->head += 1;
      while ((lil->head < lil->clen) && ateol(lil))
        lil->head++;

    }
    else
      if ((lil->code[lil->head] == '\r') || (lil->code[lil->head] == '\n'))
    {
      if (lil->ignoreeol)
      {
        lil->head += 1;
      }
      else
        break;
    }
    else
      if (isspace(lil->code[lil->head]))
    {
      lil->head += 1;
    }
    else
      break;
  }

}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
lil_var_t lil_set_var(lil_t lil, const char *name, lil_value_t val, int local)
{
  lil_var_t *nvar;
  unsigned int nvar_idx = 0;
  lil_env_t env = (local == 0) ? (lil->rootenv) : (lil->env);
  int freeval = 0;
  if (!name[0])
  {
    return 0;
  }
  if (local != 2)
  {
    helper_lil_set_var_1(&lil, &val, &env, &freeval, name, local);
  }
  nvar_idx = realloc(env->var, (sizeof(lil_var_t)) * (env->vars + 1));
  if (!(&nvar[nvar_idx]))
  {
    return 0;
  }
  env->var = &nvar[nvar_idx];
  nvar[env->vars + nvar_idx] = calloc(1, sizeof(struct _lil_var_t));
  nvar[env->vars + nvar_idx]->n = strclone(name);
  nvar[env->vars + nvar_idx]->w = 0;
  nvar[env->vars + nvar_idx]->env = env;
  nvar[env->vars + nvar_idx]->v = (freeval) ? (val) : (lil_clone_value(val));
  hm_put(&env->varmap, name, nvar[env->vars + nvar_idx]);
  return nvar[(env->vars++) + nvar_idx];
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static void hm_put(hashmap_t *hm, const char *key, void *value)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    cell->e[i].v = value;
    return;
  }

  cell->e = realloc(cell->e, (sizeof(struct hashentry_t)) * (cell->c + 1));
  cell->e[cell->c].k = strclone(key);
  cell->e[cell->c].v = value;
  cell->c += 1;
}


----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t']
void helper_helper_lil_parse_1_1(lil_t * const lil_ref, lil_value_t * const val_ref, lil_list_t words)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  lil_func_t cmd = find_cmd(lil, lil_to_string(words->v[0]));
  if (!cmd)
  {
    helper_helper_helper_lil_parse_1_1_2(&lil, &val, words);
  }
  if (cmd)
  {
    helper_helper_helper_lil_parse_1_1_1(&lil, &val, words, cmd);
  }
  *lil_ref = lil;
  *val_ref = val;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t', 'env_ref is a mutable refrence to lil_env_t', 'freeval_ref is a mutable refrence to int']
lil_var_t helper_lil_set_var_1(lil_t * const lil_ref, lil_value_t * const val_ref, lil_env_t * const env_ref, int * const freeval_ref, const char * const name, int local)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  lil_env_t env = *env_ref;
  int freeval = *freeval_ref;
  lil_var_t var = lil_find_var(lil, env, name);
  if ((((local == 3) && var) && (var->env == lil->rootenv)) && (var->env != env))
  {
    var = 0;
  }
  if ((((!var) && (env == lil->rootenv)) || (var && (var->env == lil->rootenv))) && lil->callback[6])
  {
    lil_setvar_callback_proc_t proc = (lil_setvar_callback_proc_t) lil->callback[6];
    lil_value_t newval = val;
    int r = proc(lil, name, &newval);
    if (r < 0)
    {
      return 0;
    }
    if (r)
    {
      val = newval;
      freeval = 1;
    }
  }
  if (var)
  {
    lil_free_value(var->v);
    var->v = (freeval) ? (val) : (lil_clone_value(val));
    if (var->w)
    {
      lil_env_t save_env;
      save_env = lil->env;
      lil->env = var->env;
      lil_free_value(lil_parse(lil, var->w, 0, 1));
      lil->env = save_env;
    }
    return var;
  }
  *lil_ref = lil;
  *val_ref = val;
  *env_ref = env;
  *freeval_ref = freeval;
}


/*** DEPENDENCIES:
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static lil_var_t lil_find_var(lil_t lil, lil_env_t env, const char *name)
{
  lil_var_t r = lil_find_local_var(lil, env, name);
  return (r) ? (r) : ((env == lil->rootenv) ? (0) : (lil_find_var(lil, lil->rootenv, name)));
}


----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
typedef int (*lil_setvar_callback_proc_t)(lil_t lil, const char *name, lil_value_t *value)
----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t']
void helper_helper_helper_lil_parse_1_1_2(lil_t * const lil_ref, lil_value_t * const val_ref, lil_list_t words)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  if (words->v[0]->l)
  {
    if (lil->catcher)
    {
      if (lil->in_catcher < 16384)
      {
        lil_value_t args;
        lil->in_catcher += 1;
        lil_push_env(lil);
        lil->env->catcher_for = words->v[0];
        args = lil_list_to_value(words, 1);
        lil_set_var(lil, "args", args, 2);
        lil_free_value(args);
        val = lil_parse(lil, lil->catcher, 0, 1);
        lil_pop_env(lil);
        lil->in_catcher -= 1;
      }
      else
      {
        char *msg = malloc(words->v[0]->l + 64);
        sprintf(msg, "catcher limit reached while trying to call unknown function %s", words->v[0]->d);
        lil_set_error_at(lil, lil->head, msg);
        free(msg);
        goto cleanup;
      }
    }
    else
    {
      char *msg = malloc(words->v[0]->l + 32);
      sprintf(msg, "unknown function %s", words->v[0]->d);
      lil_set_error_at(lil, lil->head, msg);
      free(msg);
      goto cleanup;
    }
  }
  *lil_ref = lil;
  *val_ref = val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_env_t lil_push_env(lil_t lil)
{
  lil_env_t env = lil_alloc_env(lil->env);
  lil->env = env;
  return env;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
void lil_pop_env(lil_t lil)
{
  if (lil->env->parent)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }
}


----------------------------
void lil_set_error_at(lil_t lil, size_t pos, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 1;
  lil->err_head = pos;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t']
void helper_helper_helper_lil_parse_1_1_1(lil_t * const lil_ref, lil_value_t * const val_ref, lil_list_t words, lil_func_t cmd)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  if (cmd->proc)
  {
    size_t shead = lil->head;
    val = cmd->proc(lil, words->c - 1, words->v + 1);
    if (lil->error == 2)
    {
      lil->error = 1;
      lil->err_head = shead;
    }
  }
  else
  {
    lil_push_env(lil);
    lil->env->func = cmd;
    if ((cmd->argnames->c == 1) && (!strcmp(lil_to_string(cmd->argnames->v[0]), "args")))
    {
      lil_value_t args = lil_list_to_value(words, 1);
      lil_set_var(lil, "args", args, 2);
      lil_free_value(args);
    }
    else
    {
      size_t i;
      for (i = 0; i < cmd->argnames->c; i += 1)
      {
        lil_set_var(lil, lil_to_string(cmd->argnames->v[i]), (i < (words->c - 1)) ? (words->v[i + 1]) : (lil->empty), 2);
      }

    }
    val = lil_parse_value(lil, cmd->code, 1);
    lil_pop_env(lil);
  }
  *lil_ref = lil;
  *val_ref = val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_env_t lil_push_env(lil_t lil)
{
  lil_env_t env = lil_alloc_env(lil->env);
  lil->env = env;
  return env;
}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
void lil_pop_env(lil_t lil)
{
  if (lil->env->parent)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
lil_value_t lil_parse_value(lil_t lil, lil_value_t val, int funclevel)
{
  if (((!val) || (!val->d)) || (!val->l))
  {
    return alloc_value(0);
  }
  return lil_parse(lil, val->d, val->l, funclevel);
}


/*** DEPENDENCIES:
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
None
----------------------------
***/


// element in merge of:  lil_parse,lil_set_var,helper_helper_lil_parse_1_1,helper_lil_set_var_1,helper_helper_helper_lil_parse_1_1_2,helper_helper_helper_lil_parse_1_1_1,lil_parse_value,helper_lil_parse_1
// hint:  ['lil_ref is a mutable refrence to lil_t', 'val_ref is a mutable refrence to lil_value_t', 'words_ref is a mutable refrence to lil_list_t']
void helper_lil_parse_1(lil_t * const lil_ref, lil_value_t * const val_ref, lil_list_t * const words_ref)
{
  lil_t lil = *lil_ref;
  lil_value_t val = *val_ref;
  lil_list_t words = *words_ref;
  if (words)
  {
    lil_free_list(words);
  }
  if (val)
  {
    lil_free_value(val);
  }
  val = 0;
  words = substitute(lil);
  if ((!words) || lil->error)
  {
    goto cleanup;
  }
  if (words->c)
  {
    helper_helper_lil_parse_1_1(&lil, &val, words);
  }
  if (lil->env->breakrun)
  {
    goto cleanup;
  }
  skip_spaces(lil);
  while (ateol(lil))
    lil->head++;

  skip_spaces(lil);
  *lil_ref = lil;
  *val_ref = val;
  *words_ref = words;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
static int ateol(lil_t lil)
{
  return (!lil->ignoreeol) && (((lil->code[lil->head] == '\n') || (lil->code[lil->head] == '\r')) || (lil->code[lil->head] == ';'));
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static void skip_spaces(lil_t lil)
{
  while (lil->head < lil->clen)
  {
    if (lil->code[lil->head] == '#')
    {
      if ((lil->code[lil->head + 1] == '#') && (lil->code[lil->head + 2] != '#'))
      {
        lil->head += 2;
        while (lil->head < lil->clen)
        {
          if (((lil->code[lil->head] == '#') && (lil->code[lil->head + 1] == '#')) && (lil->code[lil->head + 2] != '#'))
          {
            lil->head += 2;
            break;
          }
          lil->head += 1;
        }

      }
      else
      {
        while ((lil->head < lil->clen) && (!ateol(lil)))
          lil->head++;

      }
    }
    else
      if ((lil->code[lil->head] == '\\') && ((lil->code[lil->head + 1] == '\r') || (lil->code[lil->head + 1] == '\n')))
    {
      lil->head += 1;
      while ((lil->head < lil->clen) && ateol(lil))
        lil->head++;

    }
    else
      if ((lil->code[lil->head] == '\r') || (lil->code[lil->head] == '\n'))
    {
      if (lil->ignoreeol)
      {
        lil->head += 1;
      }
      else
        break;
    }
    else
      if (isspace(lil->code[lil->head]))
    {
      lil->head += 1;
    }
    else
      break;
  }

}


----------------------------
static lil_list_t substitute(lil_t lil)
{
  lil_list_t words = lil_alloc_list();
  skip_spaces(lil);
  while (((lil->head < lil->clen) && (!ateol(lil))) && (!lil->error))
  {
    lil_value_t w = alloc_value(0);
    do
    {
      size_t head = lil->head;
      lil_value_t wp = next_word(lil);
      if (head == lil->head)
      {
        lil_free_value(w);
        lil_free_value(wp);
        lil_free_list(words);
        return 0;
      }
      lil_append_val(w, wp);
      lil_free_value(wp);
    }
    while ((((lil->head < lil->clen) && (!ateol(lil))) && (!isspace(lil->code[lil->head]))) && (!lil->error));
    skip_spaces(lil);
    lil_list_append(words, w);
  }

  return words;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static int hm_has(hashmap_t *hm, const char *key)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    return 1;
  }

  return 0;
}


/*** DEPENDENCIES:
struct hashcell_t
{
  struct hashentry_t *e;
  size_t c;
}
----------------------------
static unsigned long hm_hash(const char *key)
{
  unsigned long hash = 5381;
  int c;
  while (c = *(key++))
    hash = ((hash << 5) + hash) + c;

  return hash;
}


----------------------------
typedef struct _hashmap_t
{
  struct hashcell_t cell[256];
} hashmap_t
----------------------------
***/


static lil_value_t fnc_reflect(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_func_t func;
  const char *type;
  unsigned int type_idx = 0;
  size_t i;
  lil_value_t r;
  if (!argc)
  {
    return 0;
  }
  type_idx = lil_to_string(argv[0]);
  if (!strcmp(type, "version"))
  {
    return lil_alloc_string("0.1");
  }
  if (!strcmp(type, "args"))
  {
    if (argc < 2)
    {
      return 0;
    }
    func = find_cmd(lil, lil_to_string(argv[1]));
    if ((!func) || (!func->argnames))
    {
      return 0;
    }
    return lil_list_to_value(func->argnames, 1);
  }
  if (!strcmp(type, "body"))
  {
    if (argc < 2)
    {
      return 0;
    }
    func = find_cmd(lil, lil_to_string(argv[1]));
    if ((!func) || func->proc)
    {
      return 0;
    }
    return lil_clone_value(func->code);
  }
  if (!strcmp(type, "func-count"))
  {
    return lil_alloc_integer(lil->cmds);
  }
  if (!strcmp(type, "funcs"))
  {
    lil_list_t funcs = lil_alloc_list();
    for (i = 0; i < lil->cmds; i += 1)
      lil_list_append(funcs, lil_alloc_string(lil->cmd[i]->name));

    r = lil_list_to_value(funcs, 1);
    lil_free_list(funcs);
    return r;
  }
  if (!strcmp(type, "vars"))
  {
    lil_list_t vars = lil_alloc_list();
    lil_env_t env = lil->env;
    while (env)
    {
      for (i = 0; i < env->vars; i += 1)
        lil_list_append(vars, lil_alloc_string(env->var[i]->n));

      env = env->parent;
    }

    r = lil_list_to_value(vars, 1);
    lil_free_list(vars);
    return r;
  }
  if (!strcmp(type, "globals"))
  {
    lil_list_t vars = lil_alloc_list();
    for (i = 0; i < lil->rootenv->vars; i += 1)
      lil_list_append(vars, lil_alloc_string(lil->rootenv->var[i]->n));

    r = lil_list_to_value(vars, 1);
    lil_free_list(vars);
    return r;
  }
  if (!strcmp(type, "has-func"))
  {
    const char *target;
    unsigned int target_idx = 0;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    return (hm_has(&lil->cmdmap, target)) ? (lil_alloc_string("1")) : (0);
  }
  if (!strcmp(type, "has-var"))
  {
    const char *target;
    unsigned int target_idx = 0;
    lil_env_t env = lil->env;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    while (env)
    {
      if (hm_has(&env->varmap, target))
      {
        return lil_alloc_string("1");
      }
      env = env->parent;
    }

    return 0;
  }
  if (!strcmp(type, "has-global"))
  {
    const char *target;
    unsigned int target_idx = 0;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    for (i = 0; i < lil->rootenv->vars; i += 1)
      if (!strcmp(target, lil->rootenv->var[i]->n))
    {
      return lil_alloc_string("1");
    }

    return 0;
  }
  if (!strcmp(type, "error"))
  {
    return (lil->err_msg) ? (lil_alloc_string(lil->err_msg)) : (0);
  }
  if (!strcmp(type, "dollar-prefix"))
  {
    lil_value_t r;
    if (argc == 1)
    {
      return lil_alloc_string(lil->dollarprefix);
    }
    r = lil_alloc_string(lil->dollarprefix);
    free(lil->dollarprefix);
    lil->dollarprefix = strclone(lil_to_string(argv[1]));
    return r;
  }
  if (!strcmp(type, "this"))
  {
    lil_env_t env = lil->env;
    while (((env != lil->rootenv) && (!env->catcher_for)) && (!env->func))
      env = env->parent;

    if (env->catcher_for)
    {
      return lil_alloc_string(lil->catcher);
    }
    if (env == lil->rootenv)
    {
      return lil_alloc_string(lil->rootcode);
    }
    return (env->func) ? (env->func->code) : (0);
  }
  if (!strcmp(type, "name"))
  {
    lil_env_t env = lil->env;
    while (((env != lil->rootenv) && (!env->catcher_for)) && (!env->func))
      env = env->parent;

    if (env->catcher_for)
    {
      return env->catcher_for;
    }
    if (env == lil->rootenv)
    {
      return 0;
    }
    return (env->func) ? (lil_alloc_string(env->func->name)) : (0);
  }
  return 0;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
static int hm_has(hashmap_t *hm, const char *key)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    return 1;
  }

  return 0;
}


----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
***/


static lil_value_t fnc_strcmp(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 2)
  {
    return 0;
  }
  return lil_alloc_integer(strcmp(lil_to_string(argv[0]), lil_to_string(argv[1])));
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t real_trim(const char *str, const char *chars, int left, int right)
{
  int base = 0;
  lil_value_t r = 0;
  if (left)
  {
    while (str[base] && strchr(chars, str[base]))
      base++;

    base += 1;
    if (!right)
      r = lil_alloc_string((str[base]) ? (str + base) : (0));
  }
  if (right)
  {
    size_t len;
    char *s;
    unsigned int s_idx = 0;
    s_idx = strclone(str + base);
    len = strlen(s);
    while (len && strchr(chars, s[(len - 1) + s_idx]))
      len--;

    len -= 1;
    s[len + s_idx] = 0;
    r = lil_alloc_string(s);
    free(s);
  }
  return r;
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_trim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 1, 1);
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
static lil_value_t real_trim(const char *str, const char *chars, int left, int right)
{
  int base = 0;
  lil_value_t r = 0;
  if (left)
  {
    while (str[base] && strchr(chars, str[base]))
      base++;

    base += 1;
    if (!right)
      r = lil_alloc_string((str[base]) ? (str + base) : (0));
  }
  if (right)
  {
    size_t len;
    char *s;
    unsigned int s_idx = 0;
    s_idx = strclone(str + base);
    len = strlen(s);
    while (len && strchr(chars, s[(len - 1) + s_idx]))
      len--;

    len -= 1;
    s[len + s_idx] = 0;
    r = lil_alloc_string(s);
    free(s);
  }
  return r;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static lil_list_t substitute(lil_t lil)
{
  lil_list_t words = lil_alloc_list();
  skip_spaces(lil);
  while (((lil->head < lil->clen) && (!ateol(lil))) && (!lil->error))
  {
    lil_value_t w = alloc_value(0);
    do
    {
      size_t head = lil->head;
      lil_value_t wp = next_word(lil);
      if (head == lil->head)
      {
        lil_free_value(w);
        lil_free_value(wp);
        lil_free_list(words);
        return 0;
      }
      lil_append_val(w, wp);
      lil_free_value(wp);
    }
    while ((((lil->head < lil->clen) && (!ateol(lil))) && (!isspace(lil->code[lil->head]))) && (!lil->error));
    skip_spaces(lil);
    lil_list_append(words, w);
  }

  return words;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
***/


static lil_value_t fnc_concat(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_value_t r;
  lil_value_t tmp;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  r = lil_alloc_string("");
  for (i = 0; i < argc; i += 1)
  {
    list = lil_subst_to_list(lil, argv[i]);
    tmp = lil_list_to_value(list, 1);
    lil_free_list(list);
    lil_append_val(r, tmp);
    lil_free_value(tmp);
  }

  return r;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static lil_value_t fnc_indexof(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t index;
  lil_value_t r = 0;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  for (index = 0; index < list->c; index += 1)
    if (!strcmp(lil_to_string(list->v[index]), lil_to_string(argv[1])))
  {
    r = lil_alloc_integer(index);
    break;
  }

  lil_free_list(list);
  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_rand(lil_t lil, size_t argc, lil_value_t *argv)
{
  return lil_alloc_double(rand() / ((double) 32767));
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_double(double num)
{
  char buff[128];
  sprintf(buff, "%f", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


lil_value_t lil_subst_to_value(lil_t lil, lil_value_t code)
{
  lil_list_t words = lil_subst_to_list(lil, code);
  lil_value_t val;
  val = lil_list_to_value(words, 0);
  lil_free_list(words);
  return val;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
***/


static lil_value_t fnc_subst(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return lil_subst_to_value(lil, argv[0]);
}


/*** DEPENDENCIES:
lil_value_t lil_subst_to_value(lil_t lil, lil_value_t code)
{
  lil_list_t words = lil_subst_to_list(lil, code);
  lil_value_t val;
  val = lil_list_to_value(words, 0);
  lil_free_list(words);
  return val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_return(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil->env->breakrun = 1;
  lil_free_value(lil->env->retval);
  lil->env->retval = (argc < 1) ? (0) : (lil_clone_value(argv[0]));
  lil->env->retval_set = 1;
  return (argc < 1) ? (0) : (lil_clone_value(argv[0]));
}


/*** DEPENDENCIES:
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
***/


static lil_value_t fnc_quote(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  r = alloc_value(0);
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      lil_append_char(r, ' ');
    }
    lil_append_val(r, argv[i]);
  }

  return r;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_eval(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_parse_value(lil, argv[0], 0);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_parse_value(lil, val, 0);
    lil_free_value(val);
    return r;
  }
  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_downeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_env_t upenv = lil->env;
  lil_env_t downenv = lil->downenv;
  if (!downenv)
  {
    return fnc_eval(lil, argc, argv);
  }
  lil->downenv = 0;
  lil->env = downenv;
  r = fnc_eval(lil, argc, argv);
  lil->downenv = downenv;
  lil->env = upenv;
  return r;
}


/*** DEPENDENCIES:
static lil_value_t fnc_eval(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_parse_value(lil, argv[0], 0);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_parse_value(lil, val, 0);
    lil_free_value(val);
    return r;
  }
  return 0;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


lil_value_t lil_unused_name(lil_t lil, const char *part)
{
  char *name = malloc(strlen(part) + 64);
  lil_value_t val;
  size_t i;
  for (i = 0; i < ((size_t) (-1)); i += 1)
  {
    sprintf(name, "!!un!%s!%09u!nu!!", part, (unsigned int) i);
    if (find_cmd(lil, name))
    {
      continue;
    }
    if (lil_find_var(lil, lil->env, name))
    {
      continue;
    }
    val = lil_alloc_string(name);
    free(name);
    return val;
  }

  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
static lil_var_t lil_find_var(lil_t lil, lil_env_t env, const char *name)
{
  lil_var_t r = lil_find_local_var(lil, env, name);
  return (r) ? (r) : ((env == lil->rootenv) ? (0) : (lil_find_var(lil, lil->rootenv, name)));
}


----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_func(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t name;
  lil_func_t cmd;
  lil_list_t fargs;
  if (argc < 1)
  {
    return 0;
  }
  if (argc >= 3)
  {
    name = lil_clone_value(argv[0]);
    fargs = lil_subst_to_list(lil, argv[1]);
    cmd = add_func(lil, lil_to_string(argv[0]));
    cmd->argnames = fargs;
    cmd->code = lil_clone_value(argv[2]);
  }
  else
  {
    name = lil_unused_name(lil, "anonymous-function");
    if (argc < 2)
    {
      lil_value_t tmp = lil_alloc_string("args");
      fargs = lil_subst_to_list(lil, tmp);
      lil_free_value(tmp);
      cmd = add_func(lil, lil_to_string(name));
      cmd->argnames = fargs;
      cmd->code = lil_clone_value(argv[0]);
    }
    else
    {
      fargs = lil_subst_to_list(lil, argv[0]);
      cmd = add_func(lil, lil_to_string(name));
      cmd->argnames = fargs;
      cmd->code = lil_clone_value(argv[1]);
    }
  }
  return name;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
lil_value_t lil_unused_name(lil_t lil, const char *part)
{
  char *name = malloc(strlen(part) + 64);
  lil_value_t val;
  size_t i;
  for (i = 0; i < ((size_t) (-1)); i += 1)
  {
    sprintf(name, "!!un!%s!%09u!nu!!", part, (unsigned int) i);
    if (find_cmd(lil, name))
    {
      continue;
    }
    if (lil_find_var(lil, lil->env, name))
    {
      continue;
    }
    val = lil_alloc_string(name);
    free(name);
    return val;
  }

  return 0;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
static lil_func_t add_func(lil_t lil, const char *name)
{
  lil_func_t cmd;
  lil_func_t *ncmd;
  unsigned int ncmd_idx = 0;
  cmd = find_cmd(lil, name);
  if (cmd)
  {
    if (cmd->argnames)
    {
      lil_free_list(cmd->argnames);
    }
    lil_free_value(cmd->code);
    cmd->argnames = 0;
    cmd->code = 0;
    cmd->proc = 0;
    return cmd;
  }
  cmd = calloc(1, sizeof(struct _lil_func_t));
  cmd->name = strclone(name);
  ncmd_idx = realloc(lil->cmd, (sizeof(lil_func_t)) * (lil->cmds + 1));
  if (!(&ncmd[ncmd_idx]))
  {
    free(cmd);
    return 0;
  }
  lil->cmd = &ncmd[ncmd_idx];
  ncmd[(lil->cmds++) + ncmd_idx] = cmd;
  hm_put(&lil->cmdmap, name, cmd);
  return cmd;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static lil_value_t fnc_error(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_set_error(lil, (argc > 0) ? (lil_to_string(argv[0])) : (0));
  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_set_error(lil_t lil, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 2;
  lil->err_head = 0;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_catcher(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 0)
  {
    return lil_alloc_string(lil->catcher);
  }
  else
  {
    const char *catcher = lil_to_string(argv[0]);
    free(lil->catcher);
    lil->catcher = (catcher[0]) ? (strclone(catcher)) : (0);
  }
  return 0;
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_slice(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t slice;
  size_t i;
  lilint_t from;
  lilint_t to;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 2)
  {
    return lil_clone_value(argv[0]);
  }
  from = lil_to_integer(argv[1]);
  if (from < 0)
  {
    from = 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  to = (argc > 2) ? (lil_to_integer(argv[2])) : ((lilint_t) list->c);
  if (to > ((lilint_t) list->c))
  {
    to = list->c;
  }
  if (to < from)
  {
    to = from;
  }
  slice = lil_alloc_list();
  for (i = (size_t) from; i < ((size_t) to); i += 1)
    lil_list_append(slice, lil_clone_value(list->v[i]));

  lil_free_list(list);
  r = lil_list_to_value(slice, 1);
  lil_free_list(slice);
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef int64_t lilint_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
***/


static lil_value_t fnc_count(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  char buff[64];
  if (!argc)
  {
    return alloc_value("0");
  }
  list = lil_subst_to_list(lil, argv[0]);
  sprintf(buff, "%u", (unsigned int) list->c);
  lil_free_list(list);
  return alloc_value(buff);
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_list(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list = lil_alloc_list();
  lil_value_t r;
  size_t i;
  for (i = 0; i < argc; i += 1)
    lil_list_append(list, lil_clone_value(argv[i]));

  r = lil_list_to_value(list, 1);
  lil_free_list(list);
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
***/


static lil_value_t fnc_length(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  size_t total = 0;
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      total += 1;
    }
    total += strlen(lil_to_string(argv[i]));
  }

  return lil_alloc_integer((lilint_t) total);
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t real_inc(lil_t lil, const char *varname, float v)
{
  lil_value_t pv = lil_get_var(lil, varname);
  double dv = lil_to_double(pv) + v;
  if (fmod(dv, 1))
  {
    pv = lil_alloc_double(dv);
  }
  else
    pv = lil_alloc_integer((lilint_t) dv);
  lil_set_var(lil, varname, pv, 1);
  return pv;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_alloc_double(double num)
{
  char buff[128];
  sprintf(buff, "%f", num);
  return alloc_value(buff);
}


----------------------------
double lil_to_double(lil_value_t val)
{
  return atof(lil_to_string(val));
}


----------------------------
lil_value_t lil_get_var(lil_t lil, const char *name)
{
  return lil_get_var_or(lil, name, lil->empty);
}


----------------------------
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_inc(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return real_inc(lil, lil_to_string(argv[0]), (argc > 1) ? (lil_to_double(argv[1])) : (1));
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static lil_value_t real_inc(lil_t lil, const char *varname, float v)
{
  lil_value_t pv = lil_get_var(lil, varname);
  double dv = lil_to_double(pv) + v;
  if (fmod(dv, 1))
  {
    pv = lil_alloc_double(dv);
  }
  else
    pv = lil_alloc_integer((lilint_t) dv);
  lil_set_var(lil, varname, pv, 1);
  return pv;
}


----------------------------
double lil_to_double(lil_value_t val)
{
  return atof(lil_to_string(val));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_charat(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t index;
  char chstr[2];
  const char *str;
  unsigned int str_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= strlen(str))
  {
    return 0;
  }
  chstr[0] = str[index + str_idx];
  chstr[1] = 0;
  return lil_alloc_string(chstr);
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_ltrim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 1, 0);
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
static lil_value_t real_trim(const char *str, const char *chars, int left, int right)
{
  int base = 0;
  lil_value_t r = 0;
  if (left)
  {
    while (str[base] && strchr(chars, str[base]))
      base++;

    base += 1;
    if (!right)
      r = lil_alloc_string((str[base]) ? (str + base) : (0));
  }
  if (right)
  {
    size_t len;
    char *s;
    unsigned int s_idx = 0;
    s_idx = strclone(str + base);
    len = strlen(s);
    while (len && strchr(chars, s[(len - 1) + s_idx]))
      len--;

    len -= 1;
    s[len + s_idx] = 0;
    r = lil_alloc_string(s);
    free(s);
  }
  return r;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_source(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  size_t size;
  char *buffer;
  unsigned int buffer_idx = 0;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->callback[4])
  {
    lil_source_callback_proc_t proc = (lil_source_callback_proc_t) lil->callback[4];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
    if (lil->callback[2])
  {
    lil_read_callback_proc_t proc = (lil_read_callback_proc_t) lil->callback[2];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "rb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    fseek(f, 0, 2);
    size = ftell(f);
    fseek(f, 0, 0);
    buffer_idx = malloc(size + 1);
    fread(buffer, 1, size, f);
    buffer[size + buffer_idx] = 0;
    fclose(f);
  }
  r = lil_parse(lil, buffer, 0, 0);
  free(buffer);
  return r;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef char *(*lil_source_callback_proc_t)(lil_t lil, const char *name)
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_lmap(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t i;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  for (i = 1; i < argc; i += 1)
    lil_set_var(lil, lil_to_string(argv[i]), lil_list_get(list, i - 1), 1);

  lil_free_list(list);
  return 0;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_list_get(lil_list_t list, size_t index)
{
  return (index >= list->c) ? (0) : (list->v[index]);
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_try(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->error)
  {
    return 0;
  }
  r = lil_parse_value(lil, argv[0], 0);
  if (lil->error)
  {
    lil->error = 0;
    lil_free_value(r);
    if (argc > 1)
    {
      r = lil_parse_value(lil, argv[1], 0);
    }
    else
      r = 0;
  }
  return r;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


/*** DEPENDENCIES:
typedef struct _expreval_t
{
  const char *code;
  size_t len;
  size_t head;
  lilint_t ival;
  double dval;
  int type;
  int error;
} expreval_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_alloc_double(double num)
{
  char buff[128];
  sprintf(buff, "%f", num);
  return alloc_value(buff);
}


----------------------------
lil_value_t lil_subst_to_value(lil_t lil, lil_value_t code)
{
  lil_list_t words = lil_subst_to_list(lil, code);
  lil_value_t val;
  val = lil_list_to_value(words, 0);
  lil_free_list(words);
  return val;
}


----------------------------
void lil_set_error(lil_t lil, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 2;
  lil->err_head = 0;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


----------------------------
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_if(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  int base = 0;
  int not = 0;
  int v;
  if (argc < 1)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "not"))
  {
    base = (not = 1);
  }
  if (argc < (((size_t) base) + 2))
  {
    return 0;
  }
  val = lil_eval_expr(lil, argv[base]);
  if ((!val) || lil->error)
  {
    return 0;
  }
  v = lil_to_boolean(val);
  if (not)
  {
    v = !v;
  }
  if (v)
  {
    r = lil_parse_value(lil, argv[base + 1], 0);
  }
  else
    if (argc > (((size_t) base) + 2))
  {
    r = lil_parse_value(lil, argv[base + 2], 0);
  }
  lil_free_value(val);
  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
int lil_to_boolean(lil_value_t val)
{
  const char *s = lil_to_string(val);
  size_t i;
  size_t dots = 0;
  if (!s[0])
  {
    return 0;
  }
  for (i = 0; s[i]; i += 1)
  {
    if ((s[i] != '0') && (s[i] != '.'))
    {
      return 1;
    }
    if (s[i] == '.')
    {
      if (dots)
      {
        return 1;
      }
      dots = 1;
    }
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_strpos(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *hay;
  unsigned int hay_idx = 0;
  const char *str;
  unsigned int str_idx = 0;
  size_t min = 0;
  if (argc < 2)
  {
    return lil_alloc_integer(-1);
  }
  hay_idx = lil_to_string(argv[0]);
  if (argc > 2)
  {
    min = (size_t) atoll(lil_to_string(argv[2]));
    if (min >= strlen(hay))
    {
      return lil_alloc_integer(-1);
    }
  }
  str_idx = strstr((&hay[hay_idx]) + min, lil_to_string(argv[1]));
  if (!(&str[str_idx]))
  {
    return lil_alloc_integer(-1);
  }
  return lil_alloc_integer((&str[str_idx]) - (&hay[hay_idx]));
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_exit(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (lil->callback[0])
  {
    lil_exit_callback_proc_t proc = (lil_exit_callback_proc_t) lil->callback[0];
    proc(lil, (argc > 0) ? (argv[0]) : (0));
  }
  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef void (*lil_exit_callback_proc_t)(lil_t lil, lil_value_t arg)
----------------------------
***/


static lil_value_t fnc_upeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_env_t thisenv = lil->env;
  lil_env_t thisdownenv = lil->downenv;
  lil_value_t r;
  if (lil->rootenv == thisenv)
  {
    return fnc_eval(lil, argc, argv);
  }
  lil->env = thisenv->parent;
  lil->downenv = thisenv;
  r = fnc_eval(lil, argc, argv);
  lil->env = thisenv;
  lil->downenv = thisdownenv;
  return r;
}


/*** DEPENDENCIES:
static lil_value_t fnc_eval(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_parse_value(lil, argv[0], 0);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_parse_value(lil, val, 0);
    lil_free_value(val);
    return r;
  }
  return 0;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


static lil_value_t fnc_dec(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return real_inc(lil, lil_to_string(argv[0]), -((argc > 1) ? (lil_to_double(argv[1])) : (1)));
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static lil_value_t real_inc(lil_t lil, const char *varname, float v)
{
  lil_value_t pv = lil_get_var(lil, varname);
  double dv = lil_to_double(pv) + v;
  if (fmod(dv, 1))
  {
    pv = lil_alloc_double(dv);
  }
  else
    pv = lil_alloc_integer((lilint_t) dv);
  lil_set_var(lil, varname, pv, 1);
  return pv;
}


----------------------------
double lil_to_double(lil_value_t val)
{
  return atof(lil_to_string(val));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static void del_func(lil_t lil, lil_func_t cmd)
{
  size_t i;
  size_t index = lil->cmds;
  for (i = 0; i < lil->cmds; i += 1)
    if (lil->cmd[i] == cmd)
  {
    index = i;
    break;
  }

  if (index == lil->cmds)
  {
    return;
  }
  hm_put(&lil->cmdmap, cmd->name, 0);
  if (cmd->argnames)
  {
    lil_free_list(cmd->argnames);
  }
  lil_free_value(cmd->code);
  free(cmd->name);
  free(cmd);
  lil->cmds -= 1;
  for (i = index; i < lil->cmds; i += 1)
    lil->cmd[i] = lil->cmd[i + 1];

}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
static void hm_put(hashmap_t *hm, const char *key, void *value)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    cell->e[i].v = value;
    return;
  }

  cell->e = realloc(cell->e, (sizeof(struct hashentry_t)) * (cell->c + 1));
  cell->e[cell->c].k = strclone(key);
  cell->e[cell->c].v = value;
  cell->c += 1;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static lil_value_t fnc_rename(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_func_t func;
  const char *oldname;
  unsigned int oldname_idx = 0;
  const char *newname;
  unsigned int newname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  oldname_idx = lil_to_string(argv[0]);
  newname_idx = lil_to_string(argv[1]);
  func = find_cmd(lil, oldname);
  if (!func)
  {
    char *msg = malloc(24 + strlen(oldname));
    sprintf(msg, "unknown function '%s'", oldname);
    lil_set_error_at(lil, lil->head, msg);
    free(msg);
    return 0;
  }
  r = lil_alloc_string(func->name);
  if (newname[0 + newname_idx])
  {
    hm_put(&lil->cmdmap, oldname, 0);
    hm_put(&lil->cmdmap, newname, func);
    free(func->name);
    func->name = strclone(newname);
  }
  else
  {
    del_func(lil, func);
  }
  return r;
}


/*** DEPENDENCIES:
void lil_set_error_at(lil_t lil, size_t pos, const char *msg)
{
  if (lil->error)
  {
    return;
  }
  free(lil->err_msg);
  lil->error = 1;
  lil->err_head = pos;
  lil->err_msg = strclone((msg) ? (msg) : (""));
}


----------------------------
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static void del_func(lil_t lil, lil_func_t cmd)
{
  size_t i;
  size_t index = lil->cmds;
  for (i = 0; i < lil->cmds; i += 1)
    if (lil->cmd[i] == cmd)
  {
    index = i;
    break;
  }

  if (index == lil->cmds)
  {
    return;
  }
  hm_put(&lil->cmdmap, cmd->name, 0);
  if (cmd->argnames)
  {
    lil_free_list(cmd->argnames);
  }
  lil_free_value(cmd->code);
  free(cmd->name);
  free(cmd);
  lil->cmds -= 1;
  for (i = index; i < lil->cmds; i += 1)
    lil->cmd[i] = lil->cmd[i + 1];

}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
static void hm_put(hashmap_t *hm, const char *key, void *value)
{
  struct hashcell_t *cell = hm->cell + (hm_hash(key) & 0xFF);
  size_t i;
  for (i = 0; i < cell->c; i += 1)
    if (!strcmp(key, cell->e[i].k))
  {
    cell->e[i].v = value;
    return;
  }

  cell->e = realloc(cell->e, (sizeof(struct hashentry_t)) * (cell->c + 1));
  cell->e[cell->c].k = strclone(key);
  cell->e[cell->c].v = value;
  cell->c += 1;
}


----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_split(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  const char *sep = " ";
  unsigned int sep_idx = 0;
  size_t i;
  lil_value_t val;
  const char *str;
  unsigned int str_idx = 0;
  if (argc == 0)
  {
    return 0;
  }
  if (argc > 1)
  {
    sep_idx = lil_to_string(argv[1]);
    if ((!(&sep[sep_idx])) || (!sep[0 + sep_idx]))
    {
      return lil_clone_value(argv[0]);
    }
  }
  val = lil_alloc_string("");
  str_idx = lil_to_string(argv[0]);
  list = lil_alloc_list();
  for (i = 0; str[i + str_idx]; i += 1)
  {
    if (strchr(sep, str[i + str_idx]))
    {
      lil_list_append(list, val);
      val = lil_alloc_string("");
    }
    else
    {
      lil_append_char(val, str[i + str_idx]);
    }
  }

  lil_list_append(list, val);
  val = lil_list_to_value(list, 1);
  lil_free_list(list);
  return val;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
***/


static lil_value_t fnc_local(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  for (i = 0; i < argc; i += 1)
  {
    const char *varname = lil_to_string(argv[i]);
    if (!lil_find_local_var(lil, lil->env, varname))
    {
      lil_set_var(lil, varname, lil->empty, 2);
    }
  }

  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
static lil_var_t lil_find_local_var(lil_t lil, lil_env_t env, const char *name)
{
  return hm_get(&env->varmap, name);
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_append(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_value_t r;
  size_t i;
  size_t base = 1;
  int access = 1;
  const char *varname;
  unsigned int varname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  varname_idx = lil_to_string(argv[0]);
  if (!strcmp(varname, "global"))
  {
    if (argc < 3)
    {
      return 0;
    }
    varname_idx = lil_to_string(argv[1]);
    base = 2;
    access = 0;
  }
  list = lil_subst_to_list(lil, lil_get_var(lil, varname));
  for (i = base; i < argc; i += 1)
    lil_list_append(list, lil_clone_value(argv[i]));

  r = lil_list_to_value(list, 1);
  lil_free_list(list);
  lil_set_var(lil, varname, r, access);
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_get_var(lil_t lil, const char *name)
{
  return lil_get_var_or(lil, name, lil->empty);
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_streq(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 2)
  {
    return 0;
  }
  return lil_alloc_integer((strcmp(lil_to_string(argv[0]), lil_to_string(argv[1]))) ? (0) : (1));
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_filter(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t filtered;
  size_t i;
  lil_value_t r;
  const char *varname = "x";
  unsigned int varname_idx = 0;
  int base = 0;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 2)
  {
    return lil_clone_value(argv[0]);
  }
  if (argc > 2)
  {
    base = 1;
    varname_idx = lil_to_string(argv[0]);
  }
  list = lil_subst_to_list(lil, argv[base]);
  filtered = lil_alloc_list();
  for (i = 0; (i < list->c) && (!lil->env->breakrun); i += 1)
  {
    lil_set_var(lil, varname, list->v[i], 3);
    r = lil_eval_expr(lil, argv[base + 1]);
    if (lil_to_boolean(r))
    {
      lil_list_append(filtered, lil_clone_value(list->v[i]));
    }
    lil_free_value(r);
  }

  lil_free_list(list);
  r = lil_list_to_value(filtered, 1);
  lil_free_list(filtered);
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
int lil_to_boolean(lil_value_t val)
{
  const char *s = lil_to_string(val);
  size_t i;
  size_t dots = 0;
  if (!s[0])
  {
    return 0;
  }
  for (i = 0; s[i]; i += 1)
  {
    if ((s[i] != '0') && (s[i] != '.'))
    {
      return 1;
    }
    if (s[i] == '.')
    {
      if (dots)
      {
        return 1;
      }
      dots = 1;
    }
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_topeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_env_t thisenv = lil->env;
  lil_env_t thisdownenv = lil->downenv;
  lil_value_t r;
  lil->env = lil->rootenv;
  lil->downenv = thisenv;
  r = fnc_eval(lil, argc, argv);
  lil->downenv = thisdownenv;
  lil->env = thisenv;
  return r;
}


/*** DEPENDENCIES:
static lil_value_t fnc_eval(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_parse_value(lil, argv[0], 0);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_parse_value(lil, val, 0);
    lil_free_value(val);
    return r;
  }
  return 0;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_env_t *lil_env_t
----------------------------
***/


static lil_value_t fnc_index(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t index;
  lil_value_t r;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= list->c)
  {
    r = 0;
  }
  else
    r = lil_clone_value(list->v[index]);
  lil_free_list(list);
  return r;
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


----------------------------
***/


static lil_value_t fnc_while(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  int base = 0;
  int not = 0;
  int v;
  if (argc < 1)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "not"))
  {
    base = (not = 1);
  }
  if (argc < (((size_t) base) + 2))
  {
    return 0;
  }
  while ((!lil->error) && (!lil->env->breakrun))
  {
    val = lil_eval_expr(lil, argv[base]);
    if ((!val) || lil->error)
    {
      return 0;
    }
    v = lil_to_boolean(val);
    if (not)
    {
      v = !v;
    }
    if (!v)
    {
      lil_free_value(val);
      break;
    }
    if (r)
    {
      lil_free_value(r);
    }
    r = lil_parse_value(lil, argv[base + 1], 0);
    lil_free_value(val);
  }

  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
int lil_to_boolean(lil_value_t val)
{
  const char *s = lil_to_string(val);
  size_t i;
  size_t dots = 0;
  if (!s[0])
  {
    return 0;
  }
  for (i = 0; s[i]; i += 1)
  {
    if ((s[i] != '0') && (s[i] != '.'))
    {
      return 1;
    }
    if (s[i] == '.')
    {
      if (dots)
      {
        return 1;
      }
      dots = 1;
    }
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_rtrim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 0, 1);
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
static lil_value_t real_trim(const char *str, const char *chars, int left, int right)
{
  int base = 0;
  lil_value_t r = 0;
  if (left)
  {
    while (str[base] && strchr(chars, str[base]))
      base++;

    base += 1;
    if (!right)
      r = lil_alloc_string((str[base]) ? (str + base) : (0));
  }
  if (right)
  {
    size_t len;
    char *s;
    unsigned int s_idx = 0;
    s_idx = strclone(str + base);
    len = strlen(s);
    while (len && strchr(chars, s[(len - 1) + s_idx]))
      len--;

    len -= 1;
    s[len + s_idx] = 0;
    r = lil_alloc_string(s);
    free(s);
  }
  return r;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_unusedname(lil_t lil, size_t argc, lil_value_t *argv)
{
  return lil_unused_name(lil, (argc > 0) ? (lil_to_string(argv[0])) : ("unusedname"));
}


/*** DEPENDENCIES:
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
lil_value_t lil_unused_name(lil_t lil, const char *part)
{
  char *name = malloc(strlen(part) + 64);
  lil_value_t val;
  size_t i;
  for (i = 0; i < ((size_t) (-1)); i += 1)
  {
    sprintf(name, "!!un!%s!%09u!nu!!", part, (unsigned int) i);
    if (find_cmd(lil, name))
    {
      continue;
    }
    if (lil_find_var(lil, lil->env, name))
    {
      continue;
    }
    val = lil_alloc_string(name);
    free(name);
    return val;
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_foreach(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t rlist;
  lil_value_t r;
  size_t i;
  size_t listidx = 0;
  size_t codeidx = 1;
  const char *varname = "i";
  unsigned int varname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  if (argc >= 3)
  {
    varname_idx = lil_to_string(argv[0]);
    listidx = 1;
    codeidx = 2;
  }
  rlist = lil_alloc_list();
  list = lil_subst_to_list(lil, argv[listidx]);
  for (i = 0; i < list->c; i += 1)
  {
    lil_value_t rv;
    lil_set_var(lil, varname, list->v[i], 3);
    rv = lil_parse_value(lil, argv[codeidx], 0);
    if (rv->l)
    {
      lil_list_append(rlist, rv);
    }
    else
      lil_free_value(rv);
    if (lil->env->breakrun || lil->error)
    {
      break;
    }
  }

  r = lil_list_to_value(rlist, 1);
  lil_free_list(list);
  lil_free_list(rlist);
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_substr(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *str;
  unsigned int str_idx = 0;
  lil_value_t r;
  size_t start;
  size_t end;
  size_t i;
  size_t slen;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  if (!str[0 + str_idx])
  {
    return 0;
  }
  slen = strlen(str);
  start = (size_t) atoll(lil_to_string(argv[1]));
  end = (argc > 2) ? ((size_t) atoll(lil_to_string(argv[2]))) : (slen);
  if (end > slen)
  {
    end = slen;
  }
  if (start >= end)
  {
    return 0;
  }
  r = lil_alloc_string("");
  for (i = start; i < end; i += 1)
    lil_append_char(r, str[i + str_idx]);

  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_read(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  size_t size;
  char *buffer;
  unsigned int buffer_idx = 0;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->callback[2])
  {
    lil_read_callback_proc_t proc = (lil_read_callback_proc_t) lil->callback[2];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "rb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    fseek(f, 0, 2);
    size = ftell(f);
    fseek(f, 0, 0);
    buffer_idx = malloc(size + 1);
    fread(buffer, 1, size, f);
    buffer[size + buffer_idx] = 0;
    fclose(f);
  }
  r = lil_alloc_string(buffer);
  free(buffer);
  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
typedef char *(*lil_read_callback_proc_t)(lil_t lil, const char *name)
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_enveval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_list_t invars = 0;
  lil_list_t outvars = 0;
  lil_value_t *varvalues = 0;
  unsigned int varvalues_idx = 0;
  int codeindex;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  if (argc == 1)
  {
    codeindex = 0;
  }
  else
    if (argc >= 2)
  {
    invars = lil_subst_to_list(lil, argv[0]);
    varvalues_idx = malloc((sizeof(lil_value_t)) * lil_list_size(invars));
    for (i = 0; i < lil_list_size(invars); i += 1)
      varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(invars, i))));

    if (argc > 2)
    {
      codeindex = 2;
      outvars = lil_subst_to_list(lil, argv[1]);
    }
    else
    {
      codeindex = 1;
    }
  }
  lil_push_env(lil);
  if (invars)
  {
    for (i = 0; i < lil_list_size(invars); i += 1)
    {
      lil_set_var(lil, lil_to_string(lil_list_get(invars, i)), varvalues[i + varvalues_idx], 2);
      lil_free_value(varvalues[i + varvalues_idx]);
    }

  }
  r = lil_parse_value(lil, argv[codeindex], 0);
  if (invars || outvars)
  {
    if (outvars)
    {
      varvalues_idx = realloc(varvalues_idx, (sizeof(lil_value_t)) * lil_list_size(outvars));
      for (i = 0; i < lil_list_size(outvars); i += 1)
        varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(outvars, i))));

    }
    else
    {
      for (i = 0; i < lil_list_size(invars); i += 1)
        varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(invars, i))));

    }
  }
  lil_pop_env(lil);
  if (invars)
  {
    if (outvars)
    {
      for (i = 0; i < lil_list_size(outvars); i += 1)
      {
        lil_set_var(lil, lil_to_string(lil_list_get(outvars, i)), varvalues[i + varvalues_idx], 1);
        lil_free_value(varvalues[i + varvalues_idx]);
      }

    }
    else
    {
      for (i = 0; i < lil_list_size(invars); i += 1)
      {
        lil_set_var(lil, lil_to_string(lil_list_get(invars, i)), varvalues[i + varvalues_idx], 1);
        lil_free_value(varvalues[i + varvalues_idx]);
      }

    }
    lil_free_list(invars);
    if (outvars)
    {
      lil_free_list(outvars);
    }
    free(varvalues);
  }
  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
lil_value_t lil_list_get(lil_list_t list, size_t index)
{
  return (index >= list->c) ? (0) : (list->v[index]);
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
size_t lil_list_size(lil_list_t list)
{
  return list->c;
}


----------------------------
lil_list_t lil_subst_to_list(lil_t lil, lil_value_t code)
{
  const char *save_code = lil->code;
  size_t save_clen = lil->clen;
  size_t save_head = lil->head;
  int save_igeol = lil->ignoreeol;
  lil_list_t words;
  lil->code = lil_to_string(code);
  lil->clen = code->l;
  lil->head = 0;
  lil->ignoreeol = 1;
  words = substitute(lil);
  if (!words)
  {
    words = lil_alloc_list();
  }
  lil->code = save_code;
  lil->clen = save_clen;
  lil->head = save_head;
  lil->ignoreeol = save_igeol;
  return words;
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_env_t lil_push_env(lil_t lil)
{
  lil_env_t env = lil_alloc_env(lil->env);
  lil->env = env;
  return env;
}


----------------------------
lil_value_t lil_get_var(lil_t lil, const char *name)
{
  return lil_get_var_or(lil, name, lil->empty);
}


----------------------------
void lil_pop_env(lil_t lil)
{
  if (lil->env->parent)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


static lil_value_t fnc_result(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc > 0)
  {
    lil_free_value(lil->env->retval);
    lil->env->retval = lil_clone_value(argv[0]);
    lil->env->retval_set = 1;
  }
  return (lil->env->retval_set) ? (lil_clone_value(lil->env->retval)) : (0);
}


/*** DEPENDENCIES:
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
***/


static lil_value_t fnc_store(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  const char *buffer;
  unsigned int buffer_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  if (lil->callback[3])
  {
    lil_store_callback_proc_t proc = (lil_store_callback_proc_t) lil->callback[3];
    proc(lil, lil_to_string(argv[0]), lil_to_string(argv[1]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "wb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    buffer_idx = lil_to_string(argv[1]);
    fwrite(buffer, 1, strlen(buffer), f);
    fclose(f);
  }
  return lil_clone_value(argv[1]);
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef void (*lil_store_callback_proc_t)(lil_t lil, const char *name, const char *data)
----------------------------
***/


static lil_value_t fnc_codeat(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t index;
  const char *str;
  unsigned int str_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= strlen(str))
  {
    return 0;
  }
  return lil_alloc_integer(str[index + str_idx]);
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


----------------------------
lil_value_t lil_alloc_integer(lilint_t num)
{
  char buff[128];
  sprintf(buff, "%i", num);
  return alloc_value(buff);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_write(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  lil_value_t msg = lil_alloc_string(0);
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      lil_append_char(msg, ' ');
    }
    lil_append_val(msg, argv[i]);
  }

  lil_write(lil, lil_to_string(msg));
  lil_free_value(msg);
  return 0;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
void lil_write(lil_t lil, const char *msg)
{
  if (lil->callback[1])
  {
    lil_write_callback_proc_t proc = (lil_write_callback_proc_t) lil->callback[1];
    proc(lil, msg);
  }
  else
    printf("%s", msg);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static lil_value_t fnc_set(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i = 0;
  lil_var_t var = 0;
  int access = 1;
  if (!argc)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "global"))
  {
    i = 1;
    access = 0;
  }
  while (i < argc)
  {
    if (argc == (i + 1))
    {
      return lil_clone_value(lil_get_var(lil, lil_to_string(argv[i])));
    }
    var = lil_set_var(lil, lil_to_string(argv[i]), argv[i + 1], access);
    i += 2;
  }

  return (var) ? (lil_clone_value(var->v)) : (0);
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
lil_value_t lil_get_var(lil_t lil, const char *name)
{
  return lil_get_var_or(lil, name, lil->empty);
}


----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_watch(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  const char *wcode;
  unsigned int wcode_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  wcode_idx = lil_to_string(argv[argc - 1]);
  for (i = 0; (i + 1) < argc; i += 1)
  {
    const char *vname = lil_to_string(argv[i]);
    lil_var_t v;
    if (!vname[0])
    {
      continue;
    }
    v = lil_find_var(lil, lil->env, lil_to_string(argv[i]));
    if (!v)
    {
      v = lil_set_var(lil, vname, 0, 2);
    }
    free(v->w);
    v->w = (wcode[0 + wcode_idx]) ? (strclone(wcode)) : (0);
  }

  return 0;
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static lil_var_t lil_find_var(lil_t lil, lil_env_t env, const char *name)
{
  lil_var_t r = lil_find_local_var(lil, env, name);
  return (r) ? (r) : ((env == lil->rootenv) ? (0) : (lil_find_var(lil, lil->rootenv, name)));
}


----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


static lil_value_t fnc_expr(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_eval_expr(lil, argv[0]);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_eval_expr(lil, val);
    lil_free_value(val);
    return r;
  }
  return 0;
}


/*** DEPENDENCIES:
typedef struct _lil_t *lil_t
----------------------------
int lil_append_val(lil_value_t val, lil_value_t v)
{
  char *new;
  unsigned int new_idx = 0;
  if ((!v) || (!v->l))
  {
    return 1;
  }
  new_idx = realloc(val->d, (val->l + v->l) + 1);
  if (!(&new[new_idx]))
  {
    return 0;
  }
  memcpy((&new[new_idx]) + val->l, v->d, v->l + 1);
  val->d = &new[new_idx];
  val->l += v->l;
  return 1;
}


----------------------------
int lil_append_char(lil_value_t val, char ch)
{
  char *new = realloc(val->d, val->l + 2);
  if (!new)
  {
    return 0;
  }
  new[val->l++] = ch;
  new[val->l] = 0;
  val->d = new;
  return 1;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
***/


static lil_value_t fnc_repstr(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *from;
  unsigned int from_idx = 0;
  const char *to;
  unsigned int to_idx = 0;
  char *src;
  unsigned int src_idx = 0;
  const char *sub;
  unsigned int sub_idx = 0;
  size_t idx;
  size_t fromlen;
  size_t tolen;
  size_t srclen;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 3)
  {
    return lil_clone_value(argv[0]);
  }
  from_idx = lil_to_string(argv[1]);
  to_idx = lil_to_string(argv[2]);
  if (!from[0 + from_idx])
  {
    return 0;
  }
  src_idx = strclone(lil_to_string(argv[0]));
  srclen = strlen(src);
  fromlen = strlen(from);
  tolen = strlen(to);
  while (sub_idx = strstr(src, from))
  {
    char *newsrc = malloc(((srclen - fromlen) + tolen) + 1);
    idx = (&sub[sub_idx]) - (&src[src_idx]);
    if (idx)
    {
      memcpy(newsrc, src, idx);
    }
    memcpy(newsrc + idx, to, tolen);
    memcpy((newsrc + idx) + tolen, ((&src[src_idx]) + idx) + fromlen, (srclen - idx) - fromlen);
    srclen = (srclen - fromlen) + tolen;
    free(src);
    src_idx = newsrc;
    src[srclen + src_idx] = 0;
  }

  r = lil_alloc_string(src);
  free(src);
  return r;
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
***/


static lil_value_t fnc_char(lil_t lil, size_t argc, lil_value_t *argv)
{
  char s[2];
  if (!argc)
  {
    return 0;
  }
  s[0] = (char) lil_to_integer(argv[0]);
  s[1] = 0;
  return lil_alloc_string(s);
}


/*** DEPENDENCIES:
lil_value_t lil_alloc_string(const char *str)
{
  return alloc_value(str);
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lilint_t lil_to_integer(lil_value_t val)
{
  return (lilint_t) atoll(lil_to_string(val));
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
***/


static lil_value_t fnc_print(lil_t lil, size_t argc, lil_value_t *argv)
{
  fnc_write(lil, argc, argv);
  lil_write(lil, "\n");
  return 0;
}


/*** DEPENDENCIES:
static lil_value_t fnc_write(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  lil_value_t msg = lil_alloc_string(0);
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      lil_append_char(msg, ' ');
    }
    lil_append_val(msg, argv[i]);
  }

  lil_write(lil, lil_to_string(msg));
  lil_free_value(msg);
  return 0;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_write(lil_t lil, const char *msg)
{
  if (lil->callback[1])
  {
    lil_write_callback_proc_t proc = (lil_write_callback_proc_t) lil->callback[1];
    proc(lil, msg);
  }
  else
    printf("%s", msg);
}


----------------------------
***/


static lil_value_t fnc_for(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  if (argc < 4)
  {
    return 0;
  }
  lil_free_value(lil_parse_value(lil, argv[0], 0));
  while ((!lil->error) && (!lil->env->breakrun))
  {
    val = lil_eval_expr(lil, argv[1]);
    if ((!val) || lil->error)
    {
      return 0;
    }
    if (!lil_to_boolean(val))
    {
      lil_free_value(val);
      break;
    }
    if (r)
    {
      lil_free_value(r);
    }
    r = lil_parse_value(lil, argv[3], 0);
    lil_free_value(val);
    lil_free_value(lil_parse_value(lil, argv[2], 0));
  }

  return r;
}


/*** DEPENDENCIES:
int lil_to_boolean(lil_value_t val)
{
  const char *s = lil_to_string(val);
  size_t i;
  size_t dots = 0;
  if (!s[0])
  {
    return 0;
  }
  for (i = 0; s[i]; i += 1)
  {
    if ((s[i] != '0') && (s[i] != '.'))
    {
      return 1;
    }
    if (s[i] == '.')
    {
      if (dots)
      {
        return 1;
      }
      dots = 1;
    }
  }

  return 0;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
lil_value_t lil_eval_expr(lil_t lil, lil_value_t code)
{
  expreval_t ee;
  code = lil_subst_to_value(lil, code);
  if (lil->error)
  {
    return 0;
  }
  ee.code = lil_to_string(code);
  if (!ee.code[0])
  {
    lil_free_value(code);
    return lil_alloc_integer(0);
  }
  ee.head = 0;
  ee.len = code->l;
  ee.ival = 0;
  ee.dval = 0;
  ee.type = 0;
  ee.error = 0;
  ee_expr(&ee);
  lil_free_value(code);
  if (ee.error)
  {
    switch (ee.error)
    {
      case 3:
      {
        lil_set_error(lil, "division by zero in expression");
        break;
      }

      case 2:
      {
        lil_set_error(lil, "mixing invalid types in expression");
        break;
      }

      case 1:
      {
        lil_set_error(lil, "expression syntax error");
        break;
      }

    }

    return 0;
  }
  if (ee.type == 0)
  {
    return lil_alloc_integer(ee.ival);
  }
  else
    return lil_alloc_double(ee.dval);
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


// element in merge of:  register_stdcmds,lil_new,fnc_jaileval
static void register_stdcmds(lil_t lil)
{
  lil_register(lil, "reflect", fnc_reflect);
  lil_register(lil, "func", fnc_func);
  lil_register(lil, "rename", fnc_rename);
  lil_register(lil, "unusedname", fnc_unusedname);
  lil_register(lil, "quote", fnc_quote);
  lil_register(lil, "set", fnc_set);
  lil_register(lil, "local", fnc_local);
  lil_register(lil, "write", fnc_write);
  lil_register(lil, "print", fnc_print);
  lil_register(lil, "eval", fnc_eval);
  lil_register(lil, "topeval", fnc_topeval);
  lil_register(lil, "upeval", fnc_upeval);
  lil_register(lil, "downeval", fnc_downeval);
  lil_register(lil, "enveval", fnc_enveval);
  lil_register(lil, "jaileval", fnc_jaileval);
  lil_register(lil, "count", fnc_count);
  lil_register(lil, "index", fnc_index);
  lil_register(lil, "indexof", fnc_indexof);
  lil_register(lil, "filter", fnc_filter);
  lil_register(lil, "list", fnc_list);
  lil_register(lil, "append", fnc_append);
  lil_register(lil, "slice", fnc_slice);
  lil_register(lil, "subst", fnc_subst);
  lil_register(lil, "concat", fnc_concat);
  lil_register(lil, "foreach", fnc_foreach);
  lil_register(lil, "return", fnc_return);
  lil_register(lil, "result", fnc_result);
  lil_register(lil, "expr", fnc_expr);
  lil_register(lil, "inc", fnc_inc);
  lil_register(lil, "dec", fnc_dec);
  lil_register(lil, "read", fnc_read);
  lil_register(lil, "store", fnc_store);
  lil_register(lil, "if", fnc_if);
  lil_register(lil, "while", fnc_while);
  lil_register(lil, "for", fnc_for);
  lil_register(lil, "char", fnc_char);
  lil_register(lil, "charat", fnc_charat);
  lil_register(lil, "codeat", fnc_codeat);
  lil_register(lil, "substr", fnc_substr);
  lil_register(lil, "strpos", fnc_strpos);
  lil_register(lil, "length", fnc_length);
  lil_register(lil, "trim", fnc_trim);
  lil_register(lil, "ltrim", fnc_ltrim);
  lil_register(lil, "rtrim", fnc_rtrim);
  lil_register(lil, "strcmp", fnc_strcmp);
  lil_register(lil, "streq", fnc_streq);
  lil_register(lil, "repstr", fnc_repstr);
  lil_register(lil, "split", fnc_split);
  lil_register(lil, "try", fnc_try);
  lil_register(lil, "error", fnc_error);
  lil_register(lil, "exit", fnc_exit);
  lil_register(lil, "source", fnc_source);
  lil_register(lil, "lmap", fnc_lmap);
  lil_register(lil, "rand", fnc_rand);
  lil_register(lil, "catcher", fnc_catcher);
  lil_register(lil, "watch", fnc_watch);
  lil->syscmds = lil->cmds;
}


/*** DEPENDENCIES:
static lil_value_t fnc_reflect(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_func_t func;
  const char *type;
  unsigned int type_idx = 0;
  size_t i;
  lil_value_t r;
  if (!argc)
  {
    return 0;
  }
  type_idx = lil_to_string(argv[0]);
  if (!strcmp(type, "version"))
  {
    return lil_alloc_string("0.1");
  }
  if (!strcmp(type, "args"))
  {
    if (argc < 2)
    {
      return 0;
    }
    func = find_cmd(lil, lil_to_string(argv[1]));
    if ((!func) || (!func->argnames))
    {
      return 0;
    }
    return lil_list_to_value(func->argnames, 1);
  }
  if (!strcmp(type, "body"))
  {
    if (argc < 2)
    {
      return 0;
    }
    func = find_cmd(lil, lil_to_string(argv[1]));
    if ((!func) || func->proc)
    {
      return 0;
    }
    return lil_clone_value(func->code);
  }
  if (!strcmp(type, "func-count"))
  {
    return lil_alloc_integer(lil->cmds);
  }
  if (!strcmp(type, "funcs"))
  {
    lil_list_t funcs = lil_alloc_list();
    for (i = 0; i < lil->cmds; i += 1)
      lil_list_append(funcs, lil_alloc_string(lil->cmd[i]->name));

    r = lil_list_to_value(funcs, 1);
    lil_free_list(funcs);
    return r;
  }
  if (!strcmp(type, "vars"))
  {
    lil_list_t vars = lil_alloc_list();
    lil_env_t env = lil->env;
    while (env)
    {
      for (i = 0; i < env->vars; i += 1)
        lil_list_append(vars, lil_alloc_string(env->var[i]->n));

      env = env->parent;
    }

    r = lil_list_to_value(vars, 1);
    lil_free_list(vars);
    return r;
  }
  if (!strcmp(type, "globals"))
  {
    lil_list_t vars = lil_alloc_list();
    for (i = 0; i < lil->rootenv->vars; i += 1)
      lil_list_append(vars, lil_alloc_string(lil->rootenv->var[i]->n));

    r = lil_list_to_value(vars, 1);
    lil_free_list(vars);
    return r;
  }
  if (!strcmp(type, "has-func"))
  {
    const char *target;
    unsigned int target_idx = 0;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    return (hm_has(&lil->cmdmap, target)) ? (lil_alloc_string("1")) : (0);
  }
  if (!strcmp(type, "has-var"))
  {
    const char *target;
    unsigned int target_idx = 0;
    lil_env_t env = lil->env;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    while (env)
    {
      if (hm_has(&env->varmap, target))
      {
        return lil_alloc_string("1");
      }
      env = env->parent;
    }

    return 0;
  }
  if (!strcmp(type, "has-global"))
  {
    const char *target;
    unsigned int target_idx = 0;
    if (argc == 1)
    {
      return 0;
    }
    target_idx = lil_to_string(argv[1]);
    for (i = 0; i < lil->rootenv->vars; i += 1)
      if (!strcmp(target, lil->rootenv->var[i]->n))
    {
      return lil_alloc_string("1");
    }

    return 0;
  }
  if (!strcmp(type, "error"))
  {
    return (lil->err_msg) ? (lil_alloc_string(lil->err_msg)) : (0);
  }
  if (!strcmp(type, "dollar-prefix"))
  {
    lil_value_t r;
    if (argc == 1)
    {
      return lil_alloc_string(lil->dollarprefix);
    }
    r = lil_alloc_string(lil->dollarprefix);
    free(lil->dollarprefix);
    lil->dollarprefix = strclone(lil_to_string(argv[1]));
    return r;
  }
  if (!strcmp(type, "this"))
  {
    lil_env_t env = lil->env;
    while (((env != lil->rootenv) && (!env->catcher_for)) && (!env->func))
      env = env->parent;

    if (env->catcher_for)
    {
      return lil_alloc_string(lil->catcher);
    }
    if (env == lil->rootenv)
    {
      return lil_alloc_string(lil->rootcode);
    }
    return (env->func) ? (env->func->code) : (0);
  }
  if (!strcmp(type, "name"))
  {
    lil_env_t env = lil->env;
    while (((env != lil->rootenv) && (!env->catcher_for)) && (!env->func))
      env = env->parent;

    if (env->catcher_for)
    {
      return env->catcher_for;
    }
    if (env == lil->rootenv)
    {
      return 0;
    }
    return (env->func) ? (lil_alloc_string(env->func->name)) : (0);
  }
  return 0;
}


----------------------------
static lil_value_t fnc_strcmp(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 2)
  {
    return 0;
  }
  return lil_alloc_integer(strcmp(lil_to_string(argv[0]), lil_to_string(argv[1])));
}


----------------------------
static lil_value_t fnc_trim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 1, 1);
}


----------------------------
static lil_value_t fnc_concat(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_value_t r;
  lil_value_t tmp;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  r = lil_alloc_string("");
  for (i = 0; i < argc; i += 1)
  {
    list = lil_subst_to_list(lil, argv[i]);
    tmp = lil_list_to_value(list, 1);
    lil_free_list(list);
    lil_append_val(r, tmp);
    lil_free_value(tmp);
  }

  return r;
}


----------------------------
static lil_value_t fnc_indexof(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t index;
  lil_value_t r = 0;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  for (index = 0; index < list->c; index += 1)
    if (!strcmp(lil_to_string(list->v[index]), lil_to_string(argv[1])))
  {
    r = lil_alloc_integer(index);
    break;
  }

  lil_free_list(list);
  return r;
}


----------------------------
static lil_value_t fnc_rand(lil_t lil, size_t argc, lil_value_t *argv)
{
  return lil_alloc_double(rand() / ((double) 32767));
}


----------------------------
int lil_register(lil_t lil, const char *name, lil_func_proc_t proc)
{
  lil_func_t cmd = add_func(lil, name);
  if (!cmd)
  {
    return 0;
  }
  cmd->proc = proc;
  return 1;
}


----------------------------
static lil_value_t fnc_subst(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return lil_subst_to_value(lil, argv[0]);
}


----------------------------
static lil_value_t fnc_return(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil->env->breakrun = 1;
  lil_free_value(lil->env->retval);
  lil->env->retval = (argc < 1) ? (0) : (lil_clone_value(argv[0]));
  lil->env->retval_set = 1;
  return (argc < 1) ? (0) : (lil_clone_value(argv[0]));
}


----------------------------
static lil_value_t fnc_quote(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  r = alloc_value(0);
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      lil_append_char(r, ' ');
    }
    lil_append_val(r, argv[i]);
  }

  return r;
}


----------------------------
static lil_value_t fnc_downeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_env_t upenv = lil->env;
  lil_env_t downenv = lil->downenv;
  if (!downenv)
  {
    return fnc_eval(lil, argc, argv);
  }
  lil->downenv = 0;
  lil->env = downenv;
  r = fnc_eval(lil, argc, argv);
  lil->downenv = downenv;
  lil->env = upenv;
  return r;
}


----------------------------
static lil_value_t fnc_func(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t name;
  lil_func_t cmd;
  lil_list_t fargs;
  if (argc < 1)
  {
    return 0;
  }
  if (argc >= 3)
  {
    name = lil_clone_value(argv[0]);
    fargs = lil_subst_to_list(lil, argv[1]);
    cmd = add_func(lil, lil_to_string(argv[0]));
    cmd->argnames = fargs;
    cmd->code = lil_clone_value(argv[2]);
  }
  else
  {
    name = lil_unused_name(lil, "anonymous-function");
    if (argc < 2)
    {
      lil_value_t tmp = lil_alloc_string("args");
      fargs = lil_subst_to_list(lil, tmp);
      lil_free_value(tmp);
      cmd = add_func(lil, lil_to_string(name));
      cmd->argnames = fargs;
      cmd->code = lil_clone_value(argv[0]);
    }
    else
    {
      fargs = lil_subst_to_list(lil, argv[0]);
      cmd = add_func(lil, lil_to_string(name));
      cmd->argnames = fargs;
      cmd->code = lil_clone_value(argv[1]);
    }
  }
  return name;
}


----------------------------
static lil_value_t fnc_error(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_set_error(lil, (argc > 0) ? (lil_to_string(argv[0])) : (0));
  return 0;
}


----------------------------
static lil_value_t fnc_catcher(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 0)
  {
    return lil_alloc_string(lil->catcher);
  }
  else
  {
    const char *catcher = lil_to_string(argv[0]);
    free(lil->catcher);
    lil->catcher = (catcher[0]) ? (strclone(catcher)) : (0);
  }
  return 0;
}


----------------------------
static lil_value_t fnc_slice(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t slice;
  size_t i;
  lilint_t from;
  lilint_t to;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 2)
  {
    return lil_clone_value(argv[0]);
  }
  from = lil_to_integer(argv[1]);
  if (from < 0)
  {
    from = 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  to = (argc > 2) ? (lil_to_integer(argv[2])) : ((lilint_t) list->c);
  if (to > ((lilint_t) list->c))
  {
    to = list->c;
  }
  if (to < from)
  {
    to = from;
  }
  slice = lil_alloc_list();
  for (i = (size_t) from; i < ((size_t) to); i += 1)
    lil_list_append(slice, lil_clone_value(list->v[i]));

  lil_free_list(list);
  r = lil_list_to_value(slice, 1);
  lil_free_list(slice);
  return r;
}


----------------------------
static lil_value_t fnc_count(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  char buff[64];
  if (!argc)
  {
    return alloc_value("0");
  }
  list = lil_subst_to_list(lil, argv[0]);
  sprintf(buff, "%u", (unsigned int) list->c);
  lil_free_list(list);
  return alloc_value(buff);
}


----------------------------
static lil_value_t fnc_list(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list = lil_alloc_list();
  lil_value_t r;
  size_t i;
  for (i = 0; i < argc; i += 1)
    lil_list_append(list, lil_clone_value(argv[i]));

  r = lil_list_to_value(list, 1);
  lil_free_list(list);
  return r;
}


----------------------------
static lil_value_t fnc_length(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  size_t total = 0;
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      total += 1;
    }
    total += strlen(lil_to_string(argv[i]));
  }

  return lil_alloc_integer((lilint_t) total);
}


----------------------------
static lil_value_t fnc_inc(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return real_inc(lil, lil_to_string(argv[0]), (argc > 1) ? (lil_to_double(argv[1])) : (1));
}


----------------------------
static lil_value_t fnc_charat(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t index;
  char chstr[2];
  const char *str;
  unsigned int str_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= strlen(str))
  {
    return 0;
  }
  chstr[0] = str[index + str_idx];
  chstr[1] = 0;
  return lil_alloc_string(chstr);
}


----------------------------
static lil_value_t fnc_ltrim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 1, 0);
}


----------------------------
static lil_value_t fnc_source(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  size_t size;
  char *buffer;
  unsigned int buffer_idx = 0;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->callback[4])
  {
    lil_source_callback_proc_t proc = (lil_source_callback_proc_t) lil->callback[4];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
    if (lil->callback[2])
  {
    lil_read_callback_proc_t proc = (lil_read_callback_proc_t) lil->callback[2];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "rb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    fseek(f, 0, 2);
    size = ftell(f);
    fseek(f, 0, 0);
    buffer_idx = malloc(size + 1);
    fread(buffer, 1, size, f);
    buffer[size + buffer_idx] = 0;
    fclose(f);
  }
  r = lil_parse(lil, buffer, 0, 0);
  free(buffer);
  return r;
}


----------------------------
static lil_value_t fnc_lmap(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t i;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  for (i = 1; i < argc; i += 1)
    lil_set_var(lil, lil_to_string(argv[i]), lil_list_get(list, i - 1), 1);

  lil_free_list(list);
  return 0;
}


----------------------------
static lil_value_t fnc_try(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->error)
  {
    return 0;
  }
  r = lil_parse_value(lil, argv[0], 0);
  if (lil->error)
  {
    lil->error = 0;
    lil_free_value(r);
    if (argc > 1)
    {
      r = lil_parse_value(lil, argv[1], 0);
    }
    else
      r = 0;
  }
  return r;
}


----------------------------
static lil_value_t fnc_if(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  int base = 0;
  int not = 0;
  int v;
  if (argc < 1)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "not"))
  {
    base = (not = 1);
  }
  if (argc < (((size_t) base) + 2))
  {
    return 0;
  }
  val = lil_eval_expr(lil, argv[base]);
  if ((!val) || lil->error)
  {
    return 0;
  }
  v = lil_to_boolean(val);
  if (not)
  {
    v = !v;
  }
  if (v)
  {
    r = lil_parse_value(lil, argv[base + 1], 0);
  }
  else
    if (argc > (((size_t) base) + 2))
  {
    r = lil_parse_value(lil, argv[base + 2], 0);
  }
  lil_free_value(val);
  return r;
}


----------------------------
static lil_value_t fnc_strpos(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *hay;
  unsigned int hay_idx = 0;
  const char *str;
  unsigned int str_idx = 0;
  size_t min = 0;
  if (argc < 2)
  {
    return lil_alloc_integer(-1);
  }
  hay_idx = lil_to_string(argv[0]);
  if (argc > 2)
  {
    min = (size_t) atoll(lil_to_string(argv[2]));
    if (min >= strlen(hay))
    {
      return lil_alloc_integer(-1);
    }
  }
  str_idx = strstr((&hay[hay_idx]) + min, lil_to_string(argv[1]));
  if (!(&str[str_idx]))
  {
    return lil_alloc_integer(-1);
  }
  return lil_alloc_integer((&str[str_idx]) - (&hay[hay_idx]));
}


----------------------------
static lil_value_t fnc_exit(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (lil->callback[0])
  {
    lil_exit_callback_proc_t proc = (lil_exit_callback_proc_t) lil->callback[0];
    proc(lil, (argc > 0) ? (argv[0]) : (0));
  }
  return 0;
}


----------------------------
static lil_value_t fnc_eval(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_parse_value(lil, argv[0], 0);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_parse_value(lil, val, 0);
    lil_free_value(val);
    return r;
  }
  return 0;
}


----------------------------
static lil_value_t fnc_upeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_env_t thisenv = lil->env;
  lil_env_t thisdownenv = lil->downenv;
  lil_value_t r;
  if (lil->rootenv == thisenv)
  {
    return fnc_eval(lil, argc, argv);
  }
  lil->env = thisenv->parent;
  lil->downenv = thisenv;
  r = fnc_eval(lil, argc, argv);
  lil->env = thisenv;
  lil->downenv = thisdownenv;
  return r;
}


----------------------------
static lil_value_t fnc_dec(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 1)
  {
    return 0;
  }
  return real_inc(lil, lil_to_string(argv[0]), -((argc > 1) ? (lil_to_double(argv[1])) : (1)));
}


----------------------------
static lil_value_t fnc_rename(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_func_t func;
  const char *oldname;
  unsigned int oldname_idx = 0;
  const char *newname;
  unsigned int newname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  oldname_idx = lil_to_string(argv[0]);
  newname_idx = lil_to_string(argv[1]);
  func = find_cmd(lil, oldname);
  if (!func)
  {
    char *msg = malloc(24 + strlen(oldname));
    sprintf(msg, "unknown function '%s'", oldname);
    lil_set_error_at(lil, lil->head, msg);
    free(msg);
    return 0;
  }
  r = lil_alloc_string(func->name);
  if (newname[0 + newname_idx])
  {
    hm_put(&lil->cmdmap, oldname, 0);
    hm_put(&lil->cmdmap, newname, func);
    free(func->name);
    func->name = strclone(newname);
  }
  else
  {
    del_func(lil, func);
  }
  return r;
}


----------------------------
static lil_value_t fnc_split(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  const char *sep = " ";
  unsigned int sep_idx = 0;
  size_t i;
  lil_value_t val;
  const char *str;
  unsigned int str_idx = 0;
  if (argc == 0)
  {
    return 0;
  }
  if (argc > 1)
  {
    sep_idx = lil_to_string(argv[1]);
    if ((!(&sep[sep_idx])) || (!sep[0 + sep_idx]))
    {
      return lil_clone_value(argv[0]);
    }
  }
  val = lil_alloc_string("");
  str_idx = lil_to_string(argv[0]);
  list = lil_alloc_list();
  for (i = 0; str[i + str_idx]; i += 1)
  {
    if (strchr(sep, str[i + str_idx]))
    {
      lil_list_append(list, val);
      val = lil_alloc_string("");
    }
    else
    {
      lil_append_char(val, str[i + str_idx]);
    }
  }

  lil_list_append(list, val);
  val = lil_list_to_value(list, 1);
  lil_free_list(list);
  return val;
}


----------------------------
static lil_value_t fnc_local(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  for (i = 0; i < argc; i += 1)
  {
    const char *varname = lil_to_string(argv[i]);
    if (!lil_find_local_var(lil, lil->env, varname))
    {
      lil_set_var(lil, varname, lil->empty, 2);
    }
  }

  return 0;
}


----------------------------
static lil_value_t fnc_append(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_value_t r;
  size_t i;
  size_t base = 1;
  int access = 1;
  const char *varname;
  unsigned int varname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  varname_idx = lil_to_string(argv[0]);
  if (!strcmp(varname, "global"))
  {
    if (argc < 3)
    {
      return 0;
    }
    varname_idx = lil_to_string(argv[1]);
    base = 2;
    access = 0;
  }
  list = lil_subst_to_list(lil, lil_get_var(lil, varname));
  for (i = base; i < argc; i += 1)
    lil_list_append(list, lil_clone_value(argv[i]));

  r = lil_list_to_value(list, 1);
  lil_free_list(list);
  lil_set_var(lil, varname, r, access);
  return r;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
static lil_value_t fnc_streq(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc < 2)
  {
    return 0;
  }
  return lil_alloc_integer((strcmp(lil_to_string(argv[0]), lil_to_string(argv[1]))) ? (0) : (1));
}


----------------------------
static lil_value_t fnc_filter(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t filtered;
  size_t i;
  lil_value_t r;
  const char *varname = "x";
  unsigned int varname_idx = 0;
  int base = 0;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 2)
  {
    return lil_clone_value(argv[0]);
  }
  if (argc > 2)
  {
    base = 1;
    varname_idx = lil_to_string(argv[0]);
  }
  list = lil_subst_to_list(lil, argv[base]);
  filtered = lil_alloc_list();
  for (i = 0; (i < list->c) && (!lil->env->breakrun); i += 1)
  {
    lil_set_var(lil, varname, list->v[i], 3);
    r = lil_eval_expr(lil, argv[base + 1]);
    if (lil_to_boolean(r))
    {
      lil_list_append(filtered, lil_clone_value(list->v[i]));
    }
    lil_free_value(r);
  }

  lil_free_list(list);
  r = lil_list_to_value(filtered, 1);
  lil_free_list(filtered);
  return r;
}


----------------------------
static lil_value_t fnc_topeval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_env_t thisenv = lil->env;
  lil_env_t thisdownenv = lil->downenv;
  lil_value_t r;
  lil->env = lil->rootenv;
  lil->downenv = thisenv;
  r = fnc_eval(lil, argc, argv);
  lil->downenv = thisdownenv;
  lil->env = thisenv;
  return r;
}


----------------------------
static lil_value_t fnc_index(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  size_t index;
  lil_value_t r;
  if (argc < 2)
  {
    return 0;
  }
  list = lil_subst_to_list(lil, argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= list->c)
  {
    r = 0;
  }
  else
    r = lil_clone_value(list->v[index]);
  lil_free_list(list);
  return r;
}


----------------------------
static lil_value_t fnc_while(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  int base = 0;
  int not = 0;
  int v;
  if (argc < 1)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "not"))
  {
    base = (not = 1);
  }
  if (argc < (((size_t) base) + 2))
  {
    return 0;
  }
  while ((!lil->error) && (!lil->env->breakrun))
  {
    val = lil_eval_expr(lil, argv[base]);
    if ((!val) || lil->error)
    {
      return 0;
    }
    v = lil_to_boolean(val);
    if (not)
    {
      v = !v;
    }
    if (!v)
    {
      lil_free_value(val);
      break;
    }
    if (r)
    {
      lil_free_value(r);
    }
    r = lil_parse_value(lil, argv[base + 1], 0);
    lil_free_value(val);
  }

  return r;
}


----------------------------
static lil_value_t fnc_rtrim(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (!argc)
  {
    return 0;
  }
  return real_trim(lil_to_string(argv[0]), (argc < 2) ? (" \f\n\r\t\v") : (lil_to_string(argv[1])), 0, 1);
}


----------------------------
static lil_value_t fnc_unusedname(lil_t lil, size_t argc, lil_value_t *argv)
{
  return lil_unused_name(lil, (argc > 0) ? (lil_to_string(argv[0])) : ("unusedname"));
}


----------------------------
static lil_value_t fnc_foreach(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_list_t list;
  lil_list_t rlist;
  lil_value_t r;
  size_t i;
  size_t listidx = 0;
  size_t codeidx = 1;
  const char *varname = "i";
  unsigned int varname_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  if (argc >= 3)
  {
    varname_idx = lil_to_string(argv[0]);
    listidx = 1;
    codeidx = 2;
  }
  rlist = lil_alloc_list();
  list = lil_subst_to_list(lil, argv[listidx]);
  for (i = 0; i < list->c; i += 1)
  {
    lil_value_t rv;
    lil_set_var(lil, varname, list->v[i], 3);
    rv = lil_parse_value(lil, argv[codeidx], 0);
    if (rv->l)
    {
      lil_list_append(rlist, rv);
    }
    else
      lil_free_value(rv);
    if (lil->env->breakrun || lil->error)
    {
      break;
    }
  }

  r = lil_list_to_value(rlist, 1);
  lil_free_list(list);
  lil_free_list(rlist);
  return r;
}


----------------------------
static lil_value_t fnc_substr(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *str;
  unsigned int str_idx = 0;
  lil_value_t r;
  size_t start;
  size_t end;
  size_t i;
  size_t slen;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  if (!str[0 + str_idx])
  {
    return 0;
  }
  slen = strlen(str);
  start = (size_t) atoll(lil_to_string(argv[1]));
  end = (argc > 2) ? ((size_t) atoll(lil_to_string(argv[2]))) : (slen);
  if (end > slen)
  {
    end = slen;
  }
  if (start >= end)
  {
    return 0;
  }
  r = lil_alloc_string("");
  for (i = start; i < end; i += 1)
    lil_append_char(r, str[i + str_idx]);

  return r;
}


----------------------------
static lil_value_t fnc_read(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  size_t size;
  char *buffer;
  unsigned int buffer_idx = 0;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (lil->callback[2])
  {
    lil_read_callback_proc_t proc = (lil_read_callback_proc_t) lil->callback[2];
    buffer_idx = proc(lil, lil_to_string(argv[0]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "rb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    fseek(f, 0, 2);
    size = ftell(f);
    fseek(f, 0, 0);
    buffer_idx = malloc(size + 1);
    fread(buffer, 1, size, f);
    buffer[size + buffer_idx] = 0;
    fclose(f);
  }
  r = lil_alloc_string(buffer);
  free(buffer);
  return r;
}


----------------------------
static lil_value_t fnc_enveval(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t r;
  lil_list_t invars = 0;
  lil_list_t outvars = 0;
  lil_value_t *varvalues = 0;
  unsigned int varvalues_idx = 0;
  int codeindex;
  size_t i;
  if (argc < 1)
  {
    return 0;
  }
  if (argc == 1)
  {
    codeindex = 0;
  }
  else
    if (argc >= 2)
  {
    invars = lil_subst_to_list(lil, argv[0]);
    varvalues_idx = malloc((sizeof(lil_value_t)) * lil_list_size(invars));
    for (i = 0; i < lil_list_size(invars); i += 1)
      varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(invars, i))));

    if (argc > 2)
    {
      codeindex = 2;
      outvars = lil_subst_to_list(lil, argv[1]);
    }
    else
    {
      codeindex = 1;
    }
  }
  lil_push_env(lil);
  if (invars)
  {
    for (i = 0; i < lil_list_size(invars); i += 1)
    {
      lil_set_var(lil, lil_to_string(lil_list_get(invars, i)), varvalues[i + varvalues_idx], 2);
      lil_free_value(varvalues[i + varvalues_idx]);
    }

  }
  r = lil_parse_value(lil, argv[codeindex], 0);
  if (invars || outvars)
  {
    if (outvars)
    {
      varvalues_idx = realloc(varvalues_idx, (sizeof(lil_value_t)) * lil_list_size(outvars));
      for (i = 0; i < lil_list_size(outvars); i += 1)
        varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(outvars, i))));

    }
    else
    {
      for (i = 0; i < lil_list_size(invars); i += 1)
        varvalues[i + varvalues_idx] = lil_clone_value(lil_get_var(lil, lil_to_string(lil_list_get(invars, i))));

    }
  }
  lil_pop_env(lil);
  if (invars)
  {
    if (outvars)
    {
      for (i = 0; i < lil_list_size(outvars); i += 1)
      {
        lil_set_var(lil, lil_to_string(lil_list_get(outvars, i)), varvalues[i + varvalues_idx], 1);
        lil_free_value(varvalues[i + varvalues_idx]);
      }

    }
    else
    {
      for (i = 0; i < lil_list_size(invars); i += 1)
      {
        lil_set_var(lil, lil_to_string(lil_list_get(invars, i)), varvalues[i + varvalues_idx], 1);
        lil_free_value(varvalues[i + varvalues_idx]);
      }

    }
    lil_free_list(invars);
    if (outvars)
    {
      lil_free_list(outvars);
    }
    free(varvalues);
  }
  return r;
}


----------------------------
static lil_value_t fnc_result(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc > 0)
  {
    lil_free_value(lil->env->retval);
    lil->env->retval = lil_clone_value(argv[0]);
    lil->env->retval_set = 1;
  }
  return (lil->env->retval_set) ? (lil_clone_value(lil->env->retval)) : (0);
}


----------------------------
static lil_value_t fnc_store(lil_t lil, size_t argc, lil_value_t *argv)
{
  FILE *f;
  unsigned int f_idx = 0;
  const char *buffer;
  unsigned int buffer_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  if (lil->callback[3])
  {
    lil_store_callback_proc_t proc = (lil_store_callback_proc_t) lil->callback[3];
    proc(lil, lil_to_string(argv[0]), lil_to_string(argv[1]));
  }
  else
  {
    f_idx = fopen(lil_to_string(argv[0]), "wb");
    if (!(&f[f_idx]))
    {
      return 0;
    }
    buffer_idx = lil_to_string(argv[1]);
    fwrite(buffer, 1, strlen(buffer), f);
    fclose(f);
  }
  return lil_clone_value(argv[1]);
}


----------------------------
static lil_value_t fnc_codeat(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t index;
  const char *str;
  unsigned int str_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  str_idx = lil_to_string(argv[0]);
  index = (size_t) lil_to_integer(argv[1]);
  if (index >= strlen(str))
  {
    return 0;
  }
  return lil_alloc_integer(str[index + str_idx]);
}


----------------------------
static lil_value_t fnc_write(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  lil_value_t msg = lil_alloc_string(0);
  for (i = 0; i < argc; i += 1)
  {
    if (i)
    {
      lil_append_char(msg, ' ');
    }
    lil_append_val(msg, argv[i]);
  }

  lil_write(lil, lil_to_string(msg));
  lil_free_value(msg);
  return 0;
}


----------------------------
static lil_value_t fnc_set(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i = 0;
  lil_var_t var = 0;
  int access = 1;
  if (!argc)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "global"))
  {
    i = 1;
    access = 0;
  }
  while (i < argc)
  {
    if (argc == (i + 1))
    {
      return lil_clone_value(lil_get_var(lil, lil_to_string(argv[i])));
    }
    var = lil_set_var(lil, lil_to_string(argv[i]), argv[i + 1], access);
    i += 2;
  }

  return (var) ? (lil_clone_value(var->v)) : (0);
}


----------------------------
static lil_value_t fnc_watch(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  const char *wcode;
  unsigned int wcode_idx = 0;
  if (argc < 2)
  {
    return 0;
  }
  wcode_idx = lil_to_string(argv[argc - 1]);
  for (i = 0; (i + 1) < argc; i += 1)
  {
    const char *vname = lil_to_string(argv[i]);
    lil_var_t v;
    if (!vname[0])
    {
      continue;
    }
    v = lil_find_var(lil, lil->env, lil_to_string(argv[i]));
    if (!v)
    {
      v = lil_set_var(lil, vname, 0, 2);
    }
    free(v->w);
    v->w = (wcode[0 + wcode_idx]) ? (strclone(wcode)) : (0);
  }

  return 0;
}


----------------------------
static lil_value_t fnc_expr(lil_t lil, size_t argc, lil_value_t *argv)
{
  if (argc == 1)
  {
    return lil_eval_expr(lil, argv[0]);
  }
  if (argc > 1)
  {
    lil_value_t val = alloc_value(0);
    lil_value_t r;
    size_t i;
    for (i = 0; i < argc; i += 1)
    {
      if (i)
      {
        lil_append_char(val, ' ');
      }
      lil_append_val(val, argv[i]);
    }

    r = lil_eval_expr(lil, val);
    lil_free_value(val);
    return r;
  }
  return 0;
}


----------------------------
static lil_value_t fnc_repstr(lil_t lil, size_t argc, lil_value_t *argv)
{
  const char *from;
  unsigned int from_idx = 0;
  const char *to;
  unsigned int to_idx = 0;
  char *src;
  unsigned int src_idx = 0;
  const char *sub;
  unsigned int sub_idx = 0;
  size_t idx;
  size_t fromlen;
  size_t tolen;
  size_t srclen;
  lil_value_t r;
  if (argc < 1)
  {
    return 0;
  }
  if (argc < 3)
  {
    return lil_clone_value(argv[0]);
  }
  from_idx = lil_to_string(argv[1]);
  to_idx = lil_to_string(argv[2]);
  if (!from[0 + from_idx])
  {
    return 0;
  }
  src_idx = strclone(lil_to_string(argv[0]));
  srclen = strlen(src);
  fromlen = strlen(from);
  tolen = strlen(to);
  while (sub_idx = strstr(src, from))
  {
    char *newsrc = malloc(((srclen - fromlen) + tolen) + 1);
    idx = (&sub[sub_idx]) - (&src[src_idx]);
    if (idx)
    {
      memcpy(newsrc, src, idx);
    }
    memcpy(newsrc + idx, to, tolen);
    memcpy((newsrc + idx) + tolen, ((&src[src_idx]) + idx) + fromlen, (srclen - idx) - fromlen);
    srclen = (srclen - fromlen) + tolen;
    free(src);
    src_idx = newsrc;
    src[srclen + src_idx] = 0;
  }

  r = lil_alloc_string(src);
  free(src);
  return r;
}


----------------------------
static lil_value_t fnc_char(lil_t lil, size_t argc, lil_value_t *argv)
{
  char s[2];
  if (!argc)
  {
    return 0;
  }
  s[0] = (char) lil_to_integer(argv[0]);
  s[1] = 0;
  return lil_alloc_string(s);
}


----------------------------
static lil_value_t fnc_print(lil_t lil, size_t argc, lil_value_t *argv)
{
  fnc_write(lil, argc, argv);
  lil_write(lil, "\n");
  return 0;
}


----------------------------
static lil_value_t fnc_for(lil_t lil, size_t argc, lil_value_t *argv)
{
  lil_value_t val;
  lil_value_t r = 0;
  if (argc < 4)
  {
    return 0;
  }
  lil_free_value(lil_parse_value(lil, argv[0], 0));
  while ((!lil->error) && (!lil->env->breakrun))
  {
    val = lil_eval_expr(lil, argv[1]);
    if ((!val) || lil->error)
    {
      return 0;
    }
    if (!lil_to_boolean(val))
    {
      lil_free_value(val);
      break;
    }
    if (r)
    {
      lil_free_value(r);
    }
    r = lil_parse_value(lil, argv[3], 0);
    lil_free_value(val);
    lil_free_value(lil_parse_value(lil, argv[2], 0));
  }

  return r;
}


----------------------------
None
----------------------------
***/


// element in merge of:  register_stdcmds,lil_new,fnc_jaileval
lil_t lil_new(void)
{
  lil_t lil = calloc(1, sizeof(struct _lil_t));
  lil->rootenv = (lil->env = lil_alloc_env(0));
  lil->empty = alloc_value(0);
  lil->dollarprefix = strclone("set ");
  hm_init(&lil->cmdmap);
  register_stdcmds(lil);
  return lil;
}


/*** DEPENDENCIES:
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
lil_env_t lil_alloc_env(lil_env_t parent)
{
  lil_env_t env;
  env = calloc(1, sizeof(struct _lil_env_t));
  env->parent = parent;
  return env;
}


----------------------------
static lil_value_t alloc_value(const char *str)
{
  return alloc_value_len(str, (str) ? (strlen(str)) : (0));
}


----------------------------
static void hm_init(hashmap_t *hm)
{
  memset(hm, 0, sizeof(hashmap_t));
}


----------------------------
None
----------------------------
***/


// element in merge of:  register_stdcmds,lil_new,fnc_jaileval
static lil_value_t fnc_jaileval(lil_t lil, size_t argc, lil_value_t *argv)
{
  size_t i;
  lil_t sublil;
  lil_value_t r;
  size_t base = 0;
  if (!argc)
  {
    return 0;
  }
  if (!strcmp(lil_to_string(argv[0]), "clean"))
  {
    base = 1;
    if (argc == 1)
    {
      return 0;
    }
  }
  sublil = lil_new();
  if (base != 1)
  {
    for (i = lil->syscmds; i < lil->cmds; i += 1)
    {
      lil_func_t fnc = lil->cmd[i];
      if (!fnc->proc)
      {
        continue;
      }
      lil_register(sublil, fnc->name, fnc->proc);
    }

  }
  r = lil_parse_value(sublil, argv[base], 1);
  lil_free(sublil);
  return r;
}


/*** DEPENDENCIES:
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free(lil_t lil)
{
  size_t i;
  if (!lil)
  {
    return;
  }
  free(lil->err_msg);
  lil_free_value(lil->empty);
  while (lil->env)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }

  for (i = 0; i < lil->cmds; i += 1)
  {
    if (lil->cmd[i]->argnames)
    {
      lil_free_list(lil->cmd[i]->argnames);
    }
    lil_free_value(lil->cmd[i]->code);
    free(lil->cmd[i]->name);
    free(lil->cmd[i]);
  }

  hm_destroy(&lil->cmdmap);
  free(lil->cmd);
  free(lil->dollarprefix);
  free(lil->catcher);
  free(lil);
}


----------------------------
int lil_register(lil_t lil, const char *name, lil_func_proc_t proc)
{
  lil_func_t cmd = add_func(lil, name);
  if (!cmd)
  {
    return 0;
  }
  cmd->proc = proc;
  return 1;
}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
None
----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
None
----------------------------
***/


char *lil_embedded(lil_t lil, const char *code, unsigned int flags)
{
  char *prev_embed = lil->embed;
  size_t prev_embedlen = lil->embedlen;
  lil_callback_proc_t prev_write = lil->callback[1];
  char *lilcode = 0;
  unsigned int lilcode_idx = 0;
  size_t lilcodelen = 0;
  char *cont = 0;
  unsigned int cont_idx = 0;
  size_t contlen = 0;
  size_t head = 0;
  size_t codelen = strlen(code);
  char *result;
  unsigned int result_idx = 0;
  lil->callback[1] = (lil_callback_proc_t) fnc_embed_write;
  lil->embed = 0;
  lil->embedlen = 0;
  while (head < codelen)
  {
    helper_lil_embedded_1(&lilcode_idx, &lilcodelen, &cont_idx, &contlen, &head, code, lilcode, cont, codelen);
  }

  if (contlen)
  {
    lilcode_idx = realloc(lilcode_idx, (lilcodelen + contlen) + 10);
    memcpy((&lilcode[lilcode_idx]) + lilcodelen, "\nwrite {", 8);
    memcpy(((&lilcode[lilcode_idx]) + lilcodelen) + 8, cont, contlen);
    lilcode[((lilcodelen + contlen) + 8) + lilcode_idx] = '}';
    lilcode[((lilcodelen + contlen) + 9) + lilcode_idx] = '\n';
    lilcodelen += contlen + 10;
    free(cont);
  }
  lilcode_idx = realloc(lilcode_idx, lilcodelen + 1);
  lilcode[lilcodelen + lilcode_idx] = 0;
  lil_free_value(lil_parse(lil, lilcode, 0, 1));
  free(lilcode);
  result_idx = (lil->embed) ? (lil->embed) : (strclone(""));
  lil->embed = prev_embed;
  lil->embedlen = prev_embedlen;
  lil->callback[1] = prev_write;
  return result;
}


/*** DEPENDENCIES:
void helper_lil_embedded_1(unsigned int * const lilcode_idx_ref, size_t * const lilcodelen_ref, unsigned int * const cont_idx_ref, size_t * const contlen_ref, size_t * const head_ref, const char * const code, char * const lilcode, char * const cont, size_t codelen)
{
  unsigned int lilcode_idx = *lilcode_idx_ref;
  size_t lilcodelen = *lilcodelen_ref;
  unsigned int cont_idx = *cont_idx_ref;
  size_t contlen = *contlen_ref;
  size_t head = *head_ref;
  if ((((((head < (codelen - 4)) && (code[head] == '<')) && (code[head + 1] == '?')) && (code[head + 2] == 'l')) && (code[head + 3] == 'i')) && (code[head + 4] == 'l'))
  {
    helper_helper_lil_embedded_1_2(&lilcode_idx, &lilcodelen, &cont_idx, &contlen, &head, code, lilcode, cont, codelen);
  }
  else
  {
    helper_helper_lil_embedded_1_1(&cont_idx, &contlen, &head, code, cont);
  }
  *lilcode_idx_ref = lilcode_idx;
  *lilcodelen_ref = lilcodelen;
  *cont_idx_ref = cont_idx;
  *contlen_ref = contlen;
  *head_ref = head;
}


----------------------------
static char *strclone(const char *s)
{
  size_t len = strlen(s) + 1;
  char *ns = malloc(len);
  if (!ns)
  {
    return 0;
  }
  memcpy(ns, s, len);
  return ns;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
typedef void (*lil_callback_proc_t)(void)
----------------------------
static void fnc_embed_write(lil_t lil, const char *msg)
{
  size_t len = strlen(msg) + 1;
  lil->embed = realloc(lil->embed, lil->embedlen + len);
  memcpy(lil->embed + lil->embedlen, msg, len);
  lil->embedlen += len - 1;
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
None
----------------------------
***/


lil_value_t lil_call(lil_t lil, const char *funcname, size_t argc, lil_value_t *argv)
{
  lil_func_t cmd = find_cmd(lil, funcname);
  lil_value_t r = 0;
  if (cmd)
  {
    if (cmd->proc)
    {
      r = cmd->proc(lil, argc, argv);
    }
    else
    {
      size_t i;
      lil_push_env(lil);
      lil->env->func = cmd;
      if ((cmd->argnames->c == 1) && (!strcmp(lil_to_string(cmd->argnames->v[0]), "args")))
      {
        lil_list_t args = lil_alloc_list();
        lil_value_t argsval;
        for (i = 0; i < argc; i += 1)
          lil_list_append(args, lil_clone_value(argv[i]));

        argsval = lil_list_to_value(args, 0);
        lil_set_var(lil, "args", argsval, 2);
        lil_free_value(argsval);
        lil_free_list(args);
      }
      else
      {
        for (i = 0; i < cmd->argnames->c; i += 1)
          lil_set_var(lil, lil_to_string(cmd->argnames->v[i]), (i < argc) ? (argv[i]) : (0), 2);

      }
      r = lil_parse_value(lil, cmd->code, 1);
      lil_pop_env(lil);
    }
  }
  return r;
}


/*** DEPENDENCIES:
void lil_list_append(lil_list_t list, lil_value_t val)
{
  if (list->c == list->cap)
  {
    size_t cap = (list->cap) ? (list->cap + (list->cap / 2)) : (32);
    lil_value_t *nv = realloc(list->v, (sizeof(lil_value_t)) * cap);
    if (!nv)
    {
      return;
    }
    list->cap = cap;
    list->v = nv;
  }
  list->v[list->c++] = val;
}


----------------------------
typedef struct _lil_value_t *lil_value_t
----------------------------
const char *lil_to_string(lil_value_t val)
{
  return (val && val->l) ? (val->d) : ("");
}


----------------------------
void lil_free_value(lil_value_t val)
{
  if (!val)
  {
    return;
  }
  free(val->d);
  free(val);
}


----------------------------
lil_value_t lil_clone_value(lil_value_t src)
{
  lil_value_t val;
  if (!src)
  {
    return 0;
  }
  val = calloc(1, sizeof(struct _lil_value_t));
  if (!val)
  {
    return 0;
  }
  val->l = src->l;
  if (src->l)
  {
    val->d = malloc(val->l + 1);
    if (!val->d)
    {
      free(val);
      return 0;
    }
    memcpy(val->d, src->d, val->l + 1);
  }
  else
  {
    val->d = 0;
  }
  return val;
}


----------------------------
typedef struct _lil_t *lil_t
----------------------------
void lil_free_list(lil_list_t list)
{
  size_t i;
  if (!list)
  {
    return;
  }
  for (i = 0; i < list->c; i += 1)
    lil_free_value(list->v[i]);

  free(list->v);
  free(list);
}


----------------------------
typedef struct _lil_list_t *lil_list_t
----------------------------
lil_env_t lil_push_env(lil_t lil)
{
  lil_env_t env = lil_alloc_env(lil->env);
  lil->env = env;
  return env;
}


----------------------------
typedef struct _lil_func_t *lil_func_t
----------------------------
void lil_pop_env(lil_t lil)
{
  if (lil->env->parent)
  {
    lil_env_t next = lil->env->parent;
    lil_free_env(lil->env);
    lil->env = next;
  }
}


----------------------------
static lil_func_t find_cmd(lil_t lil, const char *name)
{
  return hm_get(&lil->cmdmap, name);
}


----------------------------
lil_value_t lil_list_to_value(lil_list_t list, int do_escape)
{
  lil_value_t val = alloc_value(0);
  size_t i;
  size_t j;
  for (i = 0; i < list->c; i += 1)
  {
    int escape = (do_escape) ? (needs_escape(lil_to_string(list->v[i]))) : (0);
    if (i)
    {
      lil_append_char(val, ' ');
    }
    if (escape)
    {
      lil_append_char(val, '{');
      for (j = 0; j < list->v[i]->l; j += 1)
      {
        if (list->v[i]->d[j] == '{')
        {
          lil_append_string(val, "}\"\\o\"{");
        }
        else
          if (list->v[i]->d[j] == '}')
        {
          lil_append_string(val, "}\"\\c\"{");
        }
        else
          lil_append_char(val, list->v[i]->d[j]);
      }

      lil_append_char(val, '}');
    }
    else
      lil_append_val(val, list->v[i]);
  }

  return val;
}


----------------------------
lil_list_t lil_alloc_list(void)
{
  lil_list_t list;
  list = calloc(1, sizeof(struct _lil_list_t));
  list->v = 0;
  return list;
}


----------------------------
None
----------------------------
***/


