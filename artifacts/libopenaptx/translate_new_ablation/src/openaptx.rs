...
pub fn aptx_reconstructed_differences_update(
    ...
    // Return mutable reference to the updated rd2[new_p]
    &mut rd1[rd2_start + new_p as usize]
}
...
pub fn aptx_init(hd: bool) -> Option<Box<AptxContext>> {
    ...
    let mut boxed_ctx = Box::new(AptxContext {
        hd: if hd { 1 } else { 0 },
        channels: [
            AptxChannel {
                // SAFETY: assuming AptxChannel is a plain-old-data FFI struct that can be zero-initialized
        ...
        decode_sync_buffer: [0; 6], // keep consistent with the translated definition
    });

    // Call the reset routine with a mutable reference to the inner AptxContext
    // NOTE: aptx_reset expects &mut AptxContext (not *mut), so no casting is needed.
    aptx_reset(&mut *boxed_ctx);
...
pub fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    let reconstructed_sample = clip_intp2(reconstructed_difference.wrapping_add(prediction.predicted_sample), 23);
    
    let predictor = clip_intp2((((i64::from(prediction.s_weight[0]) * i64::from(prediction.previous_reconstructed_sample))
        .wrapping_add(i64::from(prediction.s_weight[1]) * i64::from(reconstructed_sample))) >> 22) as i32, 23);
    ...
    let reconstructed_differences_ptr = reconstructed_differences as *mut i32;
    ...
    for i in 0..order {
        let offset1 = (-i - 1) as isize;
        let srd = (unsafe { *reconstructed_differences_ptr.offset(offset1) } >> 31) | 1;
        prediction.d_weight[i as usize] -= rshift32(prediction.d_weight[i as usize].wrapping_sub(srd.wrapping_mul(srd0)), 8);
        let offset2 = (-i) as isize;
        predicted_difference += i64::from(unsafe { *reconstructed_differences_ptr.offset(offset2) }) * i64::from(prediction.d_weight[i as usize]);
    }
...
pub fn aptx_insert_sync(channels: &mut [AptxChannel; 2], sync_idx: &mut u8) {
    ...
    if aptx_check_parity(channels, sync_idx) != 0 {
        let mut min_channel_idx = 1;
        let mut min_quantize_idx = MAP[0];
        
        for i in (0..2).rev() {
            let c = &channels[i];
            for j in 0..4 {
                let quantize_idx = MAP[j];
                if c.quantize[quantize_idx].error < channels[min_channel_idx].quantize[min_quantize_idx].error {
                    min_channel_idx = i;
                    min_quantize_idx = quantize_idx;
                }
            }
        }
...
pub fn aptx_quantize_difference(quantize: &mut AptxQuantize, sample_difference: i32, dither: i32, quantization_factor: i32, tables: &AptxTables) {
    ...
    d = rshift64((d as i64) * (tables.quantize_dither_factors.as_ref().unwrap()[quantized_sample as usize] as i64), 23) as i32;
    
    let interval_ptr = &intervals[quantized_sample as usize..];
    mean = (interval_ptr[1] + interval_ptr[0]) / 2;
    interval = (interval_ptr[1] - interval_ptr[0]) * ((-(i32::from(sample_difference < 0))) | 1);
    ...
    inv = -i32::from(sample_difference < 0);
...
pub fn aptx_qmf_tree_analysis(
    ...
    for i in 0..2 {
        ...
        let (low, high) = subband_samples.split_at_mut(2);
        aptx_qmf_polyphase_analysis(
            ...
            &intermediate_slice,
            &mut low[2 * i],
            &mut high[2 * i - 2], // Adjust index since we split at 2
        );
    }
}
