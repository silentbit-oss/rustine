...
pub fn aptx_prediction_filtering(
    ...
    // Store the index first to avoid holding the mutable borrow
    ...
    for i in 0..order {
        let srd = (prediction.reconstructed_differences
            [((-i - 1) + reconstructed_differences_idx as i32) as usize]
            >> 31)
            | 1;
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (prediction.reconstructed_differences[(-i + reconstructed_differences_idx as i32) as usize] as i64)
