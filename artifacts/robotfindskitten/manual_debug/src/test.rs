...
pub fn cleanup_test_environment() {
	println!("* cleanup_test_environment");
...
pub fn test_validchar_function() {
	println!("* test_validchar_function");
...
pub fn setup_test_environment() {
    ...
    // Update LINES and COLS from ncurses
    let lines = ncurses::LINES();
    let cols = ncurses::COLS();
    // If the values are invalid, set to default 24x80
    let lines = if lines < 1 { 24 } else { lines };
    let cols = if cols < 1 { 80 } else { cols };
    LINES.store(lines, std::sync::atomic::Ordering::Relaxed);
    COLS.store(cols, std::sync::atomic::Ordering::Relaxed);
...
pub fn test_invalid_input_handling() {
	println!("* test_invalid_input_handling");
...
pub fn test_process_input_basic_movement() {
	println!("* test_process_input_basic_movement");
...
pub fn test_process_input_nethack_keys() {
	println!("* test_process_input_nethack_keys");
...
pub fn test_boundary_checking() {
	println!("* test_boundary_checking");
...
pub fn test_emacs_keys() {
    ...
    
    {
        let mut screen = SCREEN.lock().unwrap();
        if let Some(screen) = screen.as_mut() {
            screen[(orig_x + 1) as usize][orig_y as usize] = -1;
        }
    }
    ...
    
    {
        let mut screen = SCREEN.lock().unwrap();
        if let Some(screen) = screen.as_mut() {
            screen[orig_x as usize][orig_y as usize] = -1;
        }
    }
    ...
    
    {
        let mut screen = SCREEN.lock().unwrap();
        if let Some(screen) = screen.as_mut() {
            screen[orig_x as usize][(orig_y - 1) as usize] = -1;
        }
    }
...
pub fn test_initialize_functions() {
	println!("* test_initialize_functions");
    ...
    
    {
        ...
        assert!((kitten.y >= 3) && (kitten.y <= 256));
