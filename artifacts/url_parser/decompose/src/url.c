...
// hint:  ['p_idx_ref is a mutable refrence to unsigned int', 'hostname_end_idx_ref is a mutable refrence to unsigned int']
void helper_url_parse_2(unsigned int * const p_idx_ref, unsigned int * const hostname_end_idx_ref, url_data_t * const data, char * const p, const char * const p_end, const bool is_ssh, char * const hostname_end)
{
...
// hint:  ['p_idx_ref is a mutable refrence to unsigned int', 'hostname_end_idx_ref is a mutable refrence to unsigned int']
void helper_url_parse_1(unsigned int * const p_idx_ref, unsigned int * const hostname_end_idx_ref, url_data_t * const data, char * const p, const bool is_ssh, char * const hostname_end)
{
  unsigned int p_idx = *p_idx_ref;
  unsigned int hostname_end_idx = *hostname_end_idx_ref;
...
void helper_url_parse_3(unsigned int * const p_idx_ref, unsigned int * const hostname_end_idx_ref, url_data_t * const data, char * const p, const char * const p_end, const bool is_ssh, char * const hostname_end)
{
  ...
  if (!(&hostname_end[hostname_end_idx]))
  {
    do
    {
      fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 234, p);
      goto error;
    }
    while (0);
  }
  ...
  if ((((&hostname_end[hostname_end_idx]) < p_end) && (!is_ssh)) && (hostname_end[hostname_end_idx] == ':'))
  {
    ...
    else
    {
      do
      {
        fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 247, p);
        goto error;
      }
      while (0);
    }
...
void helper_url_parse_2(unsigned int * const p_idx_ref, unsigned int * const hostname_end_idx_ref, url_data_t * const data, char * const p, const bool is_ssh, char * const hostname_end)
{
  ...
  if (!(&hostname_end[hostname_end_idx]))
  {
    do
    {
      fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 255, p);
      goto error;
    }
    while (0);
  }
  ...
  if ((!is_ssh) && (hostname_end[hostname_end_idx] == ':'))
  {
    ...
    else
    {
      do
      {
        fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 267, p);
        goto error;
      }
      while (0);
    }
...
static struct url_key_value *parse_query_string(char *begin, char *end)
{
  ...
  for (unsigned element = 0; (element <= elements) && ((&p[p_idx]) < end); element += 1)
  {
    ...
    if (!kv_end)
    {
      do
      {
        fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 153, p);
        goto error;
      }
      while (0);
    }
...
void helper_url_parse_1(url_data_t * const data, char * const p)
{
    ...
    *query_end = '\0';
    ...
    if (has_fragment)
    {
      ...
      else
      {
        do
        {
          fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 319, p);
          goto error;
        }
        while (0);
      }
  ...
  else
  {
    do
    {
      fprintf(stderr, "ERROR %s Line %u! p=«%s»\n", "/c2rust_final/subjects/url_parser/url.c", 323, p);
      goto error;
    }
    while (0);
  }
