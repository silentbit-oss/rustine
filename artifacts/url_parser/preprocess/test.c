# 0 "/app/subjects/url_parser/test.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/app/subjects/url_parser/test.c"
...
# 3 "/app/subjects/url_parser/test.c" 2
# 1 "/app/pipeline/preprocess/fake_libc_include/assert.h" 1
# 4 "/app/subjects/url_parser/test.c" 2
# 1 "/app/pipeline/preprocess/fake_libc_include/string.h" 1
# 5 "/app/subjects/url_parser/test.c" 2
# 15 "/app/subjects/url_parser/test.c"
int
main (void) {
  ...
  do{ char* s = url_get_protocol(url); assert( s ); assert( strcmp("http", s) == 0); free(s); }while(0);
  do{ char* s = url_get_userinfo(url); assert( s ); assert( strcmp("user:pass", s) == 0); free(s); }while(0);
  do{ char* s = url_get_hostname(url); assert( s ); assert( strcmp("subdomain.host.com", s) == 0); free(s); }while(0);
  do{ char* s = url_get_path (url); assert( s ); assert( strcmp("/p/\xc3\xa5/t/h", s) == 0); free(s); }while(0);
  ...
  do{ char* s = url_get_fragment(url); assert( s ); assert( strcmp("hæsh", s) == 0); free(s); }while(0);
  do{ char* s = url_get_port (url); assert( s ); assert( strcmp("8080", s) == 0); free(s); }while(0);

  do{ char* s = url_get_protocol(gh_url); assert( s ); assert( strcmp("git", s) == 0); free(s); }while(0);
  do{ char* s = url_get_hostname(gh_url); assert( s ); assert( strcmp("github.com", s) == 0); free(s); }while(0);
  do{ char* s = url_get_userinfo(gh_url); assert( s ); assert( strcmp("git", s) == 0); free(s); }while(0);
  do{ char* s = url_get_path (gh_url); assert( s ); assert( strcmp("jwerle/url.h.git", s) == 0); free(s); }while(0);
