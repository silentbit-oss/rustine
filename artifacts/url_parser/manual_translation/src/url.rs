...
pub fn helper_url_parse_2(
	...
	println!("* helper_url_parse_2");
    let mut p_idx = *p_idx_ref;
    let mut hostname_end_idx: usize = 0;//*hostname_end_idx_ref;

    // Scan part and handle potential error
    let scanned = scan_part(&p[p_idx..], Category::Unreserved, ':', '/')
        .or_else(|| scan_part(&p[p_idx..], Category::SubDelim, ':', '/'));
    hostname_end_idx = match scanned {
        Some(s) => { 
            data.host = Some(s.to_string());
            s.len()
        },
        None => {
    ...
    p_idx = p_idx + hostname_end_idx;
    
    if !is_ssh && p.chars().nth(p_idx) == Some(':') {
        // In Rust we can't modify the string directly, so we'll work with slices
        p_idx = p_idx + 1;
        let port_end = scan_decimal_number(Some(&p[p_idx..]));

        if let Some(port_end_str) = port_end {
            data.port = Some(port_end_str.to_string());
            p_idx = p_idx + port_end_str.len();
        } else {
...
pub fn parse_query_string(begin: &str) -> Option<Vec<UrlKeyValue>> {
	println!("* parse_query_string");
    
    // Count the number of elements (key-value pairs) by counting delimiters
    let elements = begin.chars().filter(|c| *c == ';' || *c == '&').count();
    ...
    for element in 0..elements {
        ...
        let mut kv_end = match scan_part(&p[p_idx..], Query, '&', ';') {
        ...
        let mut key_end = match scan_part(&kv_end, Query, '=', ';') {
            Some(end) => end.to_string(),
            None => {
                eprintln!("ERROR {} Line {}! p=«{}»", "/workdir/C2RustTranslation-main/subjects/url_parser/url.c", 153, p);
                return None;
        ...
        let has_value = kv_end[key_end.len()..].starts_with('=');
        ...
        let key_end_len = key_end.len();
        let decoded_key = decode_percent(&mut key_end);
        let decoded_value = if has_value {
            decode_percent(&mut kv_end[key_end_len..])
        } else {
            None
        };

        kv.push(UrlKeyValue {
            key: Some(decoded_key.unwrap().to_string()),
            value: Some(decoded_value.unwrap().to_string()),
        });
...
pub fn helper_url_parse_1(data: &mut UrlData, p: &mut str) -> Result<(), ()> {
	println!("* helper_url_parse_1");
    let query_end = scan_part(p, Category::Query, '#', '\0');
    match query_end {
        Some(query_end_str) => {
            let has_fragment = p[query_end_str.len()..].starts_with('#');
            data.query = parse_query_string(query_end_str);
            ...
            if has_fragment {
                ...
                //let fragment_end = scan_part(&p[query_end_str.len() + 1..], Category::Fragment, '\0', '\0');
                data.fragment = decode_percent(&mut p[query_end_str.len() + 1..].to_string()).map(|s| s.to_string());
                Ok(())
                //match fragment_end {
                //    Some(fr) => {
                //        let mut t = fr.to_string();
                //        data.fragment = decode_percent(&mut t).map(|s| s.to_string());
                //        Ok(())
                //    },
                //    None => {
...
pub fn url_parse(url: Option<&str>) -> Option<Box<UrlData>> {
    ...
    let protocol_end_abs = protocol_end.len();//as_ptr() as usize - p.as_ptr() as usize;
    ...
    data.protocol = Some(protocol_end.to_string());
    ...
    if let Some(userinfo_end) = userinfo_end {
        if !userinfo_end.is_empty() && p[p_idx + userinfo_end.len()..].starts_with('@') {
            let userinfo_end_abs = userinfo_end.as_ptr() as usize - p.as_ptr() as usize;
            data.userinfo = Some(userinfo_end.to_string());
            p_idx = p_idx + userinfo_end.len() + 1;
        }
    }
    ...
    if p.chars().nth(p_idx) == Some('[') {
        ...
        helper_url_parse_2(&mut p_idx, &mut hostname_end_idx, &mut data, &p, is_ssh, &hostname_end);
    }
    ...
    let path_end = match scan_part(&p[p_idx..], Category::PCharSlash, '?', '#') {
        Some(s) => s.to_string(),
        None => { 
            return Some(data);
        }
    };
    let path_end_abs = path_end.len();// as usize - p.as_ptr() as usize;

    if path_end.is_empty() {
        eprintln!("ERROR {} Line {}! p=«{}»", "/workdir/C2RustTranslation-main/subjects/url_parser/url.c", 295, p);
        return None;
    }
    data.path = decode_percent(&mut p[p_idx..p_idx + path_end_abs].to_string()).map(|s| s.to_string());
    p_idx += path_end_abs;

    let has_query = p[p_idx..].starts_with('?');
    let has_fragment = p[p_idx..].starts_with('#');
...
pub fn url_get_query_value<'a>(url: &'a UrlData, tkey: &str) -> Option<&'a str> {
	println!("* url_get_query_value");
