...
# 19 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c"
...
# 20 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c" 2
...
# 21 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c" 2
...
# 22 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c" 2
...
static inline size_t mi_bitmap_mask_(size_t count, size_t bitidx) {
  ...
  if (count >= (8*(1<<(3)))) return (~((size_t)0));
  if (count == 0) return 0;
  return ((((size_t)1 << count) - 1) << bitidx);
}
# 43 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c"
...
bool _mi_bitmap_unclaim(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx) {
  ...
  ((bitmap_fields > idx) ? (void)0 : _mi_assert_fail("bitmap_fields > idx","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",136,__func__)); (void)(bitmap_fields);
...
bool _mi_bitmap_claim(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx, bool* any_zero) {
  ...
  const size_t bitidx = mi_bitmap_index_bit_in_field(bitmap_idx);
  const size_t mask = mi_bitmap_mask_(count, bitidx);
  ((bitmap_fields > idx) ? (void)0 : _mi_assert_fail("bitmap_fields > idx","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",149,__func__)); (void)(bitmap_fields);

  size_t prev = atomic_fetch_or_explicit(&bitmap[idx],mask,memory_order_acq_rel);
  if (any_zero != 0) { *any_zero = ((prev & mask) != mask); }
  return ((prev & mask) == 0);
}
...
static bool mi_bitmap_is_claimedx(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx, bool* any_ones) {
  ...
  const size_t bitidx = mi_bitmap_index_bit_in_field(bitmap_idx);
  const size_t mask = mi_bitmap_mask_(count, bitidx);
  ((bitmap_fields > idx) ? (void)0 : _mi_assert_fail("bitmap_fields > idx","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",161,__func__)); (void)(bitmap_fields);
  const size_t field = atomic_load_explicit(&bitmap[idx],memory_order_relaxed);
  if (any_ones != 0) { *any_ones = ((field & mask) != 0); }
...
bool _mi_bitmap_try_claim(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx) {
  ...
  const size_t bitidx = mi_bitmap_index_bit_in_field(bitmap_idx);
  const size_t mask = mi_bitmap_mask_(count, bitidx);
  ((bitmap_fields > idx) ? (void)0 : _mi_assert_fail("bitmap_fields > idx","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",173,__func__)); (void)(bitmap_fields);
  size_t expected = atomic_load_explicit(&bitmap[idx],memory_order_relaxed);
  do {
    if ((expected & mask) != 0) return 0;
  }
...
# 203 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c"
static bool mi_bitmap_try_find_claim_field_across(mi_bitmap_t bitmap, size_t bitmap_fields, size_t idx, const size_t count, const size_t retries, mi_bitmap_index_t* bitmap_idx)
{
  ...
  if (_mi_divide_up(count - initial, (8*(1<<(3)))) >= (bitmap_fields - idx)) return 0;
  ...
  ((field < &bitmap[bitmap_fields]) ? (void)0 : _mi_assert_fail("field < &bitmap[bitmap_fields]","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",228,__func__));
  ...
  while (++field < final_field) {
    newmap = (~((size_t)0));
    map = 0;
    if (!atomic_compare_exchange_strong_explicit(field,&map,newmap,memory_order_acq_rel,memory_order_acquire)) { goto rollback; }
  }


  ((field == final_field) ? (void)0 : _mi_assert_fail("field == final_field","/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c",255,__func__));
  map = atomic_load_explicit(field,memory_order_relaxed);
  do {
...
bool _mi_bitmap_try_find_from_claim_across(mi_bitmap_t bitmap, const size_t bitmap_fields, const size_t start_field_idx, const size_t count, mi_bitmap_index_t* bitmap_idx) {
  ...
  for (size_t visited = 0; visited < bitmap_fields; visited++, idx++) {
    if (idx >= bitmap_fields) { idx = 0; }
# 315 "/home/ubuntu/C2RustTranslation/subjects/mimalloc/src/bitmap.c"
...
bool _mi_bitmap_claim_across(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx, bool* pany_zero, size_t* already_set) {
  ...
  if ((prev & pre_mask) != 0) { all_zero = 0; one_count += mi_popcount(prev & pre_mask); }
  if ((prev & pre_mask) != pre_mask) any_zero = 1;
  while (mid_count-- > 0) {
    prev = atomic_fetch_or_explicit(field++,mid_mask,memory_order_acq_rel);
    if ((prev & mid_mask) != 0) { all_zero = 0; one_count += mi_popcount(prev & mid_mask); }
    if ((prev & mid_mask) != mid_mask) any_zero = 1;
  }
  if (post_mask!=0) {
    prev = atomic_fetch_or_explicit(field,post_mask,memory_order_acq_rel);
    if ((prev & post_mask) != 0) { all_zero = 0; one_count += mi_popcount(prev & post_mask); }
    if ((prev & post_mask) != post_mask) any_zero = 1;
  }
  if (pany_zero != 0) { *pany_zero = any_zero; }
  if (already_set != 0) { *already_set = one_count; };
...
static bool mi_bitmap_is_claimedx_across(mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx, bool* pany_ones, size_t* already_set) {
  ...
  if ((prev & pre_mask) != pre_mask) all_ones = 0;
  if ((prev & pre_mask) != 0) { any_ones = 1; one_count += mi_popcount(prev & pre_mask); }
  while (mid_count-- > 0) {
    prev = atomic_load_explicit(field++,memory_order_relaxed);
    if ((prev & mid_mask) != mid_mask) all_ones = 0;
    if ((prev & mid_mask) != 0) { any_ones = 1; one_count += mi_popcount(prev & mid_mask); }
  }
  if (post_mask!=0) {
    prev = atomic_load_explicit(field,memory_order_relaxed);
    if ((prev & post_mask) != post_mask) all_ones = 0;
    if ((prev & post_mask) != 0) { any_ones = 1; one_count += mi_popcount(prev & post_mask); }
  }
  if (pany_ones != 0) { *pany_ones = any_ones; }
  if (already_set != 0) { *already_set = one_count; }
