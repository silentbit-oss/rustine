...
static void mi_buffered_out(const char *msg, void *arg)
{
  ...
  for (const char *src = msg; src[src_idx] != 0; src_idx += 1)
  {
    ...
    (buf->used < buf->count) ? ((void) 0) : (_mi_assert_fail("buf->used < buf->count", "/home/ubuntu/tmp/Rustine-Dev/subjects/mimalloc/src/stats.c", 303, __func__));
    buf->buf[buf->used++] = c;
    if (c == '\n')
    {
      mi_buffered_flush(buf);
...
static void mi_heap_buf_print(mi_heap_buf_t *hbuf, const char *msg)
{
  ...
  for (const char *src = msg; src[src_idx] != 0; src_idx += 1)
  {
    ...
    (hbuf->used < hbuf->size) ? ((void) 0) : (_mi_assert_fail("hbuf->used < hbuf->size", "/home/ubuntu/tmp/Rustine-Dev/subjects/mimalloc/src/stats.c", 551, __func__));
    hbuf->buf[hbuf->used++] = c;
  }

  unsigned int src_idx = 0;
  (hbuf->used < hbuf->size) ? ((void) 0) : (_mi_assert_fail("hbuf->used < hbuf->size", "/home/ubuntu/tmp/Rustine-Dev/subjects/mimalloc/src/stats.c", 554, __func__));
