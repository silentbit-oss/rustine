...
{
  void *p = mi_malloc_aligned(size, alignment);
  ((((uintptr_t) p) % alignment) == 0) ? ((void) 0) : (_mi_assert_fail("((uintptr_t)p % alignment) == 0", "/home/ubuntu/tmp/Rustine-Dev/subjects/mimalloc/src/alloc-posix.c", 98, __func__));
  return p;
}
...
{
  (p != 0) ? ((void) 0) : (_mi_assert_fail("p != NULL", "/home/ubuntu/tmp/Rustine-Dev/subjects/mimalloc/src/alloc-posix.c", 109, __func__));
  if (p == 0)
  {
    errno = 22;
    return 22;
