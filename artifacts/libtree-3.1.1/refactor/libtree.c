...
static int recurse(char *current_file, size_t depth, struct libtree_state_t *state, struct compat_t compat, struct found_t reason);
...
static int recurse(char *current_file, size_t depth, struct libtree_state_t *s, struct compat_t compat, struct found_t reason)
{
  ...
  {
    helper_recurse_1(&curr_type, compat, fptr, header);
  }
  else
  {
    helper_recurse_2(&curr_type, compat, fptr, header);
  }
  union 
  {
  ...
  for (int cont = 1; cont;)
  {
    helper_recurse_3(&no_def_lib, &strtab, &rpath, &runpath, &soname, &cont, fptr, curr_type, pt_load_offset, pt_load_vaddr, needed);
  }
...
static int check_search_paths(struct found_t reason, size_t offset, size_t *needed_not_found, struct small_vec_u64_t *needed_buf_offsets, size_t depth, struct libtree_state_t *s, struct compat_t compat)
{
  ...
  while (st->arr[offset] != '\0')
  {
    ...
    while (((st->arr[offset] != '\0') && (st->arr[offset] != ':')) && ((&dest[dest_idx]) != path_end))
    {
      dest[dest_idx] = st->arr[offset];
      offset += 1;
      dest_idx += 1;
    }

    if (((&dest[dest_idx]) + 1) >= path_end)
    {
      continue;
    }
    if ((*((&dest[dest_idx]) - 1)) != '/')
...
void helper_check_absolute_paths_1(size_t * const depth_ref, int * const exit_code_ref, size_t * const i_ref, size_t * const needed_not_found, struct small_vec_u64_t * const needed_buf_offsets, struct libtree_state_t * const s, struct compat_t compat)
{
...
int helper_recurse_1(struct compat_t * const curr_type_ref, struct compat_t compat, FILE * const fptr, union 
{
  struct header_64_t h64;
  struct header_32_t h32;
} header)
...
int helper_recurse_2(struct compat_t * const curr_type_ref, struct compat_t compat, FILE * const fptr, union 
{
  struct header_64_t h64;
  struct header_32_t h32;
} header)
...
int helper_helper_main_2_1(struct libtree_state_t * const s_ref, int * const opt_help_ref, char * const arg, unsigned int arg_idx)
{
  struct libtree_state_t s = *s_ref;
  int opt_help = *opt_help_ref;
  switch (arg[arg_idx])
...
int helper_helper_main_2_2(struct libtree_state_t * const s_ref, int * const opt_help_ref, int * const opt_version_ref, int * const opt_raw_ref, int * const i_ref, unsigned int * const arg_idx_ref, int argc, char ** const argv, unsigned int argv_idx, char * const arg)
{
  struct libtree_state_t s = *s_ref;
  int opt_help = *opt_help_ref;
  int opt_version = *opt_version_ref;
