...
pub fn create_test_elf_file(filename: &str, valid: bool) {
    println!("\n* create_test_elf_file");
    // Only create file if valid=true or when explicitly creating an invalid file
    if valid {
    ...
    // Do not create file when valid=false (simulate non-existent file)
}
...
pub fn helper_create_elf_with_program_headers_1(f: &mut File) -> std::io::Result<()> {
	println!("\n* helper_create_elf_with_program_headers_1");
...
pub fn create_elf_with_program_headers(filename: &str, has_dynamic: bool) -> std::io::Result<()> {
	println!("\n* create_elf_with_program_headers");
...
pub fn test_string_table_operations() {
    ...
    {
        let expected = "hello";
        let arr = table.arr.as_ref().unwrap();
        let actual: String = arr[..table.n].iter().take_while(|&&c| c != '\0').collect();
        if expected != actual {
...

    {
        let expected = "world";
        let arr = table.arr.as_ref().unwrap();
        let actual: String = arr[6..].iter().take_while(|&&c| c != '\0').collect();
        if expected != actual {
...
pub fn test_string_table_growth() {
    ...
    if let Some(arr) = &table.arr {
        // We remove the null terminator at the end, if present.
        let stored_str: String = if arr.len() > 0 {
            // Remove the last character (the null terminator)
            arr[0..arr.len()-1].iter().collect()
        } else {
            String::new()
        };
...
pub fn test_config_file_nonexistent() {
    ...
    match result {
        Ok(()) => {
            println!("FAILED: Expected error, got success at line {}", 269);
            panic!("panic");
        },
        Err(_) => (),
    }
...
pub fn test_print_tree_function() {
    ...
    // __AUTOCOMMENT__:f342bd4a-390c-4fb9-b1fa-65235f941d54 create_test_elf_file("test_print.elf", true);
...
pub fn test_recurse_32bit_elf() {
    ...
    if !(result == 0 || result == 15) {
        eprintln!("FAILED: Assertion failed at line {} {}", 1758, result);
        panic!("panic");
    }
...
pub fn test_elf_file_validation() {
    ...
    // __AUTOCOMMENT__:c2e9c63c-af6a-40d9-8a21-01691ef35753 create_test_elf_file("valid.elf", true);
    // __AUTOCOMMENT__:329c8c5d-9b6a-4df8-85d9-2d37381eea0a create_test_elf_file("invalid.elf", false);
...
pub fn test_recurse_elf_with_program_headers_no_dynamic() {
    ...
    if result != 15 {
        println!("FAILED: Expected {}, got {} at line {}", 15, result, 1501);
        panic!("panic");
    }
...
pub fn test_recurse_elf_wrong_type() {
    ...
    if 15 != result {
        println!("FAILED: Expected {}, got {} at line {}", 15, result, 1548);
        panic!("panic");
    }
...
pub fn test_recurse_elf_incompatible_arch() {
    ...
    let result = recurse("test_wrong_arch.elf", 0, &mut state, compat, reason);
...
pub fn test_recurse_elf_wrong_endianness() {
    ...
    if 15 != result {
        println!("FAILED: Expected {}, got {} at line {}", 15, result, 1525);
        panic!("panic");
    }
...
pub fn test_check_absolute_paths_function() {
    ...
    println!("INJAAAAAAAAAAAA {:?}", needed_buf_offsets);
