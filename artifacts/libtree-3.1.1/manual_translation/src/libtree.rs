...
pub fn small_vec_u64_append(v: &mut SmallVecU64T, val: u64) {
    ...
    if v.n == 16 {
        v.capacity = 2 * 16;
        let mut new_vec = vec![0;v.capacity];
        for &item in v.buf.iter() {
            new_vec.push(item);
        }
        v.p = Some(new_vec);
    } else if v.n == v.capacity {
        v.capacity *= 2;
        if let Some(ref mut inner_vec) = v.p {
            inner_vec.resize(v.capacity, 0);
        }
    }
...
pub fn print_error(
    ...
    for i in 0..needed_not_found {
        ...
        if let Some(arr) = &s.string_table.arr {
            let index = if let Some(inner_vec) = &needed_buf_offsets.p {
