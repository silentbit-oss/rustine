...
pub fn helper_check_absolute_paths_1(
    ...
    if let Some(arr) = &s.string_table.arr {
        let offset = if let Some(inner_vec) = &needed_buf_offsets.p {
            inner_vec[i] as usize
        } else {
    ...
    let tmp = if let Some(inner_vec) = &needed_buf_offsets.p {
        inner_vec[i]
    } else {
        needed_buf_offsets.buf[i]
    };

    if let Some(ref mut inner_vec) = needed_buf_offsets.p {
        inner_vec[i] = inner_vec[(*needed_not_found) - 1];
        *needed_not_found -= 1;
        inner_vec[*needed_not_found] = tmp;
    } else {
        needed_buf_offsets.buf[i] = needed_buf_offsets.buf[(*needed_not_found) - 1];
        *needed_not_found -= 1;
        needed_buf_offsets.buf[*needed_not_found] = tmp;
    }
