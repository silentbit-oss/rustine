...
void string_table_store(struct string_table_t *t, char const *str) {
...
void string_table_copy_from_file(struct string_table_t *t, FILE *fptr) {
    int c;
...
void tree_preamble(struct libtree_state_t *s, size_t depth) {
    if (depth == 0)
        return;
...
int recurse(char *current_file, size_t depth,
                   struct libtree_state_t *state, struct compat_t compat,
                   struct found_t reason);

void apply_exclude_list(size_t *needed_not_found,
...
int check_absolute_paths(size_t *needed_not_found,
                                struct small_vec_u64_t *needed_buf_offsets,
...
int check_search_paths(struct found_t reason, size_t offset,
...
int interpolate_variables(struct libtree_state_t *s, size_t src,
                                 char const *ORIGIN) {
...
void print_colon_delimited_paths(char const *start, char const *indent) {
    while (1) {
...
void print_line(size_t depth, char *name, char *color_bold,
...
void print_error(size_t depth, size_t needed_not_found,
                        struct small_vec_u64_t *needed_buf_offsets,
...
int visited_files_contains(struct visited_file_array_t *files,
                                  struct stat *needle) {
...
void visited_files_append(struct visited_file_array_t *files,
                                 struct stat *new) {
...
int parse_ld_config_file(struct string_table_t *st, char *path);

int ld_conf_globbing(struct string_table_t *st, char *pattern) {
    glob_t result;
...
void parse_ld_so_conf(struct libtree_state_t *s) {
    struct string_table_t *st = &s->string_table;
...
int print_tree(int pathc, char **pathv, struct libtree_state_t *s) {
