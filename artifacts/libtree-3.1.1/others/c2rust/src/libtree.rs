...
unsafe extern "C" fn utoa(mut str: *mut libc::c_char, mut v: size_t) {
    ...
    loop {
        let fresh0 = p;
        p = p.offset(1);
        *fresh0 = ('0' as i32 as size_t).wrapping_add(v % 10 as libc::c_int as size_t)
            as libc::c_char;
        v = v / 10 as libc::c_int as size_t;
...
unsafe extern "C" fn small_vec_u64_append(
    ...
    if (*v).n < 16 as libc::c_int as size_t {
        let fresh1 = (*v).n;
    ...
    if (*v).n == 16 as libc::c_int as size_t {
        ...
        (*v).capacity = (*v).capacity * 2 as libc::c_int as size_t;
...
unsafe extern "C" fn is_ascending_order(
    ...
    while j < n {
        if *v.offset(j.wrapping_sub(1 as libc::c_int as size_t) as isize)
            >= *v.offset(j as isize)
        {
...
unsafe extern "C" fn string_table_maybe_grow(mut t: *mut string_table_t, mut n: size_t) {
    ...
    (*t).capacity = 2 as libc::c_int as size_t * ((*t).n).wrapping_add(n);
...
unsafe extern "C" fn tree_preamble(mut s: *mut libtree_state_t, mut depth: size_t) {
    ...
    let mut i: size_t = 0 as libc::c_int as size_t;
    while i < depth.wrapping_sub(1 as libc::c_int as size_t) {
        fputs(
    ...
    fputs(
        if (*s).found_all_needed[depth.wrapping_sub(1 as libc::c_int as size_t) as usize]
            as libc::c_int != 0
        {
    ...
    while i < *needed_not_found {
        ...
        ) != 0
        {
            ...
            *((*needed_buf_offsets).p)
                ...
                .offset(
                    (*needed_not_found).wrapping_sub(1 as libc::c_int as size_t) as isize,
                );
...
unsafe extern "C" fn check_absolute_paths(
    ...
    while i < *needed_not_found {
            ...
            .is_null()
        {
            ...
            if len >= 4096 as libc::c_int as size_t {
                continue;
            }
            memcpy(
            ...
            (*s)
                ...
                as usize] = (*needed_not_found <= 1 as libc::c_int as size_t)
            ...
            if !err.is_null() {
                tree_preamble(s, depth.wrapping_add(1 as libc::c_int as size_t));
                if (*s).color != 0 {
        ...
        while i < *needed_not_found {
            ...
            memcpy(
                ...
                ((*st).arr)
                    .offset(*((*needed_buf_offsets).p).offset(i as isize) as isize)
                    as *const libc::c_void,
                soname_len.wrapping_add(1 as libc::c_int as size_t),
            );
            (*s)
                .found_all_needed[depth
                as usize] = (*needed_not_found <= 1 as libc::c_int as size_t)
            ...
            if code == 0 as libc::c_int || code == 28 as libc::c_int {
                ...
                *((*needed_buf_offsets).p)
                    ...
                    .offset(
                        (*needed_not_found).wrapping_sub(1 as libc::c_int as size_t)
                            as isize,
                    );
                *needed_not_found = (*needed_not_found).wrapping_sub(1);
                *((*needed_buf_offsets).p).offset(*needed_not_found as isize) = tmp;
...
unsafe extern "C" fn interpolate_variables(
    ...
    loop {
        ...
        {
            var_val = ORIGIN;
            curr_src = curr_src.wrapping_add(6 as libc::c_int as size_t);
        } else if strncmp(
            &mut *((*st).arr).offset(curr_src as isize),
        ...
        {
            var_val = (*s).LIB;
            curr_src = curr_src.wrapping_add(3 as libc::c_int as size_t);
        } else if strncmp(
            &mut *((*st).arr).offset(curr_src as isize),
        ...
        {
            var_val = (*s).PLATFORM;
            curr_src = curr_src.wrapping_add(8 as libc::c_int as size_t);
        } else if strncmp(
            &mut *((*st).arr).offset(curr_src as isize),
        ...
        ) == 0 as libc::c_int
        {
            ...
            var_val = (*s).OSREL;
            curr_src = curr_src.wrapping_add(5 as libc::c_int as size_t);
        }
        if curly != 0 {
            if *((*st).arr).offset(curr_src as isize) as libc::c_int != '}' as i32 {
                continue;
        ...
        (*s).string_table.n = ((*s).string_table.n).wrapping_add(bytes_to_dollar);
        prev_src = curr_src;
        memcpy(
            &mut *((*st).arr).offset((*s).string_table.n as isize) as *mut libc::c_char
                as *mut libc::c_void,
        ...
        (*s).string_table.n = ((*s).string_table.n).wrapping_add(var_len);
    }
    if prev_src != src {
...
unsafe extern "C" fn print_line(
    ...
    match reason.how as libc::c_uint {
        2 => {
            if (reason.depth).wrapping_add(1 as libc::c_int as size_t) >= depth {
                fputs(b"[rpath]\0" as *const u8 as *const libc::c_char, stdout);
            } else {
                let mut num: [libc::c_char; 8] = [0; 8];
                utoa(
                    num.as_mut_ptr(),
                    (reason.depth).wrapping_add(1 as libc::c_int as size_t),
                );
...
unsafe extern "C" fn print_error(
    ...
    while i < needed_not_found {
        (*s)
            .found_all_needed[depth
            as usize] = (i.wrapping_add(1 as libc::c_int as size_t) >= needed_not_found)
            as libc::c_int as libc::c_char;
        tree_preamble(s, depth.wrapping_add(1 as libc::c_int as size_t));
        if (*s).color != 0 {
            fputs(b"\x1B[1;31m\0" as *const u8 as *const libc::c_char, stdout);
...
unsafe extern "C" fn visited_files_append(
    ...
    if (*files).n == (*files).capacity {
        (*files).capacity = (*files).capacity * 2 as libc::c_int as size_t;
        (*files)
            .arr = realloc(
...
unsafe extern "C" fn recurse(
    ...
    if curr_type.class as libc::c_int == 2 as libc::c_int {
        ...
        while i < header.h64.e_phnum as uint64_t {
            if fread(
        ...
        while i_0 < header.h32.e_phnum as uint32_t {
            if fread(
            ...
            if prog.p32.p_type == 1 as libc::c_int as uint32_t {
            ...
            } else if prog.p32.p_type == 2 as libc::c_int as uint32_t {
    ...
    if pt_load_offset.n == 0 as libc::c_int as size_t {
        fclose(fptr);
        small_vec_u64_free(&mut pt_load_offset);
        small_vec_u64_free(&mut pt_load_vaddr);
    ...
    while cont != 0 {
        ...
        match d_tag {
            ...
            1879048187 => {
                no_def_lib
                    |= (0x800 as libc::c_int as uint64_t & d_val
                        == 0x800 as libc::c_int as uint64_t) as libc::c_int;
            }
            _ => {}
        }
    }
    ...
    while vaddr_idx.wrapping_add(1 as libc::c_int as size_t) != pt_load_vaddr.n
        && strtab
            >= *(pt_load_vaddr.p)
                .offset(vaddr_idx.wrapping_add(1 as libc::c_int as size_t) as isize)
    {
...
unsafe extern "C" fn parse_ld_config_file(
    ...
    while c != -(1 as libc::c_int) {
        ...
        loop {
            ...
            if line_len < (4096 as libc::c_int - 1 as libc::c_int) as size_t {
                let fresh8 = line_len;
                ...
                != 0
        {
            ...
            if *begin as libc::c_int != '/' as i32 {
                ...
                if wd_len
                    ...
                    .wrapping_add(include_len) >= 4096 as libc::c_int as size_t
                {
                    continue;
                }
                memcpy(
            ...
            *((*st).arr)
                .offset(
                    ((*st).n).wrapping_sub(1 as libc::c_int as size_t) as isize,
...
unsafe fn main_0(
    ...
    if opt_help != 0 || opt_version == 0 && positional == 0 as libc::c_int {
        ...
        while j < num_excluded {
            ...
            if cursor_x > 60 as libc::c_int as size_t {
                cursor_x = 3 as libc::c_int as size_t;
                fputs(b"\n  \0" as *const u8 as *const libc::c_char, stdout);
            }
            fputs(exclude_list[j as usize], stdout);
            if j.wrapping_add(1 as libc::c_int as size_t) != num_excluded {
                fputs(b", \0" as *const u8 as *const libc::c_char, stdout);
            }
            j = j.wrapping_add(1);
            j;
