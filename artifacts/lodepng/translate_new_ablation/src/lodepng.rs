...
pub fn lodepng_realloc(ptr: Option<&mut [u8]>, new_size: usize) -> Option<Box<[u8]>> {
    if new_size == 0 {
...
pub fn update_adler32(adler: u32, data: Option<&[u8]>, len: u32) -> u32 {
    ...
    // Handle the case where data is None by returning the original adler
    ...
    while remaining_len != 0 {
        ...
        for i in 0..amount {
            s1 += data_slice[data_idx + i as usize] as u32;
            s2 += s1;
        }
        data_idx += amount as usize;
...
pub fn addColorBits(out: &mut [u8], index: usize, bits: u32, mut input: u32) {
    let m = match bits {
        1 => 7,
        2 => 3,
        _ => 1,
    ...
    input = input << (bits * (m - p as u32));
    
    let byte_index = (index * bits as usize) / 8;
    
    if p == 0 {
        out[byte_index] = input as u8;
    } else {
        out[byte_index] |= input as u8;
    }
}
...
pub fn lodepng_strlen(a: Option<&str>) -> usize {
    ...
    // Unwrap safely and get the bytes
    let s = a.unwrap();
    s.len()
}
...
pub fn color_tree_get(tree: &ColorTree, r: u8, g: u8, b: u8, a: u8) -> i32 {
    ...
    for bit in 0..8 {
        ...
        match &current_tree.children[i as usize] {
            Some(child) => current_tree = child,
            None => return -1,
        }
    }
    ...
    match mode.colortype {
            ...
            match mode.bitdepth {
                ...
                _ => {
                    ...
                    for _ in 0..numpixels {
                        ...
                        let converted = ((value * 255) / highest) as u8;
                        
                        buffer[buffer_idx] = converted;
                        buffer[buffer_idx + 1] = converted;
                        buffer[buffer_idx + 2] = converted;
        ...
        LodePNGColorType::LCT_RGB => {
            match mode.bitdepth {
                8 => {
                    ...
                    for i in 0..numpixels {
                        let src_start = i * 3;
                        let src_end = src_start + 3;
                        let dst_start = buffer_idx;
                        let dst_end = dst_start + 3;
                        
                        lodepng_memcpy(
                            &mut buffer[dst_start..dst_end],
                            &in_data[src_start..src_end]
                        );
                        buffer[buffer_idx + 3] = 255;
                        buffer_idx += num_channels;
                    }
                ...
                match mode.bitdepth {
                    8 => {
                        ...
                        for i in 0..numpixels {
                            ...
                            let palette_end = palette_start + 4;
                            
                            lodepng_memcpy(
                                &mut buffer[buffer_idx..buffer_idx + 4],
                                &palette[palette_start..palette_end]
                            );
                            buffer_idx += num_channels;
                        }
                    }
        ...
        LodePNGColorType::LCT_RGBA => {
            match mode.bitdepth {
                8 => {
                    lodepng_memcpy(
                        &mut buffer[0..numpixels * 4],
                        &in_data[0..numpixels * 4]
                    );
                }
                _ => {
                    let mut buffer_idx = 0;
...
pub fn getPixelColorsRGB8(buffer: &mut [u8], numpixels: usize, in_data: &[u8], mode: &LodePNGColorMode) {
    ...
    match mode.colortype {
        ...
        LodePNGColorType::LCT_RGB => {
            match mode.bitdepth {
                8 => {
                    let src_slice = &in_data[..numpixels * 3];
                    let dst_slice = &mut buffer[buffer_idx..buffer_idx + numpixels * 3];
                    lodepng_memcpy(dst_slice, src_slice);
                    buffer_idx += numpixels * 3;
                }
                _ => {
                    for i in 0..numpixels {
                        buffer[buffer_idx] = in_data[i * 6];
                ...
                match mode.bitdepth {
                    8 => {
                        for i in 0..numpixels {
                            let index = in_data[i] as usize;
                            let palette_start = index * 4;
                            let palette_slice = &palette[palette_start..palette_start + 3];
                            let buffer_slice = &mut buffer[buffer_idx..buffer_idx + 3];
                            lodepng_memcpy(buffer_slice, palette_slice);
                            buffer_idx += NUM_CHANNELS;
                        }
                    }
                    _ => {
        ...
        LodePNGColorType::LCT_RGBA => {
            match mode.bitdepth {
                8 => {
                    for i in 0..numpixels {
                        let src_start = i * 4;
                        let src_slice = &in_data[src_start..src_start + 3];
                        let dst_slice = &mut buffer[buffer_idx..buffer_idx + 3];
                        lodepng_memcpy(dst_slice, src_slice);
                        buffer_idx += NUM_CHANNELS;
                    }
                }
...
pub fn color_tree_add(tree: &mut ColorTree, r: u8, g: u8, b: u8, a: u8, index: u32) -> u32 {
    ...
    for bit in 0..8 {
        ...
        if current_tree.children[i as usize].is_none() {
            ...
            current_tree.children[i as usize] = Some(new_tree);
        }
        
        current_tree = current_tree.children[i as usize].as_mut().unwrap();
    }
    
    current_tree.index = index as i32;
    0
}
...
pub fn HuffmanTree_makeFromLengths2(tree: &mut HuffmanTree) -> u32 {
    ...
    // These values need to be obtained from external sources since they're not in the HuffmanTree struct
    // For this fix, we'll assume they're available as global variables or function parameters
    // Since we can't change the function signature, we'll need to find an alternative approach
    
    // This is a placeholder implementation that needs to be adapted to the actual data structure
    // The actual implementation should use the proper way to access numcodes, maxbitlen, lengths, and codes
    
    // For now, we'll return an error since we can't access the required data
    error = 83;
...
pub fn getTreeInflateDynamic(
    ...
    while error == 0 {
        ...
        let bitlen_cl_vec = bitlen_cl.as_mut().unwrap();
        for idx in 0..HCLEN {
            ensure_bits9(reader, 3);
            let order_idx = CLCL_ORDER[idx as usize] as usize;
            bitlen_cl_vec[order_idx] = readBits(reader, 3);
        }

        for idx in HCLEN..19 {
            let order_idx = CLCL_ORDER[idx as usize] as usize;
            bitlen_cl_vec[order_idx] = 0;
        }
        ...
        let bitlen_ll_vec = bitlen_ll.as_mut().unwrap();
        let bitlen_d_vec = bitlen_d.as_mut().unwrap();
...
pub fn readChunk_zTXt(
    ...
    while error == 0 {
        ...
        lodepng_memcpy(&mut key_vec[0..length], &data[0..length]);
        key_vec[length] = 0;
        key = Some(key_vec);
        ...
        let key_str = std::str::from_utf8(&key.as_ref().unwrap()[0..length]).ok();
        let str_slice = str.as_ref().map(|v| &v[..]);
        error = lodepng_add_text_sized(info, key_str, str_slice, size);
        break;
    }

    lodepng_free(key.map(|v| Box::new(v) as Box<dyn std::any::Any>));
    lodepng_free(str.map(|v| Box::new(v) as Box<dyn std::any::Any>));
    error
}
...
pub fn readChunk_tEXt(info: &mut LodePngInfo, data: &[u8], chunkLength: usize) -> u32 {
    ...
    while error == 0 {
        ...
        let key_str = match std::str::from_utf8(&data[0..length]) {
            Ok(s) => s,
            Err(_) => {
                error = 83;
                break;
        ...
        let str_value = match std::str::from_utf8(&data[string2_begin..string2_begin + remaining_length]) {
            Ok(s) => s,
            Err(_) => {
                error = 83;
                break;
        ...
        str = Some(str_value.to_string());
        
        error = lodepng_add_text(info, key.as_deref(), str.as_deref());
        break;
    }
...
pub fn readChunk_iTXt(
    ...
    while error == 0 {
        ...
        // Find the length of the key (until null terminator)
        let mut key_end = 0;
        while key_end < chunkLength && data[key_end] != 0 {
            key_end += 1;
        }
        length = key_end;
        ...
        // Extract key
        key = Some(String::from_utf8_lossy(&data[0..length]).into_owned());
        ...
        // Find the length of the langtag (until null terminator)
        let mut langtag_end = begin;
        while langtag_end < chunkLength && data[langtag_end] != 0 {
            langtag_end += 1;
        }
        length = langtag_end - begin;

        // Extract langtag
        langtag = Some(String::from_utf8_lossy(&data[begin..langtag_end]).into_owned());

        begin = langtag_end + 1;
        
        // Find the length of the transkey (until null terminator)
        let mut transkey_end = begin;
        while transkey_end < chunkLength && data[transkey_end] != 0 {
            transkey_end += 1;
        }
        length = transkey_end - begin;

        // Extract transkey
        transkey = Some(String::from_utf8_lossy(&data[begin..transkey_end]).into_owned());

        begin = transkey_end + 1;
        length = if chunkLength < begin { 0 } else { chunkLength - begin };

        if compressed != 0 {
            let mut str_out: Option<Vec<u8>> = None;
            ...
            if error == 0 {
                if let Some(ref str_data) = str_out {
...
pub fn unfilter_scanline(recon: &mut [u8], scanline: &[u8], precon: Option<&[u8]>, bytewidth: usize, filter_type: u8, length: usize) -> u32 {
    match filter_type {
        0 => {
        ...
        1 => {
            let (initial, rest) = recon.split_at_mut(bytewidth);
            initial.copy_from_slice(&scanline[..bytewidth]);
        ...
        3 => {
            if let Some(precon) = precon {
                ...
                while i + 3 < length {
                    ...
                    recon[i] = s0.wrapping_add(((r0 as u16 + p0 as u16) >> 1) as u8);
                    recon[i + 1] = s1.wrapping_add(((r1 as u16 + p1 as u16) >> 1) as u8);
                    recon[i + 2] = s2.wrapping_add(((r2 as u16 + p2 as u16) >> 1) as u8);
                    recon[i + 3] = s3.wrapping_add(((r3 as u16 + p3 as u16) >> 1) as u8);
                    
                    i += 4;
                }
                
                // Process in chunks of 3 where possible
                while i + 2 < length {
                    let s0 = scanline[i];
                    let s1 = scanline[i + 1];
                    let s2 = scanline[i + 2];
                ...
                // Process in chunks of 2 where possible
                while i + 1 < length {
                    let s0 = scanline[i];
                    let s1 = scanline[i + 1];
                ...
                while i < length {
                    let j = i - bytewidth;
                    recon[i] = scanline[i].wrapping_add(((recon[j] as u16 + precon[i] as u16) >> 1) as u8);
                    i += 1;
                }
            } else {
                // First bytewidth bytes
...
pub fn unfilter(out: &mut [u8], r#in: &[u8], w: u32, h: u32, bpp: u32) -> u32 {
    let bytewidth = (bpp + 7u32) / 8u32;
    let linebytes = lodepng_get_raw_size_idat(w, 1, bpp) - 1usize;
    
    // Use a vector to store previous line data instead of borrowing from out
    let mut prevline_data = vec![0u8; linebytes];
    
    for y in 0..h {
        let outindex = linebytes * y as usize;
        let inindex = (1 + linebytes) * y as usize;
        let filter_type = r#in[inindex];
        
        let precon = if y == 0 {
            Option::None
        } else {
            Some(prevline_data.as_slice())
        };
        
        {
            let error = unfilter_scanline(
                &mut out[outindex..outindex + linebytes],
                &r#in[inindex + 1..inindex + 1 + linebytes],
                precon,
                bytewidth as usize,
                filter_type,
                linebytes
        ...
        // Copy the current line to prevline_data for the next iteration
        if y < h - 1 {
            prevline_data.copy_from_slice(&out[outindex..outindex + linebytes]);
        }
    }
...
pub fn lodepng_chunk_append(out: &mut Option<Box<[u8]>>, outsize: &mut usize, chunk: &[u8]) -> u32 {
    let total_chunk_length: usize;
    let mut temp_total = 0;
    if lodepng_addofl(lodepng_chunk_length(chunk) as usize, 12, &mut temp_total) {
        return 77;
    }
    total_chunk_length = temp_total;

    let new_length: usize;
    let mut temp_new = 0;
    if lodepng_addofl(*outsize, total_chunk_length, &mut temp_new) {
        return 77;
    }
    new_length = temp_new;

    let current_buffer = out.as_mut().map(|b| &mut **b);
    let new_buffer = lodepng_realloc(current_buffer, new_length);
    if new_buffer.is_none() {
        return 83;
    }
    let new_buffer = new_buffer.unwrap();
    
    *out = Some(new_buffer);
    *outsize = new_length;
    
    let chunk_start_idx = new_length - total_chunk_length;
    let chunk_start = &mut out.as_mut().unwrap()[chunk_start_idx..];
...
pub fn decodeGeneric(
    ...
    *h = 0;
    ...
    while !IEND && state.error == 0 {
        let chunk_length = lodepng_chunk_length(chunk) as usize;
        let pos = chunk.as_ptr() as usize - in_data.as_ptr() as usize;
        ...
        let data = lodepng_chunk_data_const(Some(chunk));
        unknown = 0;
        ...
        if lodepng_chunk_type_equals(Some(chunk), Some("IDAT")) != 0 {
            ...
            if let Some(ref mut idat_vec) = idat {
                if idatsize + chunk_length <= idat_vec.len() {
                    lodepng_memcpy(&mut idat_vec[idatsize..idatsize + chunk_length], data.unwrap());
            ...
            state.error = readChunk_PLTE(&mut state.info_png.color, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
            critical_pos = 2;
        } else if lodepng_chunk_type_equals(Some(chunk), Some("tRNS")) != 0 {
            state.error = readChunk_tRNS(&mut state.info_png.color, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("bKGD")) != 0 {
            state.error = readChunk_bKGD(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("tEXt")) != 0 {
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_tEXt(&mut state.info_png, data.unwrap(), chunk_length);
                if state.error != 0 {
                    break;
                }
            }
            ...
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_zTXt(&mut state.info_png, &state.decoder, data, chunk_length);
                if state.error != 0 {
                    break;
                }
            }
            ...
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_iTXt(&mut state.info_png, &state.decoder, data.unwrap(), chunk_length);
                if state.error != 0 {
                    break;
                }
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("tIME")) != 0 {
            state.error = readChunk_tIME(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("pHYs")) != 0 {
            state.error = readChunk_pHYs(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("gAMA")) != 0 {
            state.error = readChunk_gAMA(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("cHRM")) != 0 {
            state.error = readChunk_cHRM(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("sRGB")) != 0 {
            state.error = readChunk_sRGB(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("iCCP")) != 0 {
            state.error = readChunk_iCCP(&mut state.info_png, &state.decoder, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(Some(chunk), Some("sBIT")) != 0 {
            state.error = readChunk_sBIT(&mut state.info_png, data.unwrap(), chunk_length);
            if state.error != 0 {
                break;
            }
        } else {
            ...
            if state.decoder.remember_unknown_chunks != 0 {
                // Convert Vec<u8> to Box<[u8]> for the chunk_append function
                let mut boxed_data = state.info_png.unknown_chunks_data[critical_pos - 1]
                    .as_ref()
                    .map(|vec| vec.clone().into_boxed_slice());
                let mut current_size = state.info_png.unknown_chunks_size[critical_pos - 1];
                ...
                // Convert back from Box<[u8]> to Vec<u8>
                state.info_png.unknown_chunks_data[critical_pos - 1] = boxed_data.map(|boxed| boxed.into_vec());
                state.info_png.unknown_chunks_size[critical_pos - 1] = current_size;
        ...
        if !IEND {
            chunk = lodepng_chunk_next_const(chunk, &in_data[insize..]);
        }
    }
    ...
    if state.error == 0 {
        if let Some(ref mut out_vec) = out {
            lodepng_memset(out_vec, 0);
            if let Some(ref scanlines_data) = scanlines {
                state.error = postProcessScanlines(out_vec, scanlines_data, *w, *h, &state.info_png);
            } else {
                state.error = 91;
            }
        }
...
pub fn filterScanline(out: &mut [u8], scanline: &[u8], prevline: Option<&[u8]>, length: usize, bytewidth: usize, filterType: u8) {
    match filterType {
        0 => {
            for i in 0..length {
                out[i] = scanline[i];
        ...
        3 => {
            if let Some(prevline) = prevline {
                ...
                for i in bytewidth..length {
                    out[i] = scanline[i].wrapping_sub(((scanline[i - bytewidth] as i16 + prevline[i] as i16) >> 1) as u8);
                }
            } else {
                for i in 0..bytewidth {
                    out[i] = scanline[i];
...
pub fn countZeros(data: &[u8], size: usize, pos: usize) -> u32 {
    let start = pos;
    let mut end = start + MAX_SUPPORTED_DEFLATE_LENGTH;
    if end > size {
        end = size;
    }
    
    let mut current = start;
    while current < end && data[current] == 0 {
        current += 1;
    }
    ...
    while pos < insize {
        ...
        loop {
            ...
            if current_offset > 0 {
                ...
                if numzeros >= 3 {
                    skip = hash.zeros.as_ref().unwrap()[hashpos] as usize;
            ...
            if numzeros >= 3 && length > numzeros as usize {
                let next_hashpos = hash.chainz.as_ref().unwrap()[hashpos] as usize;
                if hash.zeros.as_ref().unwrap()[next_hashpos] as u32 != numzeros {
                    break;
                }
                hashpos = next_hashpos;
            } else {
                let next_hashpos = hash.chain.as_ref().unwrap()[hashpos] as usize;
                if hash.val.as_ref().unwrap()[next_hashpos] != hashval as i32 {
                    break;
                }
                hashpos = next_hashpos;
            }
        ...
        if lazymatching {
            if lazy == 0 && length >= 3 && length <= maxlazymatch && length < MAX_SUPPORTED_DEFLATE_LENGTH
            {
                lazy = 1;
                lazylength = length;
                lazyoffset = offset;
            ...
            if lazy != 0 {
                ...
                if length > lazylength + 1 {
                    ...
                    length = lazylength;
                    offset = lazyoffset;
                    let hashval_prev = getHash(Some(in_data), insize, pos - 1);
                    hash.head.as_mut().unwrap()[hashval_prev as usize] = -1;
                    if numzeros > 0 {
...
pub fn bpmnode_create(lists: &mut BpmLists, weight: i32, index: u32, tail: Option<Box<BpmNode>>) -> Option<Box<BpmNode>> {
    if lists.nextfree >= lists.numfree {
        // Reset all memory to not in use
        if let Some(ref mut memory) = lists.memory {
            for node in memory.iter_mut() {
                node.in_use = 0;
            }
        }

        // Mark nodes in chains0 and chains1 as in use
        if let Some(ref mut chains0) = lists.chains0 {
            for node_option in chains0.iter_mut() {
                let mut current = node_option;
        ...
        if let Some(ref mut chains1) = lists.chains1 {
            for node_option in chains1.iter_mut() {
                let mut current = node_option;
                while let Some(ref mut node) = current {
                    node.in_use = 1;
                    current = &mut node.tail;
        ...
        // Rebuild freelist with unused nodes
        lists.numfree = 0;
        if let Some(ref mut freelist) = lists.freelist {
            freelist.clear();
            
            if let Some(ref memory) = lists.memory {
                for node in memory.iter() {
                    if node.in_use == 0 {
                        // We can't take direct references from memory, so we need to create new nodes
                        // This is a limitation of the Rust ownership model
                        freelist.push(Some(Box::new(BpmNode {
                            weight: node.weight,
                            index: node.index,
    ...
    if let Some(ref mut freelist) = lists.freelist {
        if lists.nextfree < lists.numfree {
            if let Some(mut result) = freelist[lists.nextfree as usize].take() {
...
pub fn lodepng_huffman_code_lengths(
    ...
    let mut leaves = Vec::with_capacity(numcodes);
    ...
    if numpresent == 0 {
        ...
        let leaf_index = leaves[0].index as usize;
        lengths[leaf_index] = 1;
        lengths[if leaf_index == 0 { 1 } else { 0 }] = 1;
    } else {
        ...
        if error == 0 {
            let freelist = lists.freelist.as_mut().unwrap();
            let memory = lists.memory.as_mut().unwrap();
            ...
            let chains0 = lists.chains0.as_mut().unwrap();
            let chains1 = lists.chains1.as_mut().unwrap();
            let memory = lists.memory.as_ref().unwrap();
            ...
            let chains1 = lists.chains1.as_ref().unwrap();
...
pub fn deflateDynamic(
    ...
    let mut frequencies_ll: Option<Box<[u32]>> = None;
    let mut frequencies_d: Option<Box<[u32]>> = None;
    let mut frequencies_cl: Option<Box<[u32]>> = None;
    let mut bitlen_lld: Option<Box<[u32]>> = None;
    let mut bitlen_lld_e: Option<Box<[u32]>> = None;
    
    let datasize = dataend - datapos;
    let bfinal = if final_val { 1 } else { 0 };
    ...
    frequencies_ll = Some(vec![0; 286].into_boxed_slice());
    frequencies_d = Some(vec![0; 30].into_boxed_slice());
    frequencies_cl = Some(vec![0; 19].into_boxed_slice());
    ...
    while error == 0 {
        if let Some(ref mut freq_ll) = frequencies_ll {
            for val in freq_ll.iter_mut() {
                *val = 0;
            }
        }
        if let Some(ref mut freq_d) = frequencies_d {
            for val in freq_d.iter_mut() {
                *val = 0;
            }
        }
        if let Some(ref mut freq_cl) = frequencies_cl {
            for val in freq_cl.iter_mut() {
                *val = 0;
            }
        }
        ...
        if let Some(ref lz77_data) = lz77_encoded.data {
            let mut i = 0;
            while i < lz77_encoded.size {
                let symbol = lz77_data[i] as usize;
                if let Some(ref mut freq_ll) = frequencies_ll {
                    if symbol < freq_ll.len() {
                        freq_ll[symbol] += 1;
                    }
                }
                
                if symbol > 256 {
                    if i + 2 < lz77_encoded.size {
                        let dist = lz77_data[i + 2] as usize;
                        if let Some(ref mut freq_d) = frequencies_d {
                            if dist < freq_d.len() {
                                freq_d[dist] += 1;
                            }
        ...
        if let Some(ref freq_ll) = frequencies_ll {
            error = HuffmanTree_makeFromFrequencies(&mut tree_ll, freq_ll, 257, 286, 15);
            if error != 0 {
                break;
            }
            // Count actual number of codes used
            numcodes_ll = 286;
            for i in (0..286).rev() {
                if freq_ll[i] != 0 {
                    numcodes_ll = i + 1;
        ...
        if let Some(ref freq_d) = frequencies_d {
            error = HuffmanTree_makeFromFrequencies(&mut tree_d, freq_d, 2, 30, 15);
            if error != 0 {
                break;
            }
            // Count actual number of codes used
        ...
        let numcodes_lld = numcodes_ll + numcodes_d;
        
        bitlen_lld = Some(vec![0; numcodes_lld].into_boxed_slice());
        bitlen_lld_e = Some(vec![0; numcodes_lld].into_boxed_slice());
        
        if bitlen_lld.is_none() || bitlen_lld_e.is_none() {
            error = 83;
            break;
        }
        ...
        if let Some(ref mut bitlen_lld_ref) = bitlen_lld {
            if let Some(ref tree_ll_lengths) = tree_ll.tree2d {
                for i in 0..numcodes_ll {
                    if i < bitlen_lld_ref.len() && i < tree_ll_lengths.len() {
                        bitlen_lld_ref[i] = tree_ll_lengths[i] as u32;
                    }
                }
            }
            
            if let Some(ref tree_d_lengths) = tree_d.tree2d {
                for i in 0..numcodes_d {
                    let idx = numcodes_ll + i;
                    if idx < bitlen_lld_ref.len() && i < tree_d_lengths.len() {
                        bitlen_lld_ref[idx] = tree_d_lengths[i] as u32;
                    }
                }
            }
        }
        
        if let Some(ref bitlen_lld_ref) = bitlen_lld {
            let mut i = 0;
            while i < numcodes_lld {
                let mut j = 0;
                while i + j + 1 < numcodes_lld && bitlen_lld_ref[i + j + 1] == bitlen_lld_ref[i] {
                    j += 1;
                }
                
                if bitlen_lld_ref[i] == 0 && j >= 2 {
                    j += 1;
                    if j <= 10 {
                        if let Some(ref mut bitlen_lld_e_ref) = bitlen_lld_e {
                            if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                bitlen_lld_e_ref[numcodes_lld_e] = 17;
                                numcodes_lld_e += 1;
                                if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                    bitlen_lld_e_ref[numcodes_lld_e] = (j - 3) as u32;
                                    numcodes_lld_e += 1;
                                }
                            }
                        }
                        ...
                        if let Some(ref mut bitlen_lld_e_ref) = bitlen_lld_e {
                            if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                bitlen_lld_e_ref[numcodes_lld_e] = 18;
                                numcodes_lld_e += 1;
                                if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                    bitlen_lld_e_ref[numcodes_lld_e] = (j_val - 11) as u32;
                                    numcodes_lld_e += 1;
                                }
                            }
                        }
                    ...
                    if let Some(ref mut bitlen_lld_e_ref) = bitlen_lld_e {
                        if numcodes_lld_e < bitlen_lld_e_ref.len() {
                            bitlen_lld_e_ref[numcodes_lld_e] = bitlen_lld_ref[i];
                            numcodes_lld_e += 1;
                        }
                    }
                    ...
                    for _ in 0..num {
                        if let Some(ref mut bitlen_lld_e_ref) = bitlen_lld_e {
                            if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                bitlen_lld_e_ref[numcodes_lld_e] = 16;
                                numcodes_lld_e += 1;
                                if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                    bitlen_lld_e_ref[numcodes_lld_e] = (6 - 3) as u32;
                                    numcodes_lld_e += 1;
                                }
                            }
                        }
                    ...
                    if rest >= 3 {
                        if let Some(ref mut bitlen_lld_e_ref) = bitlen_lld_e {
                            if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                ...
                                numcodes_lld_e += 1;
                                if numcodes_lld_e < bitlen_lld_e_ref.len() {
                                    bitlen_lld_e_ref[numcodes_lld_e] = (rest - 3) as u32;
                                    numcodes_lld_e += 1;
                                }
                            }
                        }
        ...
        if let Some(ref bitlen_lld_e_ref) = bitlen_lld_e {
            let mut i = 0;
            while i < numcodes_lld_e {
                let val = bitlen_lld_e_ref[i] as usize;
                if let Some(ref mut freq_cl) = frequencies_cl {
                    if val < freq_cl.len() {
                        freq_cl[val] += 1;
                    }
        ...
        if let Some(ref freq_cl) = frequencies_cl {
            error = HuffmanTree_makeFromFrequencies(&mut tree_cl, freq_cl, 19, 19, 7);
            if error != 0 {
                break;
            }
        }
        
        let mut numcodes_cl = 19;
        while numcodes_cl > 4 {
            let order_idx = CLCL_ORDER[numcodes_cl - 1] as usize;
            if let Some(ref tree_cl_lengths) = tree_cl.tree2d {
                if order_idx < tree_cl_lengths.len() && tree_cl_lengths[order_idx] == 0 {
                    numcodes_cl -= 1;
                } else {
                    break;
                }
        ...
        for i in 0..numcodes_cl {
            let order_idx = CLCL_ORDER[i] as usize;
            if let Some(ref tree_cl_lengths) = tree_cl.tree2d {
                if order_idx < tree_cl_lengths.len() {
                    writeBits(writer, tree_cl_lengths[order_idx] as u32, 3);
                }
            }
        }
        ...
        if let Some(ref bitlen_lld_e_ref) = bitlen_lld_e {
            ...
            while i < numcodes_lld_e {
                let val = bitlen_lld_e_ref[i] as usize;
                if let Some(ref tree_cl_codes) = tree_cl.tree2d {
                    if val < tree_cl_codes.len() {
                        writeBitsReversed(writer, tree_cl_codes[val] as u32, tree_cl.tree2d_size);
                    }
                }
                ...
                if val == 16 {
                    i += 1;
                    if i < numcodes_lld_e {
                        writeBits(writer, bitlen_lld_e_ref[i], 2);
                    }
                } else if val == 17 {
                    i += 1;
                    if i < numcodes_lld_e {
                        writeBits(writer, bitlen_lld_e_ref[i], 3);
                    }
                } else if val == 18 {
                    i += 1;
                    if i < numcodes_lld_e {
                        writeBits(writer, bitlen_lld_e_ref[i], 7);
                    }
                }
                i += 1;
            }
        ...
        if let Some(ref tree_ll_lengths) = tree_ll.tree2d {
            if 256 < tree_ll_lengths.len() && tree_ll_lengths[256] == 0 {
                error = 64;
                break;
            }
        }
        
        if let Some(ref tree_ll_codes) = tree_ll.tree2d {
            if 256 < tree_ll_codes.len() {
                writeBitsReversed(writer, tree_ll_codes[256] as u32, tree_ll.tree2d_size);
            }
        }
    ...
    match strategy {
        ...
        | LodePNGFilterStrategy::LFS_FOUR => {
            ...
            for y in 0..h {
                ...
                filterScanline(
                    &mut out[outindex + 1..outindex + 1 + linebytes],
        ...
        LodePNGFilterStrategy::LFS_MINSUM => {
            let mut attempt: [Option<Box<[u8]>>; 5] = [None, None, None, None, None];
            let mut smallest = 0;
            let mut bestType = 0;

            for i in 0..5 {
                attempt[i] = Some(vec![0u8; linebytes].into_boxed_slice());
            }

            if error == 0 {
                for y in 0..h {
                    let y_linebytes = linebytes * y as usize;
                    let mut current_smallest = 0;
                    let mut current_bestType = 0;

                    for type_val in 0..5 {
                        let attempt_slice = attempt[type_val].as_mut().unwrap();
                        filterScanline(
                            attempt_slice,
                            &in_data[y_linebytes..y_linebytes + linebytes],
                            prevline,
                            linebytes,
                            bytewidth,
                            type_val as u8,
                        ...
                        let sum = if type_val == 0 {
                            attempt_slice.iter().map(|&b| b as usize).sum()
                        } else {
                            attempt_slice
                                .iter()
                        ...
                        if type_val == 0 || sum < current_smallest {
                            current_bestType = type_val;
                            current_smallest = sum;
                        }
                    }

                    bestType = current_bestType;
                    smallest = current_smallest;
                    prevline = Some(&in_data[y_linebytes..y_linebytes + linebytes]);

                    let out_index = (linebytes + 1) * y as usize;
                    out[out_index] = bestType as u8;

                    let best_attempt = attempt[bestType].as_ref().unwrap();
                    for x in 0..linebytes {
                        out[out_index + 1 + x] = best_attempt[x];
                    }
                }
            }
            ...
            if error == 0 {
                for y in 0..h {
                    ...
                    for type_val in 0..5 {
                        ...
                        if type_val == 0 || sum > current_bestSum {
                            current_bestType = type_val;
                            current_bestSum = sum;
                        }
                    }
            ...
            if error == 0 {
                for y in 0..h {
                    ...
                    for type_val in 0..5 {
                        ...
                        let mut dummy: Option<Box<[u8]>> = None;
                        let mut outsize = 0;
                        let error_code = zlib_compress(
                            &mut dummy,
                            &mut outsize,
                            Some(attempt_slice),
                            testsize,
                        ...
                        size[type_val] = outsize;
                        dummy = None;

                        if type_val == 0 || size[type_val] < current_smallest {
                            current_bestType = type_val;
                            current_smallest = size[type_val];
                        }
                    }
...
pub fn preProcessScanlines(
    ...
    if info_png.interlace_method == 0 {
        ...
        *out = if *outsize > 0 {
            Some(vec![0; *outsize].into_boxed_slice())
        } else {
            None
        };
        ...
        if error == 0 {
            if bpp < 8 && (w * bpp) != ((((w * bpp) + 7) / 8) * 8) {
                ...
                let mut padded = if padded_size > 0 {
                    Some(vec![0; padded_size].into_boxed_slice())
                } else {
                    None
                };
                
                if padded.is_none() && padded_size > 0 {
                    error = 83;
                }
                
                if error == 0 && padded_size > 0 {
                    addPaddingBits(
                ...
                lodepng_free(padded.map(|v| Box::new(v) as Box<dyn std::any::Any>));
            } else {
        ...
        *out = if *outsize > 0 {
        ...
        let mut adam7 = if passstart[7] > 0 {
            Some(vec![0u8; passstart[7]].into_boxed_slice())
        } else {
            None
        };
        ...
        if error == 0 {
            if let Some(ref mut adam7_data) = adam7 {
                Adam7_interlace(adam7_data, in_data, w, h, bpp);
            }
            ...
            for i in 0..7 {
                if bpp < 8 {
                    ...
                    let mut padded = if padded_size > 0 {
                        Some(vec![0u8; padded_size].into_boxed_slice())
                    } else {
                        None
                    };
        ...
        lodepng_free(adam7.map(|v| Box::new(v) as Box<dyn std::any::Any>));
    }
    ...
    match mode_in.colortype {
        ...
        LodePNGColorType::LCT_PALETTE => {
            ...
            if let Some(palette) = &mode_in.palette {
                let index = (r_in * 4) as usize;
                r = palette.get(index).copied().unwrap_or(0) as u32 * 257u32;
                g = palette.get(index + 1).copied().unwrap_or(0) as u32 * 257u32;
                b = palette.get(index + 2).copied().unwrap_or(0) as u32 * 257u32;
            } else {
                return 31;
            }
        }
        ...
        LodePNGColorType::LCT_PALETTE => {
            ...
            if let Some(palette) = &mode_out.palette {
                for i in 0..mode_out.palettesize {
                    ...
                    if ((r >> 8) as u8 == palette[j]) 
                        && ((g >> 8) as u8 == palette[j + 1]) 
                        && ((b >> 8) as u8 == palette[j + 2]) 
                    {
                        *r_out = Some(i as u32);
                        return 0;
                    }
...
pub fn addUnknownChunks(out: &mut Ucvector, data: &[u8]) -> u32 {
    ...
    while inchunk_idx < data.len() {
        {
            // Convert Option<Vec<u8>> to Option<Box<[u8]>>
            let mut boxed_data = out.data.take().map(|v| v.into_boxed_slice());
            let error = lodepng_chunk_append(&mut boxed_data, &mut out.size, inchunk);
            // Convert back to Option<Vec<u8>>
        ...
        let next_chunk = lodepng_chunk_next(inchunk, &data[data.len()..]);
        inchunk_idx = next_chunk.as_ptr() as usize - data.as_ptr() as usize;
        inchunk = next_chunk;
    }
...
pub fn addChunk_IHDR(
    ...
    let chunk_slice = chunk.as_mut().unwrap();
    let data = &mut chunk_slice[8..];
    
    {
        let mut w_buffer = [0u8; 4];
        lodepng_set32bitInt(&mut w_buffer, w);
        data[0..4].copy_from_slice(&w_buffer);
    }
    
    {
        let mut h_buffer = [0u8; 4];
        lodepng_set32bitInt(&mut h_buffer, h);
        data[4..8].copy_from_slice(&h_buffer);
    }
    ...
    *outsize = 0;
    ...
    if state.encoder.auto_convert != 0 {
        ...
        if info_png.iccp_defined != 0
            && isGrayICCProfile(info_png.iccp_profile.as_deref(), info_png.iccp_profile_size as usize)
        {
            stats.allow_palette = 0;
        }

        if info_png.iccp_defined != 0
            && isRGBICCProfile(info_png.iccp_profile.as_deref(), info_png.iccp_profile_size as usize)
        {
        ...
        if info_png.sbit_defined != 0 {
            let sbit_max = if info_png.sbit_r > info_png.sbit_g {
                if info_png.sbit_r > info_png.sbit_b {
                    if info_png.sbit_r > info_png.sbit_a {
                        info_png.sbit_r
                    } else {
                        info_png.sbit_a
                    }
                } else {
                    if info_png.sbit_b > info_png.sbit_a {
                        info_png.sbit_b
                    } else {
                        info_png.sbit_a
                    }
                ...
                if info_png.sbit_g > info_png.sbit_b {
                    if info_png.sbit_g > info_png.sbit_a {
                        info_png.sbit_g
                    } else {
                        info_png.sbit_a
                    }
    ...
    if !lodepng_color_mode_equal(&state.info_raw, &info.color) {
        ...
        let mut converted: Vec<u8> = vec![0; size];

        if size > 0 && converted.is_empty() {
            state.error = 83;
            return state.error;
        }
    ...
    if let Some(unknown_data) = &info.unknown_chunks_data[0] {
        state.error = addUnknownChunks(&mut outv, unknown_data);
        if state.error != 0 {
            return state.error;
        }
    }
    ...
    for i in 0..info.text_num {
        if let (Some(keys), Some(strings)) = (&info.text_keys, &info.text_strings) {
            if lodepng_strlen(Some(&keys[i])) > 79 {
                state.error = 66;
                return state.error;
            }
            if lodepng_strlen(Some(&keys[i])) < 1 {
                state.error = 67;
                return state.error;
            }
            ...
            if state.encoder.text_compression != 0 {
                ...
                state.error = addChunk_tEXt(&mut outv, Some(&keys[i]), Some(&strings[i]));
            }
    ...
    for i in 0..info.itext_num {
        if let (Some(keys), Some(langtags), Some(transkeys), Some(strings)) = (
            &info.itext_keys,
            &info.itext_langtags,
...
pub fn lodepng_encode24(
    ...
    lodepng_encode_memory(out, outsize, image, w, h, crate::LodePNGColorType::LCT_RGB, 8)
}
pub fn lodepng_encode_file<'a>(
    filename: &str,
    image: impl Into<Option<&'a [u8]>>,
    w: u32,
    h: u32,
    colortype: LodePNGColorType,
    bitdepth: u32,
) -> u32 {
    let image_option = image.into();
    let mut buffer: Option<Vec<u8>> = None;
    let mut buffersize: usize = 0;
    let error = lodepng_encode_memory(&mut buffer, &mut buffersize, image_option, w, h, colortype, bitdepth);
    if error == 0 {
        if let Some(ref buffer_data) = buffer {
            if let Err(save_error) = lodepng_save_file(buffer_data, filename) {
                return save_error;
            }
        }
    }
    if let Some(buffer_vec) = buffer {
        lodepng_free(Some(Box::new(buffer_vec) as Box<dyn std::any::Any>));
    } else {
        lodepng_free(None);
    }
    error
}
pub fn lodepng_encode32_file(filename: &str, image: Option<&[u8]>, w: u32, h: u32) -> u32 {
    lodepng_encode_file(filename, image, w, h, LodePNGColorType::LCT_RGBA, 8)
}
pub fn lodepng_encode24_file(filename: &str, image: Option<&[u8]>, w: u32, h: u32) -> u32 {
...
pub fn lodepng_inspect_chunk(
    ...
    if lodepng_chunk_type_equals(Some(chunk), Some("PLTE")) != 0 {
...
pub fn lodepng_chunk_find_const<'a>(chunk: Option<&'a [u8]>, end: Option<&'a [u8]>, type_: Option<&str>) -> Option<&'a [u8]> {
    let mut chunk = chunk?;
    let end = end?;
    
    loop {
        // Check if chunk is beyond end or doesn't have enough bytes (12 bytes needed)
        if chunk.as_ptr() >= end.as_ptr() || 
           (end.as_ptr() as usize).checked_sub(chunk.as_ptr() as usize).map(|diff| diff < 12).unwrap_or(true) {
            return None;
        }
...
pub fn lodepng_chunk_find<'a>(chunk: &'a [u8], end: &'a [u8], type_: &str) -> Option<&'a [u8]> {
    ...
    loop {
        // Check if current_chunk is beyond end or doesn't have enough bytes (12 bytes minimum for chunk)
        if current_chunk.as_ptr() >= end.as_ptr() || (end.as_ptr() as usize) - (current_chunk.as_ptr() as usize) < 12 {
            return Option::None;
        }
        ...
        // Move to next chunk
        current_chunk = lodepng_chunk_next(current_chunk, end);
    }
}
