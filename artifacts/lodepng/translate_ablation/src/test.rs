use crate::*;
use crate::LodePNGColorType::LCT_PALETTE;
use crate::LodePNGColorType::LCT_RGBA;
...
pub fn addColor(colors: &mut Vec<u8>, colors_size: &mut usize, colors_capacity: &mut usize, r: u8, g: u8, b: u8, a: u8) {
    ...
    colors[*colors_size] = r;
    *colors_size += 1;
    colors[*colors_size] = g;
    *colors_size += 1;
    colors[*colors_size] = b;
    *colors_size += 1;
    colors[*colors_size] = a;
    *colors_size += 1;
}
...
pub fn addColor16(
    ...
    if (*colors_size + 8) > *colors_capacity {
        *colors_capacity = if *colors_capacity == 0 {
            16
        } else {
            *colors_capacity * 2
        };
        
        if let Some(ref mut vec) = colors {
            vec.resize(*colors_capacity, 0);
        } else {
            *colors = Some(vec![0; *colors_capacity]);
        }
    }

    if let Some(ref mut vec) = colors {
        vec[*colors_size] = (r & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = ((r >> 8) & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = (g & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = ((g >> 8) & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = (b & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = ((b >> 8) & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = (a & 255) as u8;
        *colors_size += 1;
        vec[*colors_size] = ((a >> 8) & 255) as u8;
        *colors_size += 1;
    }
}
...
pub fn colorConvertTest(
    ...
    println!("color convert test {} - {}", bits_in.unwrap_or(""), bits_out.unwrap_or(""));
    
    let expected_bytes = bitStringToBytes(bits_out).expect("Failed to convert expected bits to bytes");
    let image_bytes = bitStringToBytes(bits_in).expect("Failed to convert input bits to bytes");
    
    let expected_size = expected_bytes.len();
    let image_size = image_bytes.len();
    
    let mut actual = vec![0u8; expected_size];
    ...
    for (i, (expected_byte, actual_byte)) in expected_bytes.iter().zip(actual.iter()).enumerate() {
        if expected_byte != actual_byte {
            println!("Byte {}: expected {}, got {}", i, expected_byte, actual_byte);
            fail();
        }
    }
...
pub fn strtovector(numbers: Option<&str>) -> UnsignedVector {
    ...
    // Return early if input is None (NULL)
    let numbers = match numbers {
        Some(n) => n.as_bytes(),
        None => return result,
    };

    // First pass: count the number of unsigned integers
    let mut p = numbers;
    while !p.is_empty() {
        // Skip whitespace
        while !p.is_empty() && (p[0] == b' ' || p[0] == b'\t' || p[0] == b'\n') {
            p = &p[1..];
        }

        if !p.is_empty() && p[0] != b' ' && p[0] != b'\t' && p[0] != b'\n' {
            result.size += 1;
    ...
    if result.size > 0 {
        ...
        while !p.is_empty() && index < result.size {
            // Skip whitespace
            while !p.is_empty() && (p[0] == b' ' || p[0] == b'\t' || p[0] == b'\n') {
                p = &p[1..];
            }
            ...
            if !p.is_empty() {
                // Convert C string slice to Rust string for parsing
                ...
                match s.split_whitespace().next() {
                    Some(num_str) => {
                        ...
                        // Advance p by the length of the parsed number
                        p = &p[num_str.len()..];
                    }
                    None => break,
                }
...
pub fn testRGBToPaletteConvert() {
    ...
    // Allocate memory for 256 RGBA entries (256 * 4 bytes)
    let layout = Layout::array::<u8>(256 * 4).unwrap();
    let mut palette = unsafe { alloc::alloc(layout) as *mut u8 };
    ...
    for i in 0..256 {
        unsafe {
            *palette.add(i * 4 + 0) = i as u8;
            *palette.add(i * 4 + 1) = 5;
            *palette.add(i * 4 + 2) = 6;
            *palette.add(i * 4 + 3) = 128;
        }
    }
    
    // Create a slice from the raw pointer for the test function
    let palette_slice = unsafe { std::slice::from_raw_parts(palette, 256 * 4) };
    doRGBAToPaletteTest(palette_slice, 256, LCT_PALETTE);
    
    // Reallocate to 257 * 4 bytes
    let new_layout = Layout::array::<u8>(257 * 4).unwrap();
    palette = unsafe { alloc::realloc(palette as *mut u8, layout, new_layout.size()) as *mut u8 };
    ...
    // Set the new entry
    unsafe {
        *palette.add(256 * 4 + 0) = 5;
        *palette.add(256 * 4 + 1) = 6;
        *palette.add(256 * 4 + 2) = 7;
        *palette.add(256 * 4 + 3) = 8;
    }
    
    // Create a slice for the new test
    let palette_slice = unsafe { std::slice::from_raw_parts(palette, 257 * 4) };
    doRGBAToPaletteTest(palette_slice, 257, LCT_RGBA);
    
    // Free the allocated memory
    unsafe {
        alloc::dealloc(palette as *mut u8, new_layout);
    }
}
...
pub fn ColorArray_addColor(arr: &mut ColorArray, r: u8, g: u8, b: u8, a: u8) {
    if (arr.size + 4) > arr.capacity {
        ...
        // Handle the reallocation using Vec's methods
        match &mut arr.data {
            Some(vec) => {
                vec.reserve_exact(arr.capacity - vec.capacity());
            }
            None => {
                let mut new_vec = Vec::with_capacity(arr.capacity);
                new_vec.reserve_exact(arr.capacity);
                arr.data = Some(new_vec);
            }
        }
    }
    
    // Push the color components to the vector
    if let Some(vec) = &mut arr.data {
        vec.push(r);
        vec.push(g);
        vec.push(b);
        vec.push(a);
...
pub fn ColorArray_addColor16(arr: &mut ColorArray, r: u16, g: u16, b: u16, a: u16) {
    if (arr.size + 8) > arr.capacity {
        ...
        if let Some(ref mut data_vec) = arr.data {
            data_vec.resize(arr.capacity, 0);
        } else {
...
pub fn testInspectChunk() {
    ...
    let png_slice = png.as_ref().map(|v| v.as_slice());
    let mut state = LodePngState {
        decoder: LodePngDecoderSettings {
            zlibsettings: LodePngDecompressSettings {
                ignore_adler32: 0,
    ...
    lodepng_inspect(None, None, &mut state, png_slice, png_size);
    let mut chunk = lodepng_chunk_find_const(png_slice, Some(&png_slice.unwrap()[png_size..]), Some("tIME"));
    {
        if chunk.is_none() {
            println!("line {}: {} ASSERT_NOT_EQUALS failed: Expected not {} but got {}.", "1693", "(uintptr_t)chunk", 0, 0);
            fail();
        }
    }
    {
        if state.info_png.time_defined != 0 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1694", "info->time_defined", 0, state.info_png.time_defined);
            fail();
        }
    }
    let chunk_start = chunk.unwrap().as_ptr() as usize - png_slice.unwrap().as_ptr() as usize;
    lodepng_inspect_chunk(&mut state, chunk_start, png_slice.unwrap(), png_size);
    {
        if state.info_png.time_defined != 1 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1696", "info->time_defined", 1, state.info_png.time_defined);
            fail();
        }
    }
    {
        if state.info_png.time.year != 2012 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1697", "state.info_png.time.year", 2012, state.info_png.time.year);
            fail();
        }
    }
    {
        if state.info_png.time.month != 1 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1698", "info->time.month", 1, state.info_png.time.month);
            fail();
        }
    }
    {
        if state.info_png.time.day != 2 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1699", "info->time.day", 2, state.info_png.time.day);
            fail();
        }
    }
    {
        if state.info_png.time.hour != 3 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1700", "info->time.hour", 3, state.info_png.time.hour);
            fail();
        }
    }
    {
        if state.info_png.time.minute != 4 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1701", "info->time.minute", 4, state.info_png.time.minute);
            fail();
        }
    }
    {
        if state.info_png.time.second != 5 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1702", "info->time.second", 5, state.info_png.time.second);
            fail();
        }
    }
    {
        if state.info_png.text_num != 0 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1704", "info->text_num", 0, state.info_png.text_num);
            fail();
        }
    }
    chunk = lodepng_chunk_find_const(png_slice, Some(&png_slice.unwrap()[png_size..]), Some("zTXt"));
    let chunk_start = chunk.unwrap().as_ptr() as usize - png_slice.unwrap().as_ptr() as usize;
    lodepng_inspect_chunk(&mut state, chunk_start, png_slice.unwrap(), png_size);
    {
        if state.info_png.text_num != 1 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1707", "info->text_num", 1, state.info_png.text_num);
            fail();
        }
    }
    chunk = lodepng_chunk_find_const(chunk, Some(&png_slice.unwrap()[png_size..]), Some("zTXt"));
    let chunk_start = chunk.unwrap().as_ptr() as usize - png_slice.unwrap().as_ptr() as usize;
    lodepng_inspect_chunk(&mut state, chunk_start, png_slice.unwrap(), png_size);
    {
        if state.info_png.text_num != 2 {
            println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "1710", "info->text_num", 2, state.info_png.text_num);
            fail();
        }
    }
    lodepng_state_cleanup(&mut state);
...
pub fn testFuzzing() {
    ...
    let mut broken = vec![0; png_size];
    broken.copy_from_slice(png_data);
    ...
    for i in 0..png_size {
        result = None;
        broken[i] = !png_data[i];
        let error = lodepng_decode(&mut result, &mut w, &mut h, &mut state, &broken, png_size);
        if error < 1000 {
    ...
    for size in (1..=png_size).rev() {
        result = None;
        let error = lodepng_decode(&mut result, &mut w, &mut h, &mut state, &broken, size);
        if error < 1000 {
            error_counts[error as usize] += 1;
        }
    }
...
pub fn testEncoderErrors() {
    ...
    let mut png: Option<Vec<u8>> = Option::None;
    let mut png_size: usize = 0;
    let w: u32 = 32;
    let h: u32 = 32;
    let mut image = Image {
...
pub fn extractChunkNames(png: &[u8], png_size: usize) -> Option<String> {
    ...
    loop {
        lodepng_chunk_type(&mut name, chunk);
        
        // Convert name to a string slice, removing NUL characters
        let name_str = CStr::from_bytes_until_nul(&name)
            .ok()
        ...
        assertTrue(chunk.as_ptr() < end.as_ptr(), Some("jumped out of chunks"));
    }
...
pub fn test16bitColorEndianness() {
    ...
    let mut base64 = "iVBORw0KGgoAAAANSUhEUgAAACAAAAAgEAAAAAAGgflrAAAABGdBTUEAAYagMeiWXwAAAF5JREFUeJzV0jEKwDAMQ1E5Ww9/xtygk8AoezLVKgSj2Y8/OICnuFcTE2OgOoJgHQiZAN2C9kDKBOgW3AZCJkC3oD2QMgG6BbeBkAnQLWgPpExgP28H7E/0GTjPfwAW2EvYX64rn9cAAAAASUVORK5CYII=";
    let mut png: Option<Vec<u8>> = None;
    let mut png_size: usize = 0;
    fromBase64_string(&mut png, &mut png_size, base64.as_bytes(), base64.len());
    let mut w: u32 = 0;
    ...
    assertNoPNGError(lodepng_decode(&mut image, &mut w, &mut h, &mut state, png.as_ref().unwrap(), png_size), Some("line 2311"));
    {
        if let Some(ref img) = image {
            if 0x09 != img[8] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2312", "image[8]", 0x09, img[8]);
                fail();
            }
        }
    }
    {
        if let Some(ref img) = image {
            if 0x00 != img[9] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2313", "image[9]", 0x00, img[9]);
                fail();
            }
        }
    }
    ...
    assertNoPNGError(lodepng_decode(&mut image, &mut w, &mut h, &mut state, png.as_ref().unwrap(), png_size), Some("line 2321"));
    {
        if let Some(ref img) = image {
            if 0x09 != img[2] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2322", "image[2]", 0x09, img[2]);
                fail();
            }
        }
    }
    {
        if let Some(ref img) = image {
            if 0x00 != img[3] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2323", "image[3]", 0x00, img[3]);
                fail();
            }
        }
    }
    ...
    lodepng_state_cleanup(&mut state);
    png = None;
    base64 = "";
    fromBase64_string(&mut png, &mut png_size, base64.as_bytes(), base64.len());
    lodepng_state_init(&mut state);
    state.info_raw.bitdepth = 16;
    assertNoPNGError(lodepng_decode(&mut image, &mut w, &mut h, &mut state, png.as_ref().unwrap(), png_size), Some("line 2338"));
    {
        if let Some(ref img) = image {
            if 0x1f != img[258] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2339", "image[258]", 0x1f, img[258]);
                fail();
            }
        }
    }
    {
        if let Some(ref img) = image {
            if 0xf9 != img[259] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2340", "image[259]", 0xf9, img[259]);
                fail();
            }
        }
    }
    ...
    lodepng_state_init(&mut state);
    state.decoder.color_convert = 0;
    assertNoPNGError(lodepng_decode(&mut image, &mut w, &mut h, &mut state, png.as_ref().unwrap(), png_size), Some("line 2348"));
    {
        if let Some(ref img) = image {
            if 0x1f != img[194] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2349", "image[194]", 0x1f, img[194]);
                fail();
            }
        }
    }
    {
        if let Some(ref img) = image {
            if 0xf9 != img[195] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2350", "image[195]", 0xf9, img[195]);
                fail();
            }
        }
    }
    ...
    lodepng_state_cleanup(&mut state);
    png = None;
    base64 = "iVBORw0KGgoAAAANSUhEUgAAAAcAAAAHAgMAAAC5PL9AAAAABGdBTUEAAYagMeiWXwAAAANzQklUBAQEd/i1owAAAAxQTFRF/wB3AP93//8AAAD/G0OznAAAABpJREFUeJxj+P+H4WoMw605DDfmgEgg+/8fAHF5CrkeXW0HAAAAAElFTkSuQmCC";
    fromBase64_string(&mut png, &mut png_size, base64.as_bytes(), base64.len());
    lodepng_state_init(&mut state);
    state.info_raw.bitdepth = 16;
    assertNoPNGError(lodepng_decode(&mut image, &mut w, &mut h, &mut state, png.as_ref().unwrap(), png_size), Some("line 2364"));
    {
        if let Some(ref img) = image {
            if 0x77 != img[84] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2365", "image[84]", 0x77, img[84]);
                fail();
            }
        }
    }
    {
        if let Some(ref img) = image {
            if 0x77 != img[85] {
                println!("line {}: {} ASSERT_EQUALS failed: Expected {} but got {}.", "2366", "image[85]", 0x77, img[85]);
                fail();
            }
        }
    }
