...
pub fn lodepng_chunk_ancillary(chunk: Option<&[u8]>) -> bool {
    ...
    // and ensure it has at least 5 bytes (since we access chunk[4])
    match chunk {
...
pub fn lodepng_chunk_private(chunk: Option<&[u8]>) -> bool {
    // Check if chunk is None (equivalent to NULL check in C)
    // Also ensure the slice has at least 7 bytes (since we access index 6)
    match chunk {
...
pub unsafe fn lodepng_realloc(ptr: *mut u8, new_size: usize) -> *mut u8 {
    if new_size == 0 {
        if !ptr.is_null() {
    ...
    if ptr.is_null() {
        ...
        let old_layout = Layout::from_size_align(1, 1).unwrap(); // Using minimum size for old layout
...
pub fn lodepng_free(ptr: Option<*mut std::ffi::c_void>) {
    ...
    // However, if we're interfacing with C code that expects this function,
    // we might need to use unsafe to actually free the memory.
    if let Some(p) = ptr {
        unsafe {
            // Use the C library's free function directly
            free(p);
        }
    }
}

// Declare the external free function from the C library
extern "C" {
    fn free(ptr: *mut std::ffi::c_void);
}
pub fn lodepng_memcpy(dst: &mut [u8], src: &[u8]) {
    // Rust slices already include bounds checking, so we don't need to manually check size
    // The length must match between dst and src, otherwise this will panic
    dst.copy_from_slice(src);
}
pub fn lodepng_memset(dst: &mut [u8], value: u8, num: usize) {
    for i in 0..num {
        dst[i] = value;
    }
}
...
pub fn lodepng_filesize(filename: &str) -> Option<i64> {
    ...
    // Check for overflow (9223372036854775807L is i64::MAX)
    if size == i64::MAX {
        return None; // Equivalent to returning -1 in C
    }
...
pub fn readBitFromReversedStream(bitpointer: &mut usize, bitstream: &[u8]) -> u8 {
    let byte_index = *bitpointer >> 3;
    let bit_index = 7 - (*bitpointer & 0x7);
    let result = (bitstream[byte_index] >> bit_index) & 1;
    *bitpointer += 1;
    result
}
pub fn setBitOfReversedStream(bitpointer: &mut usize, bitstream: &mut [u8], bit: u8) {
    if bit == 0 {
        let byte_index = *bitpointer >> 3;
        let bit_pos = 7 - (*bitpointer & 7);
        bitstream[byte_index] &= !(1 << bit_pos);
    } else {
        let byte_index = *bitpointer >> 3;
        let bit_pos = 7 - (*bitpointer & 7);
        bitstream[byte_index] |= 1 << bit_pos;
...
pub fn lodepng_get_raw_size_idat(w: u32, h: u32, bpp: u32) -> usize {
    let line = ((((w / 8) * bpp) + 1) as usize) + ((((w & 7) * bpp) + 7) / 8) as usize;
    (h as usize) * line
}
pub fn addColorBits(out: &mut [u8], index: usize, bits: u32, input: u32) {
    let m = match bits {
        1 => 7,
        2 => 3,
        _ => 1,
    };
    let p = (index as u32) & m;
    let mut value = input & ((1u32 << bits) - 1);
    value = value << (bits * (m - p));
    
    let byte_index = (index * bits as usize) / 8;
    if p == 0 {
        out[byte_index] = value as u8;
    } else {
        out[byte_index] |= value as u8;
    }
}
...
pub fn isGrayICCProfile(profile: Option<&[u8]>, size: usize) -> bool {
    ...
    // Check size condition first (same as C code)
    if size < 20 {
        return false;
    }

    // Check the specific bytes (16-19) for "GRAY" pattern
    // Rust uses exclusive ranges, so we check up to 20 (19+1)
    if profile.len() < 20 {
        return false;
    }

    (profile[16] == b'G') && (profile[17] == b'R') && (profile[18] == b'A') && (profile[19] == b'Y')
}
pub fn isRGBICCProfile(profile: Option<&[u8]>, size: usize) -> bool {
    ...
    // Check size first (same as C version)
    if size < 20 {
        return false;
    }

    // Check the RGB signature at positions 16-19
...
pub fn lodepng_palette_clear(info: &mut LodePNGColorMode) {
    // In Rust, the Option<Vec<u8>> will handle memory deallocation automatically when set to None
    info.palette = None;
    info.palettesize = 0;
}
...
pub fn lodepng_clear_icc(info: &mut LodePngInfo) {
    // Clear the iccp_name by setting it to None (equivalent to string_cleanup in C)
    info.iccp_name = None;
    
    // Clear the iccp_profile by setting it to None (equivalent to lodepng_free in C)
    info.iccp_profile = None;
    
    // Reset the remaining ICC-related fields
...
pub fn LodePNGText_cleanup(info: &mut LodePngInfo) {
    if let Some(text_keys) = &mut info.text_keys {
        for key in text_keys.iter_mut() {
            *key = String::new(); // Equivalent to string_cleanup for Rust Strings
        }
    }
    if let Some(text_strings) = &mut info.text_strings {
        for string in text_strings.iter_mut() {
            *string = String::new(); // Equivalent to string_cleanup for Rust Strings
        }
    }

    info.text_keys = None; // Equivalent to lodepng_free
    info.text_strings = None; // Equivalent to lodepng_free
}
pub fn LodePNGIText_cleanup(info: &mut LodePngInfo) {
    if let Some(itext_keys) = &mut info.itext_keys {
        for i in 0..info.itext_num {
            if let Some(key) = itext_keys.get_mut(i) {
                *key = String::new();
            }
        }
    }
    ...
    if let Some(itext_langtags) = &mut info.itext_langtags {
        for i in 0..info.itext_num {
            if let Some(langtag) = itext_langtags.get_mut(i) {
                *langtag = String::new();
            }
        }
    }
    ...
    if let Some(itext_transkeys) = &mut info.itext_transkeys {
        for i in 0..info.itext_num {
            if let Some(transkey) = itext_transkeys.get_mut(i) {
                *transkey = String::new();
            }
        }
    }
    ...
    if let Some(itext_strings) = &mut info.itext_strings {
        for i in 0..info.itext_num {
            if let Some(string) = itext_strings.get_mut(i) {
                *string = String::new();
            }
        }
    }
...
pub fn rgba16ToPixel(out: &mut [u8], i: usize, mode: &LodePNGColorMode, r: u16, g: u16, b: u16, a: u16) {
    match mode.colortype {
        ...
        _ => {} // Other color types not handled in original C code
    }
}
    ...
    match mode.colortype {
            ...
            match mode.bitdepth {
                ...
                16 => {
                    for i in 0..numpixels {
                        ...
                        let key_val = ((in_data[i * 2] as u32) << 8) | (in_data[i * 2 + 1] as u32);
                        buffer[buffer_idx + 3] = if mode.key_defined != 0 && key_val == mode.key_r {
                            0
                        } else {
                            255
                        };
                ...
                _ => {
                    ...
                    for _ in 0..numpixels {
                        ...
                        let scaled = ((value * 255) / highest) as u8;
                        buffer[buffer_idx] = scaled;
                        buffer[buffer_idx + 1] = scaled;
                        buffer[buffer_idx + 2] = scaled;
        ...
        LodePNGColorType::LCT_RGB => {
            match mode.bitdepth {
                8 => {
                    for i in 0..numpixels {
                        buffer[buffer_idx..buffer_idx + 3].copy_from_slice(&in_data[i * 3..i * 3 + 3]);
                        buffer[buffer_idx + 3] = 255;
                        buffer_idx += num_channels;
                    }
                ...
                _ => {
                    for i in 0..numpixels {
                        ...
                        let key_r_val = ((in_data[i * 6] as u32) << 8) | (in_data[i * 6 + 1] as u32);
                        let key_g_val = ((in_data[i * 6 + 2] as u32) << 8) | (in_data[i * 6 + 3] as u32);
                        let key_b_val = ((in_data[i * 6 + 4] as u32) << 8) | (in_data[i * 6 + 5] as u32);
                        
                        buffer[buffer_idx + 3] = if mode.key_defined != 0
                            && key_r_val == mode.key_r
                            && key_g_val == mode.key_g
                            && key_b_val == mode.key_b
                        {
                            0
                        } else {
                            255
                ...
                match mode.bitdepth {
                    8 => {
                        for i in 0..numpixels {
                            ...
                            let palette_idx = index * 4;
                            buffer[buffer_idx..buffer_idx + 4].copy_from_slice(&palette[palette_idx..palette_idx + 4]);
                            buffer_idx += num_channels;
                        }
                    }
                    _ => {
        ...
        LodePNGColorType::LCT_RGBA => {
            match mode.bitdepth {
                8 => {
                    buffer[..numpixels * 4].copy_from_slice(&in_data[..numpixels * 4]);
                }
                _ => {
                    for i in 0..numpixels {
...
pub fn getPixelColorsRGB8(buffer: &mut [u8], numpixels: usize, in_data: &[u8], mode: &LodePNGColorMode) {
    ...
    match mode.colortype {
        ...
        LodePNGColorType::LCT_RGB => {
            match mode.bitdepth {
                8 => {
                    lodepng_memcpy(buffer, &in_data[..numpixels * 3]);
                }
                _ => {
                    for i in 0..numpixels {
                        buffer[buffer_idx] = in_data[(i * 6)];
    ...
    match mode.colortype {
        LodePNGColorType::LCT_GREY => {
            if mode.bitdepth == 8 {
                ...
                *g = val;
                *b = val;
                *a = if mode.key_defined != 0 && (*r as u32 == mode.key_r) {
                    0
                } else {
                    255
                };
                ...
                *g = val;
                *b = val;
                let key = (256 * in_data[i * 2] as u32) + in_data[i * 2 + 1] as u32;
        ...
        LodePNGColorType::LCT_RGB => {
            if mode.bitdepth == 8 {
                ...
                *g = in_data[i * 6 + 2];
                *b = in_data[i * 6 + 4];
                let key_r = (256 * in_data[i * 6] as u32) + in_data[i * 6 + 1] as u32;
                let key_g = (256 * in_data[i * 6 + 2] as u32) + in_data[i * 6 + 3] as u32;
                let key_b = (256 * in_data[i * 6 + 4] as u32) + in_data[i * 6 + 5] as u32;
                *a = if mode.key_defined != 0
                    && (key_r == mode.key_r)
                    && (key_g == mode.key_g)
                    && (key_b == mode.key_b)
...
pub fn color_tree_cleanup(tree: &mut ColorTree) {
    for i in 0..16 {
        if let Some(mut child) = tree.children[i].take() {
            ...
            // No need for lodepng_free as Rust's Box manages memory automatically
        }
    }
}
pub fn color_tree_init(tree: &mut ColorTree) {
    // Initialize all children to None (equivalent to memset to 0)
...
pub fn color_tree_add(
    ...
    for bit in 0..8 {
        ...
        if tree_idx.children[i as usize].is_none() {
            tree_idx.children[i as usize] = Some(Box::new(ColorTree {
                children: Default::default(),
                index: -1,
            }));
        }
        
        // Safe unwrap since we just created the child if it was None
...
pub fn lodepng_convert(
    ...
    if mode_out.colortype == LodePNGColorType::LCT_PALETTE {
        ...
        if palettesize == 0 {
            ...
            palette_idx = mode_in.key_defined;
        ...
        for i in 0..palsize {
            let p = &palette[(i * 4 + palette_idx as usize)..];
            error = color_tree_add(&mut tree, p[0], p[1], p[2], p[3], i as u32).err().unwrap_or(0);
            if error != 0 {
                break;
            }
        }
...
pub fn lodepng_color_mode_copy(dest: &mut LodePNGColorMode, source: &LodePNGColorMode) -> u32 {
    // Clean up the destination color mode
    ...
    if let Some(source_palette) = &source.palette {
        if source.palettesize > 0 {
            ...
            if new_palette.capacity() < source.palettesize * 4 {
                return 83; // Allocation failed
            }
...
pub fn alloc_string_sized(in_: Option<&[u8]>, insize: usize) -> Option<Vec<u8>> {
    ...
    // Allocate memory for output buffer (+1 for null terminator)
    let mut out = Vec::with_capacity(insize + 1);
    unsafe {
        out.set_len(insize + 1); // Set length to include null terminator
    }

    // Copy input bytes to output buffer
    lodepng_memcpy(&mut out[..insize], in_);
    out[insize] = 0; // Null-terminate the string
...
pub fn lodepng_add_text_sized(info: &mut LodePngInfo, key: Option<&str>, str: Option<&[u8]>, size: usize) -> u32 {
    ...
    let new_keys = match &info.text_keys {
        Some(keys) => {
            ...
            new_vec.push(String::new()); // Add empty string for new entry
            Some(new_vec)
        },
        None => Some(vec![String::new()]), // Initialize new vector if None
    };
    ...
    if let Some(keys) = &mut info.text_keys {
        if let Some(key_bytes) = key_bytes {
            keys[info.text_num - 1] = String::from_utf8_lossy(&key_bytes).into_owned();
        }
    }
    if let Some(strings) = &mut info.text_strings {
        if let Some(str_bytes) = str_bytes {
            strings[info.text_num - 1] = String::from_utf8_lossy(&str_bytes).into_owned();
        }
    }
...
pub fn ucvector_reserve(p: &mut Ucvector, size: usize) -> bool {
    if size > p.allocsize {
        ...
        if let Some(ref old_data) = p.data {
...
pub fn inflate_no_compression(
    ...
    let data = reader.data.as_ref().unwrap(); // Safe since we checked size
    ...
    if LEN != 0 {
        let out_data = out.data.as_mut().unwrap(); // Safe after resize
...
pub fn HuffmanTree_makeTable(tree: &mut HuffmanTree) -> u32 {
    ...
    // Allocate maxlens using lodepng_malloc
    let maxlens_ptr = unsafe { lodepng_malloc((HEADSIZE as usize) * std::mem::size_of::<u32>()) };
    let maxlens_ptr = match maxlens_ptr {
        Some(ptr) if !ptr.is_null() => ptr as *mut u32,
        _ => return 83,
    };
    let mut maxlens = unsafe { std::slice::from_raw_parts_mut(maxlens_ptr, HEADSIZE as usize) };

    // Initialize maxlens to 0
    for item in maxlens.iter_mut() {
        *item = 0;
    }

    // First pass: populate maxlens
    if let (Some(codes), Some(lengths)) = (&tree.codes, &tree.lengths) {
    ...
    tree.table_len = Some(vec![16; size as usize]);
    tree.table_value = Some(vec![0; size as usize]);

    if tree.table_len.is_none() || tree.table_value.is_none() {
        unsafe { lodepng_free(Some(maxlens_ptr as *mut std::ffi::c_void)) };
        return 83;
    }

    // Second pass: set table_len and table_value
    let mut pointer = HEADSIZE;
    for i in 0..HEADSIZE as usize {
        let l = maxlens[i];
        if l <= 9 {
            continue;
        }
        tree.table_len.as_mut().unwrap()[i] = l as u8;
        tree.table_value.as_mut().unwrap()[i] = pointer as u16;
        pointer += 1 << (l - 9);
    }

    // Third pass: populate the tables
    let mut numpresent = 0;
    if let (Some(codes), Some(lengths), Some(table_len), Some(table_value)) = (
        &tree.codes,
        &tree.lengths,
        &mut tree.table_len,
        &mut tree.table_value,
    ) {
        for i in 0..tree.numcodes as usize {
            let l = lengths[i];
            if l == 0 {
            ...
            if l <= 9 {
                ...
                let maxlen = maxlens[index];
                let tablelen = maxlen - 9;
                let start = table_value[index] as u32;
                let num = 1 << (tablelen - (l - 9));
                ...
                for j in 0..num {
                    let reverse2 = reverse >> 9;
                    let index2 = (start + (reverse2 | (j << (l - 9)))) as usize;
        ...
        if numpresent < 2 {
            for i in 0..size as usize {
                if table_len[i] == 16 {
                    table_len[i] = if i < HEADSIZE as usize { 1 } else { 10 };
                    table_value[i] = 65535;
                }
            }
        } else {
...
pub fn HuffmanTree_makeFromLengths2(tree: &mut HuffmanTree) -> u32 {
    ...
    tree.codes = Some(vec![0; tree.numcodes as usize]);
    let mut blcount = vec![0; (tree.maxbitlen + 1) as usize];
    let mut nextcode = vec![0; (tree.maxbitlen + 1) as usize];
    ...
    if error == 0 {
        // Initialize blcount and nextcode (already initialized to 0 by vec!)
        
        // Count lengths
        if let Some(lengths) = &tree.lengths {
            for &len in lengths.iter() {
                if len <= tree.maxbitlen {
                    blcount[len as usize] += 1;
                }
            }
        }
        ...
        for bits in 1..=tree.maxbitlen {
            nextcode[bits as usize] = (nextcode[(bits - 1) as usize] + blcount[(bits - 1) as usize]) << 1;
        }

        // Assign codes
        if let Some(lengths) = &tree.lengths {
            if let Some(codes) = &mut tree.codes {
                for n in 0..tree.numcodes {
                    let len = lengths[n as usize];
                    if len != 0 {
                        codes[n as usize] = nextcode[len as usize];
                        nextcode[len as usize] += 1;
                        codes[n as usize] &= (1 << len) - 1;
                    }
                }
            }
        }
    ...
    // No need for explicit free in Rust (vectors are automatically dropped)
...
pub fn HuffmanTree_makeFromLengths(
    ...
    // Check if bitlen is None (equivalent to NULL check in C)
    let bitlen = match bitlen {
        Some(bl) => bl,
        None => return 83,
    };

    // Allocate vector for lengths (replaces lodepng_malloc)
    let mut lengths = Vec::with_capacity(numcodes);
    if lengths.capacity() < numcodes {
        return 83; // Allocation failed
    }

    // Copy bitlen values into lengths vector
    for i in 0..numcodes {
        lengths.push(bitlen[i]);
    }

    // Update tree fields
    tree.lengths = Some(lengths);
    tree.numcodes = numcodes as u32;
    tree.maxbitlen = maxbitlen;

    // Call the second part of the function
...
pub fn generateFixedLitLenTree(tree: &mut HuffmanTree) -> u32 {
    ...
    // Initialize bitlen with the specified values
    bitlen.extend(std::iter::repeat(8).take(144)); // 0..144
    bitlen.extend(std::iter::repeat(9).take(112)); // 144..256
    bitlen.extend(std::iter::repeat(7).take(24)); // 256..280
    bitlen.extend(std::iter::repeat(8).take(8)); // 280..288

    error = HuffmanTree_makeFromLengths(tree, Some(&bitlen), 288, 15);
...
pub fn generateFixedDistanceTree(tree: &mut HuffmanTree) -> u32 {
    let mut error = 0;
    let mut bitlen = vec![5u32; 32]; // Create a Vec<u32> with 32 elements, all initialized to 5

    error = HuffmanTree_makeFromLengths(tree, Some(&bitlen), 32, 15);
    // No need for explicit free in Rust, Vec will be automatically dropped when out of scope
    error
}
...
pub fn huffmanDecodeSymbol(reader: &mut LodePngBitReader, codetree: &HuffmanTree) -> Option<u16> {
    let code = peekBits(reader, 9);
    
    let table_len = codetree.table_len.as_ref()?;
    let table_value = codetree.table_value.as_ref()?;
    
    let l = *table_len.get(code as usize)?;
    let mut value = *table_value.get(code as usize)?;
    ...
    if l <= 9 {
        ...
        let additional_bits = peekBits(reader, (l - 9) as usize) as u16;
        value += additional_bits;
        
        let final_len = *table_len.get(value as usize)?;
        advanceBits(reader, (final_len - 9) as usize);
        
        Some(*table_value.get(value as usize)?)
    }
}
...
pub fn getTreeInflateDynamic(
    ...
    while error == 0 {
        ...
        error = HuffmanTree_makeFromLengths(&mut tree_cl, bitlen_cl.as_deref(), 19, 7);
        if error != 0 {
            break;
        }

        bitlen_ll = Some(vec![0; HLIT as usize]);
        bitlen_d = Some(vec![0; HDIST as usize]);
        ...
        i = 0;
        while i < (HLIT + HDIST) {
            let code = match huffmanDecodeSymbol(reader, &tree_cl) {
            ...
            if code <= 15 {
                if i < HLIT {
                    if let Some(ref mut bitlen_ll_vec) = bitlen_ll {
                        bitlen_ll_vec[i as usize] = code;
                    }
                } else if let Some(ref mut bitlen_d_vec) = bitlen_d {
                    bitlen_d_vec[(i - HLIT) as usize] = code;
                }
                i += 1;
            } else if code == 16 {
                let mut replength = 3;
                ...
                if i < (HLIT + 1) {
                    if let Some(ref bitlen_ll_vec) = bitlen_ll {
                        value = bitlen_ll_vec[(i - 1) as usize];
                    } else {
                        error = 83;
                        break;
                    }
                } else if let Some(ref bitlen_d_vec) = bitlen_d {
                    value = bitlen_d_vec[((i - HLIT) - 1) as usize];
                } else {
                    error = 83;
                    break;
                }
                ...
                for n in 0..replength {
                    ...
                    if i < HLIT {
                        if let Some(ref mut bitlen_ll_vec) = bitlen_ll {
                            bitlen_ll_vec[i as usize] = value;
                        }
                    } else if let Some(ref mut bitlen_d_vec) = bitlen_d {
                        bitlen_d_vec[(i - HLIT) as usize] = value;
                    }
                    i += 1;
                }
            } else if code == 17 {
                ...
                for n in 0..replength {
                    ...
                    if i < HLIT {
                        if let Some(ref mut bitlen_ll_vec) = bitlen_ll {
                            bitlen_ll_vec[i as usize] = 0;
                        }
                    } else if let Some(ref mut bitlen_d_vec) = bitlen_d {
                        bitlen_d_vec[(i - HLIT) as usize] = 0;
                    }
                    i += 1;
                }
            } else if code == 18 {
        ...
        error = HuffmanTree_makeFromLengths(tree_ll, bitlen_ll.as_deref(), 288, 15);
        if error != 0 {
            break;
        }

        error = HuffmanTree_makeFromLengths(tree_d, bitlen_d.as_deref(), 32, 15);
        break;
    }
...
pub fn ensureBits32(reader: &mut LodePngBitReader, nbits: usize) {
    ...
    if (start + 4) < size {
        let data = reader.data.as_ref().unwrap(); // Safe unwrap since we checked size
        reader.buffer = (data[start] as u32) 
            | ((data[start + 1] as u32) << 8)
            | ((data[start + 2] as u32) << 16)
            | ((data[start + 3] as u32) << 24);
        reader.buffer >>= reader.bp & 7;
        reader.buffer |= (data[start + 4] as u32) << (24 + (8 - (reader.bp & 7)));
    } else {
        reader.buffer = 0;
...
pub fn inflateHuffmanBlock(
    ...
    while error == 0 && !done {
        ...
        let code_ll = match huffmanDecodeSymbol(reader, &tree_ll) {
            Some(code) => code,
            None => {
                error = 16;
                break;
        ...
        if code_ll <= 255 {
            ...
            let code_ll_idx = (code_ll - 257) as usize;
            let length = LENGTHBASE[code_ll_idx];
            let numextrabits_l = LENGTHEXTRA[code_ll_idx];
            ...
            let code_d = match huffmanDecodeSymbol(reader, &tree_d) {
                Some(code) => code,
                None => {
                    error = 16;
                    break;
            ...
            let code_d_idx = code_d as usize;
            let mut distance = DISTANCEBASE[code_d_idx];
            let numextrabits_d = DISTANCEEXTRA[code_d_idx];
            ...
            if let Some(data) = &mut out.data {
                if (distance as usize) < length as usize {
                    ...
                    while forward < length as usize {
                        if current_start >= data.len() || current_backward >= data.len() {
                            error = 83;
                            break;
                        }
                        data.push(data[current_backward]);
                        current_start += 1;
                        current_backward += 1;
                        forward += 1;
                    }
                } else {
                    let (left, right) = data.split_at_mut(start);
                    right[..length as usize].copy_from_slice(&left[backward..backward + length as usize]);
                }
            } else {
                error = 83;
                break;
...
pub fn LodePNGBitReader_init(reader: &mut LodePngBitReader, data: Option<Vec<u8>>, size: usize) -> u32 {
    ...
    if lodepng_mulofl(size, 8, &mut Some(reader.bitsize)) {
        return 105;
    }
...
pub fn lodepng_inflatev(
    ...
    while BFINAL == 0 {
        ...
        if BTYPE == 3 {
            ...
            error = inflateHuffmanBlock(out, &mut reader, BTYPE, Some(settings.max_output_size));
        }
...
pub fn inflatev(
    ...
    if let Some(custom_inflate) = settings.custom_inflate {
        let mut data = out.data.take();
        let mut size = out.size;
        
        let error = custom_inflate(&mut data, &mut size, in_data, insize, settings);
        ...
        if error != 0 {
            let error = if settings.max_output_size != 0 && out.size > settings.max_output_size {
                109
            } else {
                110
            };
        ...
        lodepng_inflatev(out, Some(in_data.to_vec()), insize, settings)
    }
}
...
pub fn lodepng_zlib_decompressv(
    ...
    let error = inflatev(out, &in_data[2..], insize - 2, settings);
    if error != 0 {
        return error;
    }
...
pub fn zlib_decompress(
    ...
    if let Some(custom_zlib) = settings.custom_zlib {
        error = custom_zlib(out, outsize, in_data, insize, settings);
        if error != 0 {
            error = 110;
        ...
        if expected_size != 0 {
            let new_size = *outsize + expected_size;
            ucvector_resize(&mut v, new_size);
            v.size = *outsize;
        }
...
pub fn readChunk_zTXt(
    ...
    let mut error = 0;
    let mut zlibsettings = LodePNGDecompressSettings {
        ignore_adler32: decoder.zlibsettings.ignore_adler32,
        ignore_nlen: decoder.zlibsettings.ignore_nlen,
        max_output_size: decoder.zlibsettings.max_output_size,
        custom_zlib: decoder.zlibsettings.custom_zlib,
        custom_inflate: decoder.zlibsettings.custom_inflate,
        custom_context: None, // Cannot safely clone, set to None
    };
    let mut length;
    ...
    while error == 0 {
        ...
        key = Some(vec![0; length]);
        if key.is_none() {
            error = 83;
            break;
        }

        if let Some(ref mut key_vec) = key {
            lodepng_memcpy(key_vec, &data[..length]);
            key_vec.push(0); // Add null terminator
        }
        ...
        let key_str = key.as_ref().and_then(|k| std::str::from_utf8(&k[..k.len()-1]).ok()); // Exclude null terminator
        let str_slice = str.as_ref().map(|s| s.as_slice());
        error = lodepng_add_text_sized(info, key_str, str_slice, size);
        break;
    }

    // Memory is automatically freed when key and str go out of scope
    error
}
pub fn readChunk_tRNS(color: &mut LodePNGColorMode, data: &[u8], chunkLength: usize) -> u32 {
    match color.colortype {
        ...
        LodePNGColorType::LCT_GREY => {
            ...
            color.key_defined = 1;
            let key = (u32::from(data[0]) << 8) + u32::from(data[1]);
            color.key_r = key;
            color.key_g = key;
            color.key_b = key;
        }
        LodePNGColorType::LCT_RGB => {
            ...
            color.key_defined = 1;
            color.key_r = (u32::from(data[0]) << 8) + u32::from(data[1]);
            color.key_g = (u32::from(data[2]) << 8) + u32::from(data[3]);
            color.key_b = (u32::from(data[4]) << 8) + u32::from(data[5]);
        }
        _ => return 42,
    }
    0
...
pub fn readChunk_tEXt(info: &mut LodePngInfo, data: &[u8], chunkLength: usize) -> u32 {
    ...
    while error == 0 {
        ...
        key_vec[..length].copy_from_slice(&data[..length]);
        key_vec[length] = 0;
        key = Some(key_vec);
        ...
        if remaining_length > 0 {
            str_vec[..remaining_length].copy_from_slice(&data[string2_begin..string2_begin + remaining_length]);
        }
        str_vec[remaining_length] = 0;
        str = Some(str_vec);

        // Convert to CStr and then to &str
        let key_str = unsafe { CStr::from_ptr(key.as_ref().unwrap().as_ptr() as *const u8) }.to_str().ok();
        let str_str = unsafe { CStr::from_ptr(str.as_ref().unwrap().as_ptr() as *const u8) }.to_str().ok();

        error = lodepng_add_text(info, key_str, str_str);
        break;
    }

    // Free resources (handled by Rust's drop)
    error
}
...
pub fn lodepng_add_itext_sized(
    ...
    // Helper function to extend a Vec or create a new one if None
    fn extend_or_create(vec: &mut Option<Vec<String>>, new_capacity: usize) -> bool {
        match vec {
            Some(v) => {
                v.reserve(1);
                true
            ...
            None => {
                *vec = Some(Vec::with_capacity(new_capacity));
                true
            }
        }
    }

    // Check if we can extend all vectors successfully
    let success = extend_or_create(&mut info.itext_keys, info.itext_num + 1)
        && extend_or_create(&mut info.itext_langtags, info.itext_num + 1)
        && extend_or_create(&mut info.itext_transkeys, info.itext_num + 1)
        && extend_or_create(&mut info.itext_strings, info.itext_num + 1);

    if !success {
        return 83;
    }

    // Now safely unwrap since we've ensured they exist
    let keys = info.itext_keys.as_mut().unwrap();
    let langtags = info.itext_langtags.as_mut().unwrap();
    let transkeys = info.itext_transkeys.as_mut().unwrap();
    let strings = info.itext_strings.as_mut().unwrap();

    // Push new values
    keys.push(String::from(key.unwrap_or("")));
    langtags.push(String::from(langtag.unwrap_or("")));
    transkeys.push(String::from(transkey.unwrap_or("")));
    strings.push(String::from_utf8_lossy(str.unwrap_or(&[])).into_owned());
...
pub fn readChunk_iTXt(
    ...
    let mut zlibsettings = LodePNGDecompressSettings {
        ...
        custom_zlib: decoder.zlibsettings.custom_zlib,
        custom_inflate: decoder.zlibsettings.custom_inflate,
        custom_context: decoder.zlibsettings.custom_context.as_ref().map(|b| {
            let ptr = b.as_ref() as *const std::ffi::c_void as *mut std::ffi::c_void;
            unsafe { Box::from_raw(ptr) }
        }),
    };
    let mut key: Option<Vec<u8>> = None;
    let mut langtag: Option<Vec<u8>> = None;
    let mut transkey: Option<Vec<u8>> = None;
    ...
    while error == 0 {
        ...
        // Find key length (until null byte)
        let key_length = match data.iter().take(chunkLength).position(|&x| x == 0) {
            Some(len) => len,
            None => {
                error = 75;
                break;
        ...
        // Allocate and copy key
        key = Some(vec![0; key_length + 1]);
        if let Some(ref mut key_vec) = key {
            key_vec[..key_length].copy_from_slice(&data[..key_length]);
            key_vec[key_length] = 0;
        } else {
            error = 83;
            break;
        }

        let compressed = data[key_length + 1];
        if data[key_length + 2] != 0 {
            error = 72;
            break;
        }

        let mut begin = key_length + 3;

        // Find langtag length
        let langtag_length = match data[begin..chunkLength].iter().position(|&x| x == 0) {
            Some(len) => len,
            None => {
                error = 75;
                break;
        ...
        // Allocate and copy langtag
        langtag = Some(vec![0; langtag_length + 1]);
        if let Some(ref mut langtag_vec) = langtag {
            langtag_vec[..langtag_length].copy_from_slice(&data[begin..begin + langtag_length]);
            langtag_vec[langtag_length] = 0;
        } else {
            error = 83;
            break;
        }

        begin += langtag_length + 1;

        // Find transkey length
        let transkey_length = match data[begin..chunkLength].iter().position(|&x| x == 0) {
            Some(len) => len,
            None => {
                error = 75;
                break;
        ...
        // Allocate and copy transkey
        transkey = Some(vec![0; transkey_length + 1]);
        if let Some(ref mut transkey_vec) = transkey {
            transkey_vec[..transkey_length].copy_from_slice(&data[begin..begin + transkey_length]);
            transkey_vec[transkey_length] = 0;
        } else {
            error = 83;
            break;
        }

        begin += transkey_length + 1;
        let length = if chunkLength < begin { 0 } else { chunkLength - begin };
        ...
        if compressed != 0 {
            ...
            if error == 0 {
                error = lodepng_add_itext_sized(
                    info,
                    key.as_ref().map(|k| CStr::from_bytes_with_nul(k).unwrap().to_str().unwrap()),
                    langtag.as_ref().map(|l| CStr::from_bytes_with_nul(l).unwrap().to_str().unwrap()),
                    transkey.as_ref().map(|t| CStr::from_bytes_with_nul(t).unwrap().to_str().unwrap()),
                    str.as_ref().map(|s| s.as_slice()),
                    size,
                );
            }
        } else {
    ...
    // Prevent double-free of custom_context by forgetting the Box
    if let Some(ctx) = zlibsettings.custom_context.take() {
        std::mem::forget(ctx);
    }

    // Free resources
    if let Some(k) = key {
        lodepng_free(Some(k.as_ptr() as *mut std::ffi::c_void));
    }
    if let Some(l) = langtag {
        lodepng_free(Some(l.as_ptr() as *mut std::ffi::c_void));
    }
    if let Some(t) = transkey {
        lodepng_free(Some(t.as_ptr() as *mut std::ffi::c_void));
    }
...
pub fn readChunk_iCCP(
    ...
    let mut error = 0;
    let mut size = 0;
    let zlibsettings = &decoder.zlibsettings;
    let mut length;
    let mut string2_begin;
    ...
    // Find the length of the name (until null terminator)
    length = 0;
    while length < chunkLength && data[length] != 0 {
        length += 1;
    }
    ...
    length = chunkLength - string2_begin;
    // Create a modified settings by bitwise copying the original
    let mut modified_settings = unsafe {
        std::ptr::read(&decoder.zlibsettings)
    };
    modified_settings.max_output_size = decoder.max_icc_size;
    ...
    // Prevent the custom_context from being dropped (avoid double-free)
    let ctx = std::mem::replace(&mut modified_settings.custom_context, None);
    std::mem::forget(ctx);

    if error != 0 && size > modified_settings.max_output_size {
        error = 113;
    }
...
pub fn lodepng_color_mode_alloc_palette(info: &mut LodePNGColorMode) {
    ...
    if let Some(palette) = &mut info.palette {
        for i in 0..256 {
            ...
            palette[base + 1] = 0;
            palette[base + 2] = 0;
            palette[base + 3] = LCT_MAX_OCTET_VALUE;
        }
    }
}
    ...
    match filter_type {
        ...
        2 => {
            if let Some(precon) = precon {
                for (r, (s, p)) in recon.iter_mut().zip(scanline.iter().zip(precon.iter())) {
                    *r = s.wrapping_add(*p);
                }
            } else {
                recon[..length].copy_from_slice(&scanline[..length]);
        ...
        4 => {
            if let Some(precon) = precon {
                ...
                while i + 3 < length {
                    ...
                    recon[i + 2] = s2.wrapping_add(paethPredictor(r2 as i16, p2 as i16, q2 as i16));
                    recon[i + 3] = s3.wrapping_add(paethPredictor(r3 as i16, p3 as i16, q3 as i16));
...
pub fn unfilter(
    ...
    let mut prevline_data = vec![0u8; linebytes];
    ...
    for y in 0..h {
        ...
        let current_line = &mut out[outindex..outindex + linebytes];
        let scanline = &in_data[inindex + 1..inindex + 1 + linebytes];

        let error = unfilter_scanline(
            current_line,
            scanline,
            if y == 0 { Option::None } else { Some(&prevline_data) },
            bytewidth as usize,
            filter_type,
            linebytes,
        );
        ...
        if y + 1 < h {
            prevline_data.copy_from_slice(current_line);
        }
    }

    0
}
pub fn remove_padding_bits(out: &mut [u8], in_: &[u8], olinebits: usize, ilinebits: usize, h: usize) {
...
pub fn Adam7_getpassvalues(
    ...
    for i in 0..7 {
        passw[i] = ((w + ADAM7_DX[i]).wrapping_sub(ADAM7_IX[i]) - 1) / ADAM7_DX[i];
        passh[i] = ((h + ADAM7_DY[i]).wrapping_sub(ADAM7_IY[i]) - 1) / ADAM7_DY[i];
            ...
            for y in 0..passh[i] {
                for x in 0..passw[i] {
                    ...
                    let pixeloutstart = (((ADAM7_IY[i] as usize + (y as usize * ADAM7_DY[i] as usize)) * w as usize)
                        + ADAM7_IX[i] as usize
                        + (x as usize * ADAM7_DX[i] as usize)) * bytewidth as usize;
...
pub fn postProcessScanlines(
    ...
    if info_png.interlace_method == 0 {
        ...
        for i in 0..7 {
            {
                let error = unfilter(
                    &mut temp_in[padded_passstart[i]..],
            ...
            if bpp < 8 {
                let bytes_per_line = ((passw[i] * bpp + 7) / 8) as usize;
                let padded_len = passh[i] as usize * bytes_per_line;
                let padded_data = temp_in[padded_passstart[i]..padded_passstart[i] + padded_len].to_vec();

                let pass_len = passh[i] as usize * bytes_per_line;
                let pass_slice = &mut temp_in[passstart[i]..passstart[i] + pass_len];
        ...
        Adam7_deinterlace(out, &temp_in, w, h, bpp);
    }
...
pub fn lodepng_pixel_overflow(w: usize, h: usize, pngcolor: &LodePNGColorMode, rawcolor: &LodePNGColorMode) -> bool {
    ...
    if lodepng_mulofl(numpixels.unwrap(), 8, &mut total) {
        return true;
    }
    ...
    }
    {
        ...
        if lodepng_addofl(line.unwrap(), temp, line.as_mut().unwrap()) {
            return true;
        }
    }
    if lodepng_addofl(line.unwrap(), 5, line.as_mut().unwrap()) {
        return true;
    }
    if lodepng_mulofl(line.unwrap(), h, &mut total) {
        return true;
    }
    false
}
...
pub fn lodepng_chunk_type_equals(chunk: Option<&[u8]>, type_: Option<&str>) -> u8 {
    ...
    // Check if type length is 4 (Rust strings are UTF-8, but we treat as ASCII here)
    if lodepng_strlen(Some(type_)) != 4 {
        return 0;
    }
    
    // Convert type to bytes and compare each byte
    let type_bytes = type_.as_bytes();
    if chunk.len() < 8 || type_bytes.len() < 4 {
        return 0;
    }
    
    ((chunk[4] == type_bytes[0]) as u8 &
     (chunk[5] == type_bytes[1]) as u8 &
     (chunk[6] == type_bytes[2]) as u8 &
...
pub fn lodepng_crc32(data: Option<&[u8]>, length: usize) -> u32 {
    ...
    for i in 0..length {
        ...
        r = table[((r ^ u32::from(data[i])) & 0xffu32) as usize] ^ (r >> 8u32);
    }
...
pub fn lodepng_inspect(
    ...
    if lodepng_chunk_type_equals(Some(&in_data[8..12]), Some("IHDR")) == 0 {
        state.error = 29;
        return 29;
    }
    ...
    // Update width and height if pointers are provided
    if let Some(w_ptr) = w {
        *w_ptr = width;
    }
    if let Some(h_ptr) = h {
...
pub fn lodepng_chunk_next_const<'a>(chunk: Option<&'a [u8]>, end: Option<&'a [u8]>) -> Option<&'a [u8]> {
    ...
    let available_size = end.len().checked_sub(chunk.len()).unwrap_or(0);
    
    // Check if chunk is at or beyond end, or if available_size is too small
    ...
    // Check if chunk would exceed available size
    match total_chunk_length.cmp(&available_size) {
        Ordering::Greater => Some(end),
        Ordering::Equal | Ordering::Less => Some(&chunk[total_chunk_length..]),
    }
}
...
pub fn lodepng_chunk_check_crc(chunk: Option<&[u8]>) -> u32 {
    ...
    // Calculate the position where CRC starts (length + 8)
    // Rust slices are exclusive, so we need to take 4 bytes from that position
    let crc_start = length + 8;
    ...
    // Calculate checksum for data starting at byte 4, with length + 4 bytes
    let checksum_data_start = 4;
    let checksum_data_end = checksum_data_start + length + 4;
    if checksum_data_end > chunk.len() {
        return 1; // Return error if chunk is too small
    }
    let checksum = lodepng_crc32(Some(&chunk[checksum_data_start..checksum_data_end]), length + 4);
...
pub fn lodepng_chunk_append(
    ...
    let mut new_buffer = match out {
        Some(vec) => {
            let mut new_vec = vec.clone();
            new_vec.resize(new_length, 0);
            new_vec
        }
        None => vec![0; new_length],
    };

    let chunk_start_idx = new_length - total_chunk_length;
    new_buffer[chunk_start_idx..chunk_start_idx + total_chunk_length]
        .copy_from_slice(&chunk[..total_chunk_length]);
...
pub fn decodeGeneric(
    ...
    let mut idat_idx = 0;
    let mut idatsize = 0;
    let mut scanlines: Option<Vec<u8>> = None;
    ...
    *h = 0;
    ...
    while !IEND && state.error == 0 {
        ...
        chunk = Some(&in_data[pos..]);
        chunk_length = lodepng_chunk_length(chunk.unwrap());
        ...
        if (pos + chunk_length as usize + 12) > insize || (pos + chunk_length as usize + 12) < pos {
            state.error = 64;
            break;
        }
        ...
        if lodepng_chunk_type_equals(chunk, Some("IDAT")) == 1 {
            let mut newsize = 0;
            if lodepng_addofl(idatsize, chunk_length as usize, &mut newsize) {
                state.error = 95;
                break;
            }
            if newsize > insize {
            ...
            if let (Some(idat_vec), Some(data_slice)) = (idat.as_mut(), data) {
                lodepng_memcpy(&mut idat_vec[idatsize..], data_slice);
            }
            idatsize += chunk_length as usize;
            critical_pos = 3;
        } else if lodepng_chunk_type_equals(chunk, Some("IEND")) == 1 {
            IEND = true;
        } else if lodepng_chunk_type_equals(chunk, Some("PLTE")) == 1 {
            state.error = readChunk_PLTE(&mut state.info_png.color, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
            critical_pos = 2;
        } else if lodepng_chunk_type_equals(chunk, Some("tRNS")) == 1 {
            state.error = readChunk_tRNS(&mut state.info_png.color, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("bKGD")) == 1 {
            state.error = readChunk_bKGD(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("tEXt")) == 1 {
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_tEXt(&mut state.info_png, data.unwrap(), chunk_length as usize);
                if state.error != 0 {
                    break;
                }
            }
        } else if lodepng_chunk_type_equals(chunk, Some("zTXt")) == 1 {
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_zTXt(&mut state.info_png, &state.decoder, data.unwrap(), chunk_length as usize);
                if state.error != 0 {
                    break;
                }
            }
        } else if lodepng_chunk_type_equals(chunk, Some("iTXt")) == 1 {
            if state.decoder.read_text_chunks != 0 {
                state.error = readChunk_iTXt(&mut state.info_png, &state.decoder, data.unwrap(), chunk_length as usize);
                if state.error != 0 {
                    break;
                }
            }
        } else if lodepng_chunk_type_equals(chunk, Some("tIME")) == 1 {
            state.error = readChunk_tIME(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("pHYs")) == 1 {
            state.error = readChunk_pHYs(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("gAMA")) == 1 {
            state.error = readChunk_gAMA(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("cHRM")) == 1 {
            state.error = readChunk_cHRM(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("sRGB")) == 1 {
            state.error = readChunk_sRGB(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("iCCP")) == 1 {
            state.error = readChunk_iCCP(&mut state.info_png, &state.decoder, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else if lodepng_chunk_type_equals(chunk, Some("sBIT")) == 1 {
            state.error = readChunk_sBIT(&mut state.info_png, data.unwrap(), chunk_length as usize);
            if state.error != 0 {
                break;
            }
        } else {
            ...
            if state.decoder.remember_unknown_chunks != 0 {
                state.error = lodepng_chunk_append(
                    &mut state.info_png.unknown_chunks_data[critical_pos - 1],
        ...
        if !IEND {
            chunk_idx = lodepng_chunk_next_const(Some(&in_data[chunk_idx..]), Some(&in_data[insize..])).map_or(0, |c| c.as_ptr() as usize - in_data.as_ptr() as usize);
        }
    }
    ...
    idat = None; // Equivalent to lodepng_free
    ...
    if state.error == 0 {
        if let Some(out_vec) = out.as_mut() {
            lodepng_memset(out_vec, 0, outsize);
    ...
    scanlines = None; // Equivalent to lodepng_free
}
    ...
    *outsize = Some(size);
    ...
    if size > 0 {
        ...
        *out = Some(buffer);
        ...
        if let Some(ref mut out_buf) = out {
            lodepng_buffer_file(out_buf, filename)?;
        }
    } else {
        *out = None;
    }
...
pub fn lodepng_decode_file(
    ...
    *h = 0;
    ...
    error = match lodepng_load_file(&mut buffer, &mut buffersize, filename) {
        Ok(()) => 0,
        Err(e) => e,
    };

    if error == 0 {
        if let (Some(buffer_data), Some(size)) = (buffer.as_ref(), buffersize) {
    ...
    lodepng_free(buffer.map(|mut v| v.as_mut_ptr() as *mut std::ffi::c_void));
    error
}
...
pub fn hash_init(hash: &mut Hash, windowsize: usize) -> u32 {
    // Allocate memory for each field, converting to Option<Vec<T>>
    ...
    // Check if any allocation failed (vec! would panic on OOM, so this is just for completeness)
...
pub fn hash_cleanup(hash: &mut Hash) {
    // Since lodepng_free expects Option<*mut c_void>, we need to convert the Vec fields
    // Rust's Vec can be converted to raw pointers, but since we're doing cleanup,
    // we'll just set them to None (equivalent to freeing in this context)
    
    // In Rust, we don't need to manually free memory - Drop trait handles it
    // So we can just set all fields to None
    hash.head = None;
    hash.val = None;
    hash.chain = None;
    hash.zeros = None;
...
pub fn deflateNoCompression(out: &mut Ucvector, data: &[u8]) -> u32 {
    ...
    for i in 0..numdeflateblocks {
        ...
        if let Some(ref mut out_data) = out.data {
            ...
            let dst_slice = &mut out_data[pos + 5..pos + 5 + LEN as usize];
...
pub fn writeBitsReversed(writer: &mut LodePngBitWriter, value: u32, nbits: usize) {
    for i in 0..nbits {
        {
            if (writer.bp & 7u8) == 0 {
                if let Some(data) = &mut writer.data {
                    if !ucvector_resize(data, data.size + 1) {
                        return;
                    }
                    if let Some(vec_data) = &mut data.data {
...
pub fn writeLZ77data(
    ...
    while i < lz77_encoded.size {
        ...
        {
            let codes = tree_ll.codes.as_ref().unwrap();
            let lengths = tree_ll.lengths.as_ref().unwrap();
            writeBitsReversed(writer, codes[val as usize], lengths[val as usize] as usize);
        }
        ...
        if val > 256 {
            ...
            {
                let codes = tree_d.codes.as_ref().unwrap();
                let lengths = tree_d.lengths.as_ref().unwrap();
                writeBitsReversed(writer, codes[distance_code as usize], lengths[distance_code as usize] as usize);
            }
...
pub fn uivector_cleanup(p: &mut Uivector) {
    p.size = 0;
    p.allocsize = 0;
    lodepng_free(p.data.take().map(|mut v| v.as_mut_ptr() as *mut std::ffi::c_void));
}
pub fn updateHashChain(hash: &mut Hash, wpos: usize, hashval: u32, numzeros: u16) {
    // Update val array
    if let Some(ref mut val) = hash.val {
        val[wpos] = hashval as i32;
    }

    // Update chain and head for hashval
    if let Some(ref mut head) = hash.head {
        let hashval_usize = hashval as usize;
        if head.len() > hashval_usize {
            if head[hashval_usize] != -1 {
                if let Some(ref mut chain) = hash.chain {
                    chain[wpos] = head[hashval_usize] as u16;
                }
            }
            head[hashval_usize] = wpos as i32;
        }
    }

    // Update zeros array
    if let Some(ref mut zeros) = hash.zeros {
        zeros[wpos] = numzeros;
    }

    // Update chainz and headz for numzeros
    if let Some(ref mut headz) = hash.headz {
        let numzeros_usize = numzeros as usize;
        if headz.len() > numzeros_usize {
            if headz[numzeros_usize] != -1 {
                if let Some(ref mut chainz) = hash.chainz {
                    chainz[wpos] = headz[numzeros_usize] as u16;
                }
            }
            headz[numzeros_usize] = wpos as i32;
        }
    }
}
...
pub fn getHash(data: &[u8], size: usize, pos: usize) -> u32 {
    ...
    if pos + 2 < size {
        result ^= (data[pos + 0] as u32) << 0u32;
        result ^= (data[pos + 1] as u32) << 4u32;
        result ^= (data[pos + 2] as u32) << 8u32;
    } else {
        if pos >= size {
            return 0;
        }
        let amount = size - pos;
        for i in 0..amount {
            result ^= (data[pos + i] as u32) << ((i * 8) as u32);
        }
    }
...
pub fn countZeros(data: &[u8], size: usize, pos: usize) -> u32 {
    ...
    let end = if start.len() > MAX_SUPPORTED_DEFLATE_LENGTH {
        &start[..MAX_SUPPORTED_DEFLATE_LENGTH]
    } else {
        start
    };
    
    let adjusted_end = if end.as_ptr() > unsafe { data.as_ptr().add(size) } {
        &data[data_idx..][..size]
    } else {
        end
    };
    ...
    let mut count = 0;
    for &byte in start.iter().take(adjusted_end.len()) {
        if byte == 0 {
            count += 1;
        } else {
            break;
...
pub fn addLengthDistance(values: &mut Uivector, length: usize, distance: usize) {
    ...
    let pos = values.size;
    let ok = uivector_resize(values, values.size + 4);
...
pub fn encodeLZ77(
    ...
    while pos < insize {
        ...
        let hashval = getHash(in_data, insize, pos);

        if usezeros && hashval == 0 {
            if numzeros == 0 {
                numzeros = countZeros(in_data, insize, pos);
            } else if (pos as u32 + numzeros) > insize as u32 || in_data[(pos + numzeros as usize) - 1] != 0 {
                numzeros -= 1;
            }
        } else {
            numzeros = 0;
    ...
    if error == 0 {
        ...
        if settings.use_lz77 != 0 {
            ...
            for i in datapos..dataend {
                let data_i = data[i] as usize;
                if let (Some(codes), Some(lengths)) = (&tree_ll.codes, &tree_ll.lengths) {
                    if data_i < codes.len() && data_i < lengths.len() {
                        writeBitsReversed(writer, codes[data_i], lengths[data_i] as usize);
                    }
                }
            }
        }
        ...
        if error == 0 {
            if let (Some(codes), Some(lengths)) = (&tree_ll.codes, &tree_ll.lengths) {
                if 256 < codes.len() && 256 < lengths.len() {
                    writeBitsReversed(writer, codes[256], lengths[256] as usize);
                }
            }
        }
    }
...
pub fn bpmnode_sort(leaves: &mut [BpmNode]) {
    ...
    let mut width = 1;
    while width < num {
        let (a, b) = if counter & 1 == 0 {
            (leaves.as_mut(), mem.as_mut_slice())
        } else {
            (mem.as_mut_slice(), leaves.as_mut())
        };
    ...
    // If counter is odd, copy from mem back to leaves
    if counter & 1 != 0 {
        leaves.clone_from_slice(&mem);
    }
}
pub fn bpmnode_create(
    ...
    if lists.nextfree >= lists.numfree {
        ...
        if let Some(chains0) = lists.chains0.as_ref() {
            for node_option in chains0.iter() {
                let mut current = node_option.as_ref();
                while let Some(node) = current {
                    if let Some(memory) = lists.memory.as_mut() {
                        if let Some(mem_node) = memory.get_mut(node.index as usize) {
                            mem_node.in_use = 1;
                        }
                    }
...
pub fn lodepng_huffman_code_lengths(
    ...
    if numpresent == 0 {
        ...
        if error == 0 {
            ...
            if let Some(chains0) = &mut lists.chains0 {
                if let Some(chains1) = &mut lists.chains1 {
                    if let Some(memory) = &lists.memory {
                        for i in 0..lists.listsize as usize {
                            chains0[i] = Some(Box::new(memory[0].clone()));
                            chains1[i] = Some(Box::new(memory[1].clone()));
                        }
                    }
                }
            }
            ...
            if let Some(chains1) = &lists.chains1 {
                if let Some(node) = &chains1[maxbitlen as usize - 1] {
                    let mut current = node;
                    loop {
                        for i in 0..current.index as usize {
                            lengths[leaves[i].index as usize] += 1;
                        }
                        match &current.tail {
...
pub fn deflateDynamic(
    ...
    let mut frequencies_ll = vec![0u32; 286];
    let mut frequencies_d = vec![0u32; 30];
    let mut frequencies_cl = vec![0u32; 19];
    ...
    loop {
        ...
        if settings.use_lz77 != 0 {
            ...
            if let Some(ref mut encoded_data) = lz77_encoded.data {
                for i in datapos..dataend {
                    encoded_data[i - datapos] = data[i] as u32;
                }
            }
        }

        if let Some(ref encoded_data) = lz77_encoded.data {
            let mut i = 0;
            while i < lz77_encoded.size {
                let symbol = encoded_data[i];
                frequencies_ll[symbol as usize] += 1;
                if symbol > 256 {
                    let dist = encoded_data[i + 2];
        ...
        numcodes_ll = if tree_ll.numcodes < 286 {
            tree_ll.numcodes as usize
        } else {
            286
        };
        numcodes_d = if tree_d.numcodes < 30 {
            tree_d.numcodes as usize
        } else {
            30
        };
        numcodes_lld = numcodes_ll + numcodes_d;

        bitlen_lld = vec![0u32; numcodes_lld];
        bitlen_lld_e = vec![0u32; numcodes_lld];

        if let Some(ref lengths_ll) = tree_ll.lengths {
            for i in 0..numcodes_ll {
                bitlen_lld[i] = lengths_ll[i];
            }
        }

        if let Some(ref lengths_d) = tree_d.lengths {
            for i in 0..numcodes_d {
                bitlen_lld[numcodes_ll + i] = lengths_d[i];
            }
        }
        ...
        while numcodes_cl > 4
            && tree_cl
                .lengths
                .as_ref()
                .map_or(false, |l| l[CLCL_ORDER[numcodes_cl - 1] as usize] == 0)
        {
            numcodes_cl -= 1;
        }
        ...
        if let Some(ref lengths_cl) = tree_cl.lengths {
            for i in 0..numcodes_cl {
                writeBits(writer, lengths_cl[CLCL_ORDER[i] as usize], 3);
            }
        }

        let mut i = 0;
        while i < numcodes_lld_e {
            if let (Some(codes), Some(lengths)) = (&tree_cl.codes, &tree_cl.lengths) {
                writeBitsReversed(
                    writer,
                    codes[bitlen_lld_e[i] as usize],
                    lengths[bitlen_lld_e[i] as usize] as usize,
                );
            }
            if bitlen_lld_e[i] == 16 {
                i += 1;
...
pub fn lodepng_deflate(
    ...
    let mut v = ucvector_init(out.take(), *outsize);
    let error = lodepng_deflatev(&mut v, in_data, settings);
    *out = v.data;
    *outsize = v.size;
    error
}
...
pub fn deflate(
    ...
    if let Some(custom_deflate) = settings.custom_deflate {
        // Since we can't safely call the C function pointer in Rust without unsafe,
        // we'll need to use unsafe here to maintain compatibility with the C API.
        // However, this is not idiomatic Rust - in a pure Rust solution, we'd
        // redesign this to use Rust closures or traits instead of function pointers.
        unsafe {
        ...
        lodepng_deflate(out, outsize, in_data, settings)
    }
}
...
pub fn lodepng_zlib_compress(
    ...
    let mut error = deflate(&mut deflatedata, &mut deflatesize, in_data, settings);
    ...
    *outsize = 0;
    ...
    if error == 0 {
        let ADLER32 = adler32(Some(in_data), in_data.len().try_into().unwrap());
        let CMF = 120;
        let FLEVEL = 0;
        let FDICT = 0;
        ...
        if let Some(ref mut out_vec) = out {
            out_vec[0] = (CMFFLG >> 8) as u8;
            out_vec[1] = (CMFFLG & 255) as u8;

            if let Some(ref deflatedata) = deflatedata {
                for i in 0..deflatesize {
                    out_vec[i + 2] = deflatedata[i];
                }
            }

            lodepng_set32bitInt(&mut out_vec[(*outsize - 4)..], ADLER32);
        }
    }

    // lodepng_free is not needed in Rust since Vec handles memory automatically
    error
}
...
pub fn zlib_compress(
    ...
    if let Some(custom_zlib) = settings.custom_zlib {
        // For FFI safety, we need to create raw pointers
        let mut raw_out: *mut u8 = std::ptr::null_mut();
        let mut raw_outsize = 0;
        let raw_in = in_data.as_ptr();
        
        // Call the custom_zlib function through FFI
        let error = custom_zlib(
            &mut raw_out as *mut *mut u8,
            &mut raw_outsize as *mut usize,
            raw_in,
            in_data.len(),
            settings as *const _,
        );
        
        // Convert the raw pointers back to safe Rust types if successful
        if error == 0 && raw_outsize > 0 {
            unsafe {
                *out = Some(Vec::from_raw_parts(raw_out, raw_outsize, raw_outsize));
                *outsize = raw_outsize;
            }
        }
        ...
        lodepng_zlib_compress(out, outsize, in_data, settings)
    }
}
pub fn filter(
    out: &mut [u8],
    ...
    let linebytes = lodepng_get_raw_size_idat(w, 1, u32::from(bpp)) - 1;
    let bytewidth = (usize::from(bpp) + 7) / 8;
    let mut prevline: Option<&[u8]> = None;
    let mut error = 0;
    let mut strategy = settings.filter_strategy;

    if settings.filter_palette_zero != 0
        && (color.colortype == LodePNGColorType::LCT_PALETTE || color.bitdepth < 8)
    {
        strategy = LodePNGFilterStrategy::LFS_ZERO;
    }
    ...
    match strategy {
        ...
        LodePNGFilterStrategy::LFS_MINSUM => {
            ...
            for attempt_type in &mut attempt {
                *attempt_type = Some(vec![0; linebytes]);
                if attempt_type.is_none() {
                    error = 83;
                }
            }
            ...
            if error == 0 {
                for y in 0..h {
                    for (type_idx, attempt_buf) in attempt.iter_mut().enumerate() {
                        if let Some(buf) = attempt_buf {
                            let mut sum = 0;
                            let in_slice = &in_data[y as usize * linebytes..(y as usize + 1) * linebytes];
                            filterScanline(
                                buf,
                                in_slice,
                                prevline,
                            ...
                            if type_idx == 0 {
                                sum = buf.iter().map(|&x| x as usize).sum();
                            } else {
                                sum = buf
                                    .iter()
                            ...
                            if type_idx == 0 || sum < smallest {
                                best_type = type_idx as u8;
                                smallest = sum;
                            }
                        }
                    }

                    prevline = Some(&in_data[y as usize * linebytes..(y as usize + 1) * linebytes]);
                    out[y as usize * (linebytes + 1)] = best_type;
                    let out_slice = &mut out[y as usize * (linebytes + 1) + 1
                        ..y as usize * (linebytes + 1) + 1 + linebytes];
                    out_slice.copy_from_slice(&attempt[best_type as usize].as_ref().unwrap());
                }
            }
        }
                ...
                for y in 0..h {
                    for (type_idx, attempt_buf) in attempt.iter_mut().enumerate() {
                        if let Some(buf) = attempt_buf {
                            ...
                            sum = count.iter().map(|&c| ilog2i(c)).sum();
...
pub fn Adam7_interlace(out: &mut [u8], in_data: &[u8], w: u32, h: u32, bpp: u32) {
    ...
    if bpp >= 8 {
        for i in 0..7 {
            ...
            for y in 0..passh[i] {
                for x in 0..passw[i] {
                    let pixelinstart = ((ADAM7_IY[i] + (y * ADAM7_DY[i])) * w + ADAM7_IX[i] + (x * ADAM7_DX[i])) * bytewidth;
                    let pixeloutstart = passstart[i] + ((y * passw[i] + x) as usize * bytewidth as usize);
                    for b in 0..bytewidth {
                        out[pixeloutstart + b as usize] = in_data[pixelinstart as usize + b as usize];
                    }
                }
            }
        }
        ...
        for i in 0..7 {
            ...
            for y in 0..passh[i] {
                for x in 0..passw[i] {
                    let ibp = ((ADAM7_IY[i] + (y * ADAM7_DY[i])) * olinebits) + (ADAM7_IX[i] + (x * ADAM7_DX[i])) * bpp;
...
pub fn preProcessScanlines(
    ...
    if info_png.interlace_method == 0 {
        *outsize = (h as usize) + (h as usize) * (((w * bpp) + 7) as usize / 8);
        *out = Some(vec![0; *outsize]);
        ...
        if error == 0 {
            if bpp < 8 && (w * bpp) != ((((w * bpp) + 7) / 8) * 8) {
                let padded_size = (h as usize) * (((w * bpp) + 7) as usize / 8);
                let mut padded = vec![0; padded_size];

                if padded.is_empty() && padded_size > 0 {
                    error = 83;
                }

                if error == 0 {
                    addPaddingBits(&mut padded, in_data, (((w * bpp) + 7) / 8) as usize * 8, (w * bpp) as usize, h);
                    error = filter(out.as_mut().unwrap(), &padded, w, h, &info_png.color, settings);
                }
            } else {
                error = filter(out.as_mut().unwrap(), in_data, w, h, &info_png.color, settings);
            }
        }
    } else {
        let mut passw = [0; 7];
        let mut passh = [0; 7];
        let mut filter_passstart = [0; 8];
        let mut padded_passstart = [0; 8];
        let mut passstart = [0; 8];
        ...
        *out = Some(vec![0; *outsize]);
        ...
        let mut adam7 = vec![0; passstart[7]];
        if adam7.is_empty() && passstart[7] > 0 {
            error = 83;
        }

        if error == 0 {
            Adam7_interlace(&mut adam7, in_data, w, h, bpp);
...
pub fn lodepng_assign_icc(info: &mut LodePngInfo, name: Option<&str>, profile: Option<&[u8]>, profile_size: u32) -> u32 {
    ...
    // Convert name to allocated string and then to String
    info.iccp_name = alloc_string(name).and_then(|v| String::from_utf8(v).ok());
    
    // Allocate memory for profile
    info.iccp_profile = match profile {
        Some(p) => {
            let mut vec = Vec::with_capacity(profile_size as usize);
            vec.resize(profile_size as usize, 0);
            Some(vec)
        }
        None => None,
    };
    ...
    // Copy profile data
    if let (Some(dst), Some(src)) = (info.iccp_profile.as_mut(), profile) {
        lodepng_memcpy(dst, src);
    }
...
pub fn LodePNGText_copy(dest: &mut LodePngInfo, source: &LodePngInfo) -> u32 {
    ...
    if let (Some(keys), Some(strings)) = (&source.text_keys, &source.text_strings) {
        for i in 0..source.text_num {
            ...
            let string = strings.get(i).map(|s| s.as_str());
...
pub fn LodePNGIText_copy(dest: &mut LodePngInfo, source: &LodePngInfo) -> u32 {
    ...
    // Ensure source vectors exist and have enough elements
    let keys = match &source.itext_keys {
        Some(v) if v.len() >= source.itext_num => v,
        _ => return 1, // Invalid state
    };
    let langtags = match &source.itext_langtags {
        Some(v) if v.len() >= source.itext_num => v,
        _ => return 1,
    };
    let transkeys = match &source.itext_transkeys {
        Some(v) if v.len() >= source.itext_num => v,
        _ => return 1,
    };
    let strings = match &source.itext_strings {
        Some(v) if v.len() >= source.itext_num => v,
        _ => return 1,
    };
    ...
    for i in 0..source.itext_num {
        let error = lodepng_add_itext(
            dest,
            keys.get(i).map(|s| s.as_str()),
            langtags.get(i).map(|s| s.as_str()),
            transkeys.get(i).map(|s| s.as_str()),
...
pub fn lodepng_info_copy(dest: &mut LodePngInfo, source: &LodePngInfo) -> u32 {
    ...
    // Since we can't do a direct memcpy of structs in safe Rust, we'll clone the source
    *dest = source.clone();
    
    // Initialize color mode and copy it
...
pub fn lodepng_convert_rgb(
    ...
    match mode_in.colortype {
        ...
        LodePNGColorType::LCT_PALETTE => {
            ...
            let palette = mode_in.palette.as_ref().ok_or(31u32)?;
            let base = usize::try_from(r_in * 4).map_err(|_| 31u32)?;
            if base + 2 >= palette.len() {
                return Err(31);
            }
            r = u32::from(palette[base]) * 257;
            g = u32::from(palette[base + 1]) * 257;
            b = u32::from(palette[base + 2]) * 257;
        }
    }
    ...
    match mode_out.colortype {
        ...
        LodePNGColorType::LCT_PALETTE => {
            ...
            let palette = mode_out.palette.as_ref().ok_or(31u32)?;
...
pub fn lodepng_has_palette_alpha(info: &LodePNGColorMode) -> u32 {
    ...
    if let Some(palette) = &info.palette {
        ...
        for i in 0..info.palettesize {
            ...
            if alpha_index < palette.len() && palette[alpha_index] < LCT_MAX_OCTET_VALUE {
                return 1;
            }
        }
    }
...
pub fn lodepng_compute_color_stats(
    ...
    if numcolors_done == 0 {
        for i in 0..stats.numcolors {
            ...
            color_tree_add(&mut tree, color[0], color[1], color[2], color[3], i)?;
        }
    }
    ...
    if sixteen != 0 {
        ...
        for i in 0..numpixels {
            ...
            if (bits_done == 0) && (stats.bits < 8) {
                let bits = getValueRequiredBits(r);
                if bits > stats.bits as u8 {
                    stats.bits = bits as u32;
                }
            }
            ...
            if numcolors_done == 0 {
                if !color_tree_has(&tree, r, g, b, a) {
                    color_tree_add(&mut tree, r, g, b, a, stats.numcolors)?;
                    if stats.numcolors < 256 {
                        let n = stats.numcolors as usize;
                        stats.palette[n * 4 + 0] = r;
...
pub fn lodepng_color_stats_add(
    ...
    let result = lodepng_compute_color_stats(stats, &image, 1, 1, &mode);
...
pub fn lodepng_palette_add(
    ...
    // Check if palette exists, allocate if not
    if info.palette.is_none() {
    ...
    // Get mutable reference to palette vector
    if let Some(palette) = &mut info.palette {
...
pub fn auto_choose_color(
    ...
    if numpixels < usize::try_from(n).unwrap_or(0) * 2 {
        palette_ok = 0;
    }
    ...
    if palette_ok != 0 {
        ...
        for i in 0..stats.numcolors {
            let idx = usize::try_from(i).unwrap_or(0) * 4;
...
pub fn lodepng_chunk_next<'a>(chunk: Option<&'a [u8]>, end: Option<&'a [u8]>) -> Option<&'a [u8]> {
    ...
    // Calculate available_size (end - chunk in C terms)
    let available_size = end.as_ptr() as usize - chunk.as_ptr() as usize;
    
    // Check conditions from line 4-7
    if chunk.as_ptr() >= end.as_ptr() || available_size < 12 {
        return Some(end);
    }

    // Check PNG header signature (line 8-11)
    if chunk.len() >= 8 && chunk[0] == 0x89 && chunk[1] == 0x50 && chunk[2] == 0x4e && 
       chunk[3] == 0x47 && chunk[4] == 0x0d && chunk[5] == 0x0a && 
       chunk[6] == 0x1a && chunk[7] == 0x0a {
        return Some(&chunk[8..]);
    }

    // Handle chunk length calculation (line 14-24)
    let chunk_length = lodepng_chunk_length(chunk) as usize;
    let mut total_chunk_length = 0;
...
pub fn addUnknownChunks(out: &mut Ucvector, data: Option<&[u8]>, datasize: usize) -> u32 {
    let data = match data {
        Some(d) => d,
        None => return 1, // Assuming 1 represents an error in this context
    };
    ...
    while inchunk_idx < datasize {
        let chunk = &inchunk[inchunk_idx..];
        let error = lodepng_chunk_append(&mut out.data, &mut out.size, chunk);
        if error != 0 {
            return error;
        }
        ...
        let next_chunk = lodepng_chunk_next(Some(&inchunk[inchunk_idx..]), Some(&data[datasize..]));
        inchunk_idx = match next_chunk {
            Some(c) => c.as_ptr() as usize - data.as_ptr() as usize,
            None => break,
        };
    }
...
pub fn lodepng_chunk_init<'a>(
    ...
    let chunk_slice = &mut data[chunk_start..chunk_start + length + 12];
    
    // Split the chunk slice into parts for different operations before storing in chunk
    let (length_part, rest) = chunk_slice.split_at_mut(4);
    let (type_part, _) = rest.split_at_mut(4);

    lodepng_set32bitInt(length_part, length as u32);
    lodepng_memcpy(type_part, type_);

    *chunk = Some(chunk_slice);

    0
}
pub fn lodepng_chunk_generate_crc(chunk: &mut [u8]) {
    let length = lodepng_chunk_length(chunk) as usize;
    let crc_data = &chunk[4..4 + length + 4];
    let crc = lodepng_crc32(Some(crc_data), crc_data.len());
    let crc_position = 8 + length;
    lodepng_set32bitInt(&mut chunk[crc_position..crc_position + 4], crc);
}
...
pub fn addChunk_zTXt(
    ...
    let text_bytes = textstring.map(|s| s.as_bytes()).unwrap_or(&[]);
    error = zlib_compress(&mut compressed, &mut compressedsize, text_bytes, zlibsettings);
    ...
    if error == 0 {
        if let Some(chunk_data) = chunk.as_mut() {
            ...
            if let Some(compressed_data) = compressed.as_ref() {
                lodepng_memcpy(
    ...
    lodepng_free(compressed.map(|v| v.as_ptr() as *mut c_void));
    error
}
    ...
    match info.colortype {
        ...
        LodePNGColorType::LCT_GREY => {
            if info.key_defined != 0 {
                ...
                if let Some(chunk_data) = &mut chunk {
                    chunk_data[8] = (info.key_r >> 8).try_into().unwrap();
                    chunk_data[9] = (info.key_r & 255).try_into().unwrap();
                }
            }
        }
        LodePNGColorType::LCT_RGB => {
            if info.key_defined != 0 {
                ...
                if let Some(chunk_data) = &mut chunk {
                    chunk_data[8] = (info.key_r >> 8).try_into().unwrap();
                    chunk_data[9] = (info.key_r & 255).try_into().unwrap();
                    chunk_data[10] = (info.key_g >> 8).try_into().unwrap();
                    chunk_data[11] = (info.key_g & 255).try_into().unwrap();
                    chunk_data[12] = (info.key_b >> 8).try_into().unwrap();
                    chunk_data[13] = (info.key_b & 255).try_into().unwrap();
                }
            }
        }
        _ => {}
...
pub fn addChunk_tIME(out: &mut Ucvector, time: &LodePngTime) -> u32 {
    ...
    if let Some(chunk_slice) = &mut chunk {
        chunk_slice[8] = (time.year >> 8) as u8;
        chunk_slice[9] = (time.year & 255) as u8;
        chunk_slice[10] = time.month.try_into().unwrap();
        chunk_slice[11] = time.day.try_into().unwrap();
        chunk_slice[12] = time.hour.try_into().unwrap();
        chunk_slice[13] = time.minute.try_into().unwrap();
        chunk_slice[14] = time.second.try_into().unwrap();
    }
...
pub fn lodepng_chunk_createv(
    ...
    if let Some(chunk) = chunk {
        lodepng_memcpy(&mut chunk[8..], data);
...
pub fn addChunk_pHYs(out: &mut Ucvector, info: &LodePngInfo) -> u32 {
    ...
    if let Some(chunk) = &mut chunk {
        lodepng_set32bitInt(&mut chunk[8..12], info.phys_x);
        lodepng_set32bitInt(&mut chunk[12..16], info.phys_y);
        chunk[16] = info.phys_unit as u8;
        lodepng_chunk_generate_crc(chunk);
    }
...
pub fn addChunk_iTXt(
    ...
    if error == 0 {
        let size = keysize + 3 + langsize + 1 + transsize + 1 + 
                  if compress != 0 { compressedsize } else { textsize };
    ...
    if error == 0 {
        ...
        lodepng_memcpy(
            &mut chunk[pos..pos + keysize],
            keyword.unwrap().as_bytes(),
        );
        pos += keysize;
        
        chunk[pos] = 0;
        pos += 1;
        chunk[pos] = if compress != 0 { 1 } else { 0 };
        pos += 1;
        chunk[pos] = 0;
        pos += 1;
        
        lodepng_memcpy(
            &mut chunk[pos..pos + langsize],
            langtag.unwrap().as_bytes(),
        );
        pos += langsize;
        chunk[pos] = 0;
        pos += 1;
        ...
        lodepng_memcpy(
            &mut chunk[pos..pos + transsize],
            transkey.unwrap().as_bytes(),
        );
        pos += transsize;
        chunk[pos] = 0;
        pos += 1;
        ...
        if compress != 0 {
            lodepng_memcpy(
                &mut chunk[pos..pos + compressedsize],
                compressed.as_ref().unwrap(),
            );
        } else {
            lodepng_memcpy(
                &mut chunk[pos..pos + textsize],
                textstring.unwrap().as_bytes(),
            );
        }
    ...
    lodepng_free(compressed.map(|mut v| v.as_mut_ptr() as *mut c_void));
    error
}
pub fn addChunk_iCCP(
    ...
    if let Some(profile) = &info.iccp_profile {
        error = zlib_compress(&mut compressed, &mut compressedsize, profile, zlibsettings);
    } else {
        return 1; // Return error if profile is None
    }
    ...
    if error == 0 {
        if let Some(chunk_data) = &mut chunk {
            if let Some(name) = &info.iccp_name {
                lodepng_memcpy(&mut chunk_data[8..8 + keysize], name.as_bytes());
            }
            chunk_data[8 + keysize] = 0;
            chunk_data[9 + keysize] = 0;
    ...
    if let Some(compressed_vec) = compressed {
        lodepng_free(Some(compressed_vec.as_ptr() as *mut std::ffi::c_void));
    }
    error
}
pub fn addChunk_gAMA(out: &mut Ucvector, info: &LodePngInfo) -> u32 {
    let mut chunk: Option<&mut [u8]> = Option::None;
    let error = {
        lodepng_chunk_init(&mut chunk, out, 4, b"gAMA")
    };
    if error != 0 {
        return error;
    }
    ...
    if let Some(chunk_slice) = &mut chunk {
        lodepng_set32bitInt(&mut chunk_slice[8..], info.gama_gamma);
        lodepng_chunk_generate_crc(chunk_slice);
    }
...
pub fn addChunk_cHRM(out: &mut Ucvector, info: &LodePngInfo) -> u32 {
    ...
    if let Some(chunk_slice) = &mut chunk {
        lodepng_set32bitInt(&mut chunk_slice[8..12], info.chrm_white_x);
        lodepng_set32bitInt(&mut chunk_slice[12..16], info.chrm_white_y);
        lodepng_set32bitInt(&mut chunk_slice[16..20], info.chrm_red_x);
        lodepng_set32bitInt(&mut chunk_slice[20..24], info.chrm_red_y);
        lodepng_set32bitInt(&mut chunk_slice[24..28], info.chrm_green_x);
        lodepng_set32bitInt(&mut chunk_slice[28..32], info.chrm_green_y);
        lodepng_set32bitInt(&mut chunk_slice[32..36], info.chrm_blue_x);
        lodepng_set32bitInt(&mut chunk_slice[36..40], info.chrm_blue_y);
...
pub fn addChunk_IHDR(
    ...
    let chunk = chunk.unwrap(); // Safe because we checked error above
    let data = &mut chunk[8..]; // data starts after 8-byte header
...
pub fn addChunk_IDAT(
    ...
    let error = zlib_compress(&mut zlib, &mut zlibsize, data, zlibsettings);
    
    if error == 0 {
        let error = lodepng_chunk_createv(
            out,
            zlibsize,
            b"IDAT",
            zlib.as_ref().unwrap_or(&Vec::new()).as_slice(),
        );
        lodepng_free(zlib.map(|v| v.as_ptr() as *mut c_void));
        error
    } else {
        lodepng_free(zlib.map(|v| v.as_ptr() as *mut c_void));
        error
...
pub fn lodepng_encode(
    ...
    *outsize = 0;
    ...
    if state.encoder.auto_convert != 0 {
        ...
        match lodepng_compute_color_stats(&mut stats, image, w, h, &state.info_raw) {
            Ok(()) => (),
            Err(e) => {
                state.error = e;
                return state.error;
        ...
        if info_png.background_defined != 0 {
            ...
            match lodepng_color_stats_add(&mut stats, r, g, b, 65535) {
                Ok(()) => (),
                Err(e) => {
                    state.error = e;
                    return state.error;
        ...
        if info_png.sbit_defined != 0 {
            ...
            let sbit_max = sbit_values.iter().max().unwrap_or(&0);
    ...
    if !lodepng_color_mode_equal(&state.info_raw, &info.color) {
        ...
        let mut converted = vec![0; size];
        
        state.error = lodepng_convert(&mut converted, image, &info.color, &state.info_raw, w, h);
        if state.error != 0 {
            return state.error;
        }

        state.error = preProcessScanlines(&mut data, &mut datasize, &converted, w, h, &info, &state.encoder);
        if state.error != 0 {
            return state.error;
        }
    } else {
    ...
    if let Some(chunk_data) = &info.unknown_chunks_data[0] {
        state.error = addUnknownChunks(&mut outv, Some(chunk_data), info.unknown_chunks_size[0]);
        if state.error != 0 {
            return state.error;
        }
    }
    ...
    if let Some(chunk_data) = &info.unknown_chunks_data[1] {
        state.error = addUnknownChunks(&mut outv, Some(chunk_data), info.unknown_chunks_size[1]);
        if state.error != 0 {
            return state.error;
        }
    }

    state.error = addChunk_IDAT(&mut outv, data.as_ref().unwrap(), &state.encoder.zlibsettings);
    if state.error != 0 {
        return state.error;
    }
    ...
    for i in 0..info.text_num {
        let key = info.text_keys.as_ref().unwrap()[i].as_str();
        if lodepng_strlen(Some(key)) > 79 {
            state.error = 66;
            return state.error;
        }
        if lodepng_strlen(Some(key)) < 1 {
            state.error = 67;
            return state.error;
        }

        let text = info.text_strings.as_ref().unwrap()[i].as_str();
        if state.encoder.text_compression != 0 {
            state.error = addChunk_zTXt(&mut outv, Some(key), Some(text), &state.encoder.zlibsettings);
        } else {
            state.error = addChunk_tEXt(&mut outv, Some(key), Some(text));
        }
        if state.error != 0 {
            return state.error;
        }
    ...
    if state.encoder.add_id != 0 {
        ...
        for i in 0..info.text_num {
            let k = info.text_keys.as_ref().unwrap()[i].as_str();
            if k == "LodePNG" {
        ...
        if !already_added_id_text {
            state.error = addChunk_tEXt(&mut outv, Some("LodePNG"), Some(&LODEPNG_VERSION_STRING));
            if state.error != 0 {
                return state.error;
            }
        }
    ...
    for i in 0..info.itext_num {
        let key = info.itext_keys.as_ref().unwrap()[i].as_str();
        if lodepng_strlen(Some(key)) > 79 {
            state.error = 66;
            return state.error;
        }
        ...
        let lang = info.itext_langtags.as_ref().unwrap()[i].as_str();
        let trans = info.itext_transkeys.as_ref().unwrap()[i].as_str();
        let text = info.itext_strings.as_ref().unwrap()[i].as_str();
    ...
    if let Some(chunk_data) = &info.unknown_chunks_data[2] {
        state.error = addUnknownChunks(&mut outv, Some(chunk_data), info.unknown_chunks_size[2]);
        if state.error != 0 {
            return state.error;
        }
    }
...
pub fn lodepng_chunk_find<'a>(
    ...
    loop {
        ...
        // Check bounds - Rust slices are exclusive, so we use < instead of <=
        if chunk_idx >= current_chunk.len() || 
           (current_end.as_ptr() as usize) <= (current_chunk[chunk_idx..].as_ptr() as usize) ||
           (current_end.as_ptr() as usize) - (current_chunk[chunk_idx..].as_ptr() as usize) < 12 
        {
            return None;
        }

        // Check if current chunk matches the type
        if lodepng_chunk_type_equals(Some(&current_chunk[chunk_idx..]), type_) != 0 {
            return Some(&current_chunk[chunk_idx..]);
        }

        // Get next chunk position
        let next_chunk = lodepng_chunk_next(Some(&current_chunk[chunk_idx..]), end);
        match next_chunk {
            Some(next) => {
                chunk_idx = next.as_ptr() as usize - current_chunk.as_ptr() as usize;
            }
            None => return None,
        }
    }
...
pub fn lodepng_chunk_find_const<'a>(chunk: Option<&'a [u8]>, end: Option<&'a [u8]>, type_: Option<&'a str>) -> Option<&'a [u8]> {
    ...
    loop {
        if chunk_idx >= chunk.len() || 
           end.as_ptr() as usize <= chunk.as_ptr() as usize + chunk_idx || 
           (end.as_ptr() as usize).saturating_sub((chunk.as_ptr() as usize) + chunk_idx) < 12 {
            return Option::None;
        }
        ...
        match lodepng_chunk_next_const(Some(current_chunk), Some(end)) {
            Some(next_chunk) => {
...
impl Clone for LodePNGState {
    fn clone(&self) -> Self {
        LodePNGState {
            decoder: LodePNGDecoderSettings {
                zlibsettings: LodePNGDecompressSettings {
                    ignore_adler32: self.decoder.zlibsettings.ignore_adler32,
                    ignore_nlen: self.decoder.zlibsettings.ignore_nlen,
                    max_output_size: self.decoder.zlibsettings.max_output_size,
                    custom_zlib: self.decoder.zlibsettings.custom_zlib,
                    custom_inflate: self.decoder.zlibsettings.custom_inflate,
                    custom_context: None, // Can't clone Box<c_void>, set to None
                },
                ignore_crc: self.decoder.ignore_crc,
                ignore_critical: self.decoder.ignore_critical,
                ignore_end: self.decoder.ignore_end,
                color_convert: self.decoder.color_convert,
                read_text_chunks: self.decoder.read_text_chunks,
                remember_unknown_chunks: self.decoder.remember_unknown_chunks,
                max_text_size: self.decoder.max_text_size,
                max_icc_size: self.decoder.max_icc_size,
            },
            encoder: self.encoder.clone(),
            info_raw: self.info_raw.clone(),
            info_png: self.info_png.clone(),
            error: self.error,
        }
    }
}
...
pub fn lodepng_state_copy(dest: &mut LodePNGState, source: &LodePNGState) {
    // Cleanup the destination state first
    lodepng_state_cleanup(dest);
    
    // Copy the entire state (this performs a bitwise copy)
    *dest = source.clone();
    ...
    // Copy color mode and check for errors
    dest.error = lodepng_color_mode_copy(&mut dest.info_raw, &source.info_raw);
    if dest.error != 0 {
        return;
    }
    
    // Copy PNG info and check for errors
