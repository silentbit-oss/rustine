 ...
 */
#[no_mangle]
pub unsafe extern "C" fn ti_stoch_start(options: *const f64) -> i32 {
    let options_slice = std::slice::from_raw_parts(options, 3);
    
    let kperiod = options_slice[0] as i32;
    let kslow = options_slice[1] as i32;
    let dperiod = options_slice[2] as i32;
...
pub unsafe extern "C" fn ti_stoch(mut size: std::os::raw::c_int,
 ...
 -> std::os::raw::c_int {
    ...
    let k_sum_box = Box::from_raw(k_sum);
ti_buffer_free(k_sum_box);
    let d_sum_box = Box::from_raw(d_sum);
ti_buffer_free(d_sum_box);
