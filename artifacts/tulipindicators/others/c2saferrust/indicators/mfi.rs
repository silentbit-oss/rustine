...
#[no_mangle]
pub unsafe extern "C" fn ti_mfi_start(options: *const f64) -> i32 {
    if options.is_null() {
        return 0;
    }
    *options as i32
...
pub unsafe extern "C" fn ti_mfi(mut size: std::os::raw::c_int,
 ...
 -> std::os::raw::c_int {
    ...
    let up_box = Box::from_raw(up);
ti_buffer_free(up_box);
    let down_box = Box::from_raw(down);
ti_buffer_free(down_box);
