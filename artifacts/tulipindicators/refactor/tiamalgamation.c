...
int ti_aroon(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  for (i = period; i < size; i += 1, trail += 1)
  {
    ...
    adown_idx += 1;
    aup[aup_idx] = (((double) period) - (i - maxi)) * scale;
    aup_idx += 1;
  }

  assert(((&adown[adown_idx]) - outputs[0]) == (size - ti_aroon_start(options)));
  assert(((&aup[aup_idx]) - outputs[1]) == (size - ti_aroon_start(options)));
  return 0;
}
...
int ti_fisher(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  assert(((&fisher[fisher_idx]) - outputs[0]) == (size - ti_fisher_start(options)));
...
int ti_macd(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  assert(((&hist[hist_idx]) - outputs[2]) == (size - ti_macd_start(options)));
  return 0;
}
