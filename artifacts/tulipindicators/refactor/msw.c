...
int ti_msw(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  {
    helper_ti_msw_1(&sine_idx, &lead_idx, &weight, &phase, &rp, &ip, &j, input, sine, lead, period, pi, tpi, i);
  }

  assert(((&sine[sine_idx]) - outputs[0]) == (size - ti_msw_start(options)));
  assert(((&lead[lead_idx]) - outputs[1]) == (size - ti_msw_start(options)));
  return 0;
}
...
int ti_msw(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  for (i = period; i < size; i += 1)
  {
    ...
    sine_idx += 1;
    lead[lead_idx] = sin(phase + (pi / 4.0));
    lead_idx += 1;
  }
...
void helper_ti_msw_1(unsigned int * const sine_idx_ref, unsigned int * const lead_idx_ref, double * const weight_ref, double * const phase_ref, double * const rp_ref, double * const ip_ref, int * const j_ref, const double * const input, double * const sine, double * const lead, const int period, const double pi, const double tpi, int i)
{
  unsigned int sine_idx = *sine_idx_ref;
  unsigned int lead_idx = *lead_idx_ref;
