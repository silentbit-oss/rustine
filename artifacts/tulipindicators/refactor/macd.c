...
int ti_macd(int size, const double * const *inputs, const double *options, double * const *outputs)
{
  ...
  for (i = 1; i < size; i += 1)
  {
    helper_ti_macd_1(&macd_idx, &signal_idx, &hist_idx, &short_ema, &long_ema, &signal_ema, input, macd, signal, hist, long_period, short_per, long_per, signal_per, i);
  }
...
void helper_ti_macd_1(unsigned int * const macd_idx_ref, unsigned int * const signal_idx_ref, unsigned int * const hist_idx_ref, double * const short_ema_ref, double * const long_ema_ref, double * const signal_ema_ref, const double * const input, double * const macd, double * const signal, double * const hist, const int long_period, double short_per, double long_per, double signal_per, int i)
{
  unsigned int macd_idx = *macd_idx_ref;
  unsigned int signal_idx = *signal_idx_ref;
  unsigned int hist_idx = *hist_idx_ref;
