...
pub fn ti_dema_start(options: Option<&[f64]>) -> Option<i32> {
    ...
    // Extract period from options[0] and cast to i32
...
pub fn ti_dema(
    ...
    let start = ti_dema_start(Some(options?))?;
    if size <= start {
        return Some(0);
    }
    ...
    let calculated_size = size - start;
    assert!((output_idx as i32) == calculated_size);
