...
pub fn helper_ti_linregintercept_1(
    ...
    let mut y = 0.0;
    let mut xy = 0.0;
    let p = 1.0 / f64::from(period);

    for i in 0..(period - 1) {
        let i_plus_1 = f64::from(i + 1);
        x += i_plus_1;
    ...
    for i in (period - 1)..size {
        ...
        {
            let a = (y - (b * x)) * p;
            output[output_idx as usize] = a + (b * 1.0);
            output_idx += 1;
        }
...
pub fn ti_linregintercept(
    ...
    let start = match ti_linregintercept_start(options) {
        Some(s) => s,
        None => return Some(1),
    };
    ...
    helper_ti_linregintercept_1(&mut output_idx, size, input, period, output);

    // Assert equivalent - we'll just check the condition and return an error if it fails
    let expected_diff = size - start;
    let actual_diff = (output.len() - output_idx as usize) as i32;
