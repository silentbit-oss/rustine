...
pub fn tc_run(patterns: tc_set, size: i32, inputs: &[&[TC_REAL]], options: &TcConfig, output: &mut TcResult) -> i32 {
    ...
    let period = options.period;
    let div = 1.0 / period as TC_REAL;
    let mut avg_body_sum: TC_REAL = 0.0;
    let mut avg_total_sum: TC_REAL = 0.0;
    if period < 1 {
        return 1;
    }
    if size < period {
    ...
    while i < size {
        ...
        if ((1 << 0) & patterns) != 0 && i >= 2 
            && open[(i - 2) as usize] < close[(i - 2) as usize]
            && f64::abs(open[(i - 2) as usize] - close[(i - 2) as usize]) > (options.body_long * avg_body)
            && low[(i - 1) as usize] >= high[((i - 1) - 1) as usize]
            && f64::abs(open[(i - 1) as usize] - close[(i - 1) as usize]) < (options.body_none * avg_total)
            && open[i as usize] > close[i as usize]
            && high[i as usize] <= low[(i - 1) as usize]
            && close[i as usize] <= close[(i - 2) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 0 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 1: Abandoned Baby Bull
        if ((1 << 1) & patterns) != 0 && i >= 2 
            && open[(i - 2) as usize] > close[(i - 2) as usize]
            && f64::abs(open[(i - 2) as usize] - close[(i - 2) as usize]) > (options.body_long * avg_body)
            && high[(i - 1) as usize] <= low[((i - 1) - 1) as usize]
            && f64::abs(open[(i - 1) as usize] - close[(i - 1) as usize]) < (options.body_none * avg_total)
            && open[i as usize] < close[i as usize]
            && low[i as usize] >= high[(i - 1) as usize]
            && close[i as usize] >= close[(i - 2) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 1 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 2) & patterns) != 0 
            && open[i as usize] > close[i as usize]
            && f64::abs(open[i as usize] - close[i as usize]) > (options.body_long * avg_body) {
            let hit = TcHit { index: i, patterns: 1 << 2 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 3) & patterns) != 0 
            && open[i as usize] < close[i as usize]
            && f64::abs(open[i as usize] - close[i as usize]) > (options.body_long * avg_body) {
            let hit = TcHit { index: i, patterns: 1 << 3 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 4) & patterns) != 0 
            ...
            && open[i as usize] > close[i as usize] {
            let hit = TcHit { index: i, patterns: 1 << 4 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        // Pattern 5: Doji
        if ((1 << 5) & patterns) != 0 
            && f64::abs(open[i as usize] - close[i as usize]) < (options.body_none * avg_total) {
            let hit = TcHit { index: i, patterns: 1 << 5 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 6) & patterns) != 0 
            ...
            && (high[i as usize] - top) < (options.wick_none * avg_total)
            && (bottom - low[i as usize]) > (options.wick_long * avg_body) {
            let hit = TcHit { index: i, patterns: 1 << 6 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 7: Engulfing Bear
        if ((1 << 7) & patterns) != 0 && i >= 1 
            && open[i as usize] > close[i as usize]
            && open[(i - 1) as usize] < close[(i - 1) as usize]
            && close[i as usize] < open[(i - 1) as usize]
            && open[i as usize] > close[(i - 1) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 7 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 8: Engulfing Bull
        if ((1 << 8) & patterns) != 0 && i >= 1 
            && open[i as usize] < close[i as usize]
            && open[(i - 1) as usize] > close[(i - 1) as usize]
            && close[i as usize] > open[(i - 1) as usize]
            && open[i as usize] < close[(i - 1) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 8 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 9: Evening Doji Star
        if ((1 << 9) & patterns) != 0 && i >= 2 
            && open[(i - 2) as usize] < close[(i - 2) as usize]
            && f64::abs(open[(i - 2) as usize] - close[(i - 2) as usize]) > (options.body_long * avg_body)
            && (if open[(i - 1) as usize] < close[(i - 1) as usize] { open[(i - 1) as usize] } else { close[(i - 1) as usize] }) >= (if open[((i - 1) - 1) as usize] > close[((i - 1) - 1) as usize] { open[((i - 1) - 1) as usize] } else { close[((i - 1) - 1) as usize] })
            && f64::abs(open[(i - 1) as usize] - close[(i - 1) as usize]) < (options.body_none * avg_total)
            && open[i as usize] > close[i as usize]
            && (if open[i as usize] > close[i as usize] { open[i as usize] } else { close[i as usize] }) <= (if open[(i - 1) as usize] < close[(i - 1) as usize] { open[(i - 1) as usize] } else { close[(i - 1) as usize] })
            && close[i as usize] <= close[(i - 2) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 9 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 10: Evening Star
        if ((1 << 10) & patterns) != 0 && i >= 2 
            && open[(i - 2) as usize] < close[(i - 2) as usize]
            && f64::abs(open[(i - 2) as usize] - close[(i - 2) as usize]) > (options.body_long * avg_body)
            && (if open[(i - 1) as usize] < close[(i - 1) as usize] { open[(i - 1) as usize] } else { close[(i - 1) as usize] }) >= (if open[((i - 1) - 1) as usize] > close[((i - 1) - 1) as usize] { open[((i - 1) - 1) as usize] } else { close[((i - 1) - 1) as usize] })
            && f64::abs(open[(i - 1) as usize] - close[(i - 1) as usize]) < (options.body_short * avg_body)
            && open[i as usize] > close[i as usize]
            && (if open[i as usize] > close[i as usize] { open[i as usize] } else { close[i as usize] }) <= (if open[(i - 1) as usize] < close[(i - 1) as usize] { open[(i - 1) as usize] } else { close[(i - 1) as usize] })
            && close[i as usize] <= close[(i - 2) as usize] {
            let hit = TcHit { index: i, patterns: 1 << 10 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 11) & patterns) != 0 
            ...
            && (high[i as usize] - top) < (options.wick_none * avg_total)
            && (bottom - low[i as usize]) < (options.wick_none * avg_total) {
            let hit = TcHit { index: i, patterns: 1 << 11 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        ...
        if ((1 << 12) & patterns) != 0 
            ...
            && (bottom - low[i as usize]) < (options.wick_none * avg_total) {
            let hit = TcHit { index: i, patterns: 1 << 12 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 13: Hammer
        if ((1 << 13) & patterns) != 0 && i >= 1 
            && f64::abs(open[i as usize] - close[i as usize]) < (options.body_short * avg_body)
            && (high[i as usize] - top) < (options.wick_none * avg_total)
            && (bottom - low[i as usize]) > (options.wick_long * avg_body)
            && (if open[i as usize] < close[i as usize] { open[i as usize] } else { close[i as usize] }) <= (low[(i - 1) as usize] + (options.near * avg_total)) {
            let hit = TcHit { index: i, patterns: 1 << 13 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
        
        // Pattern 14: Hanging Man
        if ((1 << 14) & patterns) != 0 && i >= 1 
            && f64::abs(open[i as usize] - close[i as usize]) < (options.body_short * avg_body)
            && (high[i as usize] - top) < (options.wick_none * avg_total)
            && (bottom - low[i as usize]) > (options.wick_long * avg_body)
            && (if open[i as usize] < close[i as usize] { open[i as usize] } else { close[i as usize] }) >= (high[(i - 1) as usize] - (options.near * avg_total)) {
            let hit = TcHit { index: i, patterns: 1 << 14 };
            let r = tc_result_add(output, hit);
            if r != 0 { return r; }
        }
