...
pub fn ti_mfi(
    ...
    if inputs.len() < 4 {
        return 0;//Err("inputs must have at least 4 elements".into());
    }
    let options = options.unwrap();//ok_or("options is None")?;
    if options.is_empty() {
        return 0;//Err("options must have at least 1 element".into());
    }
    let outputs = outputs.unwrap();//ok_or("outputs is None")?;
    if outputs.is_empty() {
        return 0;//Err("outputs must have at least 1 element".into());
    }
    ...
    let mut up = ti_buffer_new(period).unwrap();
    let mut down = ti_buffer_new(period).unwrap();
