...
pub fn next_line(reader: &mut BufReader<File>) -> Option<String> {
	println!("\n* next_line");
    ...
    for line in reader.lines() {
        ...
        //    if buf[0] == b'#' || buf[0] == 10 || buf[0] == 13 || buf[0] == 0 {
        //        continue;
        //    }
        //    
...
pub fn get_array(reader: &mut BufReader<File>, s: &mut [f64]) -> Option<usize> {
	println!("\n* get_array");

    let mut line = String::new();
    reader.read_line(&mut line).expect("Failed to read options line");
    
    let numbers: Vec<f64> = line
        .trim() // Remove newline
        .trim_matches(|c| c == '{' || c == '}') // Remove braces
        .split(',') // Split by comma
        .filter_map(|s| s.trim().parse::<f64>().ok()) // Convert to f64, ignore invalid parses
        .collect();
    
    s[..numbers.len()].copy_from_slice(&numbers);
    Some(numbers.len())
}
...
pub fn test_ind_name(reader: &mut BufReader<File>, name: Option<&str>, count: i32) {
	println!("\n* test_ind_name");
    let name: &str = name.unwrap().split_whitespace().collect::<Vec<_>>()[0];
    let (info, ind_num) = match ti_find_indicator(Some(name)) {
        Some(i) => {
    ...
    //    .filter_map(|s| s.trim().parse::<f64>().ok()) // Convert to f64, ignore invalid parses
    //    .collect();
    //for (i, &num) in numbers.iter().enumerate().take(options.len()) {
    //    options[i] = num;
    //    o_idx += 1
    //}
    //println!("===> {:?} \n\n {:?}", line, options);
    ...
    for i in 0..info.inputs {
        ...
        input_size = get_array(reader, &mut inputs[i as usize]).expect("Failed to read input array");
    }
    ...
    for i in 0..info.outputs {
        ...
        answer_size = get_array(reader, &mut answers[i as usize]).expect("Failed to read answer array");
    }
...
pub fn test(fname: Option<&str>, count: i32) {
    ...
    loop {
        let mut line = match next_line(&mut reader) {
            Some(l) => l,
            None => break,
        };
        if !(line.starts_with(|c: char| c.is_ascii_lowercase() && ('a'..='z').contains(&c))) {
            continue;
        }
        ...
        if name == "candle" {
            ...
            let start = Instant::now();
            print!("\t{:16}", name);
            test_ind_name(&mut reader, Some(name), count);
            let elapsed = start.elapsed().as_millis();
            println!("pass:{:2}   fail:{:2}   {:4}ms", 
                     LTESTS.load(Ordering::SeqCst) - ts - (LFAILS.load(Ordering::SeqCst) - fs),
                     LFAILS.load(Ordering::SeqCst) - fs,
