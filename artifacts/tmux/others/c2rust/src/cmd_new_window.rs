...
pub static mut cmd_new_window_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"new-window\0" as *const u8 as *const libc::c_char,
            alias: b"neww\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"ac:dF:kn:Pt:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: -(1 as libc::c_int),
                };
                init
            },
            usage: b"[-adkP] [-c start-directory] [-F format] [-n window-name] [-t target-window] [command]\0"
...
unsafe extern "C" fn cmd_new_window_exec(
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 {
        ...
        if idx == -(1 as libc::c_int) {
            cmdq_error(
                item,
                b"no free window indexes\0" as *const u8 as *const libc::c_char,
            );
    ...
    if !tmp.is_null() {
        name = format_single(item, tmp, c, s, 0 as *mut winlink, 0 as *mut window_pane);
    } else {
    ...
    if !wl.is_null() && args_has(args, 'k' as i32 as u_char) != 0 {
    ...
    wl = session_new(s, name, argc, argv, path, cwd, idx, &mut cause);
    if wl.is_null() {
        cmdq_error(
            item,
            b"create window failed: %s\0" as *const u8 as *const libc::c_char,
            cause,
        );
        ...
        if args_has(args, 'P' as i32 as u_char) != 0 {
            ...
            cp = format_single(item, template, c, s, wl, 0 as *mut window_pane);
            cmdq_print(item, b"%s\0" as *const u8 as *const libc::c_char, cp);
        ...
        hooks_insert(
            ...
            b"after-new-window\0" as *const u8 as *const libc::c_char,
        );
        free(name as *mut libc::c_void);
        free(cwd as *mut libc::c_void);
