...
extern "C" {
    ...
    fn window_copy_init_from_pane(_: *mut window_pane, _: libc::c_int);
        ...
        let mut init = cmd_entry {
            ...
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"Met:u\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-Mu] [-t target-pane]\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn cmd_copy_mode_exec(
    ...
    if args_has(args, 'M' as i32 as u_char) != 0 {
        wp = cmd_mouse_pane(&mut (*shared).mouse, &mut s, 0 as *mut *mut winlink);
        if wp.is_null() {
    ...
    if (*self_0).entry == &cmd_clock_mode_entry as *const cmd_entry {
    ...
    if (*wp).mode != &window_copy_mode as *const window_mode {
        ...
        window_copy_init_from_pane(wp, args_has((*self_0).args, 'e' as i32 as u_char));
    }
    if args_has(args, 'M' as i32 as u_char) != 0 {
        if !((*wp).mode).is_null()
        ...
        window_copy_start_drag(c, &mut (*shared).mouse);
    }
    if (*wp).mode == &window_copy_mode as *const window_mode
        && args_has((*self_0).args, 'u' as i32 as u_char) != 0
    {
