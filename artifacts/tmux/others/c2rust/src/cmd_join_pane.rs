...
pub static mut cmd_join_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"join-pane\0" as *const u8 as *const libc::c_char,
            alias: b"joinp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"bdhvp:l:s:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-bdhv] [-p percentage|-l size] [-s src-pane] [-t dst-pane]\0"
...
pub static mut cmd_move_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"move-pane\0" as *const u8 as *const libc::c_char,
            alias: b"movep\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"bdhvp:l:s:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
...
unsafe extern "C" fn cmd_join_pane_exec(
    ...
    let mut not_same_window: libc::c_int = 0;
    if (*self_0).entry == &cmd_join_pane_entry as *const cmd_entry {
        not_same_window = 1 as libc::c_int;
    } else {
        not_same_window = 0 as libc::c_int;
    }
    dst_s = (*item).target.s;
    ...
    if not_same_window != 0 && src_w == dst_w {
        cmdq_error(
            item,
            b"can't join a pane to its own window\0" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    if not_same_window == 0 && src_wp == dst_wp {
        cmdq_error(
            item,
    ...
    if args_has(args, 'l' as i32 as u_char) != 0 {
        ...
        if !cause.is_null() {
            cmdq_error(item, b"size %s\0" as *const u8 as *const libc::c_char, cause);
        ...
        if !cause.is_null() {
            cmdq_error(
                item,
                b"percentage %s\0" as *const u8 as *const libc::c_char,
                cause,
            );
            free(cause as *mut libc::c_void);
            return CMD_RETURN_ERROR;
        ...
        if type_0 as libc::c_uint == LAYOUT_TOPBOTTOM as libc::c_int as libc::c_uint {
            size = ((*dst_wp).sy * percentage as u_int / 100 as libc::c_int as u_int)
                as libc::c_int;
        } else {
            size = ((*dst_wp).sx * percentage as u_int / 100 as libc::c_int as u_int)
                as libc::c_int;
        }
    }
    lc = layout_split_pane(
        ...
        type_0,
        size,
        args_has(args, 'b' as i32 as u_char),
        0 as libc::c_int,
    );
    if lc.is_null() {
        cmdq_error(
            item,
            b"create pane failed: pane too small\0" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    ...
    *(*src_wp).entry.tqe_prev = (*src_wp).entry.tqe_next;
    ...
    if window_count_panes(src_w) == 0 as libc::c_int as u_int {
    ...
    notify_window(b"window-layout-changed\0" as *const u8 as *const libc::c_char, dst_w);
