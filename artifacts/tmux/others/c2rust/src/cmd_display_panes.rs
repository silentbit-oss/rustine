...
extern "C" {
    ...
    fn server_client_set_identify(_: *mut client, _: u_int);
}
...
pub static mut cmd_display_panes_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"display-panes\0" as *const u8 as *const libc::c_char,
            alias: b"displayp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"d:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-d duration] [-t target-client]\0" as *const u8
...
unsafe extern "C" fn cmd_display_panes_exec(
    ...
    c = cmd_find_client(item, args_get(args, 't' as i32 as u_char), 0 as libc::c_int);
    if c.is_null() {
    ...
    (*c)
        .identify_callback = Some(
        cmd_display_panes_callback
            as unsafe extern "C" fn(*mut client, *mut window_pane) -> (),
    );
    if (*args).argc != 0 as libc::c_int {
        (*c)
            .identify_callback_data = xstrdup(
            *((*args).argv).offset(0 as libc::c_int as isize),
        ) as *mut libc::c_void;
    } else {
        (*c)
            .identify_callback_data = xstrdup(
            b"select-pane -t '%%'\0" as *const u8 as *const libc::c_char,
        ) as *mut libc::c_void;
    }
    s = (*c).session;
    if args_has(args, 'd' as i32 as u_char) != 0 {
        ...
        if !cause.is_null() {
            cmdq_error(item, b"delay %s\0" as *const u8 as *const libc::c_char, cause);
        ...
        delay = options_get_number(
            (*s).options,
            b"display-panes-time\0" as *const u8 as *const libc::c_char,
        ) as u_int;
    }
    server_client_set_identify(c, delay);
    return CMD_RETURN_NORMAL;
}
unsafe extern "C" fn cmd_display_panes_error(
    mut item: *mut cmdq_item,
...
unsafe extern "C" fn cmd_display_panes_callback(
    mut c: *mut client,
    ...
    let mut cause: *mut libc::c_char = 0 as *mut libc::c_char;
    template = (*c).identify_callback_data as *mut libc::c_char;
    if !wp.is_null() {
        xasprintf(
        ...
        cmd = cmd_template_replace(template, expanded, 1 as libc::c_int);
        ...
        if cmdlist.is_null() {
            if !cause.is_null() {
                new_item = cmdq_get_callback1(
                    b"cmd_display_panes_error\0" as *const u8 as *const libc::c_char,
                    Some(
    ...
    (*c).identify_callback_data = 0 as *mut libc::c_void;
    (*c).identify_callback = None;
}
