...
extern "C" {
    ...
    fn control_write_buffer(_: *mut client, _: *mut evbuffer);
}
...
#[no_mangle]
pub unsafe extern "C" fn control_notify_input(
    mut c: *mut client,
    ...
    buf = evbuffer_pullup(input, -(1 as libc::c_int) as ssize_t);
    len = evbuffer_get_length(input);
    if !(winlink_find_by_window(&mut (*(*c).session).windows, (*wp).window)).is_null() {
        message = evbuffer_new();
        evbuffer_add_printf(
            message,
            b"%%output %%%u \0" as *const u8 as *const libc::c_char,
            (*wp).id,
        );
...
pub unsafe extern "C" fn control_notify_pane_mode_changed(mut pane: libc::c_int) {
    ...
    while !c.is_null() {
        if !c.is_null() && (*c).flags & 0x2000 as libc::c_int != 0 {
            control_write(
                c,
                b"%%pane-mode-changed %%%u\0" as *const u8 as *const libc::c_char,
                pane,
            );
        }
        c = (*c).entry.tqe_next;
...
pub unsafe extern "C" fn control_notify_window_layout_changed(mut w: *mut window) {
    ...
    template = b"%layout-change #{window_id} #{window_layout} #{window_visible_layout} #{window_flags}\0"
...
pub unsafe extern "C" fn control_notify_window_pane_changed(mut w: *mut window) {
    ...
    while !c.is_null() {
        if !c.is_null() && (*c).flags & 0x2000 as libc::c_int != 0 {
            control_write(
                c,
                b"%%window-pane-changed @%u %%%u\0" as *const u8 as *const libc::c_char,
                (*w).id,
                (*(*w).active).id,
            );
        }
...
#[no_mangle]
pub unsafe extern "C" fn control_notify_window_unlinked(
    mut s: *mut session,
    mut w: *mut window,
) {
    let mut c: *mut client = 0 as *mut client;
    ...
    while !c.is_null() {
            ...
            || ((*c).session).is_null())
        {
            ...
            if !(winlink_find_by_window_id(&mut (*cs).windows, (*w).id)).is_null() {
                control_write(
                    c,
                    b"%%window-close @%u\0" as *const u8 as *const libc::c_char,
                    (*w).id,
                );
            } else {
                control_write(
                    c,
                    b"%%unlinked-window-close @%u\0" as *const u8 as *const libc::c_char,
                    (*w).id,
                );
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn control_notify_window_linked(
    mut s: *mut session,
    mut w: *mut window,
) {
    let mut c: *mut client = 0 as *mut client;
    ...
    while !c.is_null() {
            ...
            || ((*c).session).is_null())
        {
            ...
            if !(winlink_find_by_window_id(&mut (*cs).windows, (*w).id)).is_null() {
                control_write(
                    c,
                    b"%%window-add @%u\0" as *const u8 as *const libc::c_char,
                    (*w).id,
                );
            } else {
                control_write(
                    c,
                    b"%%unlinked-window-add @%u\0" as *const u8 as *const libc::c_char,
                    (*w).id,
                );
            }
        }
    ...
    while !c.is_null() {
            ...
            || ((*c).session).is_null())
        {
            ...
            if !(winlink_find_by_window_id(&mut (*cs).windows, (*w).id)).is_null() {
                control_write(
                    c,
                    b"%%window-renamed @%u %s\0" as *const u8 as *const libc::c_char,
                    (*w).id,
                    (*w).name,
                );
            } else {
    ...
    while !c.is_null() {
            ...
            || ((*c).session).is_null())
        {
            if cc == c {
                control_write(
                    c,
                    b"%%session-changed $%u %s\0" as *const u8 as *const libc::c_char,
                    (*s).id,
                    (*s).name,
                );
            } else {
...
pub unsafe extern "C" fn control_notify_session_renamed(mut s: *mut session) {
    ...
    while !c.is_null() {
        if !c.is_null() && (*c).flags & 0x2000 as libc::c_int != 0 {
            control_write(
                c,
                b"%%session-renamed $%u %s\0" as *const u8 as *const libc::c_char,
                (*s).id,
                (*s).name,
            );
        }
...
pub unsafe extern "C" fn control_notify_session_created(mut s: *mut session) {
    ...
    while !c.is_null() {
        if !c.is_null() && (*c).flags & 0x2000 as libc::c_int != 0 {
            control_write(
                c,
                b"%%sessions-changed\0" as *const u8 as *const libc::c_char,
            );
        }
        c = (*c).entry.tqe_next;
    }
...
pub unsafe extern "C" fn control_notify_session_window_changed(mut s: *mut session) {
    ...
    while !c.is_null() {
        if !c.is_null() && (*c).flags & 0x2000 as libc::c_int != 0 {
            control_write(
                c,
                b"%%session-window-changed $%u @%u\0" as *const u8
                    as *const libc::c_char,
                (*s).id,
                (*(*(*s).curw).window).id,
            );
