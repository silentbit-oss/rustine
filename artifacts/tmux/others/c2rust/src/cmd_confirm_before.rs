...
pub static mut cmd_confirm_before_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"confirm-before\0" as *const u8 as *const libc::c_char,
            alias: b"confirm\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"p:t:\0" as *const u8 as *const libc::c_char,
                    lower: 1 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-p prompt] [-t target-client] command\0" as *const u8
...
unsafe extern "C" fn cmd_confirm_before_exec(
    ...
    if !prompt.is_null() {
        ...
        ptr = copy;
        cmd = strsep(&mut ptr, b" \t\0" as *const u8 as *const libc::c_char);
        xasprintf(
            &mut new_prompt as *mut *mut libc::c_char,
            b"Confirm '%s'? (y/n) \0" as *const u8 as *const libc::c_char,
            cmd,
        );
        free(copy as *mut libc::c_void);
    }
    cdata = xmalloc(::core::mem::size_of::<cmd_confirm_before_data>() as libc::c_ulong)
    ...
    status_prompt_set(
        ...
        Some(cmd_confirm_before_free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
...
unsafe extern "C" fn cmd_confirm_before_error(
    mut item: *mut cmdq_item,
    ...
    if cmdlist.is_null() {
        if !cause.is_null() {
            new_item = cmdq_get_callback1(
                b"cmd_confirm_before_error\0" as *const u8 as *const libc::c_char,
                Some(
