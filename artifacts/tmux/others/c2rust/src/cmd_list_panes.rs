...
pub static mut cmd_list_panes_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"list-panes\0" as *const u8 as *const libc::c_char,
            alias: b"lsp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"asF:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-as] [-F format] [-t target-window]\0" as *const u8
...
unsafe extern "C" fn cmd_list_panes_window(
    ...
    if template.is_null() {
        match type_0 {
            0 => {
                template = b"#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\0"
                    as *const u8 as *const libc::c_char;
            }
            1 => {
                template = b"#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\0"
                    as *const u8 as *const libc::c_char;
            }
            2 => {
                template = b"#{session_name}:#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\0"
                    as *const u8 as *const libc::c_char;
            }
            _ => {}
        }
