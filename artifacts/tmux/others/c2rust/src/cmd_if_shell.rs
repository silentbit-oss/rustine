...
pub static mut cmd_if_shell_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"if-shell\0" as *const u8 as *const libc::c_char,
            alias: b"if\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"bFt:\0" as *const u8 as *const libc::c_char,
                    lower: 2 as libc::c_int,
                    upper: 3 as libc::c_int,
                };
                init
            },
            usage: b"[-bF] [-t target-pane] shell-command command [command]\0"
...
unsafe extern "C" fn cmd_if_shell_exec(
    ...
    if args_has(args, 'F' as i32 as u_char) != 0 {
        ...
        if *shellcmd as libc::c_int != '0' as i32
            && *shellcmd as libc::c_int != '\0' as i32
        {
    ...
    cdata = xcalloc(
        ...
        ::core::mem::size_of::<cmd_if_shell_data>() as libc::c_ulong,
    ) as *mut cmd_if_shell_data;
    if !((*self_0).file).is_null() {
        (*cdata).file = xstrdup((*self_0).file);
        (*cdata).line = (*self_0).line;
    }
    ...
    memcpy(
        &mut (*cdata).mouse as *mut mouse_event as *mut libc::c_void,
        &mut (*shared).mouse as *mut mouse_event as *const libc::c_void,
    ...
    job_run(
        ...
        Some(cmd_if_shell_free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
...
unsafe extern "C" fn cmd_if_shell_callback(mut job: *mut job) {
    let mut cdata: *mut cmd_if_shell_data = (*job).data as *mut cmd_if_shell_data;
    let mut c: *mut client = (*cdata).client;
    let mut cmdlist: *mut cmd_list = 0 as *mut cmd_list;
    let mut new_item: *mut cmdq_item = 0 as *mut cmdq_item;
    let mut cause: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut cmd: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut file: *mut libc::c_char = (*cdata).file;
    let mut line: u_int = (*cdata).line;
    if !((*job).status & 0x7f as libc::c_int == 0 as libc::c_int)
        || ((*job).status & 0xff00 as libc::c_int) >> 8 as libc::c_int
            != 0 as libc::c_int
    {
        cmd = (*cdata).cmd_else;
    } else {
    ...
    if !cmd.is_null() {
        cmdlist = cmd_string_parse(cmd, file, line, &mut cause);
        if cmdlist.is_null() {
            if !cause.is_null() && !((*cdata).item).is_null() {
                cmdq_error(
                    (*cdata).item,
...
unsafe extern "C" fn cmd_if_shell_free(mut data: *mut libc::c_void) {
    ...
    free((*cdata).file as *mut libc::c_void);
