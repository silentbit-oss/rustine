...
pub static mut cmd_show_environment_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"show-environment\0" as *const u8 as *const libc::c_char,
            alias: b"showenv\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"gst:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-gs] [-t target-session] [name]\0" as *const u8
...
unsafe extern "C" fn cmd_show_environment_print(
    ...
    if args_has((*self_0).args, 's' as i32 as u_char) == 0 {
    ...
    if !((*envent).value).is_null() {
        ...
        cmdq_print(
            item,
            b"%s=\"%s\"; export %s;\0" as *const u8 as *const libc::c_char,
            (*envent).name,
            escaped,
            (*envent).name,
        );
...
unsafe extern "C" fn cmd_show_environment_exec(
    ...
    if (*args).argc != 0 as libc::c_int {
        ...
        if envent.is_null() {
            cmdq_error(
                item,
                b"unknown variable: %s\0" as *const u8 as *const libc::c_char,
