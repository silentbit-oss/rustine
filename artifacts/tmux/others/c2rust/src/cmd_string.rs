...
unsafe extern "C" fn cmd_string_getc(
    mut s: *const libc::c_char,
    mut p: *mut size_t,
) -> libc::c_int {
    let mut ucs: *const u_char = s as *const u_char;
    if *ucs.offset(*p as isize) as libc::c_int == '\0' as i32 {
        return -(1 as libc::c_int);
    }
    let fresh0 = *p;
    *p = (*p).wrapping_add(1);
    return *ucs.offset(fresh0 as isize) as libc::c_int;
}
...
#[no_mangle]
pub unsafe extern "C" fn cmd_string_split(
    mut s: *const libc::c_char,
    mut rargc: *mut libc::c_int,
    mut rargv: *mut *mut *mut libc::c_char,
) -> libc::c_int {
    ...
    loop {
        ...
        match ch {
            39 => {
                ...
                if t.is_null() {
                    current_block = 17402115628192833749;
                    break;
                }
                cmd_string_copy(&mut buf, t, &mut len);
            ...
            35 => {
                ...
                current_block = 17399044847211354252;
            }
            -1 | 32 | 9 => {
                current_block = 17399044847211354252;
            }
            126 => {
                if !buf.is_null() {
                    ...
                    t = cmd_string_expand_tilde(s, &mut p);
                    if t.is_null() {
                        current_block = 17402115628192833749;
                        break;
                    }
            ...
            17399044847211354252 => {
                ...
                if !(ch != -(1 as libc::c_int)) {
                    while argc != 0 as libc::c_int {
                        ...
                        whitespace = (*argv.offset(0 as libc::c_int as isize))
                            .offset(
                                strcspn(
                                    *argv.offset(0 as libc::c_int as isize),
                        ...
                        if equals.is_null() || equals > whitespace {
                            break;
                        }
                        environ_put(
                            global_environ,
                    ...
                    current_block = 10871435903431652734;
                    break;
                }
            }
            _ => {}
        ...
        if append != 0 {
            ...
            *buf.offset(fresh3 as isize) = ch as libc::c_char;
        }
        append = 0 as libc::c_int;
    }
    match current_block {
        17402115628192833749 => {
            if !argv.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_string_parse(
    mut s: *const libc::c_char,
    ...
    *cause = 0 as *mut libc::c_char;
    if cmd_string_split(s, &mut argc, &mut argv) != 0 as libc::c_int {
        xasprintf(
            cause,
            b"invalid or unknown command: %s\0" as *const u8 as *const libc::c_char,
            s,
        );
...
unsafe extern "C" fn cmd_string_copy(
    ...
    *dst = xrealloc(
        *dst as *mut libc::c_void,
        (*len).wrapping_add(srclen).wrapping_add(1 as libc::c_int as size_t),
    ) as *mut libc::c_char;
    strlcpy(
        (*dst).offset(*len as isize),
        src,
        srclen.wrapping_add(1 as libc::c_int as size_t),
    );
    *len = (*len).wrapping_add(srclen);
    free(src as *mut libc::c_void);
}
unsafe extern "C" fn cmd_string_string(
    mut s: *const libc::c_char,
    mut p: *mut size_t,
    ...
    loop {
        ...
        match ch {
            -1 => {
                current_block = 2501879902745990142;
                break;
            }
            92 => {
                if !(esc == 0) {
                    ...
                    match ch {
                        ...
                        101 => {
                            current_block = 12101231480398343267;
                            match current_block {
                                3477507786307003161 => {
                                    ch = '\t' as i32;
                                }
                                14358046763198985234 => {
                                    ch = '\r' as i32;
                                }
                                10674667901149900829 => {
                                    ch = '\n' as i32;
                                }
                                _ => {
                                    ch = '\u{1b}' as i32;
                        ...
                        114 => {
                            current_block = 14358046763198985234;
                            match current_block {
                                3477507786307003161 => {
                                    ch = '\t' as i32;
                                }
                        ...
                        110 => {
                            current_block = 10674667901149900829;
                            match current_block {
                                3477507786307003161 => {
                                    ch = '\t' as i32;
                                }
                        ...
                        116 => {
                            current_block = 3477507786307003161;
                            match current_block {
                                3477507786307003161 => {
                                    ch = '\t' as i32;
                                }
        ...
        let fresh4 = len;
        len = len.wrapping_add(1);
        *buf.offset(fresh4 as isize) = ch as libc::c_char;
    }
    match current_block {
        2501879902745990142 => {
            free(buf as *mut libc::c_void);
...
unsafe extern "C" fn cmd_string_variable(
    mut s: *const libc::c_char,
    mut p: *mut size_t,
) -> *mut libc::c_char {
    let mut current_block: u64;
    ...
    match ch {
        -1 => {
            current_block = 18156639247413854209;
        }
        123 => {
            fch = '{' as i32;
            ch = cmd_string_getc(s, p);
            if !(ch == '_' as i32 || ch >= 'a' as i32 && ch <= 'z' as i32
                || ch >= 'A' as i32 && ch <= 'Z' as i32)
            {
                current_block = 18156639247413854209;
            } else {
                current_block = 7123524975239680475;
            }
        }
        _ => {
            current_block = 7123524975239680475;
        ...
        7123524975239680475 => {
            ...
            *buf.offset(fresh5 as isize) = ch as libc::c_char;
            loop {
                ch = cmd_string_getc(s, p);
                if ch == -(1 as libc::c_int)
                    || !(ch == '_' as i32 || ch >= 'a' as i32 && ch <= 'z' as i32
                        || ch >= 'A' as i32 && ch <= 'Z' as i32
            ...
            match current_block {
                18156639247413854209 => {}
                _ => {
                    if !(fch == '{' as i32 && ch != '}' as i32) {
                        if ch != -(1 as libc::c_int) && fch != '{' as i32 {
                            cmd_string_ungetc(p);
                        }
                        buf = xrealloc(
                            buf as *mut libc::c_void,
...
unsafe extern "C" fn cmd_string_expand_tilde(
    mut s: *const libc::c_char,
    mut p: *mut size_t,
) -> *mut libc::c_char {
    ...
    if last == -(1 as libc::c_int) || last == '/' as i32 || last == ' ' as i32
        || last == '\t' as i32
    {
        envent = environ_find(
            global_environ,
