...
pub static mut cmd_list_buffers_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"list-buffers\0" as *const u8 as *const libc::c_char,
            alias: b"lsb\0" as *const u8 as *const libc::c_char,
            args: {
...
unsafe extern "C" fn cmd_list_buffers_exec(
    ...
    if template.is_null() {
        template = b"#{buffer_name}: #{buffer_size} bytes: \"#{buffer_sample}\"\0"
