...
pub static mut window_clock_mode: window_mode = unsafe {
    {
        let mut init = window_mode {
            name: b"clock-mode\0" as *const u8 as *const libc::c_char,
            init: Some(
                window_clock_init
            ...
            free: Some(
                window_clock_free as unsafe extern "C" fn(*mut window_pane) -> (),
            ),
            resize: Some(
                window_clock_resize
...
unsafe extern "C" fn window_clock_timer_callback(
    ...
    let mut data: *mut window_clock_mode_data = (*wp).modedata
...
unsafe extern "C" fn window_clock_init(
    ...
    data = xmalloc(::core::mem::size_of::<window_clock_mode_data>() as libc::c_ulong)
        as *mut window_clock_mode_data;
...
unsafe extern "C" fn window_clock_draw_screen(mut wp: *mut window_pane) {
    ...
    colour = options_get_number(
        (*(*wp).window).options,
        b"clock-mode-colour\0" as *const u8 as *const libc::c_char,
    ) as libc::c_int;
    style = options_get_number(
        (*(*wp).window).options,
        b"clock-mode-style\0" as *const u8 as *const libc::c_char,
    ) as libc::c_int;
    ...
    if style == 0 as libc::c_int {
        strftime(
            ...
            b"%l:%M \0" as *const u8 as *const libc::c_char,
            localtime(&mut t),
        );
    ...
    if ((*(*s).grid).sx as libc::c_ulong)
        ...
        || (*(*s).grid).sy < 6 as libc::c_int as u_int
    {
        if (*(*s).grid).sx as libc::c_ulong >= strlen(tim.as_mut_ptr())
            && (*(*s).grid).sy != 0 as libc::c_int as u_int
        {
            x = (((*(*s).grid).sx / 2 as libc::c_int as u_int) as libc::c_ulong)
                .wrapping_sub(
                    (strlen(tim.as_mut_ptr()))
                        .wrapping_div(2 as libc::c_int as libc::c_ulong),
                ) as u_int;
            y = (*(*s).grid).sy / 2 as libc::c_int as u_int;
            screen_write_cursormove(&mut ctx, x, y);
            memcpy(
    ...
    x = (((*(*s).grid).sx / 2 as libc::c_int as u_int) as libc::c_ulong)
        .wrapping_sub(
            (3 as libc::c_int as libc::c_ulong).wrapping_mul(strlen(tim.as_mut_ptr())),
        ) as u_int;
    y = ((*(*s).grid).sy / 2 as libc::c_int as u_int)
    ...
    while *ptr as libc::c_int != '\0' as i32 {
        if *ptr as libc::c_int >= '0' as i32 && *ptr as libc::c_int <= '9' as i32 {
            ...
            current_block_47 = 2873832966593178012;
            ...
            x = x.wrapping_add(6 as libc::c_int as u_int);
            current_block_47 = 15768484401365413375;
        }
