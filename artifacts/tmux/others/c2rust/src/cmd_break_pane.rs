...
pub static mut cmd_break_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"break-pane\0" as *const u8 as *const libc::c_char,
            alias: b"breakp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"dPF:n:s:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-dP] [-F format] [-n window-name] [-s src-pane] [-t dst-window]\0"
...
unsafe extern "C" fn cmd_break_pane_exec(
        ...
        && !(winlink_find_by_index(&mut (*dst_s).windows, idx)).is_null()
    {
        cmdq_error(
            item,
            b"index %d already in use\0" as *const u8 as *const libc::c_char,
            idx,
        );
    ...
    if window_count_panes(w) == 1 as libc::c_int as u_int {
        cmdq_error(
            item,
            b"can't break with only one pane\0" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    ...
    *(*wp).entry.tqe_prev = (*wp).entry.tqe_next;
    ...
    if args_has(args, 'P' as i32 as u_char) != 0 {
        ...
        if template.is_null() {
            template = b"#{session_name}:#{window_index}.#{pane_index}\0" as *const u8
