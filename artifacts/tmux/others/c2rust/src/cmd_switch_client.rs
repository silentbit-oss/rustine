...
pub static mut cmd_switch_client_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"switch-client\0" as *const u8 as *const libc::c_char,
            alias: b"switchc\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"lc:Enpt:rT:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-Elnpr] [-c target-client] [-t target-session] [-T key-table]\0"
...
unsafe extern "C" fn cmd_switch_client_exec(
    ...
    c = cmd_find_client(item, args_get(args, 'c' as i32 as u_char), 0 as libc::c_int);
    if c.is_null() {
        return CMD_RETURN_ERROR;
    }
    if !tflag.is_null()
        && *tflag
            .offset(strcspn(tflag, b":.\0" as *const u8 as *const libc::c_char) as isize)
    ...
    if cmd_find_target(&mut (*item).target, item, tflag, type_0, flags)
        != 0 as libc::c_int
    {
        return CMD_RETURN_ERROR;
    }
    ...
    if args_has(args, 'n' as i32 as u_char) != 0 {
        ...
        if s.is_null() {
            cmdq_error(
                item,
                b"can't find next session\0" as *const u8 as *const libc::c_char,
            );
            return CMD_RETURN_ERROR;
        }
        ...
        if s.is_null() {
            cmdq_error(
                item,
                b"can't find previous session\0" as *const u8 as *const libc::c_char,
            );
            return CMD_RETURN_ERROR;
        }
    } else if args_has(args, 'l' as i32 as u_char) != 0 {
        if !((*c).last_session).is_null() && session_alive((*c).last_session) != 0 {
            s = (*c).last_session;
        } else {
            s = 0 as *mut session;
        }
        if s.is_null() {
            cmdq_error(
                item,
                b"can't find last session\0" as *const u8 as *const libc::c_char,
            );
            return CMD_RETURN_ERROR;
        }
    } else {
        ...
        if !wl.is_null() {
            ...
            cmd_find_from_session(&mut (*(*item).shared).current, s, 0 as libc::c_int);
        }
    }
    if args_has(args, 'E' as i32 as u_char) == 0 {
        environ_update((*s).options, (*c).environ, (*s).environ);
    }
    if !((*c).session).is_null() && (*c).session != s {
    ...
    notify_client(b"client-session-changed\0" as *const u8 as *const libc::c_char, c);
