...
pub static mut cmd_list_clients_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"list-clients\0" as *const u8 as *const libc::c_char,
            alias: b"lsc\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"F:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-F format] [-t target-session]\0" as *const u8
...
unsafe extern "C" fn cmd_list_clients_exec(
    ...
    if template.is_null() {
        template = b"#{client_name}: #{session_name} [#{client_width}x#{client_height} #{client_termname}]#{?client_utf8, (utf8),} #{?client_readonly, (ro),}\0"
