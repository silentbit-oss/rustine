...
pub static mut cmd_resize_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"resize-pane\0" as *const u8 as *const libc::c_char,
            alias: b"resizep\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"DLMRt:Ux:y:Z\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-DLMRUZ] [-x width] [-y height] [-t target-pane] [adjustment]\0"
...
unsafe extern "C" fn cmd_resize_pane_exec(
    ...
    if args_has(args, 'M' as i32 as u_char) != 0 {
        if (cmd_mouse_window(&mut (*shared).mouse, &mut s)).is_null() {
        ...
        cmd_resize_pane_mouse_update(c, &mut (*shared).mouse);
        return CMD_RETURN_NORMAL;
    }
    ...
    if (*args).argc == 0 as libc::c_int {
        ...
        if !errstr.is_null() {
            cmdq_error(
                item,
                b"adjustment %s\0" as *const u8 as *const libc::c_char,
                errstr,
            );
            return CMD_RETURN_ERROR;
        }
    }
    if args_has((*self_0).args, 'x' as i32 as u_char) != 0 {
        x = args_strtonum(
            (*self_0).args,
            'x' as i32 as u_char,
        ...
        if !cause.is_null() {
            cmdq_error(item, b"width %s\0" as *const u8 as *const libc::c_char, cause);
            free(cause as *mut libc::c_void);
            return CMD_RETURN_ERROR;
        }
        layout_resize_pane_to(wp, LAYOUT_LEFTRIGHT, x as u_int);
    }
    if args_has((*self_0).args, 'y' as i32 as u_char) != 0 {
        y = args_strtonum(
            (*self_0).args,
            'y' as i32 as u_char,
            2 as libc::c_int as libc::c_longlong,
        ...
        if !cause.is_null() {
            cmdq_error(item, b"height %s\0" as *const u8 as *const libc::c_char, cause);
            free(cause as *mut libc::c_void);
            return CMD_RETURN_ERROR;
        }
...
unsafe extern "C" fn cmd_resize_pane_mouse_update(
    ...
    let mut wp_x: *mut window_pane = 0 as *mut window_pane;
    let mut wp_y: *mut window_pane = 0 as *mut window_pane;
    let mut y: u_int = 0;
    let mut ly: u_int = 0;
    let mut x: u_int = 0;
    let mut lx: u_int = 0;
    ...
    ly = (*m).ly;
    lx = (*m).lx;
    if (*m).statusat == 0 as libc::c_int && ly > 0 as libc::c_int as u_int {
        ly = ly.wrapping_sub(1);
        ly;
    } else if (*m).statusat > 0 as libc::c_int && ly >= (*m).statusat as u_int {
        ly = ((*m).statusat - 1 as libc::c_int) as u_int;
    }
    wp_y = 0 as *mut window_pane;
    wp_x = wp_y;
    loop_0 = (*(*wl).window).panes.tqh_first;
    while !loop_0.is_null() {
        if !(window_pane_visible(loop_0) == 0) {
            sx = (*loop_0).xoff;
            if sx != 0 as libc::c_int as u_int {
                sx = sx.wrapping_sub(1);
                sx;
            }
            ex = ((*loop_0).xoff).wrapping_add((*loop_0).sx);
            sy = (*loop_0).yoff;
            if sy != 0 as libc::c_int as u_int {
                sy = sy.wrapping_sub(1);
                sy;
            }
            ey = ((*loop_0).yoff).wrapping_add((*loop_0).sy);
            if (lx == sx || lx == ex) && (ly >= sy && ly <= ey)
                && (wp_x.is_null() || (*loop_0).sy > (*wp_x).sy)
            {
                wp_x = loop_0;
            }
            if (ly == sy || ly == ey) && (lx >= sx && lx <= ex)
                && (wp_y.is_null() || (*loop_0).sx > (*wp_y).sx)
            {
                wp_y = loop_0;
            }
        }
