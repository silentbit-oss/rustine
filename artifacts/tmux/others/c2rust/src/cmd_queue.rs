...
static mut global_queue: cmdq_list = cmdq_list {
    tqh_first: 0 as *const cmdq_item as *mut cmdq_item,
    tqh_last: 0 as *const *mut cmdq_item as *mut *mut cmdq_item,
};
unsafe extern "C" fn cmdq_name(mut c: *mut client) -> *const libc::c_char {
    static mut s: [libc::c_char; 32] = [0; 32];
    if c.is_null() {
        return b"<global>\0" as *const u8 as *const libc::c_char;
    }
    xsnprintf(
        s.as_mut_ptr(),
...
unsafe extern "C" fn cmdq_get(mut c: *mut client) -> *mut cmdq_list {
    if c.is_null() {
        return &mut global_queue;
    }
    return &mut (*c).queue;
}
#[no_mangle]
pub unsafe extern "C" fn cmdq_append(mut c: *mut client, mut item: *mut cmdq_item) {
    ...
    loop {
        ...
        *(*queue).tqh_last = item;
        (*queue).tqh_last = &mut (*item).entry.tqe_next;
        item = next;
        if item.is_null() {
            break;
        }
...
pub unsafe extern "C" fn cmdq_insert_after(
    ...
    loop {
        ...
        if !((*after).next).is_null() {
            ...
            (*item).entry.tqe_prev = &mut (*(*after).next).entry.tqe_next;
        } else {
...
unsafe extern "C" fn cmdq_remove(mut item: *mut cmdq_item) {
    if !((*item).shared).is_null()
        && {
            (*(*item).shared).references -= 1;
            (*(*item).shared).references == 0 as libc::c_int
        }
    {
        if !((*(*item).shared).formats).is_null() {
    ...
    if (*item).type_0 as libc::c_uint == CMDQ_COMMAND as libc::c_int as libc::c_uint {
...
pub unsafe extern "C" fn cmdq_get_command(
    ...
    shared = xcalloc(
        ...
        ::core::mem::size_of::<cmdq_shared>() as libc::c_ulong,
    ) as *mut cmdq_shared;
    if !current.is_null() {
        cmd_find_copy_state(&mut (*shared).current, current);
    } else {
        cmd_find_clear_state(&mut (*shared).current, 0 as libc::c_int);
    }
    if !m.is_null() {
        memcpy(
            &mut (*shared).mouse as *mut mouse_event as *mut libc::c_void,
    ...
    while !cmd.is_null() {
        xasprintf(
            ...
            b"command[%s]\0" as *const u8 as *const libc::c_char,
            (*(*cmd).entry).name,
        );
        item = xcalloc(
            1 as libc::c_int as size_t,
            ::core::mem::size_of::<cmdq_item>() as libc::c_ulong,
        ) as *mut cmdq_item;
        (*item).name = tmp;
...
unsafe extern "C" fn cmdq_fire_command(mut item: *mut cmdq_item) -> cmd_retval {
    ...
    flags = ((*cmd).flags & 0x1 as libc::c_int != 0) as libc::c_int;
    cmdq_guard(item, b"begin\0" as *const u8 as *const libc::c_char, flags);
    if ((*item).client).is_null() {
        (*item)
            .client = cmd_find_client(item, 0 as *const libc::c_char, 1 as libc::c_int);
    }
            ...
            if !(retval as libc::c_int == CMD_RETURN_ERROR as libc::c_int) {
                if (*entry).flags & 0x4 as libc::c_int != 0 {
                    ...
                    ) == 0 as libc::c_int
                    {
                        ...
                        current_block = 15835115521126477143;
                    }
                    match current_block {
                        15835115521126477143 => {}
                        _ => {
                            hooks_insert(
                                (*(*fsp).s).hooks,
                                item,
    ...
    (*item).client = c;
    if retval as libc::c_int == CMD_RETURN_ERROR as libc::c_int {
        cmdq_guard(item, b"error\0" as *const u8 as *const libc::c_char, flags);
    } else {
        cmdq_guard(item, b"end\0" as *const u8 as *const libc::c_char, flags);
    }
    return retval;
}
#[no_mangle]
pub unsafe extern "C" fn cmdq_get_callback1(
    ...
    xasprintf(
        &mut tmp as *mut *mut libc::c_char,
        b"callback[%s]\0" as *const u8 as *const libc::c_char,
        name,
    );
    item = xcalloc(
        1 as libc::c_int as size_t,
...
pub unsafe extern "C" fn cmdq_next(mut c: *mut client) -> u_int {
    ...
    if ((*queue).tqh_first).is_null() {
        log_debug(
            b"%s %s: empty\0" as *const u8 as *const libc::c_char,
            (*::core::mem::transmute::<&[u8; 10], &[libc::c_char; 10]>(b"cmdq_next\0"))
                .as_ptr(),
            name,
        );
        return 0 as libc::c_int as u_int;
    }
    if (*(*queue).tqh_first).flags & 0x2 as libc::c_int != 0 {
        log_debug(
            b"%s %s: waiting\0" as *const u8 as *const libc::c_char,
            (*::core::mem::transmute::<&[u8; 10], &[libc::c_char; 10]>(b"cmdq_next\0"))
                .as_ptr(),
            name,
        );
    ...
    log_debug(
        b"%s %s: enter\0" as *const u8 as *const libc::c_char,
        (*::core::mem::transmute::<&[u8; 10], &[libc::c_char; 10]>(b"cmdq_next\0"))
            .as_ptr(),
        name,
    );
    loop {
        ...
        log_debug(
            b"%s %s: %s (%d), flags %x\0" as *const u8 as *const libc::c_char,
            (*::core::mem::transmute::<&[u8; 10], &[libc::c_char; 10]>(b"cmdq_next\0"))
                .as_ptr(),
            name,
            (*item).name,
        ...
        if (*item).flags & 0x2 as libc::c_int != 0 {
            current_block = 4007330957536088182;
            break;
        }
    ...
    match current_block {
        4007330957536088182 => {
            log_debug(
                b"%s %s: exit (wait)\0" as *const u8 as *const libc::c_char,
                (*::core::mem::transmute::<
                    &[u8; 10],
                    &[libc::c_char; 10],
                >(b"cmdq_next\0"))
        ...
        _ => {
            log_debug(
                b"%s %s: exit (empty)\0" as *const u8 as *const libc::c_char,
                (*::core::mem::transmute::<
                    &[u8; 10],
                    &[libc::c_char; 10],
                >(b"cmdq_next\0"))
...
pub unsafe extern "C" fn cmdq_guard(
    ...
    let mut c: *mut client = (*item).client;
    if c.is_null() || (*c).flags & 0x2000 as libc::c_int == 0 {
        return;
    }
    evbuffer_add_printf(
        (*c).stdout_data,
...
unsafe extern "C" fn run_static_initializers() {
    global_queue = {
        let mut init = cmdq_list {
            tqh_first: 0 as *mut cmdq_item,
            tqh_last: &mut global_queue.tqh_first,
        };
        init
    };
}
