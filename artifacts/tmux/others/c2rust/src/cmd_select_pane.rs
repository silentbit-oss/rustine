...
extern "C" {
    ...
    fn window_set_active_pane(_: *mut window, _: *mut window_pane) -> libc::c_int;
...
pub static mut cmd_select_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"select-pane\0" as *const u8 as *const libc::c_char,
            alias: b"selectp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"DdegLlMmP:RT:t:U\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-DdegLlMmRU] [-P style] [-T title] [-t target-pane]\0" as *const u8
...
pub static mut cmd_last_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"last-pane\0" as *const u8 as *const libc::c_char,
            alias: b"lastp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"det:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-de] [-t target-window]\0" as *const u8 as *const libc::c_char,
            source: cmd_entry_flag {
                flag: 0,
                type_0: CMD_FIND_PANE,
                flags: 0,
...
unsafe extern "C" fn cmd_select_pane_exec(
    ...
    let mut current: *mut cmd_find_state = &mut (*(*item).shared).current;
    ...
    let mut pane_title: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut style: *const libc::c_char = 0 as *const libc::c_char;
    if (*self_0).entry == &cmd_last_pane_entry as *const cmd_entry
        || args_has(args, 'l' as i32 as u_char) != 0
    {
        lastwp = (*w).last;
        if lastwp.is_null() {
            cmdq_error(item, b"no last pane\0" as *const u8 as *const libc::c_char);
            return CMD_RETURN_ERROR;
        }
        if args_has((*self_0).args, 'e' as i32 as u_char) != 0 {
            (*lastwp).flags &= !(0x40 as libc::c_int);
        } else if args_has((*self_0).args, 'd' as i32 as u_char) != 0 {
            (*lastwp).flags |= 0x40 as libc::c_int;
        } else {
            server_unzoom_window(w);
            window_redraw_active_switch(w, lastwp);
            if window_set_active_pane(w, lastwp) != 0 {
    ...
    if args_has((*self_0).args, 'P' as i32 as u_char) != 0
        || args_has((*self_0).args, 'g' as i32 as u_char) != 0
    {
        if args_has(args, 'P' as i32 as u_char) != 0 {
            style = args_get(args, 'P' as i32 as u_char);
            if style_parse(&grid_default_cell, &mut (*wp).colgc, style)
                == -(1 as libc::c_int)
            {
                cmdq_error(
                    item,
        ...
        if args_has((*self_0).args, 'g' as i32 as u_char) != 0 {
            cmdq_print(
                ...
                style_tostring(&mut (*wp).colgc),
            );
        }
        return CMD_RETURN_NORMAL;
    }
    if args_has((*self_0).args, 'L' as i32 as u_char) != 0 {
        server_unzoom_window((*wp).window);
        wp = window_pane_find_left(wp);
    } else if args_has((*self_0).args, 'R' as i32 as u_char) != 0 {
        server_unzoom_window((*wp).window);
        wp = window_pane_find_right(wp);
    } else if args_has((*self_0).args, 'U' as i32 as u_char) != 0 {
        server_unzoom_window((*wp).window);
    ...
    if args_has((*self_0).args, 'd' as i32 as u_char) != 0 {
        (*wp).flags |= 0x40 as libc::c_int;
        return CMD_RETURN_NORMAL;
    }
    if args_has((*self_0).args, 'T' as i32 as u_char) != 0 {
        pane_title = format_single(
            item,
            args_get((*self_0).args, 'T' as i32 as u_char),
            c,
            s,
            wl,
            wp,
        );
        screen_set_title(&mut (*wp).base, pane_title);
        server_status_window((*wp).window);
        free(pane_title as *mut libc::c_void);
    }
    if wp == (*w).active {
        return CMD_RETURN_NORMAL;
    }
    ...
    if window_pane_visible(wp) == 0 {
        cmdq_error(item, b"pane not visible\0" as *const u8 as *const libc::c_char);
        return CMD_RETURN_ERROR;
    }
    window_redraw_active_switch(w, wp);
    if window_set_active_pane(w, wp) != 0 {
        cmd_find_from_winlink_pane(current, wl, wp, 0 as libc::c_int);
        hooks_insert(
            (*s).hooks,
            item,
            current,
            b"after-select-pane\0" as *const u8 as *const libc::c_char,
        );
        server_status_window(w);
        server_redraw_window_borders(w);
    }
