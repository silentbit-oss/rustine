...
pub static mut cmd_pipe_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"pipe-pane\0" as *const u8 as *const libc::c_char,
            alias: b"pipep\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"IOot:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-IOo] [-t target-pane] [command]\0" as *const u8
...
unsafe extern "C" fn cmd_pipe_pane_exec(
    ...
    if args_has((*self_0).args, 'o' as i32 as u_char) != 0
        && old_fd != -(1 as libc::c_int)
    {
        return CMD_RETURN_NORMAL;
    }
    if args_has((*self_0).args, 'I' as i32 as u_char) != 0 {
        in_0 = 1 as libc::c_int;
        out = args_has((*self_0).args, 'O' as i32 as u_char);
    } else {
    ...
    ) != 0 as libc::c_int
    {
        cmdq_error(
            item,
            b"socketpair error: %s\0" as *const u8 as *const libc::c_char,
    ...
    match fork() {
        -1 => {
            ...
            cmdq_error(
                item,
                b"fork error: %s\0" as *const u8 as *const libc::c_char,
                strerror(*__errno_location()),
            );
        ...
        _ => {
            ...
            (*wp).pipe_off = evbuffer_get_length((*(*wp).event).input);
            setblocking((*wp).pipe_fd, 0 as libc::c_int);
            (*wp)
                .pipe_event = bufferevent_new(
                (*wp).pipe_fd,
                Some(
...
unsafe extern "C" fn cmd_pipe_pane_read_callback(
    ...
    log_debug(
        b"%%%u pipe read %zu\0" as *const u8 as *const libc::c_char,
        (*wp).id,
        available,
    );
    bufferevent_write(
...
unsafe extern "C" fn cmd_pipe_pane_write_callback(
    ...
    let mut wp: *mut window_pane = data as *mut window_pane;
    log_debug(b"%%%u pipe empty\0" as *const u8 as *const libc::c_char, (*wp).id);
    if window_pane_destroy_ready(wp) != 0 {
        server_destroy_pane(wp, 1 as libc::c_int);
    }
}
unsafe extern "C" fn cmd_pipe_pane_error_callback(
    ...
    let mut wp: *mut window_pane = data as *mut window_pane;
    log_debug(b"%%%u pipe error\0" as *const u8 as *const libc::c_char, (*wp).id);
    bufferevent_free((*wp).pipe_event);
    close((*wp).pipe_fd);
    (*wp).pipe_fd = -(1 as libc::c_int);
    if window_pane_destroy_ready(wp) != 0 {
