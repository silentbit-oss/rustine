...
pub static mut cmd_attach_session_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"attach-session\0" as *const u8 as *const libc::c_char,
            alias: b"attach\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"c:dErt:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-dEr] [-c working-directory] [-t target-session]\0" as *const u8
...
pub unsafe extern "C" fn cmd_attach_session(
    ...
    if (sessions.rbh_root).is_null() {
        cmdq_error(item, b"no sessions\0" as *const u8 as *const libc::c_char);
    ...
    if !((*c).session).is_null() {
        ...
        notify_client(b"client-attached\0" as *const u8 as *const libc::c_char, c);
