...
extern "C" {
    ...
    fn input_reset(_: *mut window_pane, _: libc::c_int);
...
pub static mut cmd_send_keys_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"send-keys\0" as *const u8 as *const libc::c_char,
            alias: b"send\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"lXRMN:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: -(1 as libc::c_int),
                };
                init
            },
            usage: b"[-lXRM] [-N repeat-count] [-t target-pane] key ...\0" as *const u8
...
pub static mut cmd_send_prefix_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"send-prefix\0" as *const u8 as *const libc::c_char,
            alias: 0 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"2t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-2] [-t target-pane]\0" as *const u8 as *const libc::c_char,
            source: cmd_entry_flag {
                flag: 0,
                type_0: CMD_FIND_PANE,
                flags: 0,
...
unsafe extern "C" fn cmd_send_keys_inject(
    mut c: *mut client,
    ...
    if ((*wp).mode).is_null() || ((*(*wp).mode).key_table).is_none() {
        ...
        window_pane_key(wp, 0 as *mut client, s, key, 0 as *mut mouse_event);
        return;
    }
    ...
    if !bd.is_null() {
        ...
        (*table).references;
        key_bindings_dispatch(bd, item, c, 0 as *mut mouse_event, &mut (*item).target);
...
unsafe extern "C" fn cmd_send_keys_exec(
    ...
    let mut wp: *mut window_pane = (*item).target.wp;
    let mut s: *mut session = (*item).target.s;
    let mut m: *mut mouse_event = &mut (*(*item).shared).mouse;
    let mut ud: *mut utf8_data = 0 as *mut utf8_data;
    let mut uc: *mut utf8_data = 0 as *mut utf8_data;
    ...
    if args_has(args, 'N' as i32 as u_char) != 0 {
        ...
        if !cause.is_null() {
            cmdq_error(
                item,
                b"repeat count %s\0" as *const u8 as *const libc::c_char,
                cause,
            );
            free(cause as *mut libc::c_void);
            return CMD_RETURN_ERROR;
        }
        if args_has(args, 'X' as i32 as u_char) != 0 || (*args).argc == 0 as libc::c_int
        {
            (*wp).modeprefix = np;
        }
    }
    if args_has(args, 'X' as i32 as u_char) != 0 {
        if ((*wp).mode).is_null() || ((*(*wp).mode).command).is_none() {
            cmdq_error(item, b"not in a mode\0" as *const u8 as *const libc::c_char);
            return CMD_RETURN_ERROR;
        }
        if (*m).valid == 0 {
            ((*(*wp).mode).command)
                ...
                )(wp, c, s, args, 0 as *mut mouse_event);
        } else {
            ((*(*wp).mode).command)
                .expect("non-null function pointer")(wp, c, s, args, m);
        }
    ...
    if args_has(args, 'M' as i32 as u_char) != 0 {
        ...
        if wp.is_null() {
            cmdq_error(item, b"no mouse target\0" as *const u8 as *const libc::c_char);
            return CMD_RETURN_ERROR;
        }
        window_pane_key(wp, 0 as *mut client, s, (*m).key, m);
        return CMD_RETURN_NORMAL;
    }
    if (*self_0).entry == &cmd_send_prefix_entry as *const cmd_entry {
    ...
    if args_has(args, 'R' as i32 as u_char) != 0 {
        ...
        input_reset(wp, 1 as libc::c_int);
    }
    while np != 0 as libc::c_int as u_int {
        ...
        while i < (*args).argc {
            ...
            if literal == 0 {
                key = key_string_lookup_string(*((*args).argv).offset(i as isize));
            ...
            if literal != 0 {
                ud = utf8_fromcstr(*((*args).argv).offset(i as isize));
                uc = ud;
                while (*uc).size as libc::c_int != 0 as libc::c_int {
                    if !(utf8_combine(uc, &mut wc) as libc::c_uint
                        != UTF8_DONE as libc::c_int as libc::c_uint)
                    {
                        cmd_send_keys_inject(c, item, wc as key_code);
                    }
                    uc = uc.offset(1);
                    uc;
                }
