...
pub static mut cmd_respawn_window_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"respawn-window\0" as *const u8 as *const libc::c_char,
            alias: b"respawnw\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"c:kt:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: -(1 as libc::c_int),
                };
                init
            },
            usage: b"[-c start-directory] [-k] [-t target-window] [command]\0"
...
unsafe extern "C" fn cmd_respawn_window_exec(
    ...
    if args_has((*self_0).args, 'k' as i32 as u_char) == 0 {
        ...
        while !wp.is_null() {
            if (*wp).fd == -(1 as libc::c_int) {
                ...
                cmdq_error(
                    item,
                    b"window still active: %s:%d\0" as *const u8 as *const libc::c_char,
                    (*s).name,
                    (*wl).idx,
                );
    ...
    *(*wp).entry.tqe_prev = (*wp).entry.tqe_next;
    ...
    cp = args_get(args, 'c' as i32 as u_char);
    if !cp.is_null() {
        cwd = format_single(item, cp, c, s, 0 as *mut winlink, 0 as *mut window_pane);
    }
    ...
    ) != 0 as libc::c_int
    {
        cmdq_error(
            item,
            b"respawn window failed: %s\0" as *const u8 as *const libc::c_char,
            cause,
        );
        free(cause as *mut libc::c_void);
        environ_free(env);
        free(cwd as *mut libc::c_void);
        server_destroy_pane(wp, 0 as libc::c_int);
        return CMD_RETURN_ERROR;
    }
    environ_free(env);
    free(cwd as *mut libc::c_void);
