...
pub static mut cmd_display_message_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"display-message\0" as *const u8 as *const libc::c_char,
            alias: b"display\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"c:pt:F:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-p] [-c target-client] [-F format] [-t target-pane] [message]\0"
...
unsafe extern "C" fn cmd_display_message_exec(
    ...
    let mut target_c: *mut client = 0 as *mut client;
    ...
    if template.is_null() {
        template = b"[#{session_name}] #{window_index}:#{window_name}, current pane #{pane_index} - (%H:%M %d-%b-%y)\0"
            as *const u8 as *const libc::c_char;
    }
    c = cmd_find_client(item, args_get(args, 'c' as i32 as u_char), 1 as libc::c_int);
    ...
    msg = format_expand_time(ft, template, time(0 as *mut time_t));
