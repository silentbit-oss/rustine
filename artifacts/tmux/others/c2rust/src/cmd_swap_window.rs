...
pub static mut cmd_swap_window_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"swap-window\0" as *const u8 as *const libc::c_char,
            alias: b"swapw\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"ds:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-d] [-s src-window] [-t dst-window]\0" as *const u8
...
unsafe extern "C" fn cmd_swap_window_exec(
    ...
    if src != dst && !sg_src.is_null() && !sg_dst.is_null() && sg_src == sg_dst {
        cmdq_error(
            item,
            b"can't move window, sessions are grouped\0" as *const u8
                as *const libc::c_char,
        );
