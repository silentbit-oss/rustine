...
pub static mut cmd_find_window_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"find-window\0" as *const u8 as *const libc::c_char,
            alias: b"findw\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"CNt:T\0" as *const u8 as *const libc::c_char,
                    lower: 1 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-CNT] [-t target-pane] match-string\0" as *const u8
...
unsafe extern "C" fn cmd_find_window_exec(
    ...
    if C != 0 && N != 0 && T != 0 {
        xasprintf(
            ...
            b"#{||:#{C:%s},#{||:#{m:*%s*,#{window_name}},#{m:*%s*,#{pane_title}}}}\0"
        ...
        xasprintf(
            &mut filter as *mut *mut libc::c_char,
            b"#{||:#{C:%s},#{m:*%s*,#{window_name}}}\0" as *const u8
                as *const libc::c_char,
            s,
            s,
        );
        ...
        xasprintf(
            &mut filter as *mut *mut libc::c_char,
            b"#{||:#{C:%s},#{m:*%s*,#{pane_title}}}\0" as *const u8
                as *const libc::c_char,
            s,
            s,
        );
        ...
        xasprintf(
            &mut filter as *mut *mut libc::c_char,
            b"#{||:#{m:*%s*,#{window_name}},#{m:*%s*,#{pane_title}}}\0" as *const u8
                as *const libc::c_char,
            s,
            s,
        );
        ...
        xasprintf(
            &mut filter as *mut *mut libc::c_char,
            b"#{m:*%s*,#{window_name}}\0" as *const u8 as *const libc::c_char,
            s,
        );
    } else {
        xasprintf(
            &mut filter as *mut *mut libc::c_char,
            b"#{m:*%s*,#{pane_title}}\0" as *const u8 as *const libc::c_char,
            s,
        );
    }
    new_args = args_parse(
    ...
    window_pane_set_mode(wp, &window_tree_mode, &mut (*item).target, new_args);
    args_free(new_args);
