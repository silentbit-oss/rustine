...
pub unsafe extern "C" fn recalculate_sizes() {
    ...
    let mut is_zoomed: libc::c_int = 0;
    ...
    while !s.is_null() {
        ...
        ssy = (2147483647 as libc::c_int as libc::c_uint)
        ...
        while !c.is_null() {
            if !((*c).flags & 0x40 as libc::c_int != 0) {
                if !((*c).flags & (0x2000 as libc::c_int | 0x400000 as libc::c_int)
                    == 0x2000 as libc::c_int)
                {
                    if (*c).session == s {
                        ...
                        (*c).flags &= !(0x800000 as libc::c_int);
                        if lines != 0 as libc::c_int as u_int
                            && lines.wrapping_add(2 as libc::c_int as u_int)
                                > (*c).tty.sy
                        {
                            (*c).flags |= 0x800000 as libc::c_int;
                        }
                        if !(*c).flags & 0x800000 as libc::c_int != 0
                            && (*c).flags & 0x2000 as libc::c_int == 0
                            && (*c).tty.sy > lines
                            && ((*c).tty.sy).wrapping_sub(lines) < ssy
                        {
                            ssy = ((*c).tty.sy).wrapping_sub(lines);
                    ...
                    .wrapping_add(1 as libc::c_uint)
        {
            ...
            (*s).flags &= !(0x1 as libc::c_int);
            if lines != 0 as libc::c_int as u_int && ssy == 0 as libc::c_int as u_int {
                ssy = lines;
            }
            if !((*s).sx == ssx && (*s).sy == ssy) {
                log_debug(
                    b"session $%u size %u,%u (was %u,%u)\0" as *const u8
    ...
    while !w.is_null() {
        if !((*w).active).is_null() {
            flag = options_get_number(
                (*w).options,
                b"aggressive-resize\0" as *const u8 as *const libc::c_char,
            ) as libc::c_int;
            ssy = (2147483647 as libc::c_int as libc::c_uint)
                .wrapping_mul(2 as libc::c_uint)
                .wrapping_add(1 as libc::c_uint);
            ...
            while !s.is_null() {
                if !((*s).flags & 0x1 as libc::c_int != 0) {
                    if flag != 0 {
                        has = ((*(*s).curw).window == w) as libc::c_int as u_int;
                    } else {
                        has = session_has(s, w) as u_int;
                    }
                    if has != 0 {
                        if (*s).sx < ssx {
                            ssx = (*s).sx;
                        ...
                        .wrapping_add(1 as libc::c_uint))
            {
                ...
                limit = options_get_number(
                    (*w).options,
                    b"force-width\0" as *const u8 as *const libc::c_char,
                ) as u_int;
                if limit >= 2 as libc::c_int as u_int && ssx > limit {
                    ssx = limit;
                    forced |= 0x2000 as libc::c_int;
                }
                limit = options_get_number(
                    (*w).options,
                    b"force-height\0" as *const u8 as *const libc::c_char,
                ) as u_int;
                if limit >= 2 as libc::c_int as u_int && ssy > limit {
                    ssy = limit;
                    forced |= 0x4000 as libc::c_int;
                }
                if !((*w).sx == ssx && (*w).sy == ssy) {
                    log_debug(
                        b"window @%u size %u,%u (was %u,%u)\0" as *const u8
                            as *const libc::c_char,
                        (*w).id,
                        ssx,
                        ssy,
                    ...
                    (*w).flags &= !(0x2000 as libc::c_int | 0x4000 as libc::c_int);
                    (*w).flags |= forced;
                    is_zoomed = (*w).flags & 0x1000 as libc::c_int;
                    if is_zoomed != 0 {
                        window_unzoom(w);
                    }
                    layout_resize(w, ssx, ssy);
                    window_resize(w, ssx, ssy);
                    if is_zoomed != 0 && window_pane_visible((*w).active) != 0 {
                        window_zoom((*w).active);
                    }
                    wp = (*w).active;
                    while window_pane_visible((*w).active) == 0 {
                        (*w)
                            .active = *(*((*(*w).active).entry.tqe_prev
                            as *mut window_panes))
                            .tqh_last;
                        if ((*w).active).is_null() {
                            (*w)
                                .active = *(*((*w).panes.tqh_last as *mut window_panes))
                                .tqh_last;
                        }
                        if (*w).active == wp {
                            break;
