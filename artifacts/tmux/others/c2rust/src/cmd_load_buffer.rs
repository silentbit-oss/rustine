...
pub struct cmd_load_buffer_data {
    ...
    pub bufname: *mut libc::c_char,
}
#[no_mangle]
pub static mut cmd_load_buffer_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"load-buffer\0" as *const u8 as *const libc::c_char,
            alias: b"loadb\0" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-b buffer-name] path\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn cmd_load_buffer_exec(
    ...
    let mut new_pdata: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    if strcmp(path, b"-\0" as *const u8 as *const libc::c_char) == 0 as libc::c_int {
        cdata = xcalloc(
            ...
            ::core::mem::size_of::<cmd_load_buffer_data>() as libc::c_ulong,
        ) as *mut cmd_load_buffer_data;
        (*cdata).item = item;
        if !bufname.is_null() {
            (*cdata).bufname = xstrdup(bufname);
        }
        error = server_set_stdin_callback(
            c,
            Some(
    ...
    if f.is_null() {
        ...
        loop {
            ...
            new_pdata = realloc(
                ...
                psize.wrapping_add(2 as libc::c_int as size_t),
            ) as *mut libc::c_char;
            if new_pdata.is_null() {
                cmdq_error(
                    item,
                    b"realloc error: %s\0" as *const u8 as *const libc::c_char,
                    strerror(*__errno_location()),
                );
                current_block = 7672447332083251283;
                break;
            } else {
                pdata = new_pdata;
                let fresh0 = psize;
                psize = psize.wrapping_add(1);
                *pdata.offset(fresh0 as isize) = ch as libc::c_char;
            }
        }
        match current_block {
            7672447332083251283 => {}
            _ => {
                if ferror(f) != 0 {
                    ...
                    if !pdata.is_null() {
                        *pdata.offset(psize as isize) = '\0' as i32 as libc::c_char;
                    }
                    fclose(f);
                    free(file as *mut libc::c_void);
                    if paste_set(pdata, psize, bufname, &mut cause) != 0 as libc::c_int {
                        cmdq_error(
                            item,
...
unsafe extern "C" fn cmd_load_buffer_callback(
    mut c: *mut client,
    ...
    (*c).stdin_callback = None;
    server_client_unref(c);
    if !((*c).flags & 0x200 as libc::c_int != 0) {
        psize = evbuffer_get_length((*c).stdin_data);
        if !(psize == 0 as libc::c_int as size_t
            || {
                pdata = malloc(psize.wrapping_add(1 as libc::c_int as size_t))
                    as *mut libc::c_char;
                pdata.is_null()
            })
        {
            memcpy(
                pdata as *mut libc::c_void,
                evbuffer_pullup((*c).stdin_data, -(1 as libc::c_int) as ssize_t)
                    as *const libc::c_void,
                psize,
            );
            *pdata.offset(psize as isize) = '\0' as i32 as libc::c_char;
            evbuffer_drain((*c).stdin_data, psize);
            if paste_set(pdata, psize, (*cdata).bufname, &mut cause) != 0 as libc::c_int
            {
    ...
    (*(*cdata).item).flags &= !(0x2 as libc::c_int);
    free((*cdata).bufname as *mut libc::c_void);
