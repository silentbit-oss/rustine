...
pub static mut cmd_swap_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"swap-pane\0" as *const u8 as *const libc::c_char,
            alias: b"swapp\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"dDs:t:U\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-dDU] [-s src-pane] [-t dst-pane]\0" as *const u8
    ...
    *(*dst_wp).entry.tqe_prev = dst_wp;
    ...
    if args_has((*self_0).args, 'd' as i32 as u_char) == 0 {
        if src_w != dst_w {
            ...
            tmp_wp = dst_wp;
            if window_pane_visible(tmp_wp) == 0 {
                tmp_wp = src_wp;
            }
