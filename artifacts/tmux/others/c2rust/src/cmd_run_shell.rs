...
pub static mut cmd_run_shell_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"run-shell\0" as *const u8 as *const libc::c_char,
            alias: b"run\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"bt:\0" as *const u8 as *const libc::c_char,
                    lower: 1 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-b] [-t target-pane] shell-command\0" as *const u8
...
unsafe extern "C" fn cmd_run_shell_print(
    ...
    let mut cdata: *mut cmd_run_shell_data = (*job).data as *mut cmd_run_shell_data;
    ...
    if wp.is_null() {
        if !((*cdata).item).is_null() {
            cmdq_print((*cdata).item, b"%s\0" as *const u8 as *const libc::c_char, msg);
            return;
        }
    ...
    if (*wp).mode == &window_copy_mode as *const window_mode {
        window_copy_add(wp, b"%s\0" as *const u8 as *const libc::c_char, msg);
    }
}
unsafe extern "C" fn cmd_run_shell_exec(
    ...
    cdata = xcalloc(
        ...
        ::core::mem::size_of::<cmd_run_shell_data>() as libc::c_ulong,
    ...
    job_run(
        ...
        Some(cmd_run_shell_free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
...
unsafe extern "C" fn cmd_run_shell_callback(mut job: *mut job) {
    ...
    size = evbuffer_get_length((*(*job).event).input);
    if size != 0 as libc::c_int as size_t {
        line = xmalloc(size.wrapping_add(1 as libc::c_int as size_t))
            as *mut libc::c_char;
        memcpy(
        ...
        *line.offset(size as isize) = '\0' as i32 as libc::c_char;
        cmd_run_shell_print(job, line);
        free(line as *mut libc::c_void);
    }
    msg = 0 as *mut libc::c_char;
    if (*job).status & 0x7f as libc::c_int == 0 as libc::c_int {
        retcode = ((*job).status & 0xff00 as libc::c_int) >> 8 as libc::c_int;
        if retcode != 0 as libc::c_int {
            xasprintf(
                &mut msg as *mut *mut libc::c_char,
                b"'%s' returned %d\0" as *const u8 as *const libc::c_char,
                cmd,
                retcode,
            );
        }
    } else if (((*job).status & 0x7f as libc::c_int) + 1 as libc::c_int) as libc::c_schar
        as libc::c_int >> 1 as libc::c_int > 0 as libc::c_int
    {
        retcode = (*job).status & 0x7f as libc::c_int;
        xasprintf(
            &mut msg as *mut *mut libc::c_char,
