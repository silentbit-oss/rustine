...
pub static mut cmd_respawn_pane_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"respawn-pane\0" as *const u8 as *const libc::c_char,
            alias: b"respawnp\0" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-c start-directory] [-k] [-t target-pane] [command]\0" as *const u8
...
unsafe extern "C" fn cmd_respawn_pane_exec(
        ...
        && (*wp).fd != -(1 as libc::c_int)
    {
        ...
        cmdq_error(
            item,
            b"pane still active: %s:%d.%u\0" as *const u8 as *const libc::c_char,
            (*s).name,
            (*wl).idx,
            idx,
        );
    ...
    ) != 0 as libc::c_int
    {
        cmdq_error(
            item,
            b"respawn pane failed: %s\0" as *const u8 as *const libc::c_char,
            cause,
        );
