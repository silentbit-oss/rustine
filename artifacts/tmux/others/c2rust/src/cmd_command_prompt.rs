...
pub static mut cmd_command_prompt_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"command-prompt\0" as *const u8 as *const libc::c_char,
            alias: 0 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"1iI:Np:t:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-1Ni] [-I inputs] [-p prompts] [-t target-client] [template]\0"
...
unsafe extern "C" fn cmd_command_prompt_exec(
    ...
    cdata = xcalloc(
        ...
        ::core::mem::size_of::<cmd_command_prompt_cdata>() as libc::c_ulong,
    ...
    if (*args).argc != 0 as libc::c_int {
        ...
        (*cdata).template = xstrdup(b"%1\0" as *const u8 as *const libc::c_char);
    }
    prompts = args_get(args, 'p' as i32 as u_char);
    if !prompts.is_null() {
        (*cdata).prompts = xstrdup(prompts);
    } else if (*args).argc != 0 as libc::c_int {
        n = strcspn((*cdata).template, b" ,\0" as *const u8 as *const libc::c_char);
        xasprintf(
        ...
        (*cdata).prompts = xstrdup(b":\0" as *const u8 as *const libc::c_char);
    }
    (*cdata).next_prompt = (*cdata).prompts;
    ptr = strsep(&mut (*cdata).next_prompt, b",\0" as *const u8 as *const libc::c_char);
    if prompts.is_null() {
        prompt = xstrdup(ptr);
    } else {
        xasprintf(
    ...
    status_prompt_set(
        ...
        Some(cmd_command_prompt_free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
...
unsafe extern "C" fn cmd_command_prompt_error(
    mut item: *mut cmdq_item,
    ...
    if cmdlist.is_null() {
        if !cause.is_null() {
            new_item = cmdq_get_callback1(
                b"cmd_command_prompt_error\0" as *const u8 as *const libc::c_char,
                Some(
