...
#[no_mangle]
pub unsafe extern "C" fn cmd_list_parse(
    mut argc: libc::c_int,
    ...
    loop {
        ...
        if !(arglen == 0 as libc::c_int as size_t
            || *(*copy_argv.offset(i as isize))
                .offset(arglen.wrapping_sub(1 as libc::c_int as size_t) as isize)
                as libc::c_int != ';' as i32)
        {
            *(*copy_argv.offset(i as isize))
                .offset(
                    arglen.wrapping_sub(1 as libc::c_int as size_t) as isize,
                ) = '\0' as i32 as libc::c_char;
            if arglen > 1 as libc::c_int as size_t
                && *(*copy_argv.offset(i as isize))
                    .offset(arglen.wrapping_sub(2 as libc::c_int as size_t) as isize)
                    as libc::c_int == '\\' as i32
            {
                *(*copy_argv.offset(i as isize))
                    .offset(
                        arglen.wrapping_sub(2 as libc::c_int as size_t) as isize,
                ...
                cmd = cmd_parse(new_argc as libc::c_int, new_argv, file, line, cause);
                if cmd.is_null() {
                    current_block = 18430944887111189161;
                    break;
                }
    ...
    match current_block {
        11298138898191919651 => {
            if lastsplit != argc {
                cmd = cmd_parse(
                    argc - lastsplit,
                    copy_argv.offset(lastsplit as isize),
                    file,
                    line,
                    cause,
                );
...
pub unsafe extern "C" fn cmd_list_print(
    ...
    while !cmd.is_null() {
        ...
        len = (len as libc::c_ulong)
            .wrapping_add((strlen(this)).wrapping_add(3 as libc::c_int as libc::c_ulong))
            as size_t as size_t;
