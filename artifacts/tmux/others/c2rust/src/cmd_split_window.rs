...
pub static mut cmd_split_window_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"split-window\0" as *const u8 as *const libc::c_char,
            alias: b"splitw\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"bc:dfF:l:hp:Pt:v\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: -(1 as libc::c_int),
                };
                init
            },
            usage: b"[-bdfhvP] [-c start-directory] [-F format] [-p percentage|-l size] [-t target-pane] [command]\0"
...
unsafe extern "C" fn cmd_split_window_exec(
    ...
    let mut new_cause: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    if !tmp.is_null() {
        cwd = format_single(item, tmp, c, s, 0 as *mut winlink, 0 as *mut window_pane);
    } else if !((*item).client).is_null() && ((*(*item).client).session).is_null() {
    ...
    if args_has(args, 'l' as i32 as u_char) != 0 {
        ...
        if !cause.is_null() {
            ...
            {
                size = ((*wp).sy * percentage as u_int / 100 as libc::c_int as u_int)
                    as libc::c_int;
            } else {
                size = ((*wp).sx * percentage as u_int / 100 as libc::c_int as u_int)
                    as libc::c_int;
            }
            current_block = 6417057564578538666;
        }
    ...
    match current_block {
        6417057564578538666 => {
            ...
            if lc.is_null() {
                cause = xstrdup(b"pane too small\0" as *const u8 as *const libc::c_char);
            } else {
                ...
                ) != 0 as libc::c_int
                {
                    ...
                    if args_has(args, 'P' as i32 as u_char) != 0 {
                        ...
                        if template.is_null() {
                            template = b"#{session_name}:#{window_index}.#{pane_index}\0"
                                as *const u8 as *const libc::c_char;
                        }
                    ...
                    hooks_insert(
                        ...
                        b"after-split-window\0" as *const u8 as *const libc::c_char,
                    );
    ...
    cmdq_error(
        item,
        b"create pane failed: %s\0" as *const u8 as *const libc::c_char,
        cause,
    );
    free(cause as *mut libc::c_void);
    free(cwd as *mut libc::c_void);
