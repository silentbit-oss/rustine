...
pub static mut cmd_set_buffer_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"set-buffer\0" as *const u8 as *const libc::c_char,
            alias: b"setb\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"ab:n:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 1 as libc::c_int,
                };
                init
            },
            usage: b"[-a] [-b buffer-name] [-n new-buffer-name] data\0" as *const u8
...
pub static mut cmd_delete_buffer_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"delete-buffer\0" as *const u8 as *const libc::c_char,
            alias: b"deleteb\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
...
unsafe extern "C" fn cmd_set_buffer_exec(
    ...
    if (*self_0).entry == &cmd_delete_buffer_entry as *const cmd_entry {
        if pb.is_null() {
            pb = paste_get_top(&mut bufname);
        }
        if pb.is_null() {
            cmdq_error(item, b"no buffer\0" as *const u8 as *const libc::c_char);
    ...
    if (*args).argc != 1 as libc::c_int {
        cmdq_error(item, b"no data specified\0" as *const u8 as *const libc::c_char);
        return CMD_RETURN_ERROR;
    }
    newsize = strlen(*((*args).argv).offset(0 as libc::c_int as isize));
    if newsize == 0 as libc::c_int as size_t {
        return CMD_RETURN_NORMAL;
    }
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 && !pb.is_null() {
        ...
        memcpy(bufdata as *mut libc::c_void, olddata as *const libc::c_void, bufsize);
    }
    bufdata = xrealloc(bufdata as *mut libc::c_void, bufsize.wrapping_add(newsize))
        as *mut libc::c_char;
    memcpy(
