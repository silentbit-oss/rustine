...
static void cmd_list_windows_session(struct cmd *, struct session *, struct cmdq_item *, int);
const struct cmd_entry cmd_list_windows_entry = {.name = "list-windows", .alias = "lsw", .args = {"F:at:", 0, 0}, .usage = "[-a] [-F format] [-t target-session]", .target = {'t', CMD_FIND_SESSION, 0}, .flags = 0x4, .exec = cmd_list_windows_exec};
