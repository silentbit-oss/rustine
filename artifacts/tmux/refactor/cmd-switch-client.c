...
static enum cmd_retval cmd_switch_client_exec(struct cmd *, struct cmdq_item *);
const struct cmd_entry cmd_switch_client_entry = {.name = "switch-client", .alias = "switchc", .args = {"lc:Enpt:rT:", 0, 0}, .usage = "[-Elnpr] [-c target-client] [-t target-session] [-T key-table]", .flags = 0x2, .exec = cmd_switch_client_exec};
static enum cmd_retval cmd_switch_client_exec(struct cmd *self, struct cmdq_item *item)
{
  ...
  if ((c_idx = cmd_find_client(item, args_get(args, 'c'), 0)) == 0)
  {
  ...
  {
    table_idx = key_bindings_get_table(tablename, 0);
    ...
    c->keytable = &table[table_idx];
    return CMD_RETURN_NORMAL;
  }
  if (args_has(args, 'n'))
  {
    if ((s_idx = session_next_session(c->session)) == 0)
    {
    ...
    if (args_has(args, 'p'))
  {
    if ((s_idx = session_previous_session(c->session)) == 0)
    {
  ...
  if ((c->session != 0) && (c->session != (&s[s_idx])))
  {
