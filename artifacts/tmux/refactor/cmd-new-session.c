...
const struct cmd_entry cmd_new_session_entry = {.name = "new-session", .alias = "new", .args = {"Ac:dDEF:n:Ps:t:x:y:", 0, -1}, .usage = "[-AdDEP] [-c start-directory] [-F format] [-n window-name] [-s session-name] [-t target-session] [-x width] [-y height] [command]", .target = {'t', CMD_FIND_SESSION, 0x40}, .flags = 0x1, .exec = cmd_new_session_exec};
const struct cmd_entry cmd_has_session_entry = {.name = "has-session", .alias = "has", .args = {"t:", 0, 0}, .usage = "[-t target-session]", .target = {'t', CMD_FIND_SESSION, 0}, .flags = 0, .exec = cmd_new_session_exec};
