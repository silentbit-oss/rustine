...
void key_bindings_RB_INSERT_COLOR(struct key_bindings *head, struct key_binding *elm)
{
  unsigned int elm_idx = 0;
  struct key_binding *parent;
  unsigned int parent_idx = 0;
  struct key_binding *gparent;
  unsigned int gparent_idx = 0;
  struct key_binding *tmp;
  unsigned int tmp_idx = 0;
  while ((parent_idx = elm->entry.rbe_parent) && (parent->entry.rbe_color == 1))
  {
    helper_key_bindings_RB_INSERT_COLOR_1(&elm_idx, &parent_idx, &gparent_idx, &tmp_idx, head, elm, parent, gparent, tmp);
  }

  head->rbh_root->entry.rbe_color = 0;
}

void key_bindings_RB_REMOVE_COLOR(struct key_bindings *head, struct key_binding *parent, struct key_binding *elm)
{
  unsigned int elm_idx = 0;
  unsigned int parent_idx = 0;
  struct key_binding *tmp;
  ...
  {
    helper_key_bindings_RB_REMOVE_COLOR_1(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
...
struct key_binding *key_bindings_RB_REMOVE(struct key_bindings *head, struct key_binding *elm)
{
  unsigned int elm_idx = 0;
  struct key_binding *child;
  unsigned int child_idx = 0;
  struct key_binding *parent;
  unsigned int parent_idx = 0;
  struct key_binding *old = elm;
  int color;
  ...
  {
    helper_key_bindings_RB_REMOVE_1(&elm_idx, &child_idx, &parent_idx, &color, head, elm, child, parent, old);
  }
...
struct key_binding *key_bindings_RB_INSERT(struct key_bindings *head, struct key_binding *elm)
{
  struct key_binding *tmp;
  unsigned int tmp_idx = 0;
...
struct key_binding *key_bindings_RB_NFIND(struct key_bindings *head, struct key_binding *elm)
{
  struct key_binding *tmp = head->rbh_root;
  unsigned int tmp_idx = 0;
  struct key_binding *res = 0;
...
struct key_binding *key_bindings_RB_PREV(struct key_binding *elm)
{
  unsigned int elm_idx = 0;
...
void key_tables_RB_INSERT_COLOR(struct key_tables *head, struct key_table *elm)
{
  unsigned int elm_idx = 0;
  struct key_table *parent;
  unsigned int parent_idx = 0;
  struct key_table *gparent;
  unsigned int gparent_idx = 0;
  struct key_table *tmp;
  unsigned int tmp_idx = 0;
  while ((parent_idx = elm->entry.rbe_parent) && (parent->entry.rbe_color == 1))
  {
    helper_key_tables_RB_INSERT_COLOR_1(&elm_idx, &parent_idx, &gparent_idx, &tmp_idx, head, elm, parent, gparent, tmp);
  }

  head->rbh_root->entry.rbe_color = 0;
}

void key_tables_RB_REMOVE_COLOR(struct key_tables *head, struct key_table *parent, struct key_table *elm)
{
  unsigned int elm_idx = 0;
  unsigned int parent_idx = 0;
  struct key_table *tmp;
  ...
  while ((((&elm[elm_idx]) == 0) || (elm->entry.rbe_color == 0)) && ((&elm[elm_idx]) != head->rbh_root))
  {
    helper_key_tables_RB_REMOVE_COLOR_1(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
...
struct key_table *key_tables_RB_REMOVE(struct key_tables *head, struct key_table *elm)
{
  unsigned int elm_idx = 0;
  struct key_table *child;
  unsigned int child_idx = 0;
  struct key_table *parent;
  unsigned int parent_idx = 0;
  struct key_table *old = elm;
  int color;
  if (elm->entry.rbe_left == 0)
  {
    child_idx = elm->entry.rbe_right;
  ...
  {
    helper_key_tables_RB_REMOVE_1(&elm_idx, &child_idx, &parent_idx, &color, head, elm, child, parent, old);
  }
  parent_idx = elm->entry.rbe_parent;
  color = elm->entry.rbe_color;
  if (child)
...
struct key_table *key_tables_RB_INSERT(struct key_tables *head, struct key_table *elm)
{
  struct key_table *tmp;
  unsigned int tmp_idx = 0;
...
struct key_table *key_tables_RB_FIND(struct key_tables *head, struct key_table *elm)
{
...
struct key_table *key_tables_RB_NFIND(struct key_tables *head, struct key_table *elm)
{
  struct key_table *tmp = head->rbh_root;
  unsigned int tmp_idx = 0;
  struct key_table *res = 0;
  unsigned int res_idx = 0;
  int comp;
  while (tmp)
  {
...
struct key_table *key_tables_RB_PREV(struct key_table *elm)
{
  unsigned int elm_idx = 0;
  if (elm->entry.rbe_left)
  {
...
int key_table_cmp(struct key_table *e1, struct key_table *e2)
{
  return strcmp(e1->name, e2->name);
}

int key_bindings_cmp(struct key_binding *bd1, struct key_binding *bd2)
{
  if (bd1->key < bd2->key)
  {
    return -1;
...
static enum cmd_retval key_bindings_read_only(struct cmdq_item *item, void *data)
{
...
void helper_key_bindings_RB_INSERT_COLOR_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const gparent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const parent, struct key_binding * const gparent, struct key_binding * const tmp)
{
  ...
  if ((&parent[parent_idx]) == gparent->entry.rbe_left)
  {
    helper_helper_key_bindings_RB_INSERT_COLOR_1_1(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, gparent, tmp, gparent_idx);
  }
  else
  {
    helper_helper_key_bindings_RB_INSERT_COLOR_1_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, gparent, tmp, gparent_idx);
  }
  *elm_idx_ref = elm_idx;
...
void helper_key_bindings_RB_REMOVE_COLOR_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const elm, struct key_binding * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if (parent->entry.rbe_left == (&elm[elm_idx]))
  {
    helper_helper_key_bindings_RB_REMOVE_COLOR_1_1(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
  else
  {
    helper_helper_key_bindings_RB_REMOVE_COLOR_1_2(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_key_bindings_RB_REMOVE_1(unsigned int * const elm_idx_ref, unsigned int * const child_idx_ref, unsigned int * const parent_idx_ref, int * const color_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const child, struct key_binding * const parent, struct key_binding * const old)
{
  unsigned int elm_idx = *elm_idx_ref;
...
void helper_key_tables_RB_INSERT_COLOR_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const gparent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const parent, struct key_table * const gparent, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int gparent_idx = *gparent_idx_ref;
  ...
  if ((&parent[parent_idx]) == gparent->entry.rbe_left)
  {
    helper_helper_key_tables_RB_INSERT_COLOR_1_1(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, gparent, tmp, gparent_idx);
  }
  else
  {
    helper_helper_key_tables_RB_INSERT_COLOR_1_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, gparent, tmp, gparent_idx);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *gparent_idx_ref = gparent_idx;
...
void helper_key_tables_RB_REMOVE_COLOR_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const elm, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if (parent->entry.rbe_left == (&elm[elm_idx]))
  {
    helper_helper_key_tables_RB_REMOVE_COLOR_1_1(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
  else
  {
    helper_helper_key_tables_RB_REMOVE_COLOR_1_2(&elm_idx, &parent_idx, &tmp_idx, head, parent, elm, tmp);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_key_tables_RB_REMOVE_1(unsigned int * const elm_idx_ref, unsigned int * const child_idx_ref, unsigned int * const parent_idx_ref, int * const color_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const child, struct key_table * const parent, struct key_table * const old)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int child_idx = *child_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
...
void helper_helper_key_bindings_RB_INSERT_COLOR_1_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const parent, struct key_binding * const gparent, struct key_binding * const tmp, unsigned int gparent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (parent->entry.rbe_right == (&elm[elm_idx]))
  {
    helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_1_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, tmp);
  }
  do
  {
    parent->entry.rbe_color = 0;
  ...
  do
  {
    helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_1_1(&tmp_idx, head, gparent, tmp, gparent_idx);
  }
  while (0);
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
...
void helper_helper_key_bindings_RB_INSERT_COLOR_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const parent, struct key_binding * const gparent, struct key_binding * const tmp, unsigned int gparent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (parent->entry.rbe_left == (&elm[elm_idx]))
  {
    helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_2_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, tmp);
  }
  do
  {
    parent->entry.rbe_color = 0;
  ...
  do
  {
    helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_2_1(&tmp_idx, head, gparent, tmp, gparent_idx);
  }
  while (0);
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
...
void helper_helper_key_bindings_RB_REMOVE_COLOR_1_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const elm, struct key_binding * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (tmp->entry.rbe_color == 1)
  {
    helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  if (((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0)) && ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0)))
  {
    tmp->entry.rbe_color = 1;
  ...
  {
    helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2(&elm_idx, &tmp_idx, head, parent, tmp, parent_idx);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_helper_key_bindings_RB_REMOVE_COLOR_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const elm, struct key_binding * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (tmp->entry.rbe_color == 1)
  {
    helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  if (((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0)) && ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0)))
  {
    tmp->entry.rbe_color = 1;
  ...
  {
    helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2(&elm_idx, &tmp_idx, head, parent, tmp, parent_idx);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_helper_key_tables_RB_INSERT_COLOR_1_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const parent, struct key_table * const gparent, struct key_table * const tmp, unsigned int gparent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (parent->entry.rbe_right == (&elm[elm_idx]))
  {
    helper_helper_helper_key_tables_RB_INSERT_COLOR_1_1_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, tmp);
  }
  do
  {
    parent->entry.rbe_color = 0;
  ...
  do
  {
    helper_helper_helper_key_tables_RB_INSERT_COLOR_1_1_1(&tmp_idx, head, gparent, tmp, gparent_idx);
  }
  while (0);
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
...
void helper_helper_key_tables_RB_INSERT_COLOR_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const parent, struct key_table * const gparent, struct key_table * const tmp, unsigned int gparent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (parent->entry.rbe_left == (&elm[elm_idx]))
  {
    helper_helper_helper_key_tables_RB_INSERT_COLOR_1_2_2(&elm_idx, &parent_idx, &tmp_idx, head, elm, parent, tmp);
  }
  do
  {
    parent->entry.rbe_color = 0;
  ...
  do
  {
    helper_helper_helper_key_tables_RB_INSERT_COLOR_1_2_1(&tmp_idx, head, gparent, tmp, gparent_idx);
  }
  while (0);
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
...
void helper_helper_key_tables_RB_REMOVE_COLOR_1_1(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const elm, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (tmp->entry.rbe_color == 1)
  {
    helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  if (((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0)) && ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0)))
  {
    tmp->entry.rbe_color = 1;
  ...
  {
    helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2(&elm_idx, &tmp_idx, head, parent, tmp, parent_idx);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_helper_key_tables_RB_REMOVE_COLOR_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const elm, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  ...
  if (tmp->entry.rbe_color == 1)
  {
    helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  if (((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0)) && ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0)))
  {
    tmp->entry.rbe_color = 1;
  ...
  {
    helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2(&elm_idx, &tmp_idx, head, parent, tmp, parent_idx);
  }
  *elm_idx_ref = elm_idx;
  *parent_idx_ref = parent_idx;
  *tmp_idx_ref = tmp_idx;
...
void helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_1_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const gparent, struct key_binding * const tmp, unsigned int gparent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = gparent->entry.rbe_left;
  if (gparent->entry.rbe_left = tmp->entry.rbe_right)
...
void helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const parent, struct key_binding * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
...
void helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_2_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const gparent, struct key_binding * const tmp, unsigned int gparent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = gparent->entry.rbe_right;
  if (gparent->entry.rbe_right = tmp->entry.rbe_left)
...
void helper_helper_helper_key_bindings_RB_INSERT_COLOR_1_2_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const elm, struct key_binding * const parent, struct key_binding * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
...
void helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  do
  {
...
void helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2(unsigned int * const elm_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0))
  {
    helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2_2(&tmp_idx, head, parent, tmp);
  }
  tmp->entry.rbe_color = parent->entry.rbe_color;
  parent->entry.rbe_color = 0;
  if (tmp->entry.rbe_right)
  ...
  do
  {
    helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  while (0);
  elm_idx = head->rbh_root;
  break;
...
void helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  do
  {
...
void helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2(unsigned int * const elm_idx_ref, unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if ((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0))
  {
    helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2_2(&tmp_idx, head, parent, tmp);
  }
  tmp->entry.rbe_color = parent->entry.rbe_color;
  parent->entry.rbe_color = 0;
  if (tmp->entry.rbe_left)
  ...
  do
  {
    helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  while (0);
  elm_idx = head->rbh_root;
  break;
...
void helper_helper_helper_key_tables_RB_INSERT_COLOR_1_1_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const gparent, struct key_table * const tmp, unsigned int gparent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = gparent->entry.rbe_left;
  if (gparent->entry.rbe_left = tmp->entry.rbe_right)
...
void helper_helper_helper_key_tables_RB_INSERT_COLOR_1_1_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const parent, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
...
void helper_helper_helper_key_tables_RB_INSERT_COLOR_1_2_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const gparent, struct key_table * const tmp, unsigned int gparent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = gparent->entry.rbe_right;
  if (gparent->entry.rbe_right = tmp->entry.rbe_left)
...
void helper_helper_helper_key_tables_RB_INSERT_COLOR_1_2_2(unsigned int * const elm_idx_ref, unsigned int * const parent_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const elm, struct key_table * const parent, struct key_table * const tmp)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int parent_idx = *parent_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
...
void helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  do
  {
...
void helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2(unsigned int * const elm_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if ((tmp->entry.rbe_right == 0) || (tmp->entry.rbe_right->entry.rbe_color == 0))
  {
    helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2_2(&tmp_idx, head, parent, tmp);
  }
  tmp->entry.rbe_color = parent->entry.rbe_color;
  parent->entry.rbe_color = 0;
  if (tmp->entry.rbe_right)
  ...
  do
  {
    helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  while (0);
  elm_idx = head->rbh_root;
  break;
...
void helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  do
  {
...
void helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2(unsigned int * const elm_idx_ref, unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int elm_idx = *elm_idx_ref;
  unsigned int tmp_idx = *tmp_idx_ref;
  if ((tmp->entry.rbe_left == 0) || (tmp->entry.rbe_left->entry.rbe_color == 0))
  {
    helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2_2(&tmp_idx, head, parent, tmp);
  }
  tmp->entry.rbe_color = parent->entry.rbe_color;
  parent->entry.rbe_color = 0;
  if (tmp->entry.rbe_left)
  ...
  do
  {
    helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2_1(&tmp_idx, head, parent, tmp, parent_idx);
  }
  while (0);
  elm_idx = head->rbh_root;
  break;
...
void helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = parent->entry.rbe_right;
  if (parent->entry.rbe_right = tmp->entry.rbe_left)
...
void helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_1_2_2(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  struct key_binding *oleft;
  unsigned int oleft_idx = 0;
...
void helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2_1(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = parent->entry.rbe_left;
  if (parent->entry.rbe_left = tmp->entry.rbe_right)
...
void helper_helper_helper_helper_key_bindings_RB_REMOVE_COLOR_1_2_2_2(unsigned int * const tmp_idx_ref, struct key_bindings * const head, struct key_binding * const parent, struct key_binding * const tmp)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  struct key_binding *oright;
  unsigned int oright_idx = 0;
...
void helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = parent->entry.rbe_right;
  if (parent->entry.rbe_right = tmp->entry.rbe_left)
...
void helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_1_2_2(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  struct key_table *oleft;
  unsigned int oleft_idx = 0;
...
void helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2_1(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp, unsigned int parent_idx)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  tmp_idx = parent->entry.rbe_left;
  if (parent->entry.rbe_left = tmp->entry.rbe_right)
...
void helper_helper_helper_helper_key_tables_RB_REMOVE_COLOR_1_2_2_2(unsigned int * const tmp_idx_ref, struct key_tables * const head, struct key_table * const parent, struct key_table * const tmp)
{
  unsigned int tmp_idx = *tmp_idx_ref;
  struct key_table *oright;
  unsigned int oright_idx = 0;
