...
static enum cmd_retval cmd_rename_session_exec(struct cmd *, struct cmdq_item *);
const struct cmd_entry cmd_rename_session_entry = {.name = "rename-session", .alias = "rename", .args = {"t:", 1, 1}, .usage = "[-t target-session] new-name", .target = {'t', CMD_FIND_SESSION, 0}, .flags = 0x4, .exec = cmd_rename_session_exec};
