...
static enum cmd_retval cmd_set_environment_exec(struct cmd *, struct cmdq_item *);
const struct cmd_entry cmd_set_environment_entry = {.name = "set-environment", .alias = "setenv", .args = {"grt:u", 1, 2}, .usage = "[-gru] [-t target-session] name [value]", .target = {'t', CMD_FIND_SESSION, 0x40}, .flags = 0x4, .exec = cmd_set_environment_exec};
