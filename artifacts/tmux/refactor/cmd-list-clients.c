...
const struct cmd_entry cmd_list_clients_entry = {.name = "list-clients", .alias = "lsc", .args = {"F:t:", 0, 0}, .usage = "[-F format] [-t target-session]", .target = {'t', CMD_FIND_SESSION, 0}, .flags = 0x2 | 0x4, .exec = cmd_list_clients_exec};
static enum cmd_retval cmd_list_clients_exec(struct cmd *self, struct cmdq_item *item)
{
  ...
  {
    template_idx = "#{client_name}: #{session_name} [#{client_width}x#{client_height} #{client_termname}]#{?client_utf8, (utf8),} #{?client_readonly, (ro),}";
  }
  idx = 0;
  for (c_idx = (&clients)->tqh_first; (&c[c_idx]) != 0; c_idx = c_idx->entry.tqe_next)
  {
    if ((c->session == 0) || (((&s[s_idx]) != 0) && ((&s[s_idx]) != c->session)))
    {
      continue;
    }
