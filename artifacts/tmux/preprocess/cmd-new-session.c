# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 29 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c" 2
...
const struct cmd_entry cmd_new_session_entry = {
 ...
 .usage = "[-AdDEP] [-c start-directory] [-F format] [-n window-name] "
   "[-s session-name] " "[-t target-session]" " [-x width] "
   "[-y height] [command]",

 .target = { 't', CMD_FIND_SESSION, 0x40 },
...
cmd_new_session_exec(struct cmd *self, struct cmdq_item *item)
{
 ...
 char **argv, *cause, *cp, *newname, *cwd = 0;
 ...
 newname = 0;
 if (args_has(args, 's')) {
  newname = format_single(item, args_get(args, 's'), c, 0,
      0, 0);
  if (!session_check_name(newname)) {
   cmdq_error(item, "bad session name: %s", newname);
   goto error;
  }
  if ((as = session_find(newname)) != 0) {
   if (args_has(args, 'A')) {
 ...
 if ((tmp = args_get(args, 'c')) != 0)
  cwd = format_single(item, tmp, c, 0, 0, 0);
 else if (c != 0 && c->session == 0 && c->cwd != 0)
  cwd = xstrdup(c->cwd);
 else
  cwd = xstrdup(".");
# 173 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-new-session.c"
 ...
 if (argc >= 0 && (tmp = args_get(args, 'n')) != 0) {
  cp = format_single(item, tmp, c, s, 0, 0);
  w = s->curw->window;
 ...
 if (args_has(args, 'P')) {
  if ((template = args_get(args, 'F')) == 0)
   template = "#{session_name}:";
  cp = format_single(item, template, c, s, 0, 0);
