# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/sys/time.h" 1
# 21 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 31 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c" 2
...
# 32 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c" 2
...
status_prompt_load_history(void)
{
 ...
 if ((history_file = status_prompt_find_history_file()) == 0)
  return;
...
static void
status_timer_callback( int fd, short events, void *arg)
{
 struct client *c = arg;
    ...
    size_t *size)
{
 ...
 left = status_replace(c, 0, template, t);
    ...
    size_t *size)
{
 ...
 right = status_replace(c, 0, template, t);
...
status_replace(struct client *c, struct winlink *wl, const char *fmt, time_t t)
{
 ...
 if (wl != 0)
  ...
  tag = 0;
 if (c->flags & 0x80000)
  ft = format_create(c, 0, tag, 0x1|0x2);
 else
  ft = format_create(c, 0, tag, 0x1);
    ...
    struct grid_cell *gc)
{
 ...
 else if (wl->flags & (0x2|0x4))
  style_apply_update(gc, oo, "window-status-activity-style");

 text = status_replace(c, wl, fmt, t);
 return (text);
}
# 623 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/status.c"
void
...
void
status_message_callback( int fd, short event, void *data)
{
...
status_prompt_key(struct client *c, key_code key)
{
 ...
 const char *histstr, *bufdata, *ws = 0;
 u_char ch;
...
process_key:
 ...
 case '\011':
  ...
  if ((s = status_prompt_complete(c->session, word)) == 0)
   break;


  n = size - (last - c->prompt_buffer) + 1;
 ...
 case 'f'|0x200000000000ULL:
  ...
  if (options_get_number(oo, "status-keys") == 1 &&
      c->prompt_index != 0)
   c->prompt_index--;
...
{
 const char **list = 0, **layout;
 const struct cmd_entry **cmdent;
 const struct options_table_entry *oe;
 const char *layouts[] = {
  "even-horizontal", "even-vertical", "main-horizontal",
  "main-vertical", "tiled", 0
 };

 *size = 0;
 for (cmdent = cmd_table; *cmdent != 0; cmdent++) {
 ...
 for (layout = layouts; *layout != 0; layout++) {
...
status_prompt_complete(struct session *session, const char *s)
{
 ...
 colon = "";
 if (*s == ':') {
  for ((wl) = winlinks_RB_MINMAX(&session->windows, -1); (wl) != 0; (wl) = winlinks_RB_NEXT(wl)) {
  ...
  for ((s_loop) = sessions_RB_MINMAX(&sessions, -1); (s_loop) != 0; (s_loop) = sessions_RB_NEXT(s_loop)) {
   for ((wl) = winlinks_RB_MINMAX(&s_loop->windows, -1); (wl) != 0; (wl) = winlinks_RB_NEXT(wl)) {
    w = wl->window;
