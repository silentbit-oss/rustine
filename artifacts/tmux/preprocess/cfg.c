# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 28 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c" 2
...
# 29 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c" 2
...
struct cfg_cond {
 ...
 struct { struct cfg_cond *tqe_next; struct cfg_cond **tqe_prev; } entry;
};
struct cfg_conds { struct cfg_cond *tqh_first; struct cfg_cond **tqh_last; };
...
cfg_client_done( struct cmdq_item *item, void *data)
{
 if (!cfg_finished)
...
static enum cmd_retval
cfg_done( struct cmdq_item *item, void *data)
{
 if (cfg_finished)
  return (CMD_RETURN_NORMAL);
 cfg_finished = 1;
...
start_cfg(void)
{
 ...
 int quiet = 0;
 struct client *c;
# 98 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c"
 c = ((&clients)->tqh_first);
 if (c != 0) {
 ...
 if (cfg_file == 0 && (home = find_home()) != 0) {
    ...
    const char *p)
{
 ...
 if (parent == 0 || parent->met)
  ...
  cond->skip = 1;
 cond->saw_else = 0;
 do { if (((cond)->entry.tqe_next = (conds)->tqh_first) != 0) (conds)->tqh_first->entry.tqe_prev = &(cond)->entry.tqe_next; else (conds)->tqh_last = &(cond)->entry.tqe_next; (conds)->tqh_first = (cond); (cond)->entry.tqe_prev = &(conds)->tqh_first; } while (0);
}
...
cfg_handle_endif(const char *path, size_t line, struct cfg_conds *conds)
{
 ...
 do { if (((cond)->entry.tqe_next) != 0) (cond)->entry.tqe_next->entry.tqe_prev = (cond)->entry.tqe_prev; else (conds)->tqh_last = (cond)->entry.tqe_prev; *(cond)->entry.tqe_prev = (cond)->entry.tqe_next; ; ; } while (0);
 free(cond);
}
...
load_cfg(const char *path, struct client *c, struct cmdq_item *item, int quiet)
{
 ...
 do { (&conds)->tqh_first = 0; (&conds)->tqh_last = &(&conds)->tqh_first; } while (0);
 ...
 while ((buf = fparseln(f, 0, &line, delim, 0)) != 0) {
  ...
  new_item = cmdq_get_command(cmdlist, 0, 0, 0);
  if (item != 0)
 ...
 for ((cond) = (*(((struct cfg_conds *)((&conds)->tqh_last))->tqh_last)); (cond) != 0 && ((cond1) = (*(((struct cfg_conds *)((cond)->entry.tqe_prev))->tqh_last)), 1); (cond) = (cond1)) {
...
# 325 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cfg.c"
void
