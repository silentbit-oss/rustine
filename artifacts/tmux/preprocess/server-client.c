# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/sys/ioctl.h" 1
# 21 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/sys/uio.h" 1
# 22 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 32 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c" 2
...
# 33 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c" 2
...
static void
server_client_callback_identify( int fd, short events,
    void *data)
{
...
server_client_check_nested(struct client *c)
{
 ...
 if (envent == 0 || *envent->value == '\0')
  return (0);

 for ((wp) = window_pane_tree_RB_MINMAX(&all_window_panes, -1); (wp) != 0; (wp) = window_pane_tree_RB_NEXT(wp)) {
...
server_client_create(int fd)
{
 ...
 do { (&c->queue)->tqh_first = 0; (&c->queue)->tqh_last = &(&c->queue)->tqh_first; } while (0);
 ...
 c->message_string = 0;
 do { (&c->message_log)->tqh_first = 0; (&c->message_log)->tqh_last = &(&c->message_log)->tqh_first; } while (0);
 ...
 do { (c)->entry.tqe_next = 0; (c)->entry.tqe_prev = (&clients)->tqh_last; *(&clients)->tqh_last = (c); (&clients)->tqh_last = &(c)->entry.tqe_next; } while (0);
...
server_client_lost(struct client *c)
{
 ...
 do { if (((c)->entry.tqe_next) != 0) (c)->entry.tqe_next->entry.tqe_prev = (c)->entry.tqe_prev; else (&clients)->tqh_last = (c)->entry.tqe_prev; *(c)->entry.tqe_prev = (c)->entry.tqe_next; ; ; } while (0);
 ...
 for ((msg) = ((&c->message_log)->tqh_first); (msg) != 0 && ((msg1) = ((msg)->entry.tqe_next), 1); (msg) = (msg1)) {
  free(msg->msg);
  do { if (((msg)->entry.tqe_next) != 0) (msg)->entry.tqe_next->entry.tqe_prev = (msg)->entry.tqe_prev; else (&c->message_log)->tqh_last = (msg)->entry.tqe_prev; *(msg)->entry.tqe_prev = (msg)->entry.tqe_next; ; ; } while (0);
  free(msg);
 }
...
static void
server_client_free( int fd, short events, void *arg)
{
 struct client *c = arg;
...
have_event:
 ...
 if (where == NOWHERE) {
  ...
  for((wp) = ((&s->curw->window->panes)->tqh_first); (wp) != 0; (wp) = ((wp)->entry.tqe_next)) {
   if ((wp->xoff + wp->sx == x &&
       wp->yoff <= 1 + y &&
       wp->yoff + wp->sy >= y) ||
       (wp->yoff + wp->sy == y &&
...
server_client_assume_paste(struct session *s)
{
 ...
 do { (&tv)->tv_sec = (&s->activity_time)->tv_sec - (&s->last_activity_time)->tv_sec; (&tv)->tv_usec = (&s->activity_time)->tv_usec - (&s->last_activity_time)->tv_usec; if ((&tv)->tv_usec < 0) { (&tv)->tv_sec--; (&tv)->tv_usec += 1000000; } } while (0);
...
server_client_loop(void)
{
 ...
 for ((w) = windows_RB_MINMAX(&windows, -1); (w) != 0; (w) = windows_RB_NEXT(w)) {
...
server_client_resize_force(struct window_pane *wp)
{
 ...
 struct winsize ws;
# 1058 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
 if (wp->flags & 0x10) {
  wp->flags &= ~0x10;
  return (0);
 }
...
static void
server_client_resize_event( int fd, short events, void *data)
{
 ...
 ws.ws_row = wp->sy;
 if (wp->fd != -1 && ioctl(wp->fd, TIOCSWINSZ, &ws) == -1)
# 1111 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
  fatal("ioctl failed");
...
# 1209 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
static void
server_client_reset_state(struct client *c)
{
 ...
 if (options_get_number(oo, "mouse")) {
  ...
  for((loop) = ((&w->panes)->tqh_first); (loop) != 0; (loop) = ((loop)->entry.tqe_next)) {
...
static void
server_client_repeat_timer( int fd, short events, void *data)
{
...
static void
server_client_click_timer( int fd, short events, void *data)
{
 struct client *c = data;
...
static void
server_client_redraw_timer( int fd, short events,
    void* data)
{
...
server_client_check_redraw(struct client *c)
{
 ...
 if (c->flags & 0x8)
  ...
  for((wp) = ((&c->session->curw->window->panes)->tqh_first); (wp) != 0; (wp) = ((wp)->entry.tqe_next)) {
   if (wp->flags & 0x1) {
    needed = 1;
    break;
   }
...
server_client_set_title(struct client *c)
{
 ...
 ft = format_create(c, 0, 0, 0);
 format_defaults(ft, c, 0, 0, 0);

 title = format_expand_time(ft, template, time(0));
...
server_client_command_done(struct cmdq_item *item, void *data)
{
...
server_client_dispatch_command(struct client *c, struct imsg *imsg)
{
 ...
 if ((cmdlist = cmd_list_parse(argc, argv, 0, 0, &cause)) == 0) {
...
static void
server_client_stdout_cb( int fd, short events, void *arg)
{
 struct client *c = arg;
...
static void
server_client_stderr_cb( int fd, short events, void *arg)
{
 struct client *c = arg;
...
# 1864 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-client.c"
const char *
