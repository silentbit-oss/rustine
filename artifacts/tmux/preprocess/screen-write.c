# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c" 2
...
# 22 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/string.h" 1
# 23 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 25 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c" 2
...
# 26 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c" 2
...
struct screen_write_collect_item {
 ...
 struct { struct screen_write_collect_item *tqe_next; struct screen_write_collect_item **tqe_prev; } entry;
};
struct screen_write_collect_line {
 struct { struct screen_write_collect_item *tqh_first; struct screen_write_collect_item **tqh_last; } items;
};
    ...
    struct screen *s)
{
 ...
 for (y = 0; y < ((ctx->s)->grid->sy); y++)
  do { (&ctx->list[y].items)->tqh_first = 0; (&ctx->list[y].items)->tqh_last = &(&ctx->list[y].items)->tqh_first; } while (0);
 ...
 log_debug("%s: size %ux%u, %s", __func__, ((ctx->s)->grid->sx),
     ((ctx->s)->grid->sy), wp == 0 ? "no pane" : tmp);
}
...
# 230 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
void
...
# 356 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
void
...
screen_write_alignmenttest(struct screen_write_ctx *ctx)
{
 ...
 for (yy = 0; yy < ((s)->grid->sy); yy++) {
  for (xx = 0; xx < ((s)->grid->sx); xx++)
...
screen_write_collect_clear(struct screen_write_ctx *ctx, u_int y, u_int n)
{
 ...
 for (i = y ; i < y + n; i++) {
  if ((((&ctx->list[i].items)->tqh_first) == 0))
   continue;
  size = 0;
  for ((ci) = ((&ctx->list[i].items)->tqh_first); (ci) != 0 && ((tmp) = ((ci)->entry.tqe_next), 1); (ci) = (tmp)) {
   size += ci->used;
   do { if (((ci)->entry.tqe_next) != 0) (ci)->entry.tqe_next->entry.tqe_prev = (ci)->entry.tqe_prev; else (&ctx->list[i].items)->tqh_last = (ci)->entry.tqe_prev; *(ci)->entry.tqe_prev = (ci)->entry.tqe_next; ; ; } while (0);
   free(ci);
  }
  ctx->skipped += size;
...
screen_write_collect_scroll(struct screen_write_ctx *ctx)
{
 ...
 for (y = s->rupper; y < s->rlower; y++) {
  cl = &ctx->list[y + 1];
  do { if (!(((&cl->items)->tqh_first) == 0)) { *(&ctx->list[y].items)->tqh_last = (&cl->items)->tqh_first; (&cl->items)->tqh_first->entry.tqe_prev = (&ctx->list[y].items)->tqh_last; (&ctx->list[y].items)->tqh_last = (&cl->items)->tqh_last; do { ((&cl->items))->tqh_first = 0; ((&cl->items))->tqh_last = &((&cl->items))->tqh_first; } while (0); } } while (0);
  do { (&cl->items)->tqh_first = 0; (&cl->items)->tqh_last = &(&cl->items)->tqh_first; } while (0);
 }
}
...
screen_write_collect_flush(struct screen_write_ctx *ctx, int scroll_only)
{
 ...
 cx = s->cx; cy = s->cy;
 for (y = 0; y < ((s)->grid->sy); y++) {
  for ((ci) = ((&ctx->list[y].items)->tqh_first); (ci) != 0 && ((tmp) = ((ci)->entry.tqe_next), 1); (ci) = (tmp)) {
   ...
   do { if (((ci)->entry.tqe_next) != 0) (ci)->entry.tqe_next->entry.tqe_prev = (ci)->entry.tqe_prev; else (&ctx->list[y].items)->tqh_last = (ci)->entry.tqe_prev; *(ci)->entry.tqe_prev = (ci)->entry.tqe_next; ; ; } while (0);
   free(ci);
  }
 }
 s->cx = cx; s->cy = cy;
...
screen_write_collect_end(struct screen_write_ctx *ctx)
{
 ...
 ci->x = s->cx;
 do { (ci)->entry.tqe_next = 0; (ci)->entry.tqe_prev = (&ctx->list[s->cy].items)->tqh_last; *(&ctx->list[s->cy].items)->tqh_last = (ci); (&ctx->list[s->cy].items)->tqh_last = &(ci)->entry.tqe_next; } while (0);
 ctx->item = xcalloc(1, sizeof *ctx->item);
    ...
    const struct grid_cell *gc)
{
 ...
 u_int sx = ((s)->grid->sx);
 int collect;
# 1338 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
 collect = 1;
 if (gc->data.width != 1 || gc->data.size != 1 || *gc->data.data >= 0x7f)
  collect = 0;
 else if (gc->attr & 0x80)
  collect = 0;
 else if (~s->mode & 0x10)
  collect = 0;
 else if (s->mode & 0x2)
...
screen_write_cell(struct screen_write_ctx *ctx, const struct grid_cell *gc)
{
 ...
 } else if (!selected && (gc->flags & 0x10)) {
  memcpy(&tmp_gc, gc, sizeof tmp_gc);
  tmp_gc.flags &= ~0x10;
  grid_view_set_cell(gd, s->cx, s->cy, &tmp_gc);
 } else if (!skip)
...
# 1578 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/screen-write.c"
static int
