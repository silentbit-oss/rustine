# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/sys/wait.h" 1
# 21 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c" 2
# 1 "/home/enumclass/Documents/C2RustTranslation/pipeline/preprocess/fake_libc_include/sys/uio.h" 1
# 22 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 29 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/server-fn.c" 2
...
server_lock_client(struct client *c)
{
 ...
 if (*cmd == '\0' || strlen(cmd) + 1 > 16384 - sizeof(struct imsg_hdr))
  return;
...
server_kill_window(struct window *w)
{
 ...
 next_s = sessions_RB_MINMAX(&sessions, -1);
 while (next_s != 0) {
  s = next_s;
  ...
  if (!session_has(s, w))
   continue;
  server_unzoom_window(w);
  while ((wl = winlink_find_by_window(&s->windows, w)) != 0) {
  ...
  if (options_get_number(s->options, "renumber-windows")) {
   if ((sg = session_group_contains(s)) != 0) {
    for((target_s) = ((&sg->sessions)->tqh_first); (target_s) != 0; (target_s) = ((target_s)->gentry.tqe_next))
    ...
    char **cause)
{
 ...
 if (src != dst && srcsg != 0 && dstsg != 0 && srcsg == dstsg) {
...
server_destroy_session_group(struct session *s)
{
 ...
 if ((sg = session_group_contains(s)) == 0)
  ...
  for ((s) = ((&sg->sessions)->tqh_first); (s) != 0 && ((s1) = ((s)->gentry.tqe_next), 1); (s) = (s1)) {
   server_destroy_session(s);
...
server_next_session(struct session *s)
{
 ...
 for ((s_loop) = sessions_RB_MINMAX(&sessions, -1); (s_loop) != 0; (s_loop) = sessions_RB_NEXT(s_loop)) {
  if (s_loop == s)
   continue;
  if (s_out == 0 ||
      (((&s_loop->activity_time)->tv_sec == (&s_out->activity_time)->tv_sec) ? ((&s_loop->activity_time)->tv_usec < (&s_out->activity_time)->tv_usec) : ((&s_loop->activity_time)->tv_sec < (&s_out->activity_time)->tv_sec)))
   s_out = s_loop;
 }
 return (s_out);
}
