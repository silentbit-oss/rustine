# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c" 2
...
# 22 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 24 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/cmd-resize-pane.c" 2
...
const struct cmd_entry cmd_resize_pane_entry = {
 ...
 .usage = "[-DLMRUZ] [-x width] [-y height] " "[-t target-pane]" " "
   "[adjustment]",
...
cmd_resize_pane_exec(struct cmd *self, struct cmdq_item *item)
{
 ...
 if (args_has(self->args, 'x')) {
  x = args_strtonum(self->args, 'x', 2, 2147483647,
      &cause);
  if (cause != 0) {
 ...
 if (args_has(self->args, 'y')) {
  y = args_strtonum(self->args, 'y', 2, 2147483647,
      &cause);
  if (cause != 0) {
...
cmd_resize_pane_mouse_update(struct client *c, struct mouse_event *m)
{
 ...
 wp_x = wp_y = 0;
 for((loop) = ((&wl->window->panes)->tqh_first); (loop) != 0; (loop) = ((loop)->entry.tqe_next)) {
  ...
  if ((lx == sx || lx == ex) &&
      (ly >= sy && ly <= ey) &&
      (wp_x == 0 || loop->sy > wp_x->sy))
   wp_x = loop;
  if ((ly == sy || ly == ey) &&
      (lx >= sx && lx <= ex) &&
      (wp_y == 0 || loop->sx > wp_y->sx))
   wp_y = loop;
 }
 if (wp_x == 0 && wp_y == 0) {
  c->tty.mouse_drag_update = 0;
