# 0 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/mode-tree.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/mode-tree.c"
# 19 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/mode-tree.c"
...
# 20 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/mode-tree.c" 2
...
int style_equal(const struct grid_cell *,
       const struct grid_cell *);
# 27 "/home/enumclass/Documents/C2RustTranslation/bkp/tmux/mode-tree.c" 2

struct mode_tree_item;
struct mode_tree_list { struct mode_tree_item *tqh_first; struct mode_tree_item **tqh_last; };
...
struct mode_tree_item {
 ...
 struct mode_tree_list children;
 struct { struct mode_tree_item *tqe_next; struct mode_tree_item **tqe_prev; } entry;
};
...
mode_tree_find_item(struct mode_tree_list *mtl, uint64_t tag)
{
 ...
 for((mti) = ((mtl)->tqh_first); (mti) != 0; (mti) = ((mti)->entry.tqe_next)) {
  if (mti->tag == tag)
   return (mti);
...
mode_tree_free_items(struct mode_tree_list *mtl)
{
 ...
 for ((mti) = ((mtl)->tqh_first); (mti) != 0 && ((mti1) = ((mti)->entry.tqe_next), 1); (mti) = (mti1)) {
  do { if (((mti)->entry.tqe_next) != 0) (mti)->entry.tqe_next->entry.tqe_prev = (mti)->entry.tqe_prev; else (mtl)->tqh_last = (mti)->entry.tqe_prev; *(mti)->entry.tqe_prev = (mti)->entry.tqe_next; ; ; } while (0);
    ...
    struct mode_tree_list *mtl, u_int depth)
{
 ...
 for((mti) = ((mtl)->tqh_first); (mti) != 0; (mti) = ((mti)->entry.tqe_next)) {
  ...
  line->item = mti;
  line->depth = depth;
  line->last = (mti == (*(((struct mode_tree_list *)((mtl)->tqh_last))->tqh_last)));
    ...
    u_int sort_size, struct screen **s)
{
 ...
 do { (&mtd->children)->tqh_first = 0; (&mtd->children)->tqh_last = &(&mtd->children)->tqh_first; } while (0);
...
mode_tree_build(struct mode_tree_data *mtd)
{
 ...
 do { if (!(((&mtd->children)->tqh_first) == 0)) { *(&mtd->saved)->tqh_last = (&mtd->children)->tqh_first; (&mtd->children)->tqh_first->entry.tqe_prev = (&mtd->saved)->tqh_last; (&mtd->saved)->tqh_last = (&mtd->children)->tqh_last; do { ((&mtd->children))->tqh_first = 0; ((&mtd->children))->tqh_last = &((&mtd->children))->tqh_first; } while (0); } } while (0);
 do { (&mtd->children)->tqh_first = 0; (&mtd->children)->tqh_last = &(&mtd->children)->tqh_first; } while (0);
 ...
 do { (&mtd->saved)->tqh_first = 0; (&mtd->saved)->tqh_last = &(&mtd->saved)->tqh_first; } while (0);
    ...
    int expanded)
{
 ...
 do { (&mti->children)->tqh_first = 0; (&mti->children)->tqh_last = &(&mti->children)->tqh_first; } while (0);

 if (parent != 0)
  do { (mti)->entry.tqe_next = 0; (mti)->entry.tqe_prev = (&parent->children)->tqh_last; *(&parent->children)->tqh_last = (mti); (&parent->children)->tqh_last = &(mti)->entry.tqe_next; } while (0);
 else
  do { (mti)->entry.tqe_next = 0; (mti)->entry.tqe_prev = (&mtd->children)->tqh_last; *(&mtd->children)->tqh_last = (mti); (&mtd->children)->tqh_last = &(mti)->entry.tqe_next; } while (0);
...
mode_tree_remove(struct mode_tree_data *mtd, struct mode_tree_item *mti)
{
 ...
 if (parent != 0)
  do { if (((mti)->entry.tqe_next) != 0) (mti)->entry.tqe_next->entry.tqe_prev = (mti)->entry.tqe_prev; else (&parent->children)->tqh_last = (mti)->entry.tqe_prev; *(mti)->entry.tqe_prev = (mti)->entry.tqe_next; ; ; } while (0);
 else
  do { if (((mti)->entry.tqe_next) != 0) (mti)->entry.tqe_next->entry.tqe_prev = (mti)->entry.tqe_prev; else (&mtd->children)->tqh_last = (mti)->entry.tqe_prev; *(mti)->entry.tqe_prev = (mti)->entry.tqe_next; ; ; } while (0);
 mode_tree_free_item(mti);
}
...
mode_tree_draw(struct mode_tree_data *mtd)
{
 ...
 for (i = 0; i < mtd->line_size; i++) {
  ...
  if (line->flat)
   symbol = "";
  else if ((((&mti->children)->tqh_first) == 0))
   symbol = "  ";
...
mode_tree_search_for(struct mode_tree_data *mtd)
{
 ...
 for (;;) {
  ...
  else if ((next = ((mti)->entry.tqe_next)) != 0)
   mti = next;
  else {
   for (;;) {
    mti = mti->parent;
...
static int
mode_tree_search_callback( struct client *c, void *data, const char *s,
    int done)
{
...
static int
mode_tree_filter_callback( struct client *c, void *data, const char *s,
    int done)
{
 struct mode_tree_data *mtd = data;
    ...
    struct mouse_event *m, u_int *xp, u_int *yp)
{
 ...
 if (*key >= '0' && *key <= '9')
  choice = (*key) - '0';
 else if (((*key) & (0x200000000000ULL|0x400000000000ULL|0x800000000000ULL|0x1000000000000ULL)) == 0x200000000000ULL) {
    ...
    const char *template, const char *name)
{
 ...
 cmdlist = cmd_string_parse(command, 0, 0, &cause);
 if (cmdlist == 0) {
  if (cause != 0 && c != 0) {
  ...
  new_item = cmdq_get_command(cmdlist, fs, 0, 0);
