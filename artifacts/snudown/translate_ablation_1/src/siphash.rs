...
pub fn siphash_nocase(in_bytes: &[u8], k: &[u8]) -> u64 {
    ...
    let mut i = 0;
    while i < end {
        helper_siphash_nocase_7(&mut v0, &mut v1, &mut v2, &mut v3, &mut m, &in_bytes[i..i+8], i);
        i += 8;
    }
    
    match left {
        7 => {
            b |= (in_bytes[i+6].to_ascii_lowercase() as u64) << 48;
            b |= (in_bytes[i+5].to_ascii_lowercase() as u64) << 40;
            b |= (in_bytes[i+4].to_ascii_lowercase() as u64) << 32;
            b |= (in_bytes[i+3].to_ascii_lowercase() as u64) << 24;
            b |= (in_bytes[i+2].to_ascii_lowercase() as u64) << 16;
            b |= (in_bytes[i+1].to_ascii_lowercase() as u64) << 8;
            b |= in_bytes[i].to_ascii_lowercase() as u64;
        }
        6 => {
            b |= (in_bytes[i+5].to_ascii_lowercase() as u64) << 40;
            b |= (in_bytes[i+4].to_ascii_lowercase() as u64) << 32;
        ...
        0 => {}
        _ => {} // Handle any other values (shouldn't occur due to inlen & 7)
    }
...
pub fn siphash(in_: &[u8], k: &[u8; 16]) -> u64 {
    ...
    while idx < end {
        let mut m = u64::from_le_bytes(in_[idx..idx+8].try_into().unwrap());
        helper_siphash_7(&mut v0, &mut v1, &mut v2, &mut v3, &mut m, in_, idx);
        idx += 8;
    }
    
    match inlen - idx {
        7 => {
            b |= (in_[idx + 6] as u64) << 48;
            b |= (in_[idx + 5] as u64) << 40;
            b |= (in_[idx + 4] as u64) << 32;
            b |= (in_[idx + 3] as u64) << 24;
            b |= (in_[idx + 2] as u64) << 16;
            b |= (in_[idx + 1] as u64) << 8;
            b |= in_[idx] as u64;
        }
        6 => {
            b |= (in_[idx + 5] as u64) << 40;
