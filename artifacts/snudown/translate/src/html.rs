...
pub fn sdhtml_is_tag(tag_data: &[u8], tagname: &str) -> i32 {
    ...
    for (tag_byte, tagname_byte) in tag_data[i..].iter().zip(tagname.bytes()) {
        if *tag_byte != tagname_byte {
            return HTML_TAG_NONE;
        }
        tagname_idx += 1;
    }

    if i + tagname_idx >= tag_data.len() {
        return HTML_TAG_NONE;
    }

    let current_byte = tag_data[i + tagname_idx];
    if (current_byte as char).is_ascii_whitespace() || current_byte == b'>' {
        if closed {
            HTML_TAG_CLOSE
        } else {
            HTML_TAG_OPEN
...
pub fn rndr_html_tag(
    ob: &mut Buf,
    text: &Buf,
    opaque: Option<&()>,  // Using Option for opaque pointer
    tagname: &str,
    whitelist: &[&str],
    tagtype: i32,
) {
    ...
    bufrelease(Some(Box::new(attr)));
    bufrelease(Some(Box::new(value)));
    bufputc(ob, '>' as i32);
}
