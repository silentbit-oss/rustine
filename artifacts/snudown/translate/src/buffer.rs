...
pub fn bufgrow(buf: &mut Buf, neosz: usize) -> Result<(), BufError> {
    // Check if buf and buf.unit are valid (equivalent to C's assert)
    if buf.unit == 0 {
        return Err(BufError::Enomem);
    }

    // Check if requested size exceeds maximum allowed (16MB)
    if neosz > (1024 * 1024 * 16) {
        return Err(BufError::Enomem);
    }

    // If current allocation size is sufficient, return success
    if buf.asize >= neosz {
        return Ok(());
    }

    // Calculate new allocation size by adding units until we reach or exceed neosz
    let mut neoasz = buf.asize + buf.unit;
    while neoasz < neosz {
        neoasz += buf.unit;
    }

    // Resize the Vec<u8> (equivalent to realloc)
    match buf.data {
        Some(ref mut vec) => {
            vec.resize(neoasz, 0);
        },
        None => {
            buf.data = Some(vec![0; neoasz]);
        }
    }
