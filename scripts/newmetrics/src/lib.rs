...
pub fn count_file_spans(tcx: TyCtxt<'_>, crate_num: rustc_span::def_id::CrateNum) -> u32 {
    ...
    for source_file in files.iter() {
        ...
        let length = source_file.end_position().0 - source_file.start_pos.0;
        bytes_count += length;
    }
    bytes_count
}
...
pub fn analyze(&tcx: &TyCtxt<'_>, config: MetricsConfig) {
    ...
    let source_map = tcx.sess.source_map();
    let mut par = Par::new(&tcx, selected_fns.clone());
    tcx.hir_visit_all_item_likes_in_crate(&mut par);
    println!("Pointer arithmetic: {}", par.unsafe_calls.len());

    // let hir_map = tcx.hir();
    let source_map = tcx.sess.source_map();
    let mut unsafe_counter = UnsafeCounter::new(&tcx, selected_fns.clone());
    tcx.hir_visit_all_item_likes_in_crate(&mut unsafe_counter);
    ...
    tcx.hir_visit_all_item_likes_in_crate(&mut pointer_counter);
