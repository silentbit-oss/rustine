...
fn version_info() -> VersionMeta {
    VersionMeta::for_command(Command::new(find_newmetrics()))
...
fn cargo_package() -> cargo_metadata::Package {
    ...
    let package_index = metadata
        ...
        .unwrap_or_else(|| {
            show_error("This seems to be a workspace, which is not supported by cargo-newmetrics");
        });

    metadata.packages.remove(package_index)
}

/// Returns the path to the `newmetrics` binary
fn find_newmetrics() -> PathBuf {
    let mut path = std::env::current_exe().expect("current executable path invalid");
    path.set_file_name("newmetrics");
    path
}

/// Make sure that the `newmetrics` and `rustc` binary are from the same sysroot.
/// This can be violated e.g. when newmetrics is locally built and installed with a different
/// toolchain than what is used when `cargo newmetrics` is run.
fn test_sysroot_consistency() {
    ...
    let newmetrics_sysroot = get_sysroot(Command::new(find_newmetrics()));

    if rustc_sysroot != newmetrics_sysroot {
        show_error(format!(
            "newmetrics was built for a different sysroot than the rustc in your current toolchain.\n\
             Make sure you use the same toolchain to run newmetrics that you used to build it!\n\
             rustc sysroot: `{}`\n\
             newmetrics sysroot: `{}`",
            rustc_sysroot.display(),
            newmetrics_sysroot.display()
        ));
    }
}
...
fn main() {
    ...
    let Some(first) = args.next() else {
        show_error(
            "`cargo-newmetrics` called without first argument; please only invoke this binary through `cargo newmetrics`"
        )
    };
    ...
    match first.as_str() {
        ...
        "newmetrics" => {
            eprintln!("Running cargo newmetrics");
            // This arm is for when `cargo newmetrics` is called. We call `cargo rustc` for each applicable target,
            // but with the `RUSTC` env var set to the `cargo-newmetrics` binary so that we come back in the other branch,
            // and dispatch the invocations to `rustc` and `newmetrics`, respectively.
            in_cargo_newmetrics();
            eprintln!("cargo newmetrics finished");
        },
        // Check if arg is a path that ends with "/rustc"
        arg if arg.ends_with("rustc") => {
            eprintln!("Running cargo rustc");
            // This arm is executed when `cargo-newmetrics` runs `cargo rustc` with the `RUSTC_WRAPPER` env var set to itself:
            // dependencies get dispatched to `rustc`, the final test/binary to `newmetrics`.
            inside_cargo_rustc();
            eprintln!("cargo rustc finished");
        },
        _ => {
            show_error(
                "`cargo-newmetrics` must be called with either `newmetrics` or `rustc` as first argument.",
            );
        }
    }
}
...
fn inside_cargo_rustc() {
    ...
    /// invoking cargo itself with `--target`, which `in_cargo_newmetrics` ensures.
    ...
    if is_direct_target {
        let mut cmd = Command::new(find_newmetrics());
        cmd.args(std::env::args().skip(2)); // skip `cargo-newmetrics rustc`
